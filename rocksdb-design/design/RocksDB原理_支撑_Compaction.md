...
> **定位**：属"整理能力域"、后台守护线程。管 LSM 的核心整理动作：把多层重叠/冗余的 SST 归并成更少、更有序、更紧凑的文件，回收旧版本与墓碑。被【写入路径】的 Flush 产物（L0）触发，依赖【版本】记录文件变更、【SST 存储格式】读写文件。它决定 LSM 的**写/读/空间三放大**权衡。源码基准 **RocksDB 11.7.0**。
...
Flush 不断往 L0 堆不可变 SST，L0 文件间 key 可重叠、越堆读越慢。Compaction 后台把文件**逐层归并下沉**：`CompactionPicker` 按每层"分数"（数据量 / 目标容量）挑选要合并的层与文件，`CompactionJob` 用 MergingIterator 归并输入文件、输出到下一层的新 SST，同时**丢弃**被更新版本覆盖的旧值、以及不再被任何快照需要的墓碑。产出经 `VersionEdit` 原子换进新 Version（旧文件延后删除）。
...
`compaction_style`（CF Options）选三种之一：

- **Leveled（默认，`kCompactionStyleLevel`）**：经典分层，每层容量约为上层 10×，层内非重叠有序。L0→L1 特殊（L0 重叠，需全 L1 参与），L1+ 每次选一个文件与下层重叠文件合并。**低读/空间放大，高写放大**——通用首选。
- **Universal（`kCompactionStyleUniversal`）**：按时间序把相邻 SST 整段合并，尽量少合、层数少。**低写放大，高空间放大**（临时占用大）——写密集、空间宽裕时用。
- **FIFO（`kCompactionStyleFIFO`）**：只按 TTL/总大小删最老文件，几乎不合并。**极低写放大**——时序/缓存类只留近期数据的场景。
...
Leveled 下 `CompactionPickerLevel` 给每层算**分数**：L0 按文件数（`level0_file_num_compaction_trigger`），L1+ 按"当前字节 / 目标字节"。分数最高的层触发 compaction。**L0→L1 特殊**：L0 文件间 key 重叠，故一次要取所有 L0 文件 + 与之 key 范围重叠的全部 L1 文件一起归并（不能只挑一个）。L1+ 则选一个文件与下层 key 重叠的文件合并、写到下层。合并时按 SequenceNumber 保留可见版本、丢弃被覆盖的旧值与"低于最老快照"的墓碑。
