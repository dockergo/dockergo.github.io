...
读一个 block 的路径：先查 **block cache**（键=文件号+块偏移）→ 命中直接用（免磁盘、免解压）；未命中则经 **table cache**（缓存打开的 `BlockBasedTableReader`，`TableCache::FindTable`，`db/table_cache.cc:176`，首次打开走 `GetTableReader`，`db/table_cache.cc:91`，免重复解析 footer）读磁盘、解压、填入 block cache。block cache 缓存三类块：**data block**（真实 KV）、**index block**（定位）、**filter block**（布隆）。`cache_index_and_filter_blocks` 决定 index/filter 是进 block cache（可淘汰）还是常驻。table cache 本身也是一个容量受 `max_open_files` 约束的 LRU（缓存文件句柄 + reader）。
...
两种 block cache 实现：**LRUCache**（`cache/lru_cache.cc`，默认）——分片（sharded，减锁竞争）的 LRU，每片一把锁；查询 `LRUCacheShard::Lookup`（`cache/lru_cache.cc:430`）命中即把项从 LRU 链摘下（升为"被引用"），插入 `LRUCacheShard::Insert`（`cache/lru_cache.cc:548`，内部 `InsertItem` `:372`）超容量则从 LRU 尾部淘汰。**HyperClockCache**（`cache/clock_cache.cc`，`clock_cache.h:28` 起）——无锁的 clock 近似 LRU（用原子的 clock 指针 + 引用计数近似"最近使用"，Lookup/Release 常是单次原子加），高并发下吞吐更好但要求可估 key 大小。两者都**分片**：cache 切成 N 片，key 哈希到某片，各片独立锁/淘汰，降低多核锁竞争。容量满时按 LRU/clock 淘汰最久未用的块。

## 深化 · 缓存的内容与淘汰策略

![RocksDB 缓存内容与淘汰 · 三类块 + 优先级](RocksDB原理_缓存_03内容淘汰.svg)

block cache 里不只 data block：index/filter block 也占位（超大文件的它们很大）。RocksDB 支持**缓存优先级**（high/low）：index/filter 可设高优先级更不易被淘汰（读它们的收益大）。LRU 实现上，`LRUCacheShard` 把项在冷/热两条链间移动，超容量时 `EvictFromLRU`（`cache/lru_cache.cc:323`）从冷链尾部淘汰、被引用（`Ref`>0）的项经 `LRU_Remove`（`cache/lru_cache.cc:232`）暂离淘汰链、释放后 `LRU_Insert`（`cache/lru_cache.cc:256`）回插。跨 CF 的 MemTable 总内存则由 `WriteBufferManager`（`include/rocksdb/write_buffer_manager.h:37`）统管（可与 block cache 共用一个容量池）。`pin_l0_filter_and_index_blocks_in_cache` 把 L0 的 filter/index 钉住（L0 最常查）。读大表扫描时设 `fill_cache=false` 避免污染缓存。
