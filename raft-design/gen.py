...
#     家族 6 分布式协调/共识（可嵌入共识算法库 · Go · HashiCorp raft）：接口 × 能力域 × 时机。
#     全景 + 3 接口主线 + 8 支撑能力域。
# ===================================================================== #
MAINLINES = [
    ("Raft原理_全景主线框架", "pano", "◇", "全景主线框架",
     "可嵌入共识库：双维模型 · 总架构 · 库形态接口注入 · 依赖矩阵 · 三条贯穿声明"),

    ("Raft原理_接口_编程API与接口注入", "iface", "⚙", "编程 API 与接口注入",
     "NewRaft 注入 FSM/LogStore/StableStore/SnapshotStore/Transport 六件套 · Apply 写路径"),

    ("Raft原理_支撑_Leader选举", "support", "⚑", "Leader 选举",
     "三状态机 · term++ / RequestVote / PreVote · 随机超时 · 多数派授权 · 一任期一票"),
    ("Raft原理_支撑_日志复制", "support", "⇉", "日志复制",
     "AppendEntries · PrevLog 一致性检查 · nextIndex 快速回退 · pipeline · 强 Leader"),
    ("Raft原理_支撑_提交与应用", "support", "✔", "提交与应用",
     "matchIndex 中位数定 commitIndex · 本任期约束 · processLogs → FSM.Apply"),
    ("Raft原理_支撑_快照与日志压缩", "support", "▤", "快照与日志压缩",
     "阈值触发 FSM.Snapshot · TrailingLogs 保留 · InstallSnapshot 追平落后 follower"),
    ("Raft原理_支撑_成员变更", "support", "◫", "成员变更",
     "单步变更（非 joint）· Voter/Nonvoter · 串行化门 · 配置追加即生效"),
    ("Raft原理_支撑_持久状态", "support", "⛁", "持久状态",
     "StableStore 三键 CurrentTerm/LastVote · LogStore 日志 · 先落盘再改内存"),
    ("Raft原理_支撑_Transport与RPC", "support", "⇄", "Transport 与 RPC",
     "5 类 RPC 抽象 · pipeline 流水线 · 心跳快路径 · TCP/Inmem 可插拔"),
    ("Raft原理_支撑_可靠性与脑裂防护", "support", "◐", "可靠性与脑裂防护",
     "Leader Lease 退位 · 多数派选举+提交 · 单调任期 · VerifyLeader · PreVote"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接口主线 · 宿主如何用（编程 API + 5 接口注入）"),
    ("support", "支撑主线 · 共识内部（8 条能力域）"),
]
...
ARCH_HOTSPOTS = [
    ...
    # ② Raft 核心 6 盒
    (60, 248, 280, 106, "Raft原理_支撑_Leader选举"),        # 选举
    (370, 248, 280, 106, "Raft原理_支撑_日志复制"),         # 日志复制
    (680, 248, 280, 106, "Raft原理_支撑_提交与应用"),       # 提交与应用
    (60, 372, 280, 106, "Raft原理_支撑_成员变更"),          # 成员变更
    (370, 372, 280, 106, "Raft原理_支撑_可靠性与脑裂防护"), # 可靠性
    (680, 372, 280, 106, "Raft原理_支撑_Leader选举"),       # 三状态机
    # ③ 注入接口层 5 cell
    (52, 566, 172, 32, "Raft原理_支撑_提交与应用"),         # FSM → apply
    (236, 566, 172, 32, "Raft原理_支撑_持久状态"),          # LogStore
    (420, 566, 172, 32, "Raft原理_支撑_持久状态"),          # StableStore
    (604, 566, 172, 32, "Raft原理_支撑_快照与日志压缩"),    # SnapshotStore
    (788, 566, 200, 32, "Raft原理_支撑_Transport与RPC"),    # Transport
    # ④ 底层 IO band
    (40, 632, 450, 60, "Raft原理_支撑_持久状态"),           # 磁盘持久化
...
(function(){
  ...
  var saved=localStorage.getItem('raft-atlas-theme');
  ...
  function toggleTheme(){
    ...
    localStorage.setItem('raft-atlas-theme',cur);
