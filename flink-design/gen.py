...
_ap = argparse.ArgumentParser(description="生成 Flink 引擎交互式核心原理图谱(离线自包含 HTML)")
...
TAB_META = {
    "apiwalk":   ("◷", "原理详解 · 编程 API：两 API 汇入统一后端 → DataStream 累积 → Table/SQL Calcite", "原理"),
    "graphwalk": ("⬢", "原理详解 · 图变换：StreamGraph→JobGraph(链化)→ExecutionGraph(并行展开)", "原理"),
    "schedwalk": ("◱", "原理详解 · 调度部署：JobMaster/RM/TaskExecutor 三方 + slot sharing", "原理"),
    "taskwalk":  ("⚡", "原理详解 · 任务执行：Task→StreamTask→算子链 + mailbox 单线程", "原理"),
    "statewalk": ("▤", "原理详解 · 状态管理：keyed/operator + heap/RocksDB + key-group", "原理"),
...
THEMES = [
    # ── 接触面主线 · 用户可见 · Flink 是流优先统一计算引擎(原型 C):多编程 API,非 SQL 语句族 ──
    {"id": "api", "icon": "◷", "title": "编程 API(DataStream / Table-SQL)", "cat": "iface", "ord": 0,
     "desc": "多 API 接触面:命令式 DataStream + 声明式 Table/SQL,汇入统一执行管线。design 走查——API 全景(两 API 汇入统一后端)→ DataStream 转换累积成图 → Table/SQL Calcite planner 管线,附调优/误区/总纲",
     "tabs": ["apiwalk"]},

    # ── 支撑主线 · 引擎内部(7 条)· 严格以 design 走查为主干 ──
    {"id": "graph", "icon": "⬢", "title": "图变换", "cat": "support", "ord": 0,
     "desc": "编译能力域:design 走查——三级变换全景(StreamGraph→JobGraph→ExecutionGraph)→ 算子链化(forward+同 slot 合并 JobVertex)→ 并行展开(JobVertex→N 个 ExecutionVertex),附调优/误区/总纲",
     "tabs": ["graphwalk"]},
    {"id": "sched", "icon": "◱", "title": "调度与部署", "cat": "support", "ord": 1,
     "desc": "调度能力域:design 走查——三方角色(JobMaster/ResourceManager/TaskExecutor)→ slot 与 slot sharing(一条流水线共享槽位),附调优/误区/总纲",
     "tabs": ["schedwalk"]},
    {"id": "task", "icon": "⚡", "title": "任务执行", "cat": "support", "ord": 2,
     "desc": "执行能力域:design 走查——Task→StreamTask→算子链层次 → mailbox 单线程模型(记录/检查点/定时器串行,算子免加锁),附调优/误区/总纲",
     "tabs": ["taskwalk"]},
    {"id": "state", "icon": "▤", "title": "状态管理", "cat": "support", "ord": 3,
     "desc": "状态能力域:design 走查——两类状态(keyed vs operator)→ 状态后端(heap vs RocksDB)→ key-group 分片(rescale 基础),附调优/误区/总纲",
     "tabs": ["statewalk"]},
    {"id": "ckpt", "icon": "⬡", "title": "检查点与容错", "cat": "support", "ord": 4,
     "desc": "容错能力域(Flink 灵魂):design 走查——Chandy-Lamport 异步屏障快照 → 屏障对齐(对齐/非对齐)→ 快照增量 → 恢复与重分布,附调优/误区/总纲",
     "tabs": ["ckptwalk"]},
    {"id": "time", "icon": "◐", "title": "时间与窗口", "cat": "support", "ord": 5,
     "desc": "时间能力域:design 走查——事件时间 vs 处理时间 + 水位线 → 窗口(assigner/trigger/状态),附调优/误区/总纲",
     "tabs": ["timewalk"]},
    {"id": "net", "icon": "▨", "title": "网络与数据交换", "cat": "support", "ord": 6,
     "desc": "通信能力域:design 走查——生产/消费端(ResultPartition↔InputGate)→ credit-based 流控(背压不阻塞 TCP)→ 分区(keyed/forward/rebalance),附调优/误区/总纲",
     "tabs": ["netwalk"]},
    ...
    {"id": "overallarch", "icon": "◇", "title": "全景框架", "cat": "appendix", "ord": 1,
     ...
     "tabs": ["panowalk"]},
    {"id": "refolap", "icon": "▦", "title": "计算引擎对比", "cat": "appendix", "ord": 2,
     "desc": "Flink vs Spark / Kafka Streams / Storm —— 流优先统一计算引擎 vs 微批/其它流引擎的设计取舍横向对比",
     "tabs": ["compare"]},
]
...
_ARCH_SVG_B64 = open(
    os.path.join(_DESIGN_DIR, "Flink原理_总架构图.svg"),
    encoding="utf-8").read()
...
_ARCH_HOTSPOTS = [
    # API 接触面带(y=64)→ 编程 API
    (30,  64,  1020, 66, "api",   "编程 API(DataStream / Table-SQL)"),
    # 图变换行(y=144,3 盒 y178):StreamGraph/JobGraph/ExecutionGraph → 图变换
    (30,  144, 1020, 88, "graph", "图变换 · StreamGraph→JobGraph→ExecutionGraph"),
    # JobManager 行(y=246,3 盒 y280):JobMaster/RM → 调度;Checkpoint 盒 → 检查点
    (46,  280, 640,  46, "sched", "JobMaster / ResourceManager 调度"),
    (706, 280, 328,  46, "ckpt",  "CheckpointCoordinator 分布式快照"),
    # TaskManager 盒(y=350):slot 盒(task/mailbox)y384 x46 → 任务执行;网络盒 x536 → 网络
    (46,  384, 470,  112, "task", "Slot · 算子链 + mailbox 单线程"),
    (536, 384, 498,  112, "net",  "网络与数据交换 · credit 流控"),
    # 状态后端行(y=506)→ 状态管理
    (46,  506, 988,  52,  "state", "状态管理 · State Backend + key-group"),
]
_arch_hotspots_html = "\n".join(
...
_SVG_WALK_SPECS = {
    # ---- 接触面主线(1 条:编程 API)----
    "apiwalk": [("API 全景 · 两 API 汇入统一后端", "Flink原理_API_01全景.svg"),
                ("DataStream · 转换累积成图", "Flink原理_API_02DataStream.svg"),
                ("Table / SQL · Calcite planner 管线", "Flink原理_API_03TableSQL.svg")],
    # ---- 支撑主线(7 条)----
    "graphwalk": [("三级变换全景 · StreamGraph→JobGraph→ExecutionGraph", "Flink原理_图变换_01全景.svg"),
                  ("算子链化 · forward+同 slot 合并 JobVertex", "Flink原理_图变换_02链化.svg"),
                  ("并行展开 · JobVertex→N 个 ExecutionVertex", "Flink原理_图变换_03并行展开.svg")],
    "schedwalk": [("三方角色 · JobMaster/ResourceManager/TaskExecutor", "Flink原理_调度_01三方.svg"),
                  ("slot 与 slot sharing", "Flink原理_调度_02slot.svg")],
    "taskwalk": [("任务执行层次 · Task→StreamTask→算子链", "Flink原理_执行_01层次.svg"),
                 ("mailbox 单线程模型", "Flink原理_执行_02mailbox.svg")],
    "statewalk": [("两类状态 · keyed vs operator", "Flink原理_状态_01两类.svg"),
                  ("状态后端 · heap vs RocksDB", "Flink原理_状态_02后端.svg"),
                  ("key-group 分片 · rescale 基础", "Flink原理_状态_03keygroup.svg")],
    "ckptwalk": [("Chandy-Lamport 异步屏障快照", "Flink原理_检查点_01屏障快照.svg"),
                 ("屏障对齐 · 对齐 vs 非对齐", "Flink原理_检查点_02对齐.svg"),
                 ("快照 · 同步建资源+异步写+增量", "Flink原理_检查点_03快照增量.svg"),
                 ("恢复与重分布 · key-group 交集", "Flink原理_检查点_04恢复.svg")],
    "timewalk": [("事件时间 vs 处理时间 · 水位线", "Flink原理_时间_01水位线.svg"),
                 ("窗口 · assigner+trigger+状态", "Flink原理_时间_02窗口.svg")],
    ...
    "panowalk": [("双维模型 · 能力域 × 执行时机", "Flink原理_双维模型.svg"),
                 ...
                 ("依赖矩阵 · 接触面 × 能力域", "Flink原理_依赖矩阵.svg"),
                 ("能力域依赖关系图", "Flink原理_依赖关系图.svg")],
}
# 快速开始「上手总览」用独立复合渲染器 renderQsTour(总览 SVG + 5 步选择器 + 内容区),不走通用 SVG-walk
_QSTOUR_OVERVIEW_B64 = _design_b64("Flink上手_00总览.svg")  # 无此素材 → 空;quickstart 未挂载
...
_WALK_PROSE = {
    "apiwalk": "Flink原理_接触面_编程API.md",
    "graphwalk": "Flink原理_支撑_图变换.md",   "schedwalk": "Flink原理_支撑_调度与部署.md",
    "taskwalk": "Flink原理_支撑_任务执行.md",  "statewalk": "Flink原理_支撑_状态管理.md",
    "ckptwalk": "Flink原理_支撑_检查点容错.md", "timewalk": "Flink原理_支撑_时间与窗口.md",
    "netwalk": "Flink原理_支撑_网络与数据交换.md",
}
...
<title>Flink 引擎 · 交互式核心原理图谱</title>
<style>
:root{
...
<div id="loadingOverlay" role="status" aria-live="polite">
  <div class="lo-inner">
    ...
    <div class="lo-title">Flink Engine Atlas</div>
      ...
      <div class="nav-mode nav-arch active" id="navArch">
        <div class="arch-stage">
          <div class="arch-canvas">
            <img class="arch-img" src="data:image/svg+xml;base64,__ARCH_SVG_B64__" alt="Flink 总架构图" draggable="false"/>
            __ARCH_HOTSPOTS__
          </div>
        </div>
