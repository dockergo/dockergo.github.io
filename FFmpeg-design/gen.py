...
#   坐标系 = 该总架构 SVG 的 viewBox（_ARCH_VBW×_ARCH_VBH，派生自 SVG），生成期换算成百分比定位。
#   两条覆盖铁律：① 图上每个模块都有热区 ② 每条主线都被某热区覆盖（未覆盖者自动兜底成 chip）。
# ===================================================================== #
PANO_NAME = "FFmpeg原理_全景主线框架"
# 热区不再手写坐标：唯一真源是总架构 SVG 里埋的 data-tid rect，
# 由 _parse_arch_hotspots() 自动派生（viewBox + translate 层叠换算成绝对坐标）。
# 派生结果为 _ARCH_HOTSPOTS（6 元 tuple x,y,w,h,tid,lab），除数为 _ARCH_VBW/_ARCH_VBH。
...
# --- 架构热区自动派生：唯一真源 = 总架构 SVG 里埋的 data-tid rect --- #
...
_ARCH_HOTSPOTS, _ARCH_VBW, _ARCH_VBH = _parse_arch_hotspots(_read(ARCH_SVG_NAME))
...
def build_archnav():
    ...
    hots = []
    for (x, y, w, h, mid, _lab) in _ARCH_HOTSPOTS:
        if mid not in meta:
