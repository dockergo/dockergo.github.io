#!/usr/bin/env python3
"""FFmpeg-design 交互式核心原理图谱生成器（自包含 · 离线 · 双主题）。
...
用法：
  cd FFmpeg-design && python3 gen.py
...
_ap = argparse.ArgumentParser(description="生成 FFmpeg 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("FFMPEG_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("FFMPEG_DESIGN_OUT") or os.path.join(HERE, "index.html")

# ===================================================================== #
# 一、主线注册表 —— 唯一需随项目调整的数据块
#     新家族（多媒体处理管线 · FFmpeg）：元模式 = 接口 × 能力域 × 时机。
#     全景 + 1 接触面主线 + 8 支撑能力域。
# ===================================================================== #
MAINLINES = [
    ("FFmpeg原理_全景主线框架", "pano", "◇", "全景主线框架",
     "新家族多媒体处理管线：双维模型 · 总架构 · 依赖矩阵 · 依赖关系图 · 三条贯穿声明"),

    ("FFmpeg原理_接触面_命令行与API", "iface", "⚙", "接触面 · 命令行与 libav API",
     "两种接触面：命令行 ffmpeg(用户转码) + libav* C API(开发者集成) · CLI 是 API 的封装"),

    ("FFmpeg原理_支撑_库分层", "support", "▤", "库分层",
     "六库分层：avformat 容器/avcodec 编解码/avfilter 滤镜/avutil 公共/swscale 缩放/swresample 重采样 · 单向依赖"),
    ("FFmpeg原理_支撑_编解码管线", "support", "⛯", "编解码管线",
     "核心：demux→decode→filter→encode→mux · send/receive API 解耦(1:N/N:1)· remux 免解码"),
    ("FFmpeg原理_支撑_核心数据结构", "support", "▦", "核心数据结构",
     "AVPacket 压缩包(pts/dts/buf)/AVFrame 原始帧(data[8]/format)· 上下文 FormatContext/Stream/CodecContext"),
    ("FFmpeg原理_支撑_滤镜图", "support", "◫", "滤镜图",
     "AVFilterGraph 把 scale/crop/overlay 链成有向图 · buffersrc 喂帧→中间→buffersink 取帧 · decode 后 encode 前"),
    ("FFmpeg原理_支撑_容器格式", "support", "◈", "容器格式",
     "容器 vs 编码正交 · demuxer 拆/muxer 交织封 · remux(-c copy)换容器不换编码,无损极快"),
    ("FFmpeg原理_支撑_像素与采样格式", "support", "▧", "像素与采样格式",
     "AVPixelFormat(YUV/RGB·planar/packed)/AVSampleFormat · swscale/swresample 转换 · YUV420P 省数据"),
    ("FFmpeg原理_支撑_引用计数与硬件加速", "support", "◉", "引用计数与硬件加速",
     "AVBufferRef 引用计数零拷贝共享 packet/frame + 写时复制 · hwaccel GPU 解码/编码,帧留显存省 PCIe"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接触面主线 · 怎么用（命令行 + libav API）"),
    ("support", "支撑主线 · 库内部（7 条能力域）"),
]
...
# ===================================================================== #
PANO_NAME = "FFmpeg原理_全景主线框架"
ARCH_W, ARCH_H = 1080, 800  # 必须与 ARCH_SVG_NAME 的 viewBox 一致
# (x, y, w, h, 主线name) —— 一个模块可拆多行热区，一条主线可被多个区域指向
ARCH_HOTSPOTS = [
    # 标题条 → 全景总览
    (0, 0, 1080, 58, "FFmpeg原理_全景主线框架"),
    # ① 接触面 · 命令行与 API（顶部横条）
    (30, 92, 1020, 62, "FFmpeg原理_接触面_命令行与API"),
    # ② 五段管线主干（demux→decode→filter→encode→mux）
    (30, 176, 200, 92, "FFmpeg原理_支撑_容器格式"),      # demux/mux 端 = 容器
    (250, 176, 190, 92, "FFmpeg原理_支撑_编解码管线"),    # decode
    (460, 176, 190, 92, "FFmpeg原理_支撑_滤镜图"),        # filter
    (670, 176, 190, 92, "FFmpeg原理_支撑_编解码管线"),    # encode
    (880, 176, 170, 92, "FFmpeg原理_支撑_容器格式"),      # mux 端 = 容器
    # ③ 核心数据结构带（packet/frame 载体）
    (30, 292, 1020, 70, "FFmpeg原理_支撑_核心数据结构"),
    # ④~⑦ 能力域带（下排四）
    (30, 386, 245, 92, "FFmpeg原理_支撑_库分层"),
    (285, 386, 245, 92, "FFmpeg原理_支撑_像素与采样格式"),
    (540, 386, 245, 92, "FFmpeg原理_支撑_引用计数与硬件加速"),
    (795, 386, 255, 92, "FFmpeg原理_支撑_滤镜图"),
]
...
BRAND_TITLE = "一切知识皆索引"
BRAND_SUB = "FFmpeg"
HOME_DESC = ("FFmpeg 核心原理设计文档库的离线交互图谱——新家族（多媒体处理管线：把音视频的解封装→解码→滤镜→编码→封装"
             "做成一条 packet/frame 流经的管线，六库分层供 CLI 与 libav API 调用）。"
             "9 条主线、手绘原理图，全部回 FFmpeg 源码核实。点击项目总架构图任意模块即可下钻到对应主线。")
ARCH_SVG_NAME = "FFmpeg原理_总架构图.svg"
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="FFmpeg 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
(function(){
  ...
  var saved=localStorage.getItem('ffmpeg-atlas-theme');
  ...
  function toggleTheme(){
    ...
    localStorage.setItem('ffmpeg-atlas-theme',cur);
...
<header>
  ...
  <a href="https://github.com/FFmpeg/FFmpeg" target="_blank" rel="noopener" title="GitHub 源码仓库" style="display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M12 .5C5.7.5.5 5.7.5 12c0 5.1 3.3 9.4 7.9 10.9.6.1.8-.2.8-.6v-2c-3.2.7-3.9-1.4-3.9-1.4-.5-1.3-1.3-1.7-1.3-1.7-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1 1.8 2.7 1.3 3.4 1 .1-.8.4-1.3.7-1.6-2.6-.3-5.3-1.3-5.3-5.8 0-1.3.5-2.3 1.2-3.1-.1-.3-.5-1.5.1-3.1 0 0 1-.3 3.3 1.2a11.4 11.4 0 0 1 6 0C17.3 4.7 18.3 5 18.3 5c.6 1.6.2 2.8.1 3.1.8.8 1.2 1.8 1.2 3.1 0 4.5-2.7 5.5-5.3 5.8.4.4.8 1.1.8 2.2v3.3c0 .4.2.7.8.6 4.6-1.5 7.9-5.8 7.9-10.9C23.5 5.7 18.3.5 12 .5z"/></svg></a><a href="https://ffmpeg.org" target="_blank" rel="noopener" title="项目官网" style="display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><img src="data:image/svg+xml;base64," width="18" height="18" alt="官网" style="display:block"/></a><button id="themeBtn" title="切换主题" aria-label="切换主题" style="width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);background:var(--c-panel);color:var(--c-ink2);cursor:pointer;display:inline-grid;place-items:center;font-size:16px;flex:none">☾</button>
</header>
<div class="wrap">
  <div id="home">
    {archnav}
