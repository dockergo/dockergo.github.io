...
- **libavutil**:公共基础——AVFrame、AVBufferRef 引用计数、AVDictionary、数学/日志/错误码。所有库依赖它(最底层)。
- **libavcodec**:编解码——编码器/解码器,AVCodecContext、AVPacket、send/receive API。
- **libavformat**:容器——muxer/demuxer,AVFormatContext、AVStream,读写各种容器(mp4/mkv/ts…)。
- **libavfilter**:滤镜——AVFilterGraph,scale/crop/overlay 等滤镜链接成图处理帧。
- **libswscale**:图像缩放/像素格式转换(如 YUV↔RGB、分辨率缩放)。
- **libswresample**:音频重采样/采样格式/声道布局转换。
...
| 库 | 职责 | 关键结构/API |
|---|---|---|
| libavutil | 公共基础(最底层) | AVFrame、AVBufferRef、AVDictionary |
| libavcodec | 编解码 | AVCodecContext、AVPacket、send/receive |
| libavformat | 容器封装/解封装 | AVFormatContext、AVStream、muxer/demuxer |
| libavfilter | 滤镜图 | AVFilterGraph、buffersrc/buffersink |
| libswscale | 图像缩放/像素转换 | sws_scale、SwsContext |
| libswresample | 音频重采样 | swr_convert、SwrContext |
