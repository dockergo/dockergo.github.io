...
_ap = argparse.ArgumentParser(description="生成 ClickHouse 引擎交互式核心原理图谱(离线自包含 HTML)")
...
TAB_META = {
    ...
    "scwalk":   ("◈", "原理详解 · DDL 生命周期:CREATE/StorageFactory→Database 引擎→ALTER 两类路径→分布式 DDL", "原理"),
    "loadwalk": ("◈", "原理详解 · 写入生命周期:INSERT 建 Part→幂等去重→async insert→Mutation→轻量删除", "原理"),
    "qlwalk":   ("◈", "原理详解 · 查询执行全景:接入缓存→分析→规划→优化 pass→Pipeline→散射汇聚→两级聚合→Join", "原理"),
    "dclwalk":  ("◈", "原理详解 · 访问控制:判定链路→权限模型→认证→行级安全→配额约束→存储后端", "原理"),
    "stewalk":  ("◈", "原理详解 · 存储引擎:数据组织→表引擎族→Part 格式→稀疏主键 Marks→跳数索引→压缩→读写→Merge", "原理"),
    "optwalk":  ("◈", "原理详解 · 优化技术:优化全景→Analyzer passes→QueryPlan 优化→主键裁剪→Projection→缓存", "原理"),
    "execwalk": ("◈", "原理详解 · 执行引擎:Processors 框架→列式内存→Pipeline 构建→调度执行→算子族→向量化并行", "原理"),
    "txnwalk":  ("◈", "原理详解 · 复制与一致性:复制模型→复制队列→副本拉取→幂等 quorum→最终一致→znode 结构", "原理"),
    "metawalk": ("◈", "原理详解 · 元数据与协调:两层元数据→clickhouse-keeper→Raft→Database 元数据→DDL 队列→system 表", "原理"),
    "wgwalk":   ("◈", "原理详解 · 资源治理:资源管理全景→Settings 层级→Quota 限流→MemoryTracker→Workload 调度→并发溢写", "原理"),
    "tswalk":   ("◈", "原理详解 · 集群与自愈:拓扑分片→自愈线程→Part 校验恢复→RESTORE REPLICA→均衡 TTL move→parallel replicas", "原理"),
    "cpwalk":   ("◈", "原理详解 · 后台任务:调度全景→任务分配→Merge 选取放大→Mutation TTL→清理→线程池", "原理"),
    "deploysvg":("▭", "物理部署视图 · Shard/Replica/Keeper 布局 + 存算分离可选", "部署"),
    "depgraph": ("◫", "能力域依赖关系 · 8 条支撑主线的依赖与治理关系", "参考"),
    "deploywalk":("◈", "部署形态概览 · 存算一体 / 存算分离 / 冷热分离 / 湖仓查询", "部署"),
    "panowalk":("◇", "全景框架 · 双维模型 / 总架构图 / 物理部署 / 依赖矩阵 / 依赖关系 / 部署形态", "参考"),
...
# ClickHouse 版:4 接口主线 + 8 支撑能力域(按 CH 实情重映射)+ Appendix。
# 能力域与 Doris 差异:CH 无经典 2PC 事务 → 用「复制与一致性」;元数据依赖 keeper → 「元数据与协调」。
THEMES = [
    # ── 接口主线 · 用户可见(DDL / DML / DQL / DCL)· 严格以 clickhouse-design 走查为主干 ──
    {"id": "schemachange", "icon": "⇲", "title": "DDL 数据定义", "cat": "iface", "ord": 0,
     "desc": "数据定义:建库建表与在线变更。design 走查——DDL 生命周期总览 → CREATE 与 StorageFactory → Database 引擎对比 → ALTER 两类路径(元数据轻改 vs Mutation 重写)→ 分布式 DDL(ON CLUSTER),附调优/误区/总纲",
     "tabs": ["scwalk"]},
    {"id": "load", "icon": "⇥", "title": "DML 数据写入", "cat": "iface", "ord": 1,
     "desc": "数据写入:design 走查——写入生命周期总览 → INSERT 建 Part(排序建块)→ 幂等去重(block hash)→ async insert 攒批 → Mutation 整 Part 重写 → 轻量 DELETE 与 PatchParts,附调优/误区/总纲",
     "tabs": ["loadwalk"]},
    {"id": "qlife", "icon": "◷", "title": "DQL 数据查询", "cat": "iface", "ord": 2,
     "desc": "数据查询:design 走查——查询执行全景 → 生命周期总览 → 接入缓存 → 分析(AST→QueryTree)→ 规划(QueryTree→QueryPlan)→ QueryPlan 优化 → Pipeline 执行 → 分布式散射汇聚 → 两级聚合 → Join 算法 → 高并发点查,附调优/误区/总纲",
     "tabs": ["qlwalk"]},
    {"id": "dcl", "icon": "⛚", "title": "DCL 数据控制", "cat": "iface", "ord": 3,
     "desc": "数据控制:design 走查——权限判定链路 → 权限模型(User/Role/Grant)→ 认证方式 → 行级安全(RowPolicy)→ 配额约束(Quota)→ 访问控制存储后端,附调优/误区/总纲",
     "tabs": ["dclwalk"]},

    # ── 支撑主线 · 引擎内部(8 条)· 严格以 clickhouse-design 走查为主干 ──
    {"id": "storageformat", "icon": "▤", "title": "存储引擎", "cat": "support", "ord": 0,
     "desc": "存储引擎:design 走查——数据组织架构 → 表引擎族(MergeTree family)→ Part 物理格式(Wide/Compact)→ 稀疏主键与 Marks → 跳数索引 → 压缩编码链 → 读取路径 → 写入建 Part → Merge 合并 → Mutation,附调优/误区/总纲",
     "tabs": ["stewalk"]},
    {"id": "opttech", "icon": "✲", "title": "优化技术", "cat": "support", "ord": 1,
     "desc": "查询提速:design 走查——优化全景(四个位置)→ Analyzer passes → QueryPlan 优化 pass → 主键裁剪(PK/分区/PREWHERE)→ Projection 优化 → 缓存体系,附调优/误区/总纲",
     "tabs": ["optwalk"]},
    {"id": "execengine", "icon": "⚡", "title": "执行引擎", "cat": "support", "ord": 2,
     "desc": "运行时:design 走查——Processors 框架 → 列式内存模型(IColumn/Block)→ Pipeline 构建 → 调度执行(PipelineExecutor)→ 算子族 → 向量化与并行度,附调优/误区/总纲",
     "tabs": ["execwalk"]},
    {"id": "transaction", "icon": "⇄", "title": "复制与一致性", "cat": "support", "ord": 3,
     "desc": "复制模型:CH 无经典 2PC 事务。design 走查——复制模型总览 → 复制队列 → 副本拉取(fetch part)→ 写入幂等与 quorum → 一致性模型(最终一致)→ znode 结构(Keeper 路径),附调优/误区/总纲",
     "tabs": ["txnwalk"]},
    {"id": "metadata", "icon": "⬢", "title": "元数据与协调", "cat": "support", "ord": 4,
     "desc": "控制面:design 走查——两层元数据(本地 + Keeper)→ clickhouse-keeper 架构 → Raft 日志复制 → Database 元数据 → 分布式 DDL 队列 → system 自省表,附调优/误区/总纲",
     "tabs": ["metawalk"]},
    {"id": "workload", "icon": "◐", "title": "资源与负载管理", "cat": "support", "ord": 5,
     "desc": "资源治理:design 走查——资源管理全景 → Settings 层级 → Quota 限流 → 内存追踪(MemoryTracker)→ Workload 调度 → 并发与溢写,附调优/误区/总纲",
     "tabs": ["wgwalk"]},
    {"id": "tabletsched", "icon": "⬡", "title": "集群与自愈", "cat": "support", "ord": 6,
     "desc": "集群面:design 走查——集群拓扑与分片 → 副本自愈线程 → Part 校验与恢复 → RESTORE REPLICA(副本重建)→ 数据均衡与 TTL move → parallel replicas,附调优/误区/总纲",
     "tabs": ["tswalk"]},
    {"id": "compaction", "icon": "▨", "title": "后台任务", "cat": "support", "ord": 7,
     "desc": "后台维护:design 走查——后台调度全景 → 任务分配 → Merge 选取与放大 → Mutation 与 TTL → 清理任务 → 后台线程池,附调优/误区/总纲",
     "tabs": ["cpwalk"]},
...
# CH 总架构图 viewBox 1080×820,根坐标直接可用(无 <g translate> 偏移)。
# 对称架构:左侧一个 clickhouse-server 完整栈(查询路径行/存储带/后台带/贯穿带),右侧 Keeper + 对等节点。
import base64 as _b64
_ARCH_SVG_B64 = open(
    os.path.join(_DESIGN_DIR, "ClickHouse原理_总架构图.svg"),
    encoding="utf-8").read()
_ARCH_SVG_B64 = _b64.b64encode(_ARCH_SVG_B64.encode("utf-8")).decode("ascii")

# (x, y, w, h, theme_id, 标签) —— 取自 CH 总架构图各模块 rect;
# 顶部新增「外部数据生态层」后,节点主体整体下移 80(SVG 内 <g translate(0,80)>),故各 y 均 +80;
# 画布高度 900,百分比除数用 900。
_ARCH_HOTSPOTS = [
    # 查询/写入路径行(rect y=200 → +80=280):接入→分析/规划(优化)→执行
    (306, 280, 118, 52, "opttech",      "优化"),
    (436, 280, 118, 52, "execengine",   "执行"),
    # 存储引擎带(rect y=292 → +80=372)
    (46,  372, 600, 86, "storageformat","存储引擎"),
    # 后台任务带(rect y=412 → +80=492)
    (46,  492, 600, 72, "compaction",   "后台任务"),
    # 贯穿带:访问控制(DCL)/ 资源与负载(rect y=528 → +80=608)
    (60,  608, 180, 56, "dcl",          "访问控制 DCL"),
    (252, 608, 180, 56, "workload",     "资源与负载"),
    # Keeper 协调集群(rect y=140 → +80=220)→ 元数据与协调
    (690, 220, 360, 230, "metadata",    "元数据与协调 · Keeper"),
    # 对等节点 Shard×Replica(rect y=388 → +80=468)→ 集群与自愈 + 复制
    (690, 468, 360, 130, "tabletsched", "集群与自愈"),
    (710, 584, 322, 42,  "transaction", "复制 · Distributed"),
]
_arch_hotspots_html = "\n".join(
    ...
    'data-theme-id="{tid}" title="{lab} → {ttitle}"><span class="arch-hot-lab">{lab}</span></button>'.format(
        lp=x/1080*100, tp=y/900*100, wp=w/1080*100, hp=h/900*100,
...
_SVG_WALK_SPECS = {
    # ---- 接口主线(图序取自 clickhouse-design 各 prose 文档)----
    "scwalk": [("DDL 生命周期总览", "ClickHouse原理_DDL_01总览.svg"),
               ("CREATE 与 StorageFactory", "ClickHouse原理_DDL_02建表分发.svg"),
               ("Database 引擎对比", "ClickHouse原理_DDL_03Database引擎.svg"),
               ("ALTER 两类路径 · 元数据轻改 vs Mutation 重写", "ClickHouse原理_DDL_04ALTER路径.svg"),
               ("分布式 DDL 执行 · ON CLUSTER", "ClickHouse原理_DDL_05分布式DDL.svg")],
    "loadwalk": [("写入生命周期总览", "ClickHouse原理_DML_01总览.svg"),
                 ("INSERT Part 路径 · 排序建块", "ClickHouse原理_DML_02建Part.svg"),
                 ("幂等去重 · block hash 去重", "ClickHouse原理_DML_03幂等去重.svg"),
    ...
    "qlwalk": [("查询执行全景", "ClickHouse原理_DQL_09执行全景.svg"),
               ...
               ("分布式散射汇聚 · scatter/gather", "ClickHouse原理_DQL_07分布式.svg"),
               ("两级聚合", "ClickHouse原理_DQL_08两级聚合.svg"),
               ("Join 算法族", "ClickHouse原理_DQL_10Join算法.svg"),
               ("高并发点查", "ClickHouse原理_DQL_11点查.svg")],
    "dclwalk": [("权限判定链路", "ClickHouse原理_DCL_01判定链路.svg"),
                ("权限模型 · User/Role/Grant", "ClickHouse原理_DCL_02权限模型.svg"),
                ("认证 · 多种认证方式", "ClickHouse原理_DCL_03认证.svg"),
                ("行级安全 · RowPolicy", "ClickHouse原理_DCL_04行级安全.svg"),
                ("配额与约束 · Quota", "ClickHouse原理_DCL_05配额约束.svg"),
                ("访问控制存储后端", "ClickHouse原理_DCL_06存储后端.svg")],
    # ---- 支撑主线(8 条)----
    "stewalk": [("数据组织架构", "ClickHouse原理_存储_01数据组织.svg"),
                ("表引擎族 · MergeTree family", "ClickHouse原理_存储_02表引擎族.svg"),
                ("Part 物理格式 · Wide/Compact", "ClickHouse原理_存储_03Part结构.svg"),
                ("稀疏主键与 Marks", "ClickHouse原理_存储_04稀疏主键.svg"),
                ("跳数索引 · skip index", "ClickHouse原理_存储_05跳数索引.svg"),
                ("压缩编码链", "ClickHouse原理_存储_06压缩编码.svg"),
                ("读取路径 · Mark 定位", "ClickHouse原理_存储_07读取路径.svg"),
    ...
    "optwalk": [("优化全景 · 四个位置", "ClickHouse原理_优化_01全景.svg"),
                ("Analyzer passes", "ClickHouse原理_优化_02分析器.svg"),
                ("QueryPlan 优化 pass", "ClickHouse原理_优化_03计划优化.svg"),
                ("主键裁剪 · PK/分区/PREWHERE", "ClickHouse原理_优化_04主键裁剪.svg"),
                ("Projection 优化", "ClickHouse原理_优化_05Projection.svg"),
                ("缓存体系", "ClickHouse原理_优化_06缓存.svg")],
    "execwalk": [("Processors 框架", "ClickHouse原理_执行_01框架.svg"),
                 ("列式内存模型 · IColumn/Block", "ClickHouse原理_执行_02列式内存.svg"),
                 ("Pipeline 构建", "ClickHouse原理_执行_03Pipeline构建.svg"),
                 ("调度执行 · PipelineExecutor", "ClickHouse原理_执行_04调度.svg"),
                 ("算子族", "ClickHouse原理_执行_05算子族.svg"),
                 ("向量化与并行度", "ClickHouse原理_执行_06并行度.svg")],
    # 复制与一致性(替代 Doris 的事务主线 —— CH 无经典 2PC 事务)
    "txnwalk": [("复制模型总览", "ClickHouse原理_复制_01总览.svg"),
                ("复制队列 · replication queue", "ClickHouse原理_复制_02复制队列.svg"),
                ("副本拉取 · fetch part", "ClickHouse原理_复制_03副本拉取.svg"),
                ("写入幂等与 quorum", "ClickHouse原理_复制_04幂等quorum.svg"),
                ("一致性模型 · 最终一致", "ClickHouse原理_复制_05一致性.svg"),
                ("znode 结构 · Keeper 路径", "ClickHouse原理_复制_06znode结构.svg")],
    "metawalk": [("两层元数据 · 本地 + Keeper", "ClickHouse原理_元数据_01两层.svg"),
    ...
    # ---- 全局图(单图走查)----
    "deploysvg": [("物理部署视图 · Shard/Replica/Keeper", "ClickHouse原理_物理部署图.svg")],
    "depgraph": [("能力域依赖关系", "ClickHouse原理_依赖关系图.svg")],
    # 全景框架:统领全部原理的全局视图
    "panowalk": [("双维模型 · 能力域 × 执行时机", "ClickHouse原理_双维模型.svg"),
                 ("总架构图 · 位置即语义", "ClickHouse原理_总架构图.svg"),
                 ("物理部署视图", "ClickHouse原理_物理部署图.svg"),
                 ("依赖矩阵 · 接口主线 × 能力域", "ClickHouse原理_依赖矩阵.svg"),
                 ("能力域依赖关系", "ClickHouse原理_依赖关系图.svg"),
                 ("部署形态 · 物理布局对比", "ClickHouse原理_部署形态.svg")],
}
# 快速开始「上手总览」用独立复合渲染器 renderQsTour(总览 SVG + 5 步选择器 + 内容区),不走通用 SVG-walk。
# CH design 无「上手总览」素材 → 空 base64;quickstart 主题已从 THEMES 移除,该常量仅为占位不被渲染。
_QSTOUR_OVERVIEW_B64 = _design_b64("ClickHouse上手_00总览.svg")
...
# walk tid → prose md 文件(全局图无 prose);文件名对应 clickhouse-design/design/*.md
_WALK_PROSE = {
    "scwalk": "ClickHouse原理_DDL数据定义.md",  "loadwalk": "ClickHouse原理_DML数据写入.md",
    "qlwalk": "ClickHouse原理_DQL数据查询.md",  "dclwalk": "ClickHouse原理_DCL数据控制.md",
    "stewalk": "ClickHouse原理_支撑_存储引擎.md", "optwalk": "ClickHouse原理_支撑_优化技术.md",
    "execwalk": "ClickHouse原理_支撑_执行引擎.md", "txnwalk": "ClickHouse原理_支撑_复制与一致性.md",
    "metawalk": "ClickHouse原理_支撑_元数据与协调.md",  "wgwalk": "ClickHouse原理_支撑_资源与负载管理.md",
    "tswalk": "ClickHouse原理_支撑_集群与自愈.md", "cpwalk": "ClickHouse原理_支撑_后台任务.md",
}
...
<title>ClickHouse 引擎 · 交互式核心原理图谱</title>
<style>
:root{
...
<div id="loadingOverlay" role="status" aria-live="polite">
  <div class="lo-inner">
    ...
    <div class="lo-title">ClickHouse Engine Atlas</div>
...
<div id="app">
  <header>
    ...
    <a href="https://github.com/ClickHouse/ClickHouse" target="_blank" rel="noopener" title="GitHub 源码仓库" style="margin-left:auto;display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M12 .5C5.7.5.5 5.7.5 12c0 5.1 3.3 9.4 7.9 10.9.6.1.8-.2.8-.6v-2c-3.2.7-3.9-1.4-3.9-1.4-.5-1.3-1.3-1.7-1.3-1.7-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1 1.8 2.7 1.3 3.4 1 .1-.8.4-1.3.7-1.6-2.6-.3-5.3-1.3-5.3-5.8 0-1.3.5-2.3 1.2-3.1-.1-.3-.5-1.5.1-3.1 0 0 1-.3 3.3 1.2a11.4 11.4 0 0 1 6 0C17.3 4.7 18.3 5 18.3 5c.6 1.6.2 2.8.1 3.1.8.8 1.2 1.8 1.2 3.1 0 4.5-2.7 5.5-5.3 5.8.4.4.8 1.1.8 2.2v3.3c0 .4.2.7.8.6 4.6-1.5 7.9-5.8 7.9-10.9C23.5 5.7 18.3.5 12 .5z"/></svg></a><a href="https://clickhouse.com" target="_blank" rel="noopener" title="项目官网" style="display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><img src="data:image/svg+xml;base64," width="18" height="18" alt="官网" style="display:block"/></a><button class="theme-toggle" id="themeToggle" title="切换深色 / 浅色主题" aria-label="切换主题">
...
const COMPARE_SPEC={
  id:'compare', title:'架构对比 · ClickHouse 在 OLAP/数仓生态中的定位',
  note:'突出设计取舍(非优劣);同类对手看差异,异类引擎(Spark/Trino/DuckDB)看设计哲学。以 ClickHouse 为参照系:对称无中心、稀疏主键、不可变 Part、最终一致。',
  rowH:40,
  cols:[{h:'维度',w:140,accent:'var(--cv-scan)'},{h:'ClickHouse',w:180,accent:'var(--cv-ink)'},{h:'Doris',w:150,accent:'var(--cv-ink)'},{h:'StarRocks',w:130,accent:'var(--cv-ink)'},{h:'Spark / Trino / DuckDB',w:250,accent:'var(--cv-ink)'}],
  rows:[
    ['定位','单表极致 OLAP · 对称无中心','实时数仓 + 湖仓一体','实时数仓','Spark/Trino=联邦批处理;DuckDB=嵌入式分析'],
    ['集群架构','对等节点 + Keeper 仲裁(无 FE/BE)','FE/BE 主从','FE/BE(存算分离可选)','Spark/Trino=coordinator-worker;DuckDB=单进程'],
    ['执行模型','Processors 向量化 + Pipeline','MPP + Pipeline 向量化','MPP + 向量化','Spark=DAG/RDD;Trino=MPP;DuckDB=向量化 push'],
    ['优化器','Analyzer passes + QueryPlan 优化(CBO 较弱)','Nereids(Cascades CBO)','CBO','Spark Catalyst(RBO+CBO+codegen)'],
    ['存储','MergeTree 列存 · 不可变 Part · 稀疏主键','自有列存 + 湖仓外表','自有列存','Spark/Trino 无自有存储;DuckDB 有自有格式'],
    ['分布式','Distributed 表 scatter/gather(非 MPP 交换)','MPP fragment 交换','MPP 交换','Trino/Spark=stage 交换;DuckDB 单机'],
    ['更新/主键','弱:ReplacingMergeTree / 轻量删除 / Mutation 重写','MoW 主键模型(写时合并)','主键模型(强)','多为不可变 / 追加'],
    ['事务/一致性','无经典事务 · block hash 幂等 + 最终一致','2PC + MVCC 版本链','导入事务 + Publish','Spark/Trino=作业级;DuckDB=单机 ACID'],
    ['元数据','两层 · 本地 + clickhouse-keeper(Raft)','FE + BDB-JE + EditLog','FE / 元数据服务','各引擎 catalog 服务'],
    ['湖仓/外表','有限(外表引擎)','Hive/Iceberg/Hudi/Paimon','Iceberg/Hudi','Trino 湖仓最强;Spark 生态广']
  ]
};
...
(function hideLoadingOverlay(){
  ...
  /* 兜底：无论如何 4s 后必隐，防止任何异常把用户永久卡在加载页 */
