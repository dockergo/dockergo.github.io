...
会话过期若各节点各自判定，会因时钟/网络差异导致同一临时节点在不同副本上存亡不一致。ZK 让 **Leader 统一管理过期**（`LeaderSessionTracker`），过期通过 `closeSession` 事务经 ZAB 全序广播 —— 所有节点在同一 zxid 点一致地删除临时节点。这保证了"临时节点存在性"本身也是强一致的。心跳 `touchSession` 也汇聚到 leader 续期。
