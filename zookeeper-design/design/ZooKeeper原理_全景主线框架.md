...
| 主线 | 覆盖 | 核实锚点 |
|---|---|---|
| **客户端 API 与 znode** | create/getData/setData/getChildren/exists/delete/getACL/setACL/sync/multi；znode 路径模型（持久/临时/顺序/容器/TTL）；session；CLI | `zookeeper-server/.../server/FinalRequestProcessor.java`、`ZooDefs.java`、`DataTree.java` |

**支撑能力域（面向引擎内部，8 条）**

| 主线 | 一句话职责 | 核实锚点 |
|---|---|---|
| **ZAB 原子广播**（灵魂） | Proposal→Ack→Commit 广播 + Leader 选举/恢复；zxid 全序 | `server/quorum/{Leader,Follower,LearnerHandler,FastLeaderElection}.java` |
| **数据树 DataTree** | 内存层级 znode 树：nodes 映射、DataNode、ephemerals 索引 | `server/DataTree.java`、`DataNode.java`、`ZKDatabase.java` |
| **会话与临时节点** | session 心跳 + 分桶过期；ephemeral 节点绑 session | `server/SessionTrackerImpl.java`、`ExpiryQueue.java` |
| **Watch 机制** | 一次性触发通知；标准/持久/递归 watch | `server/watch/{WatchManager,WatchManagerOptimized}.java` |
| **事务日志与快照** | WAL（FileTxnLog）+ 模糊快照（FileSnap）+ 崩溃恢复 | `server/persistence/{FileTxnLog,FileSnap,FileTxnSnapLog}.java` |
| **请求处理链** | RequestProcessor 责任链；Leader/Follower/单机三种拓扑 | `server/{PrepRequestProcessor,SyncRequestProcessor,FinalRequestProcessor}.java`、`quorum/{CommitProcessor,ProposalRequestProcessor}.java` |
| **集群与 Quorum** | QuorumPeer 状态机、QuorumCnxManager 选举连接、QuorumVerifier 过半、reconfig | `server/quorum/{QuorumPeer,QuorumCnxManager}.java` |
| **ACL 权限** | scheme:id:perm，每 znode 独立 ACL；authprovider 插件 | `server/ZooKeeperServer.java:2065`、`server/auth/*`、`ZooDefs.java` |
...
- **写全序、读本地**：任何改数据的操作都编码成事务、经 ZAB 广播到过半并按 zxid 全序提交（线性化写）；读默认直接查本地内存树、不经共识（故读吞吐可随 follower/observer 扩展，但可能读到略旧值——需最新用 `sync` + read）。这是 ZK "读多写少" 场景的立身之本。
- **zxid 是一切顺序的锚**：64 位事务 id = **epoch（高 32，第几任 leader）| counter（低 32，任内递增）**（`util/ZxidUtils.java:29`）。选举比 zxid、恢复比 zxid、提交按 zxid 递增、快照记 zxid——全序由它统一。
- **全量内存 + 磁盘只为恢复**：整棵 znode 树常驻内存（读写都在内存树上），磁盘上的 TxnLog+快照只用于崩溃恢复与新成员同步。故 ZK 存的是**小数据**（配置/元数据/协调状态），不是海量业务数据——要正确与低延迟协调，而非大吞吐存储。
