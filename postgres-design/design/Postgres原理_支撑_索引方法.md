...
索引项只存 key→TID，取真数据要"回堆表"，三种形态代价迥异：**Index Scan** 逐个 TID 随机回堆表（每命中一行一次可能的随机 IO，选择率低时才划算）；**Bitmap Heap Scan**（`BitmapHeapNext`，`executor/nodeBitmapHeapscan.c:174`）先把索引给出的 TID 攒成按页排序的位图、再顺序回表，把大量随机 IO 摊成顺序 IO，适合中等选择率或多个索引 AND/OR 组合（BRIN 的 `bringetbitmap`（`access/brin/brin.c:572`）只能产位图、天然走这条路）；**Index-Only Scan** 若查询列全在索引里、且经 Visibility Map 确认整页全可见，则**完全免回表**、最快。DML 侧每个索引都要维护（`gininsert`（`access/gin/gininsert.c:865`）、`gistinsert`（`access/gist/gist.c:166`）、`brininsert`（`access/brin/brin.c:349`）、`hashinsert`（`access/hash/hash.c:271`）各按其结构插入），这正是"索引越多写越慢"的根源。
