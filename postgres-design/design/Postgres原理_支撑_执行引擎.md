...
火山接口下每类算子是一个 ExecProc 实现，按 pull 语义各司其职：**SeqScan**（`ExecSeqScan`，`executor/nodeSeqscan.c:119`，顺序取堆表行、按快照过滤）、**Sort**（`ExecSort`，`executor/nodeSort.c:50`，首次 pull 时把下游取尽排好、之后逐行吐出，超 work_mem 落盘外部归并）、**Agg**（`ExecAgg`，`executor/nodeAgg.c:2247`，HashAgg 建哈希表 / GroupAgg 靠有序分组）、**HashJoin**（`ExecHashJoin`，`executor/nodeHashjoin.c:802`，先 build 小表哈希、再 probe 大表，超内存分批）、**BitmapHeapScan**（`BitmapHeapNext`，`executor/nodeBitmapHeapscan.c:174`，先收索引 TID 位图、再按页有序回堆表）。Sort/Agg/HashJoin 是三个会"物化/落盘"的阻塞点，也是 work_mem 的主要消费者。

并行落地由 leader 侧 `LaunchParallelWorkers`（`access/transam/parallel.c:583`）经 postmaster fork 出 worker 进程、`ExecParallelCreateReaders`（`executor/execParallel.c:944`）建起共享内存 tuple queue 的读端，worker 各跑一份 plan 子树扫不同块、结果回传 leader 的 Gather 汇聚。因 worker 是独立进程，计划片段与参数经共享内存序列化传递，故并非所有算子都并行安全。
