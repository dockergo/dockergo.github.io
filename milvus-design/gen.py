...
_ap = argparse.ArgumentParser(description="生成 Milvus 引擎交互式核心原理图谱(离线自包含 HTML)")
...
TAB_META = {
    "apiwalk":   ("◷", "原理详解 · 向量 API：insert/delete/upsert + search(ANN)/query", "原理"),
    "searchwalk":("◉", "原理详解 · 向量检索：knowhere HNSW/IVF/DiskANN + bitset 预过滤", "原理"),
    "archwalk":  ("◇", "原理详解 · 分布式架构：MixCoord + 存算分离 + vchannel 分片", "原理"),
    "writewalk": ("⇥", "原理详解 · 写入路径：Proxy→streaming WAL→binlog 日志驱动", "原理"),
    "segwalk":   ("▤", "原理详解 · 段生命周期：growing→sealed→flushed→indexed + compaction", "原理"),
    "timewalk":  ("⬡", "原理详解 · 一致性：TSO + guaranteeTs 等 tSafe 快照", "原理"),
    "metawalk":  ("⬢", "原理详解 · 元数据：etcd 存 schema/段/索引元 + MVCC", "原理"),
    "panowalk":  ("◇", "全景框架 · 双维模型 → 总架构 → 依赖矩阵 → 依赖关系", "整体架构"),
    "compare":   ("▦", "向量库对比 · Milvus vs Pinecone/Weaviate/pgvector 设计取舍", "整体架构"),
}
...
THEMES = [
    # ── 接触面主线 · 用户可见 · Milvus 是分布式向量数据库(新家族):向量 CRUD + ANN 检索 ──
    {"id": "api", "icon": "◷", "title": "向量 CRUD 与 ANN 检索", "cat": "iface", "ord": 0,
     "desc": "接触面:insert/delete/upsert + search(topK 最近邻)+ query。design 走查——数据操作(insert/delete/upsert)→ 检索(search ANN / query)→ collection DDL 与 schema,附调优/误区/总纲",
     "tabs": ["apiwalk"]},

    # ── 支撑主线 · 引擎内部(6 条)· 严格以 design 走查为主干 ──
    {"id": "search", "icon": "◉", "title": "向量索引与检索", "cat": "support", "ord": 0,
     "desc": "核心能力域(灵魂):design 走查——向量索引类型(knowhere:HNSW/IVF/DiskANN/FLAT)→ 段上 ANN 检索执行 → 标量过滤 bitset 预过滤,附调优/误区/总纲",
     "tabs": ["searchwalk"]},
    {"id": "arch", "icon": "◇", "title": "分布式架构", "cat": "support", "ord": 1,
     "desc": "架构能力域:design 走查——节点拓扑(MixCoord + 工作节点)→ 存算分离(Proxy/QueryNode/DataNode)→ collection 分片(vchannel+partition+segment),附调优/误区/总纲",
     "tabs": ["archwalk"]},
    {"id": "write", "icon": "⇥", "title": "写入路径", "cat": "support", "ord": 2,
     "desc": "写入能力域:design 走查——写入全景(Proxy→WAL→binlog)→ DML 消息流(日志驱动)→ WriteBuffer 与 binlog 生成,附调优/误区/总纲",
     "tabs": ["writewalk"]},
    {"id": "seg", "icon": "▤", "title": "段与生命周期", "cat": "support", "ord": 3,
     "desc": "存储能力域:design 走查——段状态机(growing→sealed→flushed→indexed)→ Flush → Compaction → 异步索引构建,附调优/误区/总纲",
     "tabs": ["segwalk"]},
    {"id": "time", "icon": "⬡", "title": "一致性与时间", "cat": "support", "ord": 4,
     "desc": "一致性能力域:design 走查——TSO 混合逻辑时钟 → 一致性级别与 guaranteeTs → tSafe 时间闸门(MVCC),附调优/误区/总纲",
     "tabs": ["timewalk"]},
    {"id": "meta", "icon": "⬢", "title": "元数据", "cat": "support", "ord": 5,
     "desc": "元数据能力域:design 走查——元数据存储(etcd/tikv)→ 元数据内容与 MVCC,附调优/误区/总纲",
     "tabs": ["metawalk"]},

    # ── Appendix · 参考 ──
    {"id": "overallarch", "icon": "◇", "title": "全景框架", "cat": "appendix", "ord": 1,
     "desc": "全景框架:双维模型(能力域×执行时机)· 总架构图(Proxy/MixCoord/存算分离/对象存储)· 依赖矩阵 · 能力域依赖关系",
     "tabs": ["panowalk"]},
    {"id": "refolap", "icon": "▦", "title": "向量库对比", "cat": "appendix", "ord": 2,
     "desc": "Milvus vs Pinecone / Weaviate / pgvector —— 分布式 ANN 向量库 vs 托管/嵌入式向量检索的设计取舍横向对比",
     "tabs": ["compare"]},
]
...
_ARCH_SVG_B64 = open(
    os.path.join(_DESIGN_DIR, "Milvus原理_总架构图.svg"),
    encoding="utf-8").read()
...
_ARCH_HOTSPOTS = [
    # API 带(y=64)→ 接触面
    (30,  64,  1020, 46, "api",   "向量 CRUD + ANN 检索 API"),
    # Proxy 带(y=124)→ 分布式架构(接入层)
    (30,  124, 1020, 40, "arch",  "Proxy 接入层"),
    # 写入路径盒(y=178 左)→ 写入路径
    (30,  178, 340,  330, "write", "写入路径(日志驱动)"),
    # 读检索盒(y=178 中):ANN 检索行 → 向量索引检索;一致性行 → 一致性与时间
    (386, 178, 344,  158, "search", "读/检索路径 · ANN(knowhere)"),
    (402, 396, 312,  52,  "time",  "一致性 · 等 tSafe 追 guaranteeTs"),
    (402, 456, 312,  40,  "search","growing/未建索引段暴力搜"),
    # MixCoord 盒(y=178 右):root(TSO)→ 一致性;data(段)→ 段生命周期;etcd → 元数据
    (762, 228, 272,  56, "time",  "RootCoord · TSO 时间戳"),
    (762, 292, 272,  52, "seg",   "DataCoord · 段/compaction/索引"),
    (762, 472, 272,  28, "meta",  "元数据存 etcd / tikv"),
    # 对象存储带(y=524)→ 段与生命周期(数据持久层)
    (30,  524, 1020, 56, "seg",   "对象存储 · 段 binlog 共享持久层"),
]
_arch_hotspots_html = "\n".join(
...
_SVG_WALK_SPECS = {
    "apiwalk": [("数据操作 · insert/delete/upsert", "Milvus原理_API_01数据操作.svg"),
                ("检索 · search(ANN)/ query", "Milvus原理_API_02检索.svg"),
                ("collection DDL 与 schema", "Milvus原理_API_03DDL.svg")],
    "searchwalk": [("向量索引类型 · knowhere 分发", "Milvus原理_检索_01索引类型.svg"),
                   ("段上 ANN 检索执行", "Milvus原理_检索_02段检索.svg"),
                   ("标量过滤 · bitset 预过滤", "Milvus原理_检索_03预过滤.svg")],
    "archwalk": [("节点拓扑 · MixCoord + 工作节点", "Milvus原理_架构_01拓扑.svg"),
                 ("存算分离 · Proxy/QueryNode/DataNode", "Milvus原理_架构_02存算分离.svg"),
                 ("collection 分片 · vchannel+partition+segment", "Milvus原理_架构_03分片.svg")],
    "writewalk": [("写入全景 · Proxy→WAL→binlog", "Milvus原理_写入_01全景.svg"),
                  ("DML 消息流 · 日志驱动", "Milvus原理_写入_02消息流.svg"),
                  ("WriteBuffer 与 binlog 生成", "Milvus原理_写入_03binlog.svg")],
    "segwalk": [("段状态机 · growing→sealed→flushed→indexed", "Milvus原理_段_01状态机.svg"),
                ("Flush · growing→sealed→binlog", "Milvus原理_段_02Flush.svg"),
                ("Compaction · 合段 + 应用删除", "Milvus原理_段_03compaction.svg"),
                ("异步索引构建", "Milvus原理_段_04索引构建.svg")],
    "timewalk": [("TSO · 混合逻辑时钟", "Milvus原理_时间_01TSO.svg"),
                 ("一致性级别与 guaranteeTs", "Milvus原理_时间_02一致性级别.svg"),
                 ("tSafe · 读快照时间闸门(MVCC)", "Milvus原理_时间_03tSafe.svg")],
    "metawalk": [("元数据存储 · etcd / tikv", "Milvus原理_元数据_01存储.svg"),
                 ("元数据内容与 MVCC", "Milvus原理_元数据_02内容.svg")],
    "panowalk": [("双维模型 · 能力域 × 执行时机", "Milvus原理_双维模型.svg"),
                 ("总架构图 · Proxy/MixCoord/存算分离", "Milvus原理_总架构图.svg"),
                 ("依赖矩阵 · 接触面 × 能力域", "Milvus原理_依赖矩阵.svg"),
                 ("能力域依赖关系图", "Milvus原理_依赖关系图.svg")],
}
# 快速开始「上手总览」用独立复合渲染器 renderQsTour(总览 SVG + 5 步选择器 + 内容区),不走通用 SVG-walk
_QSTOUR_OVERVIEW_B64 = _design_b64("Milvus上手_00总览.svg")  # 无此素材 → 空;quickstart 未挂载
...
_WALK_PROSE = {
    "apiwalk": "Milvus原理_接触面_向量CRUD与检索API.md",
    "searchwalk": "Milvus原理_支撑_向量索引与检索.md", "archwalk": "Milvus原理_支撑_分布式架构.md",
    "writewalk": "Milvus原理_支撑_写入路径.md",  "segwalk": "Milvus原理_支撑_段与生命周期.md",
    "timewalk": "Milvus原理_支撑_一致性与时间.md", "metawalk": "Milvus原理_支撑_元数据.md",
}
...
<title>Milvus 引擎 · 交互式核心原理图谱</title>
<style>
:root{
...
<div id="loadingOverlay" role="status" aria-live="polite">
  <div class="lo-inner">
    ...
    <div class="lo-title">Milvus Engine Atlas</div>
      ...
      <div class="nav-mode nav-arch active" id="navArch">
        <div class="arch-stage">
          <div class="arch-canvas">
            <img class="arch-img" src="data:image/svg+xml;base64,__ARCH_SVG_B64__" alt="Milvus 总架构图" draggable="false"/>
            __ARCH_HOTSPOTS__
          </div>
        </div>
...
const COMPARE_SPEC={
  id:'compare', title:'向量库对比 · Milvus 在向量检索生态中的定位',
  note:'突出设计取舍(非优劣);分布式自建 vs 托管 vs 嵌入关系库的向量能力分野。',
  rowH:40,
  cols:[{h:'维度',w:150,accent:'var(--cv-scan)'},{h:'Milvus',w:200,accent:'var(--cv-ink)'},{h:'Pinecone(托管)',w:170,accent:'var(--cv-ink)'},{h:'Weaviate',w:160,accent:'var(--cv-ink)'},{h:'pgvector',w:150,accent:'var(--cv-ink)'}],
  rows:[
    ['形态','分布式自建/云','全托管 SaaS','分布式自建/云','PostgreSQL 扩展'],
    ['架构','存算分离+日志驱动','托管黑盒','存算一体','嵌入关系库'],
    ['索引','knowhere:HNSW/IVF/DiskANN/FLAT','专有(HNSW 系)','HNSW','HNSW/IVFFlat'],
    ['规模','十亿级+,水平扩','大规模托管','中大规模','受 PG 单机/分片限'],
    ['一致性','全局 TSO + 级别可选','托管保证','最终一致','PG 事务(强)'],
    ['过滤','标量 bitset 预过滤','元数据过滤','混合搜索','SQL WHERE(原生)'],
    ['写入','streaming WAL 日志驱动','托管','实时','PG 事务写'],
    ['运维','较重(多组件)','零运维','中等','极轻(装扩展)'],
    ['适合','超大规模专用向量检索','快速起步免运维','图+向量混合','已有 PG 少量向量'],
    ['可借鉴','—','托管易用性','混合搜索','复用关系库事务']
  ]
};
