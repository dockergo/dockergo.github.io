#!/usr/bin/env python3
"""milvus-design 交互式核心原理图谱生成器（自包含 · 离线 · 双主题）。
...
用法：
  cd milvus-design && python3 gen.py
...
_ap = argparse.ArgumentParser(description="生成 Milvus 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("MILVUS_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("MILVUS_DESIGN_OUT") or os.path.join(HERE, "index.html")

# ===================================================================== #
# 一、主线注册表 —— 唯一需随项目调整的数据块
#     家族（分布式向量数据库 · 存算分离 · streaming WAL 日志驱动 · knowhere ANN · 全局 TSO）。
#     全景 + 1 接触面主线 + 6 支撑能力域。
# ===================================================================== #
MAINLINES = [
    ("Milvus原理_全景主线框架", "pano", "◇", "全景主线框架",
     "分布式向量数据库：双维模型 · 总架构 · 依赖矩阵 · 三条贯穿声明"),

    ("Milvus原理_接触面_向量CRUD与检索API", "iface", "⚙", "向量 CRUD 与检索 API",
     "collection/partition DDL · insert/delete/upsert · search(ANN) · query · 经 Proxy 接入"),

    ("Milvus原理_支撑_分布式架构", "support", "◫", "分布式架构",
     "存算分离节点分工：MixCoord 统管 · Proxy 接入 · QueryNode 读 · DataNode 写 · 对象存储"),
    ("Milvus原理_支撑_写入路径", "support", "⇄", "写入路径",
     "日志驱动：Proxy→streaming WAL(按 vchannel 分片)→消费→WriteBuffer→binlog→对象存储"),
    ("Milvus原理_支撑_段与生命周期", "support", "▦", "段与生命周期",
     "growing(内存可变)→sealed(封存)→flushed(落 binlog)→indexed；compaction 合段应用删除"),
    ("Milvus原理_支撑_向量索引与检索", "support", "◉", "向量索引与检索",
     "灵魂：ANN 检索 · knowhere HNSW/IVF/DiskANN/FLAT · segcore C++ · 标量 bitset 预过滤"),
    ("Milvus原理_支撑_一致性与时间", "support", "◷", "一致性与时间",
     "全局 TSO(HLC) · 一致性级别 Strong/Bounded/Eventually/Session · guaranteeTs 等 tSafe"),
    ("Milvus原理_支撑_元数据", "support", "▤", "元数据",
     "全局状态持久化：collection schema/段信息/索引元/位点 · 存 etcd(或 tikv)"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接触面主线 · 用户可见（向量 CRUD 与 ANN 检索 API）"),
    ("support", "支撑主线 · 系统内部（6 条能力域）"),
]
...
# ===================================================================== #
PANO_NAME = "Milvus原理_全景主线框架"
ARCH_W, ARCH_H = 1080, 800  # 必须与 ARCH_SVG_NAME 的 viewBox 一致
# (x, y, w, h, 主线name) —— 一个模块可拆多行热区，一条主线可被多个区域指向
ARCH_HOTSPOTS = [
    # 顶部标题 → 全景总览
    (30, 14, 720, 44, "Milvus原理_全景主线框架"),
    # API 条 → 接触面
    (30, 64, 1020, 46, "Milvus原理_接触面_向量CRUD与检索API"),
    # Proxy 接入层 → 分布式架构
    (30, 124, 1020, 40, "Milvus原理_支撑_分布式架构"),
    # ① 写入路径（左列）
    (30, 178, 340, 34, "Milvus原理_支撑_写入路径"),   # 标题
    (46, 212, 308, 50, "Milvus原理_支撑_写入路径"),   # streaming WAL
    (46, 270, 308, 50, "Milvus原理_支撑_写入路径"),   # StreamingNode/DataNode 消费
    (46, 328, 308, 50, "Milvus原理_支撑_写入路径"),   # WriteBuffer
    (46, 386, 308, 50, "Milvus原理_支撑_写入路径"),   # binlog + delta log
    (46, 444, 308, 52, "Milvus原理_支撑_段与生命周期"),  # 段 growing→sealed
    # ② 读/检索路径（中列）
    (386, 178, 344, 34, "Milvus原理_支撑_向量索引与检索"),  # 标题
    (402, 212, 312, 50, "Milvus原理_支撑_向量索引与检索"),  # 加载 sealed + knowhere 索引
    (402, 270, 312, 58, "Milvus原理_支撑_向量索引与检索"),  # ANN 向量检索(segcore)
    (402, 336, 312, 52, "Milvus原理_支撑_向量索引与检索"),  # 标量 bitset 预过滤
    (402, 396, 312, 52, "Milvus原理_支撑_一致性与时间"),    # 等 tSafe 追上 guaranteeTs
    (402, 456, 312, 40, "Milvus原理_支撑_段与生命周期"),    # growing/未建索引段 暴力搜
    # ③ MixCoord（右列）
    (746, 178, 304, 46, "Milvus原理_支撑_分布式架构"),      # MixCoord 标题
    (762, 228, 272, 26, "Milvus原理_支撑_元数据"),          # RootCoord · DDL 元数据
    (762, 254, 272, 30, "Milvus原理_支撑_一致性与时间"),    # RootCoord · TSO 时间戳
    (762, 292, 272, 52, "Milvus原理_支撑_段与生命周期"),    # DataCoord · 段/flush/索引
    (762, 352, 272, 52, "Milvus原理_支撑_分布式架构"),      # QueryCoord · 副本/负载均衡
    (762, 412, 272, 52, "Milvus原理_支撑_写入路径"),        # StreamingCoord · PChannel 分配
    (762, 472, 272, 28, "Milvus原理_支撑_元数据"),          # 元数据存 etcd/tikv
    # ④ 对象存储（存算分离共享持久层）
    (30, 524, 1020, 56, "Milvus原理_支撑_分布式架构"),
    # ⑤ 位置即语义 → 全景
    (30, 596, 1020, 52, "Milvus原理_全景主线框架"),
    # ⑥ 贯穿层 timestamp → 一致性与时间
    (30, 662, 1020, 44, "Milvus原理_支撑_一致性与时间"),
    # ⑦ 三条贯穿声明 → 全景
    (30, 720, 1020, 66, "Milvus原理_全景主线框架"),
]
# 没有独立架构区域、需底部 chip 兜底的主线（本项目 8 主线全部落在图上 → 空）
ARCH_ALWAYS_CHIP = []

BRAND_TITLE = "Milvus 核心原理"
BRAND_SUB = "Milvus"
HOME_DESC = ("Milvus 核心原理设计文档库的离线交互图谱——分布式向量数据库（存算分离 · streaming WAL 日志驱动 · "
             "knowhere ANN 检索 · 全局 TSO 一致性；数据在对象存储）。8 条主线、25 张手绘原理图，全部回本地源码核实。"
             "点击项目总架构图任意模块即可下钻到对应主线。")
ARCH_SVG_NAME = "Milvus原理_总架构图.svg"
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="Milvus 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
(function(){
  ...
  var saved=localStorage.getItem('milvus-atlas-theme');
  ...
  function toggleTheme(){
    ...
    localStorage.setItem('milvus-atlas-theme',cur);
...
<header>
  ...
  <a href="https://github.com/milvus-io/milvus" target="_blank" rel="noopener" title="GitHub 源码仓库" style="display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M12 .5C5.7.5.5 5.7.5 12c0 5.1 3.3 9.4 7.9 10.9.6.1.8-.2.8-.6v-2c-3.2.7-3.9-1.4-3.9-1.4-.5-1.3-1.3-1.7-1.3-1.7-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1 1.8 2.7 1.3 3.4 1 .1-.8.4-1.3.7-1.6-2.6-.3-5.3-1.3-5.3-5.8 0-1.3.5-2.3 1.2-3.1-.1-.3-.5-1.5.1-3.1 0 0 1-.3 3.3 1.2a11.4 11.4 0 0 1 6 0C17.3 4.7 18.3 5 18.3 5c.6 1.6.2 2.8.1 3.1.8.8 1.2 1.8 1.2 3.1 0 4.5-2.7 5.5-5.3 5.8.4.4.8 1.1.8 2.2v3.3c0 .4.2.7.8.6 4.6-1.5 7.9-5.8 7.9-10.9C23.5 5.7 18.3.5 12 .5z"/></svg></a><a href="https://milvus.io" target="_blank" rel="noopener" title="项目官网" style="display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><img src="data:image/svg+xml;base64," width="18" height="18" alt="官网" style="display:block"/></a><button id="themeBtn" title="切换主题" aria-label="切换主题" style="width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);background:var(--c-panel);color:var(--c-ink2);cursor:pointer;display:inline-grid;place-items:center;font-size:16px;flex:none">☾</button>
</header>
<div class="wrap">
  <div id="home">
    {archnav}
