...
# 架构热区从总架构 SVG 里带 data-tid 的 rect 自动派生（单一真源：SVG）。
# viewBox 宽高也从 SVG 自动读取（_ARCH_VBW/_ARCH_VBH），不再手填 ARCH_W/ARCH_H。
# 注：RootCoord 在 SVG 里被有意拆成"元数据"与"一致性与时间"两个 data-tid rect，
#     保留原手工的双下钻粒度（单盒视觉，两个透明热区叠加）。
...
def build_archnav():
    ...
    hots = []
    for (x, y, w, h, mid, lab) in _ARCH_HOTSPOTS:
        if mid not in meta:
    ...
    covered = {t[4] for t in _ARCH_HOTSPOTS}
...
_ARCH_SVG_TEXT = _read(ARCH_SVG_NAME)
...
def build_html():
    ...
    covered = {t[4] for t in _ARCH_HOTSPOTS} | set(ARCH_ALWAYS_CHIP)
