...
> **定位**：属"元数据能力域"。管全局状态的持久化:collection schema、段信息、索引元、位点——存 etcd(或 tikv)。被所有主线依赖,是 DDL/段/索引状态的最终落点。源码基准 **Milvus(6ca0350944)**(`internal/metastore/`、`internal/kv/`)。
...
- **后端选择**(`pkg/util/paramtable/service_param.go:544`):`MetaStoreType` 默认 `etcd`,可选 `tikv`(`pkg/util/constant.go:31`)。
- **catalog 抽象**:RootCoord 的 catalog 包一个 `kv.TxnKV`(`internal/metastore/kv/rootcoord/kv_catalog.go`),结构化 key:`CollectionMeta/{id}`、`PartitionMeta/{cid}/{pid}`、`FieldMeta/…`、alias、function。DataCoord catalog 包 `kv.MetaKv` 管段/binlog 路径。
- **初始化**(`root_coord.go:390`):按类型建 metaKV(`etcdkv.NewEtcdKV` 或 `tikv.NewTiKV`),catalog 建在其上;支持嵌入式/外部 etcd、TLS。
- **事务操作**:`MultiSave`/`MultiSaveAndRemove` 带 predicate(`embed_etcd_kv.go:400`)——原子多键更新。
...
| 结构 | 定义 | 职责 |
|---|---|---|
| MetaStoreType | `pkg/util/paramtable/service_param.go:544` | etcd(默认)/ tikv |
| RootCoord catalog | `internal/metastore/kv/rootcoord/kv_catalog.go` | collection/partition/field 元 |
| DataCoord catalog | `internal/metastore/kv/datacoord/kv_catalog.go` | 段/binlog 路径元 |
| MetaKvFactory | `internal/kv/etcd/metakv_factory.go:33` | 建 etcd KV(嵌入/外部/TLS) |
| MultiSaveAndRemove | `internal/kv/etcd/embed_etcd_kv.go:471` | 原子多键事务 |
