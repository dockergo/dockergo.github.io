#!/usr/bin/env python3
"""react-design 交互式核心原理图谱生成器（自包含 · 离线 · 双主题）。
...
用法：
  cd react-design && python3 gen.py
...
_ap = argparse.ArgumentParser(description="生成 React 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("REACT_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("REACT_DESIGN_OUT") or os.path.join(HERE, "index.html")

# ===================================================================== #
# 一、主线注册表 —— 唯一需随项目调整的数据块
#     新家族（前端 UI 框架 · 声明式渲染 · React）：元模式 = 接口 × 能力域 × 时机。
#     全景 + 1 接触面主线 + 8 支撑能力域。
# ===================================================================== #
MAINLINES = [
    ("React原理_全景主线框架", "pano", "◇", "全景主线框架",
     "新家族前端 UI 框架：双维模型 · 总架构 · 依赖矩阵 · 依赖关系图 · 三条贯穿声明"),

    ("React原理_接触面_组件与JSX", "iface", "⚙", "接触面 · 组件与 JSX / Hooks",
     "声明式：JSX→createElement→ReactElement · 函数组件返回 element · Hooks API 只在顶层调用"),

    ("React原理_支撑_Fiber架构", "support", "▤", "Fiber 架构",
     "基石：FiberNode(tag/flags/lanes)+ 树链接 child/sibling/return + alternate 双缓冲 · 可中断渲染的数据结构"),
    ("React原理_支撑_协调与Diff", "support", "◫", "协调与 Diff",
     "reconcileChildren 对比新 element 与旧 Fiber · key 复用列表 · 打 flags(Placement/Update/Deletion)"),
    ("React原理_支撑_render与提交", "support", "⛯", "render 与提交",
     "双阶段：render(可中断构树/diff,无副作用)vs commit(同步三子阶段改 DOM)· workLoop beginWork/completeWork"),
    ("React原理_支撑_Hooks", "support", "▦", "Hooks",
     "Hooks 存 Fiber.memoizedState 单向链表(按调用序,故不可条件调用)· mount/update 两套 dispatcher"),
    ("React原理_支撑_Lanes与调度", "support", "◱", "Lanes 与优先级调度",
     "31 位掩码表优先级(SyncLane 急…IdleLane 闲)· getNextLanes 选批 · 位运算合并/取最高 · 映射 Scheduler"),
    ("React原理_支撑_并发特性", "support", "◉", "并发特性",
     "可中断渲染：5ms 时间切片 shouldYield · startTransition 过渡降优先级 · Suspense 挂起 fallback"),
    ("React原理_支撑_事件系统", "support", "◈", "事件系统",
     "委托到 root 容器 + 按 Fiber 树重建传播 · 插件归一 SyntheticEvent · 事件类型定优先级映射 Lane"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接触面主线 · 怎么用（组件 / JSX / Hooks）"),
...
# ===================================================================== #
PANO_NAME = "React原理_全景主线框架"
ARCH_W, ARCH_H = 1080, 760  # 必须与 ARCH_SVG_NAME 的 viewBox 一致
# (x, y, w, h, 主线name) —— 坐标取自总架构 SVG 的真实 rect（viewBox 1080×760）
ARCH_HOTSPOTS = [
    # 标题条 → 全景总览
    (0, 0, 1080, 58, "React原理_全景主线框架"),
    # 接触面带
    (30, 70, 1020, 58, "React原理_接触面_组件与JSX"),
    # 双阶段主干：render(左) / commit(右)
    (30, 146, 620, 150, "React原理_支撑_render与提交"),
    (670, 146, 380, 150, "React原理_支撑_render与提交"),
    # 协调与 Diff 落在 render 阶段内（左侧文字区）
    (30, 200, 620, 96, "React原理_支撑_协调与Diff"),
    # Fiber 基座带
    (30, 312, 1020, 66, "React原理_支撑_Fiber架构"),
    # 支撑 3 域
    (30, 394, 330, 128, "React原理_支撑_Lanes与调度"),
    (375, 394, 330, 128, "React原理_支撑_并发特性"),
    (720, 394, 330, 128, "React原理_支撑_Hooks"),
    # 事件系统带
    (30, 538, 1020, 150, "React原理_支撑_事件系统"),
]
...
BRAND_TITLE = "一切知识皆索引"
BRAND_SUB = "React"
HOME_DESC = ("React 核心原理设计文档库的离线交互图谱——新家族（前端 UI 框架 / 声明式渲染：你声明"
             "「状态→UI 长啥样」，React 用 Fiber 架构 diff 出最小 DOM 变更并可中断地提交）。"
             "9 条主线、手绘原理图，全部回 React 源码核实。点击项目总架构图任意模块即可下钻到对应主线。")
ARCH_SVG_NAME = "React原理_总架构图.svg"
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="React 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
(function(){
  ...
  var saved=localStorage.getItem('react-atlas-theme');
  ...
  function toggleTheme(){
    ...
    localStorage.setItem('react-atlas-theme',cur);
