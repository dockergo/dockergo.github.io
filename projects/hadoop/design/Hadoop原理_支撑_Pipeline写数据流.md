...
图示 pipeline 如何建立并流水传 packet。client 侧 `DFSOutputStream` 把字节切成 **packet**（默认 64KB），每 packet 内再按 512 字节算 chunk 校验和塞进 `dataQueue`，后台 `DataStreamer` 线程负责发送：

1. 向 NameNode `addBlock` 申请新块 + 3 个目标 DataNode。
2. 向 DN1 发建管请求，DN1 转发给 DN2、DN2 转发 DN3，握手成功即确立管道。
3. 从 `dataQueue` 取 packet 发给 DN1，DN1 边写本地边转发 DN2……即 DataNode 侧收 packet→校验→写本地→转发下游。

## ack 反向确认与块完成

![ack 与完成](Hadoop原理_支撑_Pipeline_02故障恢复.svg)

packet 发出后移入 `ackQueue` 等确认。DataNode 侧 `PacketResponder`：DN3 写完回 ack 给 DN2，DN2 合并自己的 ack 回 DN1，DN1 回 client；client 的 `ResponseProcessor` 收到 ack 才把 packet 从 `ackQueue` 移除。全 ack 表示该 packet 在 3 副本都落地。

**pipeline 故障恢复**：`processDatanodeOrExternalError` 检测到某 DN 挂——把 `ackQueue` 未确认 packet 移回 `dataQueue`、剔除坏 DN、用递增的 genStamp 重建剩余节点的 pipeline；副本数不足时补新节点转移数据。写满一个块或 `close` 时 `finalizeBlock` 定稿，client 回 NameNode `complete` 收尾。
