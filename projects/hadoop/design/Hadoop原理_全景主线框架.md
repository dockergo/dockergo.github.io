...
一条贯穿声明：**元数据走 NameNode、数据不经 NameNode 直连 DataNode**。写路径 `create`→`addBlock`→pipeline 传 packet→`complete`；读路径 `open`/`getBlockLocations`→client 直连最近副本 DataNode。HDFS 是「一次写入、多次读取」模型：不支持随机写，只支持追加。
