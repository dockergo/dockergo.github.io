...
`FSNamesystem` 是命名空间的容器与并发协调者，持有三大件：目录树 `FSDirectory`、块管理 `BlockManager`、租约管理 `LeaseManager`。所有元数据读写经全局读写锁串行化。

`FSDirectory` 维护以 `INodeDirectory rootDir` 为根的**全内存 INode 树**，并用 `INodeMap` 按 id 快速索引。文件是 `INodeFile`，持有它的块列表（`BlockInfo[]`）；目录是 `INodeDirectory`。创建文件时建 `INodeFile`、授租约、写 EditLog。
...
- **EditLog**——预写日志。每个变更操作（创建、删除、addBlock…）先追加一条 `FSEditLogOp`，再 `logSync` 刷盘后才对客户端返回成功。它是「自上次快照以来发生了什么」的增量。
- **FsImage**——某时刻整棵命名空间树的序列化快照（Protobuf 格式）。启动时先载最新 FsImage、再重放其后的 EditLog，恢复到崩溃前状态。
...
EditLog 不断增长会拖慢重启（重放耗时）。**检查点（checkpoint）**周期性地：载入旧 FsImage + 重放累积的 EditLog → 得到新的内存树 → 序列化为新 FsImage → 截断旧 EditLog。默认每 3600 秒或累积 100 万条事务触发其一即做。
