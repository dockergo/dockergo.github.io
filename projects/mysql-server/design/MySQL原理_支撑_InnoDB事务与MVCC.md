...
## 三、行锁分解：Next-Key = 记录锁 + 间隙锁

![锁与 Next-Key](MySQL原理_支撑_InnoDB事务与MVCC_03锁与NextKey.svg)

当前读的锁都加在**索引记录**上，分三类：**记录锁**（`LOCK_REC_NOT_GAP` `lock0lock.h:972`）只锁一行；**间隙锁**（`LOCK_GAP` `lock0lock.h:965`）锁一段开区间、禁止插入以防幻读；**Next-Key**（`LOCK_ORDINARY=0` `lock0lock.h:962`）= 间隙 + 右端记录，左开右闭，是 RR 隔离下的默认粒度。行锁按 S/X 兼容性判等待：读读（S+S）并存，只要有写（X）即互斥。等待成环即死锁，`DeadlockChecker::check_and_resolve` `lock0lock.cc:7607` 选 undo 量小者回滚（`select_victim` `:7421`）。

## 四、undo 回滚段与 purge 历史清理

![undo 与 purge](MySQL原理_支撑_InnoDB事务与MVCC_04undo与purge.svg)

旧版本存在 **undo 回滚段**（128 个段、每段 `TRX_RSEG_N_SLOTS` `trx0rseg.h:170` = 1024 个 slot）里，靠 `DB_ROLL_PTR` 串成版本链；insert undo 提交后即可丢弃，update undo 要留给 MVCC 回溯。**purge 线程**（`srv_purge_coordinator_thread` `srv0srv.cc:2768`、`purge_sys` `trx0purge.cc:68`）取最老活跃 ReadView 作水位，回收早于它、再无人可见的旧版本并清理已删除行。风险闭环：长事务持老 ReadView → purge 水位卡住 → undo 膨胀、history list 持续增长、版本链变长拖慢快照读。
