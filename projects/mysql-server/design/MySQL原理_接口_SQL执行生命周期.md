...
![火山模型](MySQL原理_接口_SQL执行生命周期_03火山模型.svg)

MySQL 5.7 的执行器是经典**火山（迭代器）模型**：计划组织成一串嵌套算子，最外层反复向内层"要下一行"，数据像水流般被逐行拉上来而非一次性物化。`sub_select` `sql/sql_executor.cc:1224` 就是这台"泵"——对当前表经 handler 取一行、交 `evaluate_join_record` `:1484` 判连接与 `WHERE`，通过则递归驱动下一张表。好处是内存省（流水线、不缓存全部中间结果、`LIMIT` 够数即短路停拉），代价是每行都穿过整条算子链——这也解释了"少返回一列、少扫一行"为何能线性省掉一整趟逐行开销。
