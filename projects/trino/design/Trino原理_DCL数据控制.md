...
- **认证（HTTP 层）**：`AuthenticationFilter`（`core/trino-main/.../server/security/AuthenticationFilter.java:39`）按配置的有序 `Authenticator` 列表逐个尝试（`for` 循环 `:84`，`authenticator.authenticate(request)` `:87`），**首个成功即确定 `Identity`**（first-success-wins）；全失败把各 `Authenticator` 的挑战头收进 `authenticateHeaders`（`:82`、`:99`）聚合成一个 401 —— `sendWwwAuthenticate`（`:117`）。安全连接用全列表，非安全 HTTP 仅用 `InsecureAuthenticator`（`core/trino-main/.../server/security/InsecureAuthenticator.java`，需显式允许）。
- **访问控制（分析/执行）**：`AccessControlManager`（`core/trino-main/.../security/AccessControlManager.java:98`）组合系统级 + 连接器级——`checkCanXxx` 抛异常式 + `filterXxx` 返回子集式。系统级先判、连接器级后判，都过才放行。
...
- **系统级**（`SystemAccessControl` 列表，全部咨询）：`systemAuthorizationCheck`（`AccessControlManager.java:1600`）遍历所有系统控制器，任一抛异常即失败。内置 `default`/`allow-all`/`read-only`/`file` 四种工厂。SPI 接口 `SystemAccessControl`（`core/trino-spi/.../spi/security/SystemAccessControl.java:115`）。
- **连接器级**（`ConnectorAccessControl`，按 catalog）：`catalogAuthorizationCheck`（`AccessControlManager.java:1631`）解析该 catalog 的连接器控制器；catalog 无控制器则**空操作**（放行）。SPI 接口 `core/trino-spi/.../spi/connector/ConnectorAccessControl.java`。
- **顺序**：系统级先判、连接器级后判，**两者都必须过**。
- **deny-by-default**：SPI 所有 default 方法都拒绝或返回空子集（如 `SystemAccessControl.getRowFilters` 默认 `return List.of();`，`SystemAccessControl.java:1067`）——permissive 插件必须显式 override。

以 `checkCanSelectFromColumns`（`AccessControlManager.java:1113`）为例：先 `checkCanAccessCatalog`（`:1569`，内部委托 boolean 版 `canAccessCatalog` `:1576`）→ 再系统级 `checkCanSelectFromColumns`（`AccessControlManager.java:1122`）→ 再连接器级（`AccessControlManager.java:1124`）——三关都过才允许读这些列。
...
`ViewExpression`（`core/trino-spi/.../spi/security/ViewExpression.java`）表示一个行过滤（boolean SQL 表达式）或列掩码（改写列值的 SQL 表达式）。`getRowFilters`（`AccessControlManager.java:1472`）/`getColumnMasks`（`:1494`）是**合并式**（不短路）：先取连接器级，再叠加每个系统级控制器的——`getRowFilters` 结果拼接（多个过滤 AND 生效），`getColumnMasks` 若两源掩同一列则抛 `INVALID_COLUMN_MASK`（`:1517`）。行过滤把 `WHERE 表达式` 注入查询、列掩码把 `列` 替换成表达式（如 `substr(ssn,1,3)||'****'`），对用户透明。
