...
## 索引内部结构 · HNSW 图与 IVF 倒排

![Milvus HNSW/IVF 索引结构](Milvus原理_检索_04索引结构.svg)

上图把两族主流索引"长什么样、怎么少比距离"画出来:**HNSW** 是分层小世界图——入口在稀疏顶层,每层贪心走到局部最近点后下沉,L0 全量层做 ef 邻域精搜取 topK(M/ef 越大越准越慢);**IVF** 先用 k-means 把向量分成 nlist 个簇(各有质心),查询只找 nprobe 个最近质心、只扫这几个簇的倒排链(nlist/nprobe 权衡,PQ/SQ 量化压向量省内存)。二者都把"和全库逐一比"降成"只比一小部分",这是 ANN 近对数级的来源;度量 L2/IP/COSINE 须与 embedding 训练一致。索引算法在 knowhere(第三方库),core 只做集成/分发。
...
## 分布式检索归并 · scatter-gather

![Milvus 分布式检索归并](Milvus原理_检索_05分布式归并.svg)

上面讲的是**单个段**上怎么搜;放到集群里,一次 search 是**散-并-合**三段:Proxy 按 vchannel 把请求散给各 shard delegator(`internal/querynodev2/delegator/delegator.go:455`);delegator 过 tSafe 闸门(`waitTSafe`)拿到一致快照后,`PinReadableSegments` 钉住 sealed/growing 段快照、`organizeSubTask`(`delegator.go:882`)按段所在节点再散(sealed→远程 QueryNode 走 `worker.SearchSegments`/Historical,growing→本地/Streaming);各段并行 ANN(bitset 预过滤在遍历内跳过被删/被过滤行);结果先在 delegator 用 `ReduceSearchOnQueryNode`(`delegator.go:567`)合成本 shard 局部 topK、再回 Proxy 合成全局 topK。两级归并成立,是因为"全局 topK ⊆ 各局部 topK 之并"——段越多并行度越高,但归并与网络开销也随之上升。
