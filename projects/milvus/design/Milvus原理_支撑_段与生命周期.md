...
> **定位**：属"存储能力域"。管数据的物理组织与演进:growing(内存可变)→ sealed(封存)→ flushed(落 binlog)→ indexed(建索引);compaction 合段应用删除。被【写入路径】产出、被【向量索引与检索】读取。源码基准 **Milvus(9a6e499)**(`internal/datacoord/`、`internal/core/src/segcore/`)。
...
- **Growing**(接口 `SegmentGrowing` `internal/core/src/segcore/SegmentGrowing.h:25`,实现 `SegmentGrowingImpl` `internal/core/src/segcore/SegmentGrowingImpl.h:84`):新写入落这;内存可变、`Insert` 追加、**无向量索引**(查询暴力搜)。DataCoord 按 (collection, partition, channel) 分配(`internal/datacoord/segment_manager.go:307`)。
- **Sealed**(封存):growing 段超 `maxSize`(默认 1024MB)或按 idle/总大小策略封;不可变。`SegmentSealed`(`internal/core/src/segcore/SegmentSealed.h:33`)是 load-only(`LoadIndex`/`LoadSegmentMeta`),主实现 `ChunkedSegmentSealedImpl`。
...
- **mix compaction**(`internal/datanode/compactor/mix_compactor.go:51`,`Compact` `:430`):合并多个小段,合并时**应用删除**——`EntityFilter`(`internal/compaction/entity_filter.go:10`,`Filtered` `:50`)按 `effectiveTs < deleteTs` 丢被删行(strict `<` 保留 insert_ts==delete_ts 的 upsert,`isEntityDeleted` `:89`)、按 TTL 丢过期行(`isEntityExpired` `:104`)。
- **L0(level-zero)compaction**(`internal/datanode/compactor/l0_compactor.go:49` 的 `LevelZeroCompactionTask`,`Compact` `:117`):处理纯删除段——把累积的删除数据按目标段拆分、写成 delta log。
...
- **查询用索引 vs 暴力**:QueryNode 段 `ExistIndex(field)` 判有无索引(定义 `internal/querynodev2/segments/segment.go:684`,search 处用 `:818`);growing 段和未建索引 sealed 段**暴力搜**(日志"search without indexes")、建了索引走 knowhere。
...
| SegmentGrowing / Impl | `core/src/segcore/SegmentGrowing.h:25` / `SegmentGrowingImpl.h:84` | 内存可变段 |
| SegmentSealed | `core/src/segcore/SegmentSealed.h:33` | 不可变 load-only 段 |
| SegmentManager | `internal/datacoord/segment_manager.go:307` | 分配 growing 段 + seal 策略 |
| mixCompactionTask | `internal/datanode/compactor/mix_compactor.go:51` | 合段 + 应用删除(Compact `:430`) |
| LevelZeroCompactionTask | `internal/datanode/compactor/l0_compactor.go:49` | 纯删除段 → delta log(Compact `:117`) |
| EntityFilter | `internal/compaction/entity_filter.go:10` | 合并时丢被删/过期行(Filtered `:50`) |
| ExistIndex | `internal/querynodev2/segments/segment.go:684` | 判段有无向量索引 |
