...
![oop 三特化布局：实例 / 数组 / 类镜像的内存分家](OpenJDK原理_支撑_对象模型oop-klass_05oop三特化布局.svg)

三种特化只在"是否落地额外字段"上分家：`instanceOopDesc`（`oops/instanceOop.hpp`）不加任何字段，布局全由 `InstanceKlass`（`oops/instanceKlass.hpp:134`）的 `_nonstatic_field_size`（`:213`）、oop-map（`:215`）、`_itable_len`（`:216`）在类一级描述；`arrayOopDesc`（`oops/arrayOop.hpp:43`）多一个 `int length`（`:84`），因长度运行期才知、必须落地。由此 GC 扫引用实例走 oop-map、数组按元素类型批量扫；取大小实例定长可编译期常量化、数组走 `size_given_klass()`（`oop.inline.hpp:161`）现算。
...
![压缩指针 base+shift 编解码与紧凑对象头演进](OpenJDK原理_支撑_对象模型oop-klass_04压缩指针与紧凑头.svg)

压缩指针拿 `_base`+`_shift` 编解码换内存：`CompressedOops`（`oops/compressedOops.hpp:37`）把 64 位堆指针压成 32 位 narrowOop，`真实地址 = _base（:42）+ (narrowOop << _shift（:46）)`，`_shift = log2(对象对齐)`（默认 8 字节即 shift=3，地址低 3 位恒 0），故寻址上限 `2^32 × 8 = 32GB`（"堆 > 32GB 压缩失效"之由来）；`CompressedKlassPointers`（`oops/compressedKlass.hpp:101`）同理压 `Klass*`，`_base` 取元空间起点。JDK 28 紧凑对象头 `UseCompactObjectHeaders` 再把类指针内嵌进 markWord 高位（`klass_shift`/`klass_bits=22`，`markWord.hpp:149`），普通对象头从 12/16 字节压到 8 字节，是近年最重要的堆内存优化。
