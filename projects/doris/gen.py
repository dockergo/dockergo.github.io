...
_ap = argparse.ArgumentParser(description="生成 Doris 引擎交互式核心原理图谱(离线自包含 HTML)")
...
THEMES = [
    ...
    {"id": "quickstart", "icon": "▶", "title": "快速开始", "cat": "appendix", "ord": 0,
     "desc": "从零到一条查询的最小闭环上手教程:上手总览页顶部是五步路线全景图,下方点击每一步(环境搭建 / 建库建表 / 数据写入 / 查询分析 / 数据导出)即展开该步可照做的命令与 SQL,默认展示第一步。整体集成架构与依赖关系见 Appendix",
     "tabs": ["qstour"]},

    # ── 接口主线 · 用户可见(DDL / DML / DQL / DCL)· 严格以 design 走查为主干 ──
    {"id": "schemachange", "icon": "⇲", "title": "DDL 数据定义", "cat": "iface", "ord": 0,
     "desc": "数据定义:建库建表与在线变更表结构(Schema Change)。design 走查:元数据变更生命周期总览 → 接入转发(单写多读)→ 执行并写 EditLog → Checkpoint → Schema Change 变更决策,附调优/误区/总纲",
     "tabs": ["scwalk", "sctree2"]},
    {"id": "load", "icon": "⇥", "title": "DML 数据写入", "cat": "iface", "ord": 1,
     "desc": "数据写入:design 走查——写入生命周期总览 → 导入方式与接入 → 2PC 事务状态机 → 写入路径(Route→MemTable→Rowset)→ 提交发布与主键更新 → 按数据来源选导入,附调优/误区/总纲",
     "tabs": ["loadwalk", "loadstruct2", "writedata2", "mowmerge2"]},
    {"id": "qlife", "icon": "◷", "title": "DQL 数据查询", "cat": "iface", "ord": 2,
     "desc": "数据查询:design 走查——查询执行全景 → 生命周期总览 → 接入缓存 → 编译(Text→Plan)→ 算子树与 Fragment → MPP 分布式执行 → Pipeline 执行 → 汇聚返回 → Join 分布决策,附调优/误区/总纲",
     "tabs": ["qlwalk", "qlifevars", "qlifeterms"]},
    {"id": "dcl", "icon": "⛚", "title": "DCL 数据控制", "cat": "iface", "ord": 3,
     "desc": "数据控制:design 走查——定义线×执行线 → RBAC 权限模型 → 请求管控时序 → 资源隔离与限流 → 可插拔认证/授权,附调优/误区/总纲",
     "tabs": ["dclwalk"]},

    # ── 支撑主线 · 引擎内部(8 条)· 严格以 design 走查为主干 ──
    {"id": "storageformat", "icon": "▤", "title": "存储引擎", "cat": "support", "ord": 0,
     "desc": "存储引擎:design 走查——数据组织架构 → 三种数据模型 → 多级跳过链 → 主键两种实现,附调优/误区/总纲。内表/存储格式/外表/索引/物化的深挖细节见「深挖 · Legacy 细节」",
     "tabs": ["stewalk"]},
    {"id": "opttech", "icon": "✲", "title": "优化技术", "cat": "support", "ord": 1,
     "desc": "查询提速:design 走查——优化的四个位置 → CBO 代价优化 → RBO/CBO/HBO 三段式,附调优/误区/总纲。优化器/RF/TOPN/统计的深挖见「深挖 · Legacy 细节」",
     "tabs": ["optwalk"]},
    {"id": "execengine", "icon": "⚡", "title": "执行引擎", "cat": "support", "ord": 2,
     "desc": "BE 运行时:design 走查——四级展开 → MPP 分布式执行 → Pipeline 拆分与阻塞点 → 向量化与线程模型 → Exchange 三种分发,附调优/误区/总纲。线程/内存深挖见「深挖 · Legacy 细节」",
     "tabs": ["execwalk"]},
    {"id": "transaction", "icon": "⇅", "title": "事务一致性", "cat": "support", "ord": 3,
     "desc": "多版本并发:design 走查——写事务×读快照 → 2PC 事务状态机 → MVCC 与主键更新 → 版本链与 Publish 对齐,附调优/误区/总纲。专家图(双泳道/VersionGraph)见「深挖 · Legacy 细节」",
     "tabs": ["txnwalk", "txntree2", "txnswim2", "versiongraph2"]},
    {"id": "metadata", "icon": "⬢", "title": "元数据", "cat": "support", "ord": 4,
     "desc": "控制面:design 走查——元数据组织架构 → 内存+日志+镜像三位一体 → 单写多读拓扑,附调优/误区/总纲。结构细节见「深挖 · Legacy 细节」",
     "tabs": ["metawalk", "metatree2"]},
    {"id": "workload", "icon": "◐", "title": "资源与负载管理", "cat": "support", "ord": 5,
     "desc": "资源治理:design 走查——拦截→约束→审计闭环 → Workload Group 多租户隔离 → 多级内存治理 → 可观测三件套,附调优/误区/总纲",
     "tabs": ["wgwalk", "wgtree2"]},
    {"id": "tabletsched", "icon": "⬡", "title": "集群自愈", "cat": "support", "ord": 6,
     "desc": "控制面:design 走查——Detect→Decide→Act 自愈闭环 → Replica 模型与健康检测 → 调度优先级,附调优/误区/总纲",
     "tabs": ["tswalk", "tablettree2"]},
    {"id": "compaction", "icon": "▨", "title": "后台任务", "cat": "support", "ord": 7,
     "desc": "后台维护:design 走查——异步守护摊平成本 → Compaction 分层合并 → 三放大权衡,附调优/误区/总纲",
     "tabs": ["cpwalk", "compacttree2"]},

    # ── Appendix · 参考 ──
    {"id": "overallarch", "icon": "◇", "title": "架构与部署", "cat": "appendix", "ord": 1,
     "desc": "全景框架 + 部署形态合一:双维模型(能力域×执行时机)· 总架构图 · 物理部署 · 依赖矩阵 · 依赖关系 · 集成架构(数仓分层)· 四种部署形态(存算一体/存算分离/冷热分离/湖仓查询)· 部署形态对比",
     "tabs": ["panowalk", "archintegrated", "deploywalk"]},
...
# 热区从总架构 SVG 里带 data-tid 的 <rect> 自动派生 —— SVG 是唯一真源,
# 消除“SVG 图 + 手写坐标”双真源;坐标含 <g transform="translate"> 偏移,标签取 data-lab。
import base64 as _b64
_ARCH_SVG_TEXT = open(
    os.path.join(_DESIGN_DIR, "Doris原理_总架构图.svg"),
    encoding="utf-8").read()
...
def _parse_arch_hotspots(svg_text):
    ...
    walk(root, 0.0, 0.0)
    return hots, vbw, vbh
# (x, y, w, h, theme_id, 标签) —— 6 元 tuple,坐标已含 translate 偏移
...
def _design_b64(fname):
    with open(os.path.join(_DESIGN_DIR, fname), encoding="utf-8") as _f:
        return _b64.b64encode(_f.read().encode("utf-8")).decode("ascii")

# SVG-walk 视图:tid → [(标题, 文件名), ...](顺序取自 prose 文档的图序)
_SVG_WALK_SPECS = {
    # ---- 接口主线 ----
    "scwalk": [("生命周期总览", "Doris原理_DDL_01总览.svg"),
               ("接入与转发 · 单写多读", "Doris原理_DDL_02接入转发.svg"),
               ("执行并写 EditLog", "Doris原理_DDL_03执行EditLog.svg"),
               ("Checkpoint · 日志压缩与恢复", "Doris原理_DDL_04Checkpoint.svg"),
               ("Schema Change 轻重之分", "Doris原理_DDL_05变更决策.svg"),
               ("重量变更 · 影子索引与双写", "Doris原理_DDL_06影子双写.svg"),
               ("两级数据划分 · Colocate", "Doris原理_DDL_08Colocate.svg"),
               ("结构对象全景", "Doris原理_DDL_07对象全景.svg")],
    "loadwalk": [("写入生命周期总览", "Doris原理_DML_01总览.svg"),
                 ("导入方式与接入", "Doris原理_DML_02导入方式.svg"),
                 ("事务状态机 · 2PC", "Doris原理_DML_03事务状态机.svg"),
                 ("写入路径 · Route→MemTable→Rowset", "Doris原理_DML_04写入路径.svg"),
                 ("提交、发布与主键更新", "Doris原理_DML_05提交发布.svg"),
                 ("Sink 分发与副本一致性", "Doris原理_DML_08分发副本.svg"),
                 ("UPDATE/DELETE 与并发控制", "Doris原理_DML_07组提交.svg"),
                 ("流式导入 · Routine/Kafka", "Doris原理_DML_09流式导入.svg"),
                 ("按数据来源选导入", "Doris原理_DML_06导入选择.svg")],
    "qlwalk": [("查询执行全景", "Doris原理_DQL_09执行全景.svg"),
               ("生命周期总览", "Doris原理_DQL_01总览.svg"),
               ("阶段一 · 接入与缓存", "Doris原理_DQL_02接入缓存.svg"),
               ("阶段二 · 编译 Text→Plan", "Doris原理_DQL_03编译.svg"),
               ("阶段三 · 算子树与 Fragment", "Doris原理_DQL_04算子树.svg"),
               ("阶段四 · MPP 分布式执行", "Doris原理_DQL_05MPP.svg"),
               ("阶段五 · Pipeline 执行", "Doris原理_DQL_06Pipeline.svg"),
               ("阶段六 · 汇聚与返回", "Doris原理_DQL_07汇聚.svg"),
               ("RF 与两阶段聚合", "Doris原理_DQL_11两阶段聚合.svg"),
               ("高并发点查 · 旁路", "Doris原理_DQL_10点查.svg"),
               ("分布式 Join 四种分布", "Doris原理_DQL_08Join分布.svg")],
    "dclwalk": [("定义线 × 执行线", "Doris原理_DCL_01定义执行.svg"),
                ("权限模型 · RBAC", "Doris原理_DCL_02权限模型.svg"),
                ("请求管控时序", "Doris原理_DCL_03请求管控.svg"),
                ("资源隔离与限流", "Doris原理_DCL_04资源隔离.svg"),
                ("行列级权限与治理", "Doris原理_DCL_06行级安全.svg"),
                ("可插拔认证 / 授权", "Doris原理_DCL_05认证授权.svg")],
    # ---- 支撑主线 ----
    "stewalk": [("数据组织架构", "Doris原理_存储_01数据组织.svg"),
                ("三种数据模型", "Doris原理_存储_02数据模型.svg"),
                ("多级跳过链", "Doris原理_存储_03跳过链.svg"),
                ("索引跳过 · ZoneMap/BF", "Doris原理_存储_07索引跳过.svg"),
                ("索引检索 · 倒排/N-gram", "Doris原理_存储_08索引检索.svg"),
                ("主键 · 向量索引", "Doris原理_存储_09索引主键向量.svg"),
                ("湖仓外表读取", "Doris原理_存储_10湖仓外表.svg"),
                ("冷热分层 · Storage Policy", "Doris原理_存储_05冷热分层.svg"),
                ("主键两种实现 · MoR/MoW", "Doris原理_存储_04主键实现.svg"),
                ("读取路径 · 选版本→裁剪→归并→收口", "Doris原理_存储_11读取路径.svg"),
                ("文件缓存 · BlockFileCache", "Doris原理_存储_06文件缓存.svg")],
    "optwalk": [("优化的四个位置", "Doris原理_优化_01四位置.svg"),
                ("优化器整体架构", "Doris原理_优化_09优化架构.svg"),
                ("CBO 代价优化", "Doris原理_优化_02代价优化.svg"),
                ("三段式 · RBO/CBO/HBO", "Doris原理_优化_03三段式.svg"),
                ("物化视图透明改写", "Doris原理_优化_04物化改写.svg"),
                ("多级缓存", "Doris原理_优化_05缓存.svg"),
                ("Runtime Filter 运行时裁行", "Doris原理_优化_06运行时过滤.svg"),
                ("TopN 极值下推", "Doris原理_优化_07TopN.svg"),
                ("统计信息 · CBO 燃料", "Doris原理_优化_08统计信息.svg")],
    "execwalk": [("四级展开", "Doris原理_执行_01四级.svg"),
                 ("MPP 分布式执行", "Doris原理_执行_02MPP.svg"),
                 ("Pipeline 拆分与阻塞点", "Doris原理_执行_03阻塞点.svg"),
                 ("线程模型", "Doris原理_执行_04线程模型.svg"),
                 ("向量化执行内核", "Doris原理_执行_08向量化内核.svg"),
                 ("Pipeline 演进 · X 引擎", "Doris原理_执行_07Pipeline演进.svg"),
                 ("Exchange 三种分发", "Doris原理_执行_05Exchange.svg"),
                 ("Spill 落盘", "Doris原理_执行_06Spill.svg"),
                 ("深入 · BE 线程模型(≈840)", "Doris执行_09线程模型.svg"),
                 ("深入 · 内存治理与仲裁", "Doris执行_10内存治理.svg"),
                 ("深入 · jemalloc 分配器", "Doris执行_11jemalloc.svg")],
    "txnwalk": [("写事务 × 读快照", "Doris原理_事务_01写读.svg"),
                ("事务状态机 · 2PC", "Doris原理_事务_02状态机.svg"),
                ("MVCC 与主键更新", "Doris原理_事务_03MVCC.svg"),
                ("多表事务", "Doris原理_事务_05多表事务.svg"),
                ("版本链与 Publish 对齐", "Doris原理_事务_04版本链.svg"),
                ("Sequence 列裁决", "Doris原理_事务_06序列裁决.svg")],
    "metawalk": [("元数据组织架构", "Doris原理_元数据_01组织.svg"),
                 ("两层元数据 · FE 逻辑 / BE 物理", "Doris原理_元数据_06两层元数据.svg"),
                 ("三位一体 · 内存+日志+镜像", "Doris原理_元数据_02三位一体.svg"),
                 ("单写多读拓扑", "Doris原理_元数据_03拓扑.svg"),
                 ("选主 · BDB-JE 多数派", "Doris原理_元数据_05选主.svg"),
                 ("EditLog 全序日志复制", "Doris原理_元数据_04日志复制.svg")],
    "wgwalk": [("治理闭环 · 拦截→约束→审计", "Doris原理_资源_01闭环.svg"),
               ("Workload Group 多租户隔离", "Doris原理_资源_02工作负载组.svg"),
               ("多级内存治理", "Doris原理_资源_03内存.svg"),
               ("资源组下发 · cgroup", "Doris原理_资源_06资源组下发.svg"),
               ("CPU 软限与查询排队", "Doris原理_资源_05CPU排队.svg"),
               ("可观测三件套", "Doris原理_资源_04可观测.svg")],
    "tswalk": [("自愈闭环 · Detect→Decide→Act", "Doris原理_自愈_01闭环.svg"),
               ("Replica 模型与健康检测", "Doris原理_自愈_02副本.svg"),
               ("副本对账 · Report", "Doris原理_自愈_05对账.svg"),
               ("调度优先级", "Doris原理_自愈_03调度优先级.svg"),
               ("扩缩容 · Decommission", "Doris原理_自愈_04扩缩容.svg")],
    "cpwalk": [("异步守护摊平成本", "Doris原理_后台_01异步.svg"),
               ("Compaction 分层合并", "Doris原理_后台_02Compaction.svg"),
               ("后台维护任务全景", "Doris原理_后台_04维护任务.svg"),
               ("三放大权衡", "Doris原理_后台_03三放大.svg")],
    # ---- 全局 2 图(单图走查)----
    "deploysvg": [("物理部署视图 · FE/BE/MPP/缓存", "Doris原理_物理部署图.svg")],
    "depgraph": [("能力域依赖关系", "Doris原理_依赖关系图.svg")],
    # 部署形态概览:4 张手绘 SVG(Apple 工业风,参物理部署视图),替代原 mermaid DEPLOY_MMS
    "deploywalk": [("存算一体部署", "Doris部署_01存算一体.svg"),
                   ("存算分离部署", "Doris部署_02存算分离.svg"),
                   ("冷热分离部署", "Doris部署_03冷热分离.svg"),
                   ("湖仓查询部署", "Doris部署_04湖仓查询.svg")],
    # 全景框架:统领全部原理的全局视图(替代 refarch;含依赖关系图,吸收 depgraph)
    "panowalk": [("双维模型 · 能力域 × 执行时机", "Doris原理_双维模型.svg"),
                 ("总架构图 · 位置即语义", "Doris原理_总架构图.svg"),
                 ("物理部署视图", "Doris原理_物理部署图.svg"),
                 ("依赖矩阵 · 接口主线 × 能力域", "Doris原理_依赖矩阵.svg"),
                 ("能力域依赖关系", "Doris原理_依赖关系图.svg")],
    # 集成架构:按总架构图风格手绘(替代原 mermaid ARCHINTEG_MM)
    "archintegrated": [("集成架构全景", "Doris原理_集成架构.svg")],
    # EXPLAIN 计划演变:5 阶段计划树手绘 SVG(替代原 mermaid EXPLAIN_MMS)
    "explaincmd": [("① PARSED · 解析未绑定", "Doris原理_EXPLAIN_01解析.svg"),
                   ("② ANALYZED · 绑定", "Doris原理_EXPLAIN_02绑定.svg"),
                   ("③ REWRITTEN · RBO 改写", "Doris原理_EXPLAIN_03改写.svg"),
                   ("④ OPTIMIZED · CBO 物理优化", "Doris原理_EXPLAIN_04优化.svg"),
                   ("⑤ DISTRIBUTED · 切 Fragment", "Doris原理_EXPLAIN_05分发.svg")],
    # 问题与诊断主题重构:Profile透视 / 诊断原理 / 常见问题(单图走查)
    "profileview": [("Profile 透视 · 格式 × 采集逻辑", "Doris原理_Profile透视.svg")],
    "diagprin": [("诊断原理 · 可观测闭环", "Doris原理_诊断原理.svg")],
    "commonqa": [("常见问题 · 读写两类", "Doris原理_常见问题.svg")],
    # DML 落盘结构(writedata 重画,并入 legacy→load)
    "writedata2": [("落盘结构 · Rowset→Segment 列存", "Doris原理_DML_10落盘结构.svg")],
    # 其余旧 JS-spec 结构图重画为工业风 SVG(替 sctree/loadstruct/mowmerge/txntree/txnswim/versiongraph/metatree/wgtree/tablettree/compacttree)
    "sctree2": [("变更结构 · 影子索引/水位/状态机", "Doris原理_DDL_09变更结构.svg")],
    "loadstruct2": [("分桶结构 · tablet 路由", "Doris原理_DML_11分桶结构.svg")],
    "mowmerge2": [("MoW 合并 · delete bitmap 标删", "Doris原理_DML_12MoW合并.svg")],
    "txntree2": [("事务与 MVCC 版本结构", "Doris原理_事务_07结构.svg")],
    "txnswim2": [("写入事务 · 双泳道", "Doris原理_事务_08双泳道.svg")],
    "versiongraph2": [("VersionGraph · 读快照选版本", "Doris原理_事务_09版本读快照.svg")],
    "metatree2": [("元数据结构 · 持久化&高可用", "Doris原理_元数据_06结构.svg")],
    "wgtree2": [("资源组隔离结构", "Doris原理_资源_07结构.svg")],
    "tablettree2": [("副本调度与修复结构", "Doris原理_自愈_06结构.svg")],
    "compacttree2": [("Compaction 数据结构", "Doris原理_后台_05结构.svg")],
}
# 快速开始「上手总览」用独立复合渲染器 renderQsTour(总览 SVG + 5 步选择器 + 内容区),不走通用 SVG-walk
_QSTOUR_OVERVIEW_B64 = _design_b64("Doris上手_00总览.svg")
...
_WALK_PROSE = {
    "scwalk": "Doris原理_DDL数据定义.md",  "loadwalk": "Doris原理_DML数据操纵.md",
    "qlwalk": "Doris原理_DQL数据查询.md",  "dclwalk": "Doris原理_DCL数据控制.md",
    "stewalk": "Doris原理_支撑_存储引擎.md", "optwalk": "Doris原理_支撑_优化技术.md",
    "execwalk": "Doris原理_支撑_执行引擎.md", "txnwalk": "Doris原理_支撑_事务一致性.md",
    "metawalk": "Doris原理_支撑_元数据.md",  "wgwalk": "Doris原理_支撑_资源与负载管理.md",
    "tswalk": "Doris原理_支撑_集群管理与自愈.md", "cpwalk": "Doris原理_支撑_后台任务.md",
}
...
.brand-intro{display:flex;flex-direction:column;align-items:flex-start;margin-left:12px;min-width:0;max-width:min(60vw,720px)}
.brand-intro .tt{font-size:15px;font-weight:600;color:var(--c-ink);line-height:1.3}
.brand-intro .sub{margin:3px 0 0;font-size:11.5px;color:var(--c-ink3);line-height:1.5;text-align:left}
...
/* 下钻页(垂直 tab 文档:svg-walk/nested/multi/steps)用块级布局贴顶,规避画布式 flex 居中造成的顶部大片空白;并隐藏冗余空 mmout */
...
.svg-walk-img{display:block;width:100%;max-width:1280px;height:auto;user-select:none;
...
<div id="loadingOverlay" role="status" aria-live="polite">
  <div class="lo-inner">
    ...
    <div class="lo-title">Doris Engine Atlas</div>
...
<div id="app">
  <header>
    ...
    <div class="brand-intro"><div class="tt">Apache Doris · 核心原理图谱</div><span class="sub">MPP 分析型数据库:FE(元数据 / 查询规划 / 事务)与 BE(向量化执行 / Rowset 存储)分离,存算一体,后台 Compaction 归并、资源组约束负载。</span></div>
    ...
    <a href="https://github.com/apache/doris" target="_blank" rel="noopener" title="GitHub 源码仓库" style="display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M12 .5C5.7.5.5 5.7.5 12c0 5.1 3.3 9.4 7.9 10.9.6.1.8-.2.8-.6v-2c-3.2.7-3.9-1.4-3.9-1.4-.5-1.3-1.3-1.7-1.3-1.7-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1 1.8 2.7 1.3 3.4 1 .1-.8.4-1.3.7-1.6-2.6-.3-5.3-1.3-5.3-5.8 0-1.3.5-2.3 1.2-3.1-.1-.3-.5-1.5.1-3.1 0 0 1-.3 3.3 1.2a11.4 11.4 0 0 1 6 0C17.3 4.7 18.3 5 18.3 5c.6 1.6.2 2.8.1 3.1.8.8 1.2 1.8 1.2 3.1 0 4.5-2.7 5.5-5.3 5.8.4.4.8 1.1.8 2.2v3.3c0 .4.2.7.8.6 4.6-1.5 7.9-5.8 7.9-10.9C23.5 5.7 18.3.5 12 .5z"/></svg></a><a href="https://doris.apache.org" target="_blank" rel="noopener" title="项目官网" style="display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><img src="data:image/svg+xml;base64," width="18" height="18" alt="官网" style="display:block"/></a><button class="theme-toggle" id="themeToggle" title="切换深色 / 浅色主题" aria-label="切换主题">
...
function activateTab(t){
  ...
  var _isDoc=!!pane.querySelector('.do-paneflow');
  ['zoomOut','zoomReset','zoomIn','fitBtn'].forEach(function(id){var el=document.getElementById(id);if(el)el.style.display=_isDoc?'none':'';});
  document.getElementById('flowPlay').style.display=(_isDoc||TABLE_TABS[t.dataset.tab])?'none':'';
