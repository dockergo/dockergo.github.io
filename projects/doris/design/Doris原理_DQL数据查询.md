...
> 下列 `file:行号` 均在用户 Doris 分支源码中 grep 核实，串起"接入 → 编译 → 分布式调度 → BE 执行"整条链路。

- **接入（MySQL 协议）**：`fe/fe-core/src/main/java/org/apache/doris/qe/ConnectProcessor.java:181`（`handleQuery`）、`:209`（`executeQuery`，落到 `StmtExecutor`）。
- **语句执行入口**：`fe/fe-core/src/main/java/org/apache/doris/qe/StmtExecutor.java:481`（`execute()`）。
- **走 Nereids**：`StmtExecutor.java:673`（`executeByNereids`）、`:1272`（`handleQueryStmt`）。
- **编译总入口**：`fe/fe-core/src/main/java/org/apache/doris/nereids/NereidsPlanner.java:216`（`planWithLock`）。
- **绑定分析**：`NereidsPlanner.java:410`（`analyze`）。
- **RBO 规则改写**：`NereidsPlanner.java:431`（`rewrite`）。
- **CBO 代价优化**：`NereidsPlanner.java:517`（`optimize`）。
- **从 Memo 取最优计划**：`NereidsPlanner.java:319`（`chooseNthPlan`）。
- **切 Fragment 与分布**：`NereidsPlanner.java:579`（`splitFragments`，在 Exchange 处切分）、`:678`（`distribute`）。
- **MPP 调度：定 BE/instance**：`fe/fe-core/src/main/java/org/apache/doris/qe/Coordinator.java:1364`（`computeFragmentExecParams`）。
- **下发各 BE 并汇聚**：`Coordinator.java:683`（`exec`）、`:814`（`sendPipelineCtx`）、`:1193`（`getNext`，汇聚返回客户端）。
- **BE Pipeline 执行**：`be/src/pipeline/pipeline_fragment_context.h:86`（`prepare`）、`:88`（`submit`）、`:171`（`_build_pipeline_tasks`）；`be/src/pipeline/pipeline_task.h:63`（`execute`）。
