...
> 下列 `file:行号` 在用户 Doris 分支 BE 源码 grep 核实，尤以调度队列印证"MLFQ + work-stealing + vruntime"。

- **Fragment → Pipeline 切分**：`be/src/pipeline/pipeline_fragment_context.h:129`（`_build_pipelines`）。
- **算子树构建**：`be/src/pipeline/pipeline_fragment_context.h:131`（`_create_tree_helper`）。
- **Pipeline Task 生成**：`be/src/pipeline/pipeline_fragment_context.h:171`（`_build_pipeline_tasks`）。
- **依赖驱动执行**：`be/src/pipeline/pipeline_task.h:63`（`execute`）、`:251`（`_read_dependencies`，显式依赖对象）。
- **调度线程主循环**：`be/src/pipeline/task_scheduler.h:46`（`TaskScheduler`）、`:78`（`_do_work`）。
- **每核 MLFQ 队列**：`be/src/pipeline/task_queue.h:69`（`PriorityTaskQueue`）、`:88`（`SUB_QUEUE_LEVEL = 6`，六级子队列）。
- **vruntime 防饥饿**：`be/src/pipeline/task_queue.h:50`（`get_vruntime`）。
- **每核一队 + 窃取**：`be/src/pipeline/task_queue.h:106`（`MultiCoreTaskQueue`）、`:130`（`_steal_take`，work-stealing）。
- **算子基类**：`be/src/pipeline/exec/operator.h:96`（`OperatorBase`）、`:865`（`OperatorXBase`）。
- **Source / Sink 接口**：`be/src/pipeline/exec/operator.h:923`（`get_block`）、`:664`（`sink`）。
- **向量化表达式**：`be/src/vec/exprs/vexpr.h:87`（`VExpr`）、`:138`（`execute`，对列式 `Block` 求值）。
- **内存不足触发 Spill**：`be/src/pipeline/exec/partitioned_aggregation_sink_operator.cpp:161`（`revoke_memory`，聚合中间态落盘）。
