...
_ap = argparse.ArgumentParser(description="生成 Apache Parquet 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("PARQUET_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("PARQUET_DESIGN_OUT") or os.path.join(HERE, "index.html")

# ===================================================================== #
# 一、主线注册表 —— 唯一需随项目调整的数据块
#     家族（列式文件格式 · 自描述列存文件规范 + Thrift 元数据 IDL · Apache Parquet）：
#     元模式 = 接触面 × 能力域 × 时机。全景 + 1 接触面主线 + 10 支撑能力域 = 12 篇。
# ===================================================================== #
MAINLINES = [
    ("Parquet原理_全景主线框架", "pano", "◇", "全景主线框架",
     "自描述列存文件规范：双维模型 · 总架构 · 依赖矩阵/关系图 · 三条贯穿声明"),

    ("Parquet原理_接触面_类型系统与schema树", "iface", "⚙", "类型系统与 schema 树",
     "8 种物理 Type · LogicalType 注解 · SchemaElement 深度优先扁平树(num_children) · ColumnOrder 排序语义"),

    ("Parquet原理_支撑_文件布局", "support", "▤", "文件布局",
     "PAR1 魔数 · N 列 × M 行组 · 行组=每列一 ColumnChunk · FileMetaData 尾置(单遍写)"),
    ("Parquet原理_支撑_Dremel嵌套编码", "support", "❁", "Dremel 嵌套编码",
     "招牌：定义级 def + 重复级 rep 展开嵌套 · null 编进 def 级 · 由 schema 算 max level"),
    ("Parquet原理_支撑_列编码", "support", "▦", "列编码",
     "PLAIN · RLE/位打包混合 · 字典(PLAIN_DICTIONARY/RLE_DICTIONARY) · DELTA 系列 · BYTE_STREAM_SPLIT"),
    ("Parquet原理_支撑_压缩与页", "support", "⛁", "压缩与页",
     "codec(SNAPPY/ZSTD/LZ4_RAW…) · PageType 四型 · DataPageV2 免解压读级 · 页级 CRC32"),
    ("Parquet原理_支撑_列块与页组装", "support", "▥", "列块与页组装",
     "ColumnChunk→ColumnMetaData(偏移/编码集/统计) · 字典页居首 · 页背靠背可跳读"),
    ("Parquet原理_支撑_统计与排序", "support", "◉", "统计与排序",
     "Statistics min/max/null_count · SizeStatistics 直方图 · ColumnOrder/BoundaryOrder · NaN/INT96 特例"),
    ("Parquet原理_支撑_PageIndex跳页", "support", "◫", "PageIndex 跳页",
     "ColumnIndex(每页 min/max/null_pages) + OffsetIndex(PageLocation) 尾部集中 · 两级裁页"),
    ("Parquet原理_支撑_布隆过滤器", "support", "◐", "布隆过滤器",
     "SplitBlock 分块布隆 + XxHash · 块内 8 word/256bit SIMD · 等值裁剪补 min/max"),
    ("Parquet原理_支撑_模块化加密", "support", "🔒", "模块化加密",
     "AES-GCM / GCM-CTR · footer 密钥 vs 列密钥 · AAD(prefix+suffix) 防换页 · 加密/明文 footer 双模式"),
    ("Parquet原理_支撑_写路径", "support", "✎", "写路径",
     "单遍写：缓冲行组→按大小切→页编码压缩→列块背靠背→FileMetaData 尾置+PAR1"),
    ("Parquet原理_支撑_读路径", "support", "◷", "读路径",
     "尾部读长度+FileMetaData · path_in_schema 列裁剪 · PageIndex/统计裁页 · 解码 rep/def 组装"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接触面主线 · 计算引擎如何用（类型系统 + schema 树 + 排序语义）"),
    ("support", "支撑主线 · 文件格式内部（10 条能力域）"),
]
...
# ===================================================================== #
PANO_NAME = "Parquet原理_全景主线框架"
# (x, y, w, h, 主线name) —— 一个模块可拆多行热区，一条主线可被多个区域指向
# 没有独立架构区域、需底部 chip 兜底的主线（全景 + 跨切/流程类能力域）
ARCH_ALWAYS_CHIP = [
    "Parquet原理_全景主线框架",
    "Parquet原理_支撑_压缩与页",
    "Parquet原理_支撑_模块化加密",
...
BRAND_TITLE = "一切知识皆索引"
BRAND_SUB = "Apache Parquet"
HOME_DESC = ("Apache Parquet 核心原理设计文档库的离线交互图谱——列式文件格式家族（自描述列存文件规范 + Thrift 元数据 IDL · "
             "被 Spark/Trino/DuckDB/Arrow/Iceberg 读写）。12 条主线、40+ 张手绘原理图，全部回本地 parquet-format 源码核实。"
             "点击项目总架构图任意模块即可下钻到对应主线。")
ARCH_SVG_NAME = "Parquet原理_总架构图.svg"
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="Apache Parquet 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
:root{
  ...
  --c-line:var(--c-border); --c-panel:var(--c-card);
}
