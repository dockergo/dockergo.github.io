...
| 论断 | 锚点 |
|---|---|
| PartitionSpec 主体：从源列派生分区（隐藏分区） | `api/src/main/java/org/apache/iceberg/PartitionSpec.java:53` |
| specId：每个 spec 有唯一 id | `api/src/main/java/org/apache/iceberg/PartitionSpec.java:60`、`:83` |
| 每个 manifest 记自己的 spec_id → 老数据留老分区规则 | `core/src/main/java/org/apache/iceberg/BaseSnapshot.java:212`（dataManifests）|
| 表可存多个 spec（演进不重写老数据） | `core/src/main/java/org/apache/iceberg/TableMetadata.java:460`（spec()）|
| 分区剪枝在 manifest 级用分区值过滤 | `core/src/main/java/org/apache/iceberg/ManifestGroup.java:177`（planFiles）|
