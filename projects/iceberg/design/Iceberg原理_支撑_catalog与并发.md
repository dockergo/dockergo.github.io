...
图注:Catalog 解决"表名 → 表"映射与生命周期(loadTable/createTable/dropTable/listTables),只存一个指针、不存数据。TableOperations 是所有 catalog 的提交契约:current(当前 TableMetadata)、refresh(重读)、commit(base, metadata)——校验 base==current 后原子换指针;BaseMetastoreTableOperations 用 writeNewMetadata 收敛公共流程。四类 catalog 差异只在"如何原子换指针"(锚点见下表)。
