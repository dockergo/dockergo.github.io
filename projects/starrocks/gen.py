...
_ap = argparse.ArgumentParser(description="生成 StarRocks 引擎交互式核心原理图谱(离线自包含 HTML)")
...
THEMES = [
    ...
    {"id": "schemachange", "icon": "⇲", "title": "DDL 数据定义", "cat": "iface", "ord": 0,
     "desc": "数据定义:建库建表(分区 Partition + 分桶 Distribution + 数据模型)与动态分区。design 走查——DDL 执行链路(DDLStmtExecutor→MetadataMgr→LocalMetastore 11 步)→ 分区与分桶两级切分 → 动态分区自动滚动,附调优/误区/总纲",
     "tabs": ["scwalk"]},
    {"id": "load", "icon": "⇥", "title": "DML 数据操纵", "cat": "iface", "ord": 1,
     "desc": "数据写入:design 走查——写入方式全景(INSERT/Stream/Broker/Routine Load/Overwrite)→ INSERT 复用 MPP 执行路径 → 三种 Load 对比,共享骨架 beginTxn→写 Rowset→commit→publish,附调优/误区/总纲",
     "tabs": ["loadwalk"]},
    {"id": "qlife", "icon": "◷", "title": "DQL 数据查询", "cat": "iface", "ord": 2,
     "desc": "数据查询:design 走查——查询执行全景 → 解析与分析(ANTLR Parser→Analyzer)→ 逻辑计划→物理计划→Fragment → MPP 分发(Coordinator→Deployer→BE),附调优/误区/总纲",
     "tabs": ["qlwalk"]},
    {"id": "dcl", "icon": "⛚", "title": "DCL 数据控制", "cat": "iface", "ord": 3,
     "desc": "数据控制:design 走查——可插拔认证(密码/LDAP/JWT/OAuth2)→ RBAC 授权(Authorizer→AuthorizationMgr,有效权限=直授+角色+组)→ 认证一次授权每语句的完整链路,附调优/误区/总纲",
     "tabs": ["dclwalk"]},

    # ── 支撑主线 · 引擎内部(8 条)· 严格以 design 走查为主干 ──
    {"id": "storageformat", "icon": "▤", "title": "存储引擎", "cat": "support", "ord": 0,
     "desc": "存储引擎:design 走查——Tablet/Rowset/Segment 三级 → 四种数据模型 → 持久化主键索引+删除向量 → 存算分离(Lake) → Segment 磁盘格式 → 本地表 vs 云原生表,附调优/误区/总纲",
     "tabs": ["stewalk"]},
    {"id": "opttech", "icon": "✲", "title": "优化技术", "cat": "support", "ord": 1,
     "desc": "查询提速:design 走查——Cascades CBO 三阶段 → 代价模型与统计 → Join 重排(左深/DP/贪心)→ Runtime Filter 与物化视图透明改写,附调优/误区/总纲",
     "tabs": ["optwalk"]},
    {"id": "execengine", "icon": "⚡", "title": "执行引擎", "cat": "support", "ord": 2,
     "desc": "BE 运行时:design 走查——Pipeline/Driver/Operator 三级 → 协作式调度与 Driver 状态机 → morsel 驱动并行 → 向量化 Chunk → 数据交换 Exchange,附调优/误区/总纲",
     "tabs": ["execwalk"]},
    {"id": "transaction", "icon": "⇅", "title": "事务一致性", "cat": "support", "ord": 3,
     "desc": "多版本并发:design 走查——导入事务 2PC 状态机(6 态)→ 版本发布 publish(COMMITTED→VISIBLE)→ 主键模型写入路径与删除向量,附调优/误区/总纲",
     "tabs": ["txnwalk"]},
    {"id": "metadata", "icon": "⬢", "title": "元数据", "cat": "support", "ord": 4,
     "desc": "控制面:design 走查——EditLog+BDB-JE(日志即真相)→ 镜像与检查点 → FE 高可用(leader/follower/observer)→ 存算分离元数据 StarManager,附调优/误区/总纲",
     "tabs": ["metawalk"]},
    {"id": "workload", "icon": "◐", "title": "资源与负载管理", "cat": "support", "ord": 5,
     "desc": "资源治理:design 走查——资源组模型(FE ResourceGroup→BE WorkGroup)→ 分类器归组 → CPU 加权调度与大查询熔断,附调优/误区/总纲",
     "tabs": ["wgwalk"]},
    {"id": "tabletsched", "icon": "⬡", "title": "集群管理与自愈", "cat": "support", "ord": 6,
     "desc": "控制面:design 走查——Tablet 健康检查与优先级 → 调度克隆修复 → 负载均衡 → 心跳与 Tablet 上报对账,附调优/误区/总纲",
     "tabs": ["tswalk"]},
    {"id": "compaction", "icon": "▨", "title": "后台任务", "cat": "support", "ord": 7,
     "desc": "后台维护:design 走查——Compaction 基线 vs 增量 → 评分驱动调度(两框架)→ 物化视图刷新(异步 MTMV/同步 Rollup)→ 动态分区,附调优/误区/总纲",
     "tabs": ["cpwalk"]},
...
_SVG_WALK_SPECS = {
    # ---- 接口主线 ----
    "scwalk": [("DDL 执行链路 · DDLStmtExecutor→LocalMetastore", "StarRocks原理_DDL_01执行链路.svg"),
               ("分区与分桶 · 两级数据切分", "StarRocks原理_DDL_02分区分桶.svg"),
               ("动态分区 · 后台自动滚动", "StarRocks原理_DDL_03动态分区.svg")],
    "loadwalk": [("写入方式全景 · 5 种导入 + 共享骨架", "StarRocks原理_DML_01写入全景.svg"),
                 ("INSERT 执行路径 · 复用 MPP", "StarRocks原理_DML_02INSERT.svg"),
                 ("Stream / Broker / Routine Load 对比", "StarRocks原理_DML_03三种Load.svg")],
    "qlwalk": [("查询执行全景 · SELECT 一生", "StarRocks原理_DQL_01执行全景.svg"),
               ("解析与分析 · ANTLR Parser→Analyzer", "StarRocks原理_DQL_02解析分析.svg"),
               ("计划生成 · 逻辑→物理→Fragment", "StarRocks原理_DQL_03计划生成.svg"),
               ("MPP 分发 · Coordinator→Deployer→BE", "StarRocks原理_DQL_04MPP分发.svg")],
    "dclwalk": [("认证 · 可插拔身份验证", "StarRocks原理_DCL_01认证.svg"),
                ("授权 · RBAC 权限模型", "StarRocks原理_DCL_02RBAC.svg"),
                ("完整链路 · 认证一次授权每语句", "StarRocks原理_DCL_03完整链路.svg")],
    # ---- 支撑主线 ----
    "stewalk": [("存储层级 · Tablet→Rowset→Segment", "StarRocks原理_存储_01层级.svg"),
                ("四种数据模型 · KeysType", "StarRocks原理_存储_02数据模型.svg"),
                ("主键索引 · 持久化索引 + 删除向量", "StarRocks原理_存储_03主键索引.svg"),
                ("存算分离 · 云原生表 / Lake", "StarRocks原理_存储_04存算分离.svg"),
                ("Segment 磁盘格式", "StarRocks原理_存储_05Segment格式.svg"),
                ("本地表 vs 云原生表", "StarRocks原理_存储_06两形态对比.svg")],
    "optwalk": [("CBO 优化器 · Cascades 三阶段", "StarRocks原理_优化_01CBO.svg"),
                ("代价模型与统计", "StarRocks原理_优化_02代价统计.svg"),
                ("Join 重排 · 左深/DP/贪心", "StarRocks原理_优化_03Join重排.svg"),
                ("Runtime Filter 与物化视图改写", "StarRocks原理_优化_04RF与MV.svg")],
    "execwalk": [("Pipeline / Driver / Operator 三级", "StarRocks原理_执行_01Pipeline.svg"),
                 ("协作式调度与 Driver 状态机", "StarRocks原理_执行_02调度.svg"),
                 ("Morsel 驱动并行", "StarRocks原理_执行_03Morsel.svg"),
                 ("向量化 Chunk", "StarRocks原理_执行_04Chunk.svg"),
                 ("数据交换 Exchange", "StarRocks原理_执行_05Exchange.svg")],
    "txnwalk": [("导入事务 2PC 状态机", "StarRocks原理_事务_01状态机.svg"),
                ("版本发布 · publish→visible", "StarRocks原理_事务_02版本发布.svg"),
                ("主键模型写入路径", "StarRocks原理_事务_03主键写入.svg")],
    "metawalk": [("EditLog + BDB-JE · 日志即真相", "StarRocks原理_元数据_01EditLog.svg"),
                 ("镜像与检查点", "StarRocks原理_元数据_02镜像检查点.svg"),
                 ("FE 高可用 · leader/follower/observer", "StarRocks原理_元数据_03高可用.svg"),
                 ("存算分离元数据 · StarManager", "StarRocks原理_元数据_04StarManager.svg")],
    "wgwalk": [("资源组模型 · FE 定义→BE 执行", "StarRocks原理_资源_01资源组.svg"),
               ("分类器 · 请求归组", "StarRocks原理_资源_02分类器.svg"),
               ("CPU 加权调度与大查询熔断", "StarRocks原理_资源_03大查询熔断.svg")],
    "tswalk": [("Tablet 健康检查与优先级", "StarRocks原理_集群_01健康检查.svg"),
               ("Tablet 调度与克隆修复", "StarRocks原理_集群_02调度克隆.svg"),
               ("负载均衡", "StarRocks原理_集群_03负载均衡.svg"),
               ("心跳与 Tablet 上报对账", "StarRocks原理_集群_04心跳上报.svg")],
    "cpwalk": [("Compaction · 基线 vs 增量", "StarRocks原理_后台_01Compaction.svg"),
    ...
    "panowalk": [("双维模型 · 能力域 × 执行时机", "StarRocks原理_双维模型.svg"),
                 ...
                 ("依赖矩阵 · 接口 × 能力域", "StarRocks原理_依赖矩阵.svg"),
                 ("能力域依赖关系图", "StarRocks原理_依赖关系图.svg")],
}
# 快速开始「上手总览」用独立复合渲染器 renderQsTour(总览 SVG + 5 步选择器 + 内容区),不走通用 SVG-walk
_QSTOUR_OVERVIEW_B64 = _design_b64("StarRocks上手_00总览.svg")  # 无此素材 → 空;quickstart 未挂载
...
_WALK_PROSE = {
    "scwalk": "StarRocks原理_DDL数据定义.md",  "loadwalk": "StarRocks原理_DML数据操纵.md",
    "qlwalk": "StarRocks原理_DQL数据查询.md",  "dclwalk": "StarRocks原理_DCL数据控制.md",
    "stewalk": "StarRocks原理_支撑_存储引擎.md", "optwalk": "StarRocks原理_支撑_优化技术.md",
    "execwalk": "StarRocks原理_支撑_执行引擎.md", "txnwalk": "StarRocks原理_支撑_事务一致性.md",
    "metawalk": "StarRocks原理_支撑_元数据.md",  "wgwalk": "StarRocks原理_支撑_资源与负载管理.md",
    "tswalk": "StarRocks原理_支撑_集群管理与自愈.md", "cpwalk": "StarRocks原理_支撑_后台任务.md",
}
...
<title>StarRocks 引擎 · 交互式核心原理图谱</title>
<style>
:root{
...
<div id="loadingOverlay" role="status" aria-live="polite">
  <div class="lo-inner">
    ...
    <div class="lo-title">StarRocks Engine Atlas</div>
      ...
      <div class="nav-mode nav-arch active" id="navArch">
        <div class="arch-stage">
          <div class="arch-canvas">
            <img class="arch-img" src="data:image/svg+xml;base64,__ARCH_SVG_B64__" alt="StarRocks 总架构图" draggable="false"/>
            __ARCH_HOTSPOTS__
          </div>
        </div>
