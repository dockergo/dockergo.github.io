...
每次导入产出一个新 Rowset(小文件),读时要归并众多 Rowset——Compaction 把它们合并成更少更大的有序文件。抽象基类 **Compaction**(纯虚 `compact` / `pick_rowsets_to_compact`),合并执行 `do_compaction_impl` 分派**水平合并**(`_merge_rowsets_horizontally`,列少)或**垂直合并**(`_merge_rowsets_vertically`,列多)。

两种粒度:
- **CumulativeCompaction**(增量):合并 cumulative point 之上的新增小 Rowset,高频轻量;`calculate_cumulative_point` 推进分界点。
- **BaseCompaction**(基线):把增量结果并入基线大文件,低频重量;`base_cumulative_delta_ratio` 控触发。

选谁由策略 **DefaultCumulativeBaseCompactionPolicy** 取二者评分较高者(平手偏增量);另有 `SizeTieredCompactionPolicy` 按大小分层。
...
调度有两套框架,由 `enable_event_based_compaction_framework` 切换:
- **传统**:每盘固定 base/cumulative 线程,`find_best_tablet_to_compaction` 选盘上最该压的 Tablet。
- **事件驱动**:**CompactionManager** 的 `_schedule` 用 `CompactionCandidateComparator` 按 **compaction score 降序**排候选,`update_tablet` 读 `tablet->compaction_score` 喂入。

云原生表的 Compaction 是 **RPC 驱动(FE→BE)**:`lake::CompactionScheduler::compact` 是 BRPC handler,worker `thread_task → do_compaction → tablet_mgr->compact`;策略 `lake::CompactionPolicy::create` 选 `PrimaryCompactionPolicy` / `SizeTiered` / `BaseAndCumulative`。
...
- **异步物化视图(MTMV)**:独立的表,后台按调度刷新。走任务框架 `TaskRun.executeTaskRun`,由 `MVRefreshProcessorFactory` 选 `MVPCTRefreshProcessor`(分区变化跟踪,PCT)/ `MVIVMRefreshProcessor`(增量)/ `MVHybridRefreshProcessor`。PCT 刷新用 insert-overwrite 计划经 `MVRefreshExecutor.executePlan` 执行;时效由 `MVTimelinessArbiter` 判定哪些分区过期需刷。
- **同步物化视图 / Rollup**:随基表写入同步维护,本质是 schema-change。`MaterializedViewHandler.createMaterializedViewJob` 建 `RollupJobV2` 后台构建。
...
时序表常按天/小时分区,老分区要删、新分区要提前建——**DynamicPartitionScheduler**(`extends LeaderDaemon`)后台周期 `executeDynamicPartitionForTable` 按 `DynamicPartitionProperty`(保留几天、预建几天、分桶数)自动加/删分区。这是"元数据能力域的异步部分"落在后台任务的典型例子:前台不感知,分区滚动自动完成。

---

## 深化 · 源码坐标（Compaction 内核 / MV）

| 结构 | 定义 | 职责 |
|---|---|---|
| Compaction 基类 | `be/src/storage/compaction.h:42` | 纯虚 compact/pick_rowsets |
| 框架开关 | `be/src/storage/olap_server.cpp:128` | enable_event_based_compaction_framework |
| DefaultCumulativeBaseCompactionPolicy | `be/src/storage/default_compaction_policy.cpp:27` | base/cumulative 评分择优 |
| CompactionCandidate | `be/src/storage/compaction_candidate.h:65` | 按 score 降序排候选 |
| lake::CompactionPolicy | `be/src/storage/lake/compaction_policy.cpp:614` | 云原生策略选择 |
| MVRefreshExecutor | `scheduler/mv/MVRefreshExecutor.java:29` | PCT insert-overwrite 刷新 |
| MaterializedViewHandler | `alter/MaterializedViewHandler.java:111` | 同步 MV/Rollup 构建 |
