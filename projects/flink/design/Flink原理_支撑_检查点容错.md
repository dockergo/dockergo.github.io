...
| CheckpointOptions | `core/execution/CheckpointOptions.java:106` | 对齐/非对齐/对齐超时选项 |
| AbstractStreamOperator.snapshotState | `streaming/api/operators/AbstractStreamOperator.java:421` | 算子快照钩子(产 OperatorSnapshotFutures) |
| CheckpointedFunction.snapshotState | `api/checkpoint/CheckpointedFunction.java:151` | 用户函数快照钩子 |
| EmbeddedRocksDBStateBackend | `state/rocksdb/EmbeddedRocksDBStateBackend.java:521` | setEnableIncrementalCheckpointing 增量开关 |
| OperatorStateHandle.Mode | `checkpoint/OperatorStateHandle.java:46` | operator state 重分布模式(SPLIT/UNION/BROADCAST) |
| CheckpointType / CheckpointProperties | `checkpoint/CheckpointType.java:30` | 区分检查点(增量共享)vs 保存点(自包含) |
