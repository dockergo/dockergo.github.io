...
| 结构 | 定义 | 职责 |
|---|---|---|
| State 基接口 | `flink-core-api/.../state/State.java:32` | 状态根接口(clear) |
| ValueState / ListState / MapState | `state/ValueState.java:40`,`ListState.java:44`,`MapState.java:45` | keyed 状态三种类型 |
| OperatorStateStore | `state/OperatorStateStore.java:78` | operator state(getListState/Union/Broadcast) |
| StateDescriptor | `flink-core/.../state/StateDescriptor.java:60` | 状态声明(类型/序列化器) |
| HashMapStateBackend / StateTable | `state/hashmap/HashMapStateBackend.java:63`,`heap/StateTable.java:79` | 堆内后端(StateMap[] 按 key-group 分) |
| EmbeddedRocksDBStateBackend / RocksDBKeyedStateBackend | `state/rocksdb/EmbeddedRocksDBStateBackend.java:98`,`RocksDBKeyedStateBackend.java:227` | RocksDB 后端(每状态一 ColumnFamily) |
