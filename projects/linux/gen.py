...
_ap = argparse.ArgumentParser(description="生成 Linux 内核交互式核心原理图谱(离线自包含 HTML)")
...
TABS = [
    # design 原理图走查(优化型混合:各主题首个「原理详解」tab)
    ("scwalk", "原理详解", ""),
    ("loadwalk", "原理详解", ""),
    ("qlwalk", "原理详解", ""),
    ("dclwalk", "原理详解", ""),
    ...
    ("dataorg", "数据组织架构", ""),
    ("seq", "时序 · 端到端", SEQ),
    ("feflow", "原理 ① FE 查询规划", FE_FLOW),
    ("lakerel", "湖仓层次关系", ""),
    ("beflow", "原理 ② BE 扫描执行", BE_FLOW),
    ("olapflow", "内表 OLAP 扫描", OLAP_FLOW),
    ("writeflow", "数据写入链路", WRITE_FLOW),
    ("memflow", "内存全景", MEM_FLOW),
    ("wgflow", "资源组模型", WG_FLOW),
    ("metaflow", "原理 · 元数据流", ""),
    ("tabletflow", "原理 · 副本调度", ""),
    ("scflow", "原理 · 变更流程", ""),
    ("icebergflow", "Iceberg 读取", ""),
    ("gcflow", "Group Commit", ""),
    ("rlflow", "Routine Load", ""),
    ("vecflow", "算子执行原理", ""),
    ("vectree", "向量化结构", ""),
    ("joinflow", "分布式 Join 策略", ""),
    ("idxarch", "索引体系架构", ""),
    ("vecsearch", "向量检索与倒排索引", ""),
    ("idxpano", "索引全景透视", ""),
    ("idxchain", "索引过滤链路", ""),
    ...
    ("optobserve", "可观测性透视", ""),
    ("optpersp", "原理概览", ""),
    ("mvcompare", "同步/异步 MV 对比", ""),
    ("optcompare", "RBO/CBO/HBO 对比", ""),
    ("metaseq", "元数据时序", ""), ("metatree", "元数据结构", ""),
    ("tabletseq", "调度时序", ""), ("tablettree", "调度结构", ""),
    ("scseq", "变更时序", ""), ("sctree", "变更结构", ""),
    ("optflow", "优化流程", OPT_FLOW),
    ("pipeflow", "调度模型", ""),
    ("mvflow", "MV 全景", ""),
    ("statflow", "统计全景", ""),
    ("loadflow", "导入导出全景", ""),
    ...
    ("olapdata", "列存结构", ""),
    ("integstruct", "存储+索引结构", ""),
    ("bedata", "结构 · ORC on-disk", ""),
    ("writedata", "落盘结构", ""),
    ("profiletree", "Profile 层级", ""),
    ("memtree", "MemTracker 树", ""),
    ("stattbl", "表统计", ""),
    ("statcol", "列统计", ""),
    ("threadtree", "线程架构", ""),
    ("wgtree", "资源组隔离", ""),
    ...
    ("versiongraph", "版本读快照", ""),
    ("profilesrc", "排障闭环", ""),
    ("compactflow", "Compaction 原理", ""),
    ("compacttree", "Compaction 结构", ""),
    ("compactseq", "Compaction 时序", ""),
    ("txnflow", "原理", ""),
    ("txntree", "事务结构", ""),
    ("txnseq", "事务时序", ""),
    ("hudiflow", "Hudi Parquet 读取", ""),
    ("hiveorcflow", "Hive ORC 读取", ""),
    ("threadseq", "线程调度", ""),
    ("memseq", "内存调度", ""),
    ("wgseq", "负载时序", ""),
    ("olapseq", "OLAP 时序", ""),
    ...
    ("profileflow", "原理", ""),
    ("profileseq", "诊断时序", ""),
    ("explaincmd", "EXPLAIN 命令", ""),
    ("profileview", "Profile 透视", ""),
    ("diagprin", "诊断原理", ""),
    ("commonqa", "常见问题", ""),
    ("writedata2", "落盘结构", ""),
    ("sctree2", "变更结构", ""),
    ...
    ("wgtree2", "资源组隔离", ""),
    ("tablettree2", "调度结构", ""),
    ("compacttree2", "Compaction 结构", ""),
    ("profilefmt", "Profile 格式", ""),
    ("rbotree", "RBO 规则", ""),
    ("cbotree", "CBO 代价", ""),
    ("hbotree", "HBO 历史", ""),
    # 优化技术:5 个顶部块(每块内部再垂直 TAB 切换,内容复用上面已定义的视图)
    ("optq", "查询优化器", ""),
    ("optpipe", "Pipeline", ""),
    ("optrf", "Runtime Filter", ""),
    ("opttopn", "TOPN", ""),
    ("optstat", "统计信息", ""),
    # 查询生命周期主题(4 tab):主线多图 / 运行时时序 / 调优开关表 / 术语表
    ("qlife", "生命周期主线", ""),
    ("qlifeseq", "运行时时序", ""),
    ("qlifevars", "调优开关", ""),
    ("qlifeterms", "术语表", ""),
    # 存储引擎(6 嵌套块,每块内部再垂直 TAB;内容复用上面已定义的视图)
    ("steOlap", "内表存储", ""),
    ("steFmt", "存储格式", ""),
    ("steExt", "外表读取", ""),
    ("steIdx", "索引与检索", ""),
    ...
    # DCL 数据控制
    ("dclprin", "权限体系", ""),
    # 原理概览(保留有独有价值的 3 个;8 个强重叠已删,内容回到各主题旧视图)
    ("optprin", "原理概览", ""),
    ("tsprin", "原理概览", ""),
    ("cpprin", "原理概览", ""),
]
...
TAB_META = {
    # design 原理图走查(权威手绘 SVG · 逐图垂直 TAB)
    "scwalk":   ("◈", "原理详解 · 元数据变更生命周期:接入转发→EditLog→Checkpoint→变更决策", "原理"),
    "loadwalk": ("◈", "原理详解 · 写入生命周期:导入方式→2PC 状态机→写入路径→提交发布→选型", "原理"),
    "qlwalk":   ("◈", "原理详解 · 查询执行全景:接入缓存→编译→算子树→MPP→Pipeline→汇聚→Join", "原理"),
    "dclwalk":  ("◈", "原理详解 · 定义线×执行线:RBAC 权限→请求管控→资源隔离→可插拔认证", "原理"),
    "stewalk":  ("◈", "原理详解 · 存储引擎:数据组织→三种数据模型→多级跳过链→主键两实现", "原理"),
    "optwalk":  ("◈", "原理详解 · 优化技术:优化四位置→CBO 代价优化→RBO/CBO/HBO 三段式", "原理"),
    "execwalk": ("◈", "原理详解 · 执行引擎:四级展开→MPP→Pipeline 阻塞点→线程模型→Exchange", "原理"),
    "txnwalk":  ("◈", "原理详解 · 事务一致性:写读并发→2PC 状态机→MVCC→版本链对齐", "原理"),
    "metawalk": ("◈", "原理详解 · 元数据:组织架构→内存+日志+镜像三位一体→单写多读拓扑", "原理"),
    "wgwalk":   ("◈", "原理详解 · 资源治理:拦截→约束→审计闭环 + WG 隔离 + 多级内存 + 可观测", "原理"),
    "tswalk":   ("◈", "原理详解 · 集群自愈:Detect→Decide→Act 闭环 + 副本健康 + 调度优先级", "原理"),
    "cpwalk":   ("◈", "原理详解 · 后台任务:异步守护摊平成本 + Compaction 分层合并 + 三放大权衡", "原理"),
    ...
    "qsingest":   ("③", "数据写入 · Stream Load / INSERT / Broker Load", "整体架构"),
    "qsexport":   ("④", "数据导出 · SELECT INTO OUTFILE / EXPORT / mysqldump", "整体架构"),
    "tourjoin":   ("⑤", "查询分析 · 逻辑流程 + 物理执行计划 + 算子流程", "整体架构"),
    "archintegrated": ("◈", "集成架构 · 数据源→接入→数仓分层(ODS/DWD/DWS/ADS)→服务→治理", "整体架构"),
    "dataorg":        ("◈", "数据组织 · Catalog→Table→Partition→Index→Tablet→Replica→Rowset→Segment→Page→Block", "数据结构"),
    "seq":       ("⇄", "端到端时序 · 一次 SELECT 的生命周期", "查询流程"),
    "lakerel": ("⇄", "湖仓层次 · 引擎→表格式→存储格式→压缩→文件存储 关系", "整体架构"),
    "feflow":    ("◱", "FE 查询规划 · 解析→CBO→Split→下发", "FE 流程"),
    "beflow":    ("◲", "BE 外表扫描 · Pipeline 调度 → FileScanner 格式分派(详见「存储格式」)", "BE 流程"),
    "olapflow":  ("▤", "内表 OLAP 扫描 · tablet→segment 向量化", "查询流程"),
    "writeflow": ("✎", "数据写入 · LSM 内存表→异步 flush→rowset", "写入流程"),
    "memflow":   ("▧", "内存管理 · Tracker 树→进程仲裁→GC/反压", "整体架构"),
    "wgflow":    ("◐", "负载管理 · 资源组→排队→CPU/内存隔离", "整体架构"),
    "optflow":   ("✲", "优化器 · 绑定→RBO→Cascades CBO", "整体架构"),
    "pipeflow":  ("⚡", "Pipeline · 构建→调度→pull执行→交换", "整体架构"),
    "mvflow":    ("▦", "物化视图 · 异步MTMV/透明改写/同步Rollup", "整体架构"),
    "statflow":  ("∿", "统计信息 · 采集→缓存→喂 CBO", "整体架构"),
    "loadflow":  ("⇥", "导入导出 · Stream/Broker→事务→tablet", "整体架构"),
    "rfflow":    ("⧗", "Runtime Filter · join build→probe scan 裁行", "整体架构"),
    "topnflow":  ("↧", "TOPN · 堆 top-k→RuntimePredicate 下推裁剪", "整体架构"),
    "pipeseq":   ("⇄", "时序 · fragment 构建到调度执行", "时序流程"),
    "writeseq":  ("⇄", "时序 · 导入到 tablet 落盘", "时序流程"),
    "insseq":    ("⇄", "时序 · 写入链路 LoadChannel→flush→rowset", "时序流程"),
    "rfseq":     ("⇄", "时序 · RF 构建到 probe 裁行", "时序流程"),
    "topnseq":   ("⇄", "时序 · 堆维护到谓词下推", "时序流程"),
    "integstruct": ("▤", "并行结构 · 内表存储布局 vs 索引加速(顶部示例表)", "数据结构"),
    "olapdata":  ("▤", "数据结构 · segment 列存 block + 稀疏索引", "数据结构"),
    "bedata":    ("▤", "数据结构 · ORC stripe + row group 索引", "数据结构"),
    "writedata": ("▤", "数据结构 · rowset/segment LSM 落盘布局", "数据结构"),
    "profiletree": ("⧉", "Profile 五段树 · Summary→Merged/Detail→算子计数器", "Profile"),
    "memtree": ("▧", "数据结构 · MemTracker 树:Process→Type→Query→算子", "数据结构"),
    "stattbl": ("∿", "数据结构 · 表级统计 TableStatsMeta + 健康度采集", "数据结构"),
    "statcol": ("∿", "数据结构 · 列级统计 ColumnStatistic + 分区 → 喂 CBO", "数据结构"),
    "threadtree": ("⚙", "线程架构 · BE 全线程模型(≈840):7 层 × 16 组 + 关键交互 + 隐藏 BlockFileCache", "数据结构"),
    "wgtree": ("◐", "数据结构 · WorkloadGroup → cgroup CPU/内存/IO 隔离", "数据结构"),
    "memotree": ("✲", "数据结构 · Cascades Memo/Group/GroupExpression 搜索空间", "数据结构"),
    "mvtree": ("▦", "数据结构 · MV 透明改写:StructInfo/MaterializationContext", "数据结构"),
    "anntree": ("✦", "数据结构 · ANN(faiss)+ 倒排索引组织", "数据结构"),
    "pipetree": ("⚡", "数据结构 · PipelineTask/Dependency/work-stealing 队列", "数据结构"),
    "jemalloctree": ("▧", "数据结构 · jemalloc ↔ JVM/JNI ↔ 应用层内存交互", "数据结构"),
    "jeflow": ("▧", "原理 · jemalloc 劫持→50ms 维护→脏页/tcache 回收→衰减调控", "整体架构"),
    "metaflow": ("⬢", "原理 · EditLog→BDB-JE 复制→选主追平→Checkpoint", "整体架构"),
    "tabletflow": ("⬡", "原理 · 巡检→健康度→调度排队→克隆修复/均衡", "整体架构"),
    "scflow": ("⇲", "原理 · ALTER→影子索引→双写水位→BE 转换→原子切换", "整体架构"),
    "icebergflow": ("❄", "原理 · 多目录元数据→Iceberg manifest 切分→BE delete 过滤", "查询流程"),
    "gcflow": ("⇥", "原理 · 高并发小批量:攒批队列+WAL→组提交合成大 rowset", "查询流程"),
    "rlflow": ("⇥", "原理 · Kafka 持续消费:作业级→任务级调度→分区区间导入", "查询流程"),
    "vecflow": ("⚡", "原理 · 列式 Block→VExpr 批量求值→算子 pull/push→filter+SIMD", "整体架构"),
    "vectree": ("▤", "数据结构 · Block/Column 家族/PODArray/Filter", "数据结构"),
    "joinflow": ("⋈", "原理 · 请求候选→代价择优→colocate/bucket/shuffle/broadcast→BE 交换", "整体架构"),
    "idxpano": ("▦", "透视表 · 9 类索引 × 16 维度(核心定位/粒度/精度/开销/收益/限制/心智模型)", "数据结构"),
    "idxarch": ("◈", "体系架构 · 存储层位置 / 查询协同流程 / 能力分层(3 图竖 tab 切换)", "数据结构"),
    "vecsearch": ("✦", "向量检索与倒排索引 · 倒排全文(INVERTED)/向量 ANN 两图,建表+查询 SQL 与执行链", "查询流程"),
    "idxchain": ("⇊", "过滤链路 · ShortKey/PK→ZoneMap→Bloom→Inverted→DeleteBitmap→向量化二次过滤", "查询流程"),
    "dedupflow": ("∪", "原理 · count(distinct)改写→Bitmap 精确/HLL 近似→预聚合合并", "整体架构"),
    "fmtflow": ("▤", "原理 · FileScanner 格式分派→ORC/Parquet/Hudi/Iceberg + 内表列存 V2", "整体架构"),
    "fmtcompare": ("▤", "并行对比 · ORC/Hudi/Iceberg/内表 V2/V3 读取方式 + 文件结构同屏", "数据结构"),
    "glossary": ("▦", "术语表 · FE/BE/CN/Tablet/Rowset/Nereids… 首次解释 + 缩写", "整体架构"),
    "compare": ("▦", "架构对比 · Doris vs ClickHouse/StarRocks/Spark/Trino/DuckDB 设计取舍", "整体架构"),
    "failure": ("▦", "失败与一致性 · 各流程失败点/重试/幂等/可见性", "整体架构"),
    "bottleneck": ("▦", "瓶颈模型 · 关键链路最易慢点与调优方向", "整体架构"),
    "archcompare": ("▦", "部署形态对比 · 湖仓/存算一体/存算分离/冷热分离 取舍", "部署形态"),
    "deployview": ("◈", "部署形态概览 · 湖仓查询/存算一体/存算分离/冷热分离 四种部署(垂直 TAB 切换)", "部署形态"),
    "mvcompare": ("▦", "物化视图对比 · 同步 Rollup vs 异步 MTMV", "数据结构"),
    "optcompare": ("▦", "优化器对比 · RBO 规则/CBO 代价/HBO 历史", "数据结构"),
    "optgoal": ("◈", "优化目标 · 按「减少什么资源消耗」归 9 类(核心目录)", "整体架构"),
    "optaxis": ("◈", "优化总表 · 9 类资源主轴 × (生命周期/FE/BE/Storage/粒度/时机/策略/收益/观察入口)", "整体架构"),
    "optrelation": ("▦", "主轴关系 · 九类资源主轴的本质问题与相互作用", "数据结构"),
    "optarch": ("◈", "优化架构图 · 9 类主轴在 FE 规划/BE 执行/Storage/写入主线上的落点", "整体架构"),
    "optlifecycle": ("▦", "生命周期透视 · SQL 接入→分析→改写→CBO→Fragment→Pipeline→Scan→返回 各阶段优化", "整体架构"),
    "optgranularity": ("▦", "数据粒度透视 · 查询/表/分区/Tablet/Bucket/文件/Rowset/Segment/Page/列/行/Batch 各级跳过", "数据结构"),
    "optphase": ("▦", "静态/动态透视 · 规划期 vs 执行期 vs 存储内生 + 常见误区纠偏", "数据结构"),
    "optoperator": ("▦", "算子透视 · Scan/Filter/Join/Agg/Sort/Exchange/Sink/Expr 的成本与核心优化", "数据结构"),
    "optworkload": ("▦", "工作负载透视 · 点查/明细/宽表/Join/聚合/TopN/Dashboard/湖仓/日志/高并发/高频导入 优先策略", "数据结构"),
    "optobserve": ("▦", "可观测性透视 · EXPLAIN/Profile 各观察入口的重点指标与对应优化", "数据结构"),
    "optpersp": ("✦", "原理概览 · 资源消耗/生命周期/数据粒度/执行算子/工作负载/可观测性 六维透视(垂直 TAB 切换)", "优化原理"),
    "metaseq": ("⇄", "时序 · DDL→日志→复制→重放→checkpoint", "时序流程"),
    "metatree": ("⬢", "数据结构 · EditLog/BDB-JE/角色/Checkpoint", "数据结构"),
    "tabletseq": ("⇄", "时序 · 巡检→排队→克隆→回收→均衡", "时序流程"),
    "tablettree": ("⬡", "数据结构 · TabletStatus/Checker/Scheduler/Clone", "数据结构"),
    "scseq": ("⇄", "时序 · ALTER→影子→双写→转换→切换", "时序流程"),
    "sctree": ("⇲", "数据结构 · 影子索引/watershed/状态机/转换器", "数据结构"),
    "mvspjg": ("▦", "原理 · SPJG 透明改写算法(S/P/J/G 逐维匹配)", "示例"),
    "mvscene": ("▦", "场景 · MV 适用/不适用边界 + 代价护栏", "示例"),
    "profilediag": ("⧉", "诊断 · 用计数器定位瓶颈/倾斜/内存/IO", "示例"),
    "aggmerge": ("▤", "数据流转 · AGG 模型预聚合→compaction 合并(真实值)", "数据结构"),
    "mowmerge": ("▤", "数据流转 · MoW delete bitmap 标删旧版本(真实值)", "数据结构"),
    "rfstruct": ("⧗", "数据结构 · RF 三型 filter(IN/BLOOM/MinMax 真实值)", "数据结构"),
    "topnstruct": ("↧", "数据结构 · top-k 最小堆 + 极值下推(真实值)", "数据结构"),
    "loadstruct": ("⇥", "数据结构 · 分区+分桶路由到 tablet(真实值)", "数据结构"),
    "txnswim": ("⇅", "专家图 · 写入事务双泳道(数据落盘 vs 事务可见)", "数据结构"),
    "versiongraph": ("⇅", "专家图 · VersionGraph 读快照选版本 + Compaction 语义保持", "数据结构"),
    "profilesrc": ("⧉", "专家图 · 排障闭环 query_id→Profile→Counter→Metrics→源码", "数据结构"),
    "compactflow": ("▨", "原理 · 调度→选 rowset→归并→bitmap→回收", "整体架构"),
    "compacttree": ("▨", "数据结构 · Version/cumulative point/version graph", "数据结构"),
    "compactseq": ("⇄", "时序 · producer→policy→merge→GC", "时序流程"),
    "txnflow": ("⇅", "原理 · prepare→commit→publish→可见", "整体架构"),
    "txntree": ("⇅", "数据结构 · rowset version/version graph/MVCC", "数据结构"),
    "txnseq": ("⇄", "时序 · 两阶段事务 + 版本生效", "时序流程"),
    "hudiflow": ("⇄", "原理 · Hudi COW 原生 / MOR JNI 合并", "查询流程"),
    "hiveorcflow": ("⇄", "原理 · ORC split→SearchArgument→延迟物化", "查询流程"),
    "threadseq": ("⇄", "线程调度 · 一条 SQL 的线程流转:brpc 接入→bthread→pipeline→scan/flush 池", "时序流程"),
    "memseq": ("⇄", "内存调度 · 分配→consume→try_reserve→仲裁→spill 落盘的时间线", "时序流程"),
    "wgseq": ("⇄", "时序 · 排队准入→cgroup→执行", "时序流程"),
    "olapseq": ("⇄", "时序 · 内表扫描 rowset→segment→延迟物化", "时序流程"),
    "optseq": ("⇄", "时序 · parse→analyze→rewrite→optimize", "时序流程"),
    "mvseq": ("⇄", "时序 · MTMV 刷新 + 查询期透明改写", "时序流程"),
    "statseq": ("⇄", "时序 · 自动采集→落库→喂 CBO", "时序流程"),
    "profileflow": ("⧉", "原理 · 埋点→上报→FE 聚合→五段树", "整体架构"),
    "profileseq": ("⇄", "时序 · profile 采集上报与聚合", "时序流程"),
    "explaincmd": ("⧉", "EXPLAIN 诊断命令 · 一条 SQL 随 PARSED→ANALYZED→REWRITTEN→OPTIMIZED→DISTRIBUTED 的计划演变(垂直 TAB)", "查询流程"),
    "profileview": ("⧉", "Profile 透视 · 五段格式结构 × 采集/上报/聚合实现逻辑", "诊断"),
    "diagprin": ("◎", "诊断原理 · 慢 SQL 反查源码闭环 + Counter→根因判读 + Metrics 交叉", "诊断"),
    "commonqa": ("▦", "常见问题 · 写侧失败恢复/一致性 + 读侧瓶颈模型(读写两类)", "诊断"),
    "writedata2": ("▤", "落盘结构 · Rowset→Segment 列存布局 + 主键索引(MoW)", "数据结构"),
    "sctree2": ("⇲", "变更结构 · 影子索引/水位双写/状态机/BE 转换器", "数据结构"),
    "loadstruct2": ("⇥", "分桶结构 · Block→分区+分桶 hash→tablet 路由→DeltaWriter", "数据结构"),
    "mowmerge2": ("◑", "MoW 合并 · delete bitmap 标删旧版本(读跳过免归并)", "数据结构"),
    "txntree2": ("⇅", "事务与 MVCC 版本结构 · 2PC + VersionGraph + MoW 可见性", "数据结构"),
    "txnswim2": ("⇅", "写入事务双泳道 · 数据落盘 ≠ 对读可见", "数据结构"),
    "versiongraph2": ("⇅", "VersionGraph · 读快照拼连续 version 区间选 rowset", "数据结构"),
    "metatree2": ("⬢", "元数据结构 · EditLog + BDB-JE 复制 + 角色 + Checkpoint", "数据结构"),
    "wgtree2": ("◐", "资源组隔离结构 · CPU/内存/并发/IO 四维 + 组内线程池", "数据结构"),
    "tablettree2": ("⬡", "副本调度修复结构 · 12 态 + Checker/Scheduler + Clone/Rebalancer", "数据结构"),
    "compacttree2": ("▨", "Compaction 数据结构 · Version/cumulative point/策略/GC", "数据结构"),
    "profilefmt": ("⧉", "Profile 数据格式 · Summary→Execution→Fragment→Pipeline→Operator 计数器", "数据结构"),
    "rbotree": ("✲", "原理 · RBO 规则集(谓词/裁剪/下推/重排)", "数据结构"),
    "cbotree": ("✲", "原理 · CBO Memo 代价枚举择优", "数据结构"),
    "hbotree": ("✲", "原理 · HBO 历史行数反馈修正估算", "数据结构"),
    "optq": ("✲", "查询优化器 · 绑定/RBO/Cascades CBO/Join 策略(内部垂直切换)", "优化技术"),
    "optpipe": ("⚡", "Pipeline · fragment→task 树/work-stealing/pull-sink/向量化(内部垂直切换)", "优化技术"),
    "optrf": ("⧗", "Runtime Filter · IN/BLOOM/MinMax 运行时裁行(内部垂直切换)", "优化技术"),
    "opttopn": ("↧", "TOPN · 堆 top-k + RuntimePredicate 极值下推(内部垂直切换)", "优化技术"),
    "optstat": ("∿", "统计信息 · 自动采集喂 CBO 行数估算(内部垂直切换)", "优化技术"),
    "qlife": ("◷", "查询生命周期主线 · Query Text→Token→Plan→Fragment→Pipeline→结果 的形态演进(垂直 TAB)", "查询流程"),
    "qlifeseq": ("⇄", "运行时时序 · FE 编译下发 ⇄ BE 并行执行 ⇄ 客户端流式返回", "时序流程"),
    "qlifevars": ("⚙", "调优开关速查 · 贯穿全链路的 session variables 与相关环节", "示例"),
    "qlifeterms": ("▤", "术语表 · LogicalPlan/PlanFragment/Memo/Breaker/MVCC 等核心概念", "示例"),
    "steOlap": ("▤", "内表存储 · OLAP 扫描/列存 Segment/聚合合并(内部垂直切换)", "存储引擎"),
    "steFmt": ("▤", "存储格式 · V2 footer + 三索引 + 页编码 + 格式并行对比(内部垂直切换)", "存储引擎"),
    "steExt": ("▤", "外表读取 · Hive ORC/Hudi/Iceberg 湖仓格式读取(内部垂直切换)", "存储引擎"),
    "steIdx": ("🔎", "索引与检索 · 向量 ANN + 全文倒排 + 索引全景/过滤链路(内部垂直切换)", "存储引擎"),
    "steMv": ("▦", "物化视图 · MTMV 异步/SPJG 改写/Rollup 同步/精确近似去重(内部垂直切换)", "存储引擎"),
    "steOrg": ("◈", "数据组织 · Catalog→Table→Partition→Tablet→Rowset→Segment→Page 层级(内部垂直切换)", "存储引擎"),
    "dclprin": ("⛚", "DCL 权限体系 · 定义线(GRANT/REVOKE→EditLog)× 执行线(AuthN→AuthZ→WG→Audit)+ RBAC", "数据控制"),
    "optprin": ("◉", "优化四位置 · Planning/Execution/Storage/Cache 分层削减(跨环节全景)", "原理概览"),
    "tsprin": ("◉", "自愈环 · Detect → Decide → Act 持续闭环", "原理概览"),
    "cpprin": ("◉", "后台任务错峰 · 异步维护把写入/维护成本移出关键路径", "原理概览"),
}
...
THEMES = [
    # ── 接触面主线 · 用户可见(Linux 内核:系统调用是唯一用户接触面)──
    {"id": "qlife", "icon": "◷", "title": "系统调用接触面", "cat": "iface", "ord": 0,
     "desc": "用户态陷入内核的唯一入口。design 走查——陷入返回全景(syscall→pt_regs→do_syscall_64→sysret)→ 调用分派(nr 边界 nospec→__x64_sys_*)→ 旁路与快路径(/proc /sys 文件 vs vDSO)→ vDSO 免陷入,附调优/误区/总纲",
     "tabs": ["qlwalk"]},

    # ── 支撑能力域 · 内核内部(11 条)──
    {"id": "storageformat", "icon": "▤", "title": "虚拟内存", "cat": "support", "ord": 0,
     "desc": "内存子系统:design 走查——地址空间与 VMA(maple tree)→ 缺页四分支(匿名/文件/交换/COW)→ 物理页分配三层(伙伴/per-cpu/slub)→ 内存回收(kswapd/直接回收/LRU),附调优/误区/总纲",
     "tabs": ["stewalk"]},
    {"id": "opttech", "icon": "◷", "title": "进程调度", "cat": "support", "ord": 1,
     "desc": "CPU 分配:design 走查——调度类栈(stop>dl>rt>fair>idle)→ __schedule 切换链路 → EEVDF 选任务(资格+虚拟截止期)→ 唤醒抢占 → 多核负载均衡(sched_domain),附调优/误区/总纲",
     "tabs": ["optwalk"]},
    {"id": "execengine", "icon": "⚡", "title": "进程与任务管理", "cat": "support", "ord": 2,
     "desc": "任务生命周期:design 走查——task_struct 生命周期(fork→exec→exit→wait)→ 写时复制(线程共享 mm vs 进程 COW)→ exec 替换映像 → 退出回收 → 信号投递处理,附调优/误区/总纲",
     "tabs": ["execwalk"]},
    {"id": "transaction", "icon": "▦", "title": "VFS 文件系统", "cat": "support", "ord": 3,
     "desc": "文件抽象:design 走查——四大对象(super_block/inode/dentry/file)→ 路径解析(link_path_walk/dcache RCU)→ 读写贯穿(页缓存命中/缺失)→ 页缓存回写 → 文件系统接入,附调优/误区/总纲",
     "tabs": ["txnwalk"]},
    {"id": "metadata", "icon": "▨", "title": "块层 IO", "cat": "support", "ord": 4,
     "desc": "块设备 IO:design 走查——对象结构(bio/bio_vec/request)→ blk-mq 两级队列(软/硬)→ IO 全路径 → plug 攒批合并 → mq-deadline(扇区红黑树+FIFO),附调优/误区/总纲",
     "tabs": ["metawalk"]},
    {"id": "workload", "icon": "◐", "title": "中断与软中断", "cat": "support", "ord": 5,
     "desc": "中断处理:design 走查——硬中断上半部→下半部移交全景 → 软中断向量表与两处时机 → softirq 处理循环(三闸门/ksoftirqd)→ tasklet/workqueue/threaded irq 对比,附调优/误区/总纲",
     "tabs": ["wgwalk"]},
    {"id": "tabletsched", "icon": "⬡", "title": "同步原语", "cat": "support", "ord": 6,
     "desc": "并发同步:design 走查——锁选择决策树 → RCU 读侧无锁+宽限期延后回收 → 原子操作与内存屏障 → 死锁与 lockdep(AB-BA 环/BFS 检测),附调优/误区/总纲",
     "tabs": ["tswalk"]},
    {"id": "compaction", "icon": "◫", "title": "网络协议栈", "cat": "support", "ord": 7,
     "desc": "网络收发:design 走查——发送路径(tcp_sendmsg→IP→qdisc→驱动)→ 接收路径(napi_schedule→NET_RX_SOFTIRQ→poll→协议栈)→ TCP 状态机(握手/挥手/拥塞控制),附调优/误区/总纲",
     "tabs": ["cpwalk"]},
    {"id": "dcl", "icon": "◔", "title": "时间与定时器", "cat": "support", "ord": 8,
     "desc": "时间体系:design 走查——时间体系概览(clocksource/clock_event/timekeeper)→ timer wheel(分级轮 9 级)→ 到期驱动(时钟中断→节拍)→ hrtimer(红黑树高精度),附调优/误区/总纲",
     "tabs": ["dclwalk"]},
    {"id": "schemachange", "icon": "⛭", "title": "设备驱动模型", "cat": "support", "ord": 9,
     "desc": "驱动框架:design 走查——kobject 层级映射 sysfs → probe 双向匹配(设备↔驱动 match→really_probe)→ 三类驱动接入(字符/块/网络),附调优/误区/总纲",
     "tabs": ["scwalk"]},
    {"id": "load", "icon": "◨", "title": "cgroup 与 namespace", "cat": "support", "ord": 10,
     "desc": "隔离与限量:design 走查——两维隔离(namespace 视图 vs cgroup 限量)→ cgroup v2 单一层级 + 控制器 → 一个容器如何由三块拼成,附调优/误区/总纲",
     "tabs": ["loadwalk"]},

    # ── Appendix · 参考 ──
    {"id": "overallarch", "icon": "◇", "title": "架构地图", "cat": "appendix", "ord": 1,
     "desc": "统领全部子系统的全局视图:双维模型(能力域×执行时机)· 总架构(用户态→系统调用→子系统→硬件)· 接触面×能力域 依赖热力图",
     "tabs": ["panowalk"]},
]
...
_ARCH_SVG_TEXT = open(
    os.path.join(_DESIGN_DIR, "Linux原理_全景_02总架构.svg"),
    encoding="utf-8").read()
...
_SVG_WALK_SPECS = {
    "qlwalk": [("陷入返回全景 · syscall→pt_regs→do_syscall_64→sysret", "Linux原理_接触面_01陷入返回全景.svg"),
               ("用户指针安全访问 · access_ok→nospec→拷贝清零", "Linux原理_接触面_05用户内存访问.svg"),
               ("调用分派 · nr 边界(nospec)→__x64_sys_*", "Linux原理_接触面_02调用分派.svg"),
               ("旁路与快路径 · 陷入 vs /proc·/sys vs vDSO", "Linux原理_接触面_03旁路与快路径.svg"),
               ("vDSO 免陷入 · seqlock 读 vvar+rdtsc", "Linux原理_接触面_04vDSO.svg")],
    "stewalk": [("地址空间与 VMA · maple tree", "Linux原理_内存_04地址空间.svg"),
                ("缺页四分支 · 匿名/文件/交换/COW", "Linux原理_内存_01缺页处理.svg"),
                ("物理页分配三层 · 伙伴/per-cpu/slub", "Linux原理_内存_02物理页分配.svg"),
                ("内存回收 · kswapd/直接回收/LRU", "Linux原理_内存_03回收.svg")],
    "optwalk": [("调度类栈 · stop>dl>rt>fair>idle", "Linux原理_调度_01调度类栈.svg"),
                ("__schedule 切换链路", "Linux原理_调度_02schedule切换.svg"),
                ("EEVDF 选任务 · 资格+虚拟截止期", "Linux原理_调度_03EEVDF选任务.svg"),
                ("唤醒与抢占 · need_resched", "Linux原理_调度_04唤醒抢占.svg"),
                ("多核负载均衡 · sched_domain", "Linux原理_调度_05负载均衡.svg")],
    "execwalk": [("task_struct 生命周期 · fork→exec→exit→wait", "Linux原理_进程_01生命周期.svg"),
                 ("写时复制 · 线程共享 mm vs 进程 COW", "Linux原理_进程_02写时复制.svg"),
                 ("exec 替换执行映像", "Linux原理_进程_04exec映像.svg"),
                 ("退出与回收 · do_exit→僵尸→release_task", "Linux原理_进程_05退出回收.svg"),
                 ("信号投递处理 · 排队→挂起→get_signal", "Linux原理_进程_03信号投递处理.svg")],
    "txnwalk": [("VFS 四大对象 · super_block/inode/dentry/file", "Linux原理_VFS_01四大对象.svg"),
                ("路径解析 · link_path_walk/dcache RCU", "Linux原理_VFS_03路径解析.svg"),
                ("读写贯穿 · 页缓存命中/缺失→块层", "Linux原理_VFS_02读写路径.svg"),
                ("页缓存回写 · 标脏→阈值→wb_workfn", "Linux原理_VFS_04页缓存回写.svg"),
                ("文件系统接入 · 注册→挂载→分派 ext4", "Linux原理_VFS_05文件系统接入.svg")],
    "metawalk": [("对象结构 · bio/bio_vec/request", "Linux原理_块层_04对象结构.svg"),
                 ("blk-mq 两级队列 · 软→硬", "Linux原理_块层_02多队列.svg"),
                 ("IO 提交到完成全路径", "Linux原理_块层_01IO全路径.svg"),
                 ("plug 攒批与合并", "Linux原理_块层_03调度器.svg"),
                 ("mq-deadline · 扇区红黑树+FIFO", "Linux原理_块层_05deadline.svg")],
    "wgwalk": [("硬中断上半部→下半部移交全景", "Linux原理_中断_01硬软中断移交.svg"),
               ("软中断向量表与两处时机", "Linux原理_中断_02软中断向量与时机.svg"),
               ("softirq 处理循环 · 三闸门/ksoftirqd", "Linux原理_中断_04软中断循环.svg"),
               ("下半部机制对比 · tasklet/workqueue/threaded", "Linux原理_中断_03下半部机制对比.svg")],
    "tswalk": [("锁选择决策树", "Linux原理_同步_01锁选择决策树.svg"),
               ("RCU 读侧无锁 + 宽限期延后回收", "Linux原理_同步_02RCU读侧无锁.svg"),
               ("原子操作与内存屏障", "Linux原理_同步_03原子与屏障.svg"),
               ("死锁与 lockdep · AB-BA 环/BFS 检测", "Linux原理_同步_04死锁检测.svg")],
    "cpwalk": [("发送路径 · tcp_sendmsg→IP→qdisc→驱动", "Linux原理_网络_01发送路径.svg"),
               ("接收路径 · napi→NET_RX_SOFTIRQ→poll→协议栈", "Linux原理_网络_02接收路径.svg"),
               ("TCP 状态机 · 握手/挥手/拥塞控制", "Linux原理_网络_03TCP状态机.svg")],
    "dclwalk": [("时间体系概览 · clocksource/clock_event/timekeeper", "Linux原理_时间_01时间体系.svg"),
                ("节拍 tick · 周期节拍 vs 无节拍(nohz)", "Linux原理_时间_05节拍模式.svg"),
                ("timer wheel · 分级轮 9 级", "Linux原理_时间_02定时器轮.svg"),
                ("到期驱动 · 时钟中断→节拍", "Linux原理_时间_03到期驱动.svg"),
                ("hrtimer · 红黑树高精度", "Linux原理_时间_04hrtimer.svg")],
    "scwalk": [("kobject 层级映射 sysfs", "Linux原理_驱动_01kobject层级.svg"),
               ("device/driver/bus 三方会合 · 总线 match 撮合", "Linux原理_驱动_04三方会合.svg"),
               ("probe 双向匹配 · match→really_probe", "Linux原理_驱动_02匹配probe.svg"),
               ("三类驱动接入 · 字符/块/网络", "Linux原理_驱动_03三类驱动.svg")],
    "loadwalk": [("两维隔离 · namespace 视图 vs cgroup 限量", "Linux原理_隔离_01两维隔离.svg"),
                 ("cgroup v2 单一层级 + 控制器", "Linux原理_隔离_02cgroupv2层级.svg"),
                 ("一个容器如何由三块拼成", "Linux原理_隔离_03容器组成.svg")],
    "panowalk": [("双维模型 · 能力域 × 执行时机", "Linux原理_全景_01双维模型.svg"),
                 ("总架构 · 用户态→系统调用→子系统→硬件", "Linux原理_全景_02总架构.svg"),
                 ("依赖矩阵 · 接触面 × 能力域 热力图", "Linux原理_全景_03依赖矩阵.svg")],
}
# 快速开始「上手总览」用独立复合渲染器 renderQsTour(总览 SVG + 5 步选择器 + 内容区),不走通用 SVG-walk
_QSTOUR_OVERVIEW_B64 = _design_b64("Linux上手_00总览.svg")  # 无此素材→空;quickstart 未挂载
...
_WALK_PROSE = {
    "qlwalk": "Linux原理_接触面_系统调用.md",
    "stewalk": "Linux原理_支撑_虚拟内存.md", "optwalk": "Linux原理_支撑_进程调度.md",
    "execwalk": "Linux原理_支撑_进程与任务管理.md", "txnwalk": "Linux原理_支撑_VFS文件系统.md",
    "metawalk": "Linux原理_支撑_块层IO.md", "wgwalk": "Linux原理_支撑_中断与软中断.md",
    "tswalk": "Linux原理_支撑_同步原语.md", "cpwalk": "Linux原理_支撑_网络协议栈.md",
    "dclwalk": "Linux原理_支撑_时间与定时器.md", "scwalk": "Linux原理_支撑_设备驱动模型.md",
    "loadwalk": "Linux原理_支撑_cgroup与namespace.md",
}
...
<title>Linux 内核 · 交互式核心原理图谱</title>
...
<div id="loadingOverlay" role="status" aria-live="polite">
  <div class="lo-inner">
    ...
    <div class="lo-title">Linux Kernel Atlas</div>
...
<div id="app">
  <header>
    ...
    <a href="https://github.com/torvalds/linux" target="_blank" rel="noopener" title="GitHub 源码仓库" style="margin-left:auto;display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M12 .5C5.7.5.5 5.7.5 12c0 5.1 3.3 9.4 7.9 10.9.6.1.8-.2.8-.6v-2c-3.2.7-3.9-1.4-3.9-1.4-.5-1.3-1.3-1.7-1.3-1.7-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1 1.8 2.7 1.3 3.4 1 .1-.8.4-1.3.7-1.6-2.6-.3-5.3-1.3-5.3-5.8 0-1.3.5-2.3 1.2-3.1-.1-.3-.5-1.5.1-3.1 0 0 1-.3 3.3 1.2a11.4 11.4 0 0 1 6 0C17.3 4.7 18.3 5 18.3 5c.6 1.6.2 2.8.1 3.1.8.8 1.2 1.8 1.2 3.1 0 4.5-2.7 5.5-5.3 5.8.4.4.8 1.1.8 2.2v3.3c0 .4.2.7.8.6 4.6-1.5 7.9-5.8 7.9-10.9C23.5 5.7 18.3.5 12 .5z"/></svg></a><a href="https://kernel.org" target="_blank" rel="noopener" title="项目官网" style="display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><img src="data:image/svg+xml;base64," width="18" height="18" alt="官网" style="display:block"/></a><button class="theme-toggle" id="themeToggle" title="切换深色 / 浅色主题" aria-label="切换主题">
      ...
      <div class="nav-mode nav-arch active" id="navArch">
        <div class="arch-stage">
          <div class="arch-canvas">
            <img class="arch-img" src="data:image/svg+xml;base64,__ARCH_SVG_B64__" alt="Doris 总架构图" draggable="false"/>
            __ARCH_HOTSPOTS__
          </div>
        </div>
...
const COMPARE_SPEC={
  id:'compare', title:'架构对比 · Doris 在 OLAP/湖仓生态中的定位',
  note:'突出设计取舍(非优劣);同类对手看差异,异类引擎(Spark/DuckDB)看设计哲学的借鉴。',
  rowH:40,
  cols:[{h:'维度',w:140,accent:'var(--cv-scan)'},{h:'Doris',w:180,accent:'var(--cv-ink)'},{h:'ClickHouse',w:145,accent:'var(--cv-ink)'},{h:'StarRocks',w:135,accent:'var(--cv-ink)'},{h:'Spark / Trino / DuckDB',w:250,accent:'var(--cv-ink)'}],
  rows:[
    ['定位','实时数仓 + 湖仓一体','单表极致 OLAP','实时数仓(同源竞品)','Spark/Trino=联邦批处理;DuckDB=嵌入式分析'],
    ['执行模型','MPP + Pipeline 向量化','向量化 + morsel 并行','MPP + 向量化','Spark=DAG/RDD;Trino=MPP;DuckDB=向量化 push'],
    ['优化器','Nereids(Cascades CBO)','规则为主,CBO 较弱','CBO','Spark Catalyst(RBO+CBO+codegen)—Nereids 借鉴'],
    ['存储','自有列存 + 湖仓外表','MergeTree 列存','自有列存','Spark/Trino 无自有存储;DuckDB 有自有格式'],
    ['并行度','编译期定 lane×BE','编译期 baked 进计划','编译期','morsel-driven 运行期动态(Spark/DuckDB)'],
    ['更新/主键','MoW 主键模型(写时合并)','弱(ReplacingMergeTree)','主键模型(强)','多为不可变 / 追加'],
    ['湖仓','Hive/Iceberg/Hudi/Paimon','有限(外表)','Iceberg/Hudi','Trino 湖仓最强;Spark 生态广'],
    ['物化视图','异步 MTMV 透明改写+Rollup','有(增量弱)','异步 MV','Spark 无透明改写'],
    ['向量检索','倒排 + ANN(FAISS)','有向量能力','有','需外接专用向量库'],
    ['可借鉴','—','向量化/morsel(§4.2)','—','Catalyst 计划生命周期 / DuckDB push 模型']
  ]
};
