...
![节拍 tick：周期模式点亮 update_process_times，无节拍靠 hrtimer 兜底](Linux原理_时间_05节拍模式.svg)

节拍即"每 `1/HZ` 秒一次时钟中断"，用来推进 `jiffies`、驱动调度与定时器扫描。**周期模式**下 `tick_handle_periodic`（`kernel/time/tick-common.c:108`）→ `tick_periodic`（`tick-common.c:86`）：仅被选为 `tick_do_timer_cpu` 的那颗 CPU `do_timer(1)` 推进全局 `jiffies`、`update_wall_time` 更新墙钟（`tick-common.c:95`），而每颗 CPU 都跑 `update_process_times`（`tick-common.c:101`）做本地记账、扫定时器、触发调度节拍。**无节拍**用两个开关省电降抖动：`NO_HZ_IDLE`（CPU 空闲即停周期 tick）、`NO_HZ_FULL`（仅剩单个可运行任务也停 tick，HPC/实时消抖动），停 tick 后靠一个 hrtimer 记录"下一个真正该醒的时刻"。
