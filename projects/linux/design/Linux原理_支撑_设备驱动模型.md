...
![device/driver/bus 解耦：两张表挂同一总线，总线 match 撮合](Linux原理_驱动_04三方会合.svg)

设备模型把**硬件实例**（`struct device`，"我这块硬件在这"）与**驱动代码**（`struct device_driver`，"我会开这类硬件"）解耦成两张表，挂在同一条**总线 `bus_type`**（`include/linux/device/bus.h:83`）上；总线提供三个关键回调：`match`（判定某设备与某驱动是否配对，`bus.h:90`）、`uevent`（生成热插拔环境变量，`bus.h:91`）、`probe`（撮合成功后绑定初始化，`bus.h:92`）。**总线是媒人**——设备与驱动互不直接查找，全靠 `match` 撮合，语义随总线而异（PCI 比 vendor/device ID、platform 比 `compatible`/name、USB 比接口描述符）；没有 `match` 回调的总线默认"全部匹配"（`driver_match_device`，`drivers/base/base.h:188`）。
