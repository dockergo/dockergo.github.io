...
![用户指针安全访问：access_ok → barrier_nospec → 拷贝清零，非法即 -EFAULT](Linux原理_接触面_05用户内存访问.svg)

系统调用参数经寄存器传入、封装在 `pt_regs` 里。**内核绝不直接解引用用户指针**（可能非法、或指向内核空间做提权），跨边界必须走专用原语 `copy_from_user`/`copy_to_user`（`include/linux/uaccess.h:166`）：先 `access_ok`（`arch/x86/include/asm/uaccess_64.h:98`，校验地址落在用户空间 `valid_user_address`）、插 `barrier_nospec` 防投机执行泄漏（`uaccess.h:182`）再拷贝，短拷贝时把目标剩余清零（`uaccess.h:190`）防信息泄漏——这是用户/内核地址空间隔离的**强制关卡**。
