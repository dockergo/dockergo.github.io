...
## 源码锚点（src/parallel · src/execution · src/common 精确定位）

> 以下 `文件:行号` 在 duckdb `src` 源码 grep 核实，把 push 流水线推进、morsel 调度与列批表示落到实现。

- **Pipeline 推进（push 循环）**：`src/parallel/pipeline_executor.cpp:271`（`PipelineExecutor::Execute`，从 Source 拉块推过算子到 Sink）、`:127`（`PrepareForExecution`）。
- **Pipeline/Event 调度成 DAG**：`src/parallel/executor.cpp:302`（`ScheduleEvents`）、`:77`（`SchedulePipeline`）。
- **线程池 morsel 领取**：`src/parallel/task_scheduler.cpp:57`（`CreateProducer`）、`:61`（`ScheduleTask`）、`:69`（`GetTaskFromProducer`，工作线程抢占式取任务）。
- **DataChunk 列批**：`src/common/types/data_chunk.cpp:32`（`InitializeEmpty`）、`:39`（`Initialize`，按列分配 2048 宽向量）。
- **表达式求值器**：`src/execution/expression_executor.cpp:15`（`ExpressionExecutor` 构造，向量化逐批求值入口）。
