...
## 源码锚点（src/storage · src/execution/index 精确定位）

> 以下 `文件:行号` 在 duckdb `src` 源码 grep 核实，把 RowGroup 扫描、单文件块管理、压缩选择与 ART 落到实现。

- **RowGroup 扫描**：`src/storage/table/row_group.cpp:413`（`InitializeScan`）、`:800`（`Scan`，向量化按段扫出 DataChunk）。
- **单文件块管理 / free list**：`src/storage/single_file_block_manager.cpp:910`（`GetFreeBlockId`，优先复用空闲块）、`:861`（`LoadFreeList`）、`:1311`（`WriteHeader`，双写 Header）。
- **列压缩自选编码**：`src/storage/table/column_data_checkpointer.cpp:172`（`DetectBestCompressionMethod`，逐候选试算取最省）、`:317`（`WriteToDisk`）、`:418`（`Checkpoint`）。
- **ART 自适应索引**：`src/execution/index/art/art.cpp:50`（`ART` 构造）、`:512`（`Insert`，主键/唯一约束强制与点查）。
- **提交并入全局**：`src/storage/data_table.cpp:1205`（`DataTable::Append`）。
