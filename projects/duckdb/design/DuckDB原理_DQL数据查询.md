...
![编译计划形态演变](DuckDB原理_DQL_08编译计划形态.svg)

图注：同一条 SELECT 的算子树在编译期逐段变形——Binder 产出未优化逻辑树 → Optimizer 谓词下推/列裁剪/Join 定序/Top-N 合并使树更矮、数据更少 → PhysicalPlanGenerator 为每个逻辑算子选具体实现（HashJoin/RadixAgg/TopN）。下表补每段的输入输出与源码锚点。
...
## 源码锚点（src/main · src/planner · src/execution 精确定位）

> 以下 `文件:行号` 在 duckdb `src` 源码 grep 核实，把编译四段、Hash Join 三面接口与 Join 物理选择落到实现。

- **编译链（Planner→Optimizer→PhysicalPlan）**：`src/main/client_context.cpp:469`（`CreatePreparedStatementInternal`）、`src/main/client_context.cpp:488`（`planner.CreatePlan`）、`src/main/client_context.cpp:538`（`PhysicalPlanGenerator`）；`src/planner/planner.cpp:78`（`Planner::CreatePlan`）。
- **Hash Join 的 Sink/Execute/Source 三面**：`src/execution/operator/join/physical_hash_join.cpp:752`（`Sink`，build 端建 radix 哈希表）、`src/execution/operator/join/physical_hash_join.cpp:1880`（`Finalize`，合并并生成动态过滤）、`src/execution/operator/join/physical_hash_join.cpp:2125`（`ExecuteInternal`，probe 端探测）、`src/execution/operator/join/physical_hash_join.cpp:2654`（`GetDataInternal`，产出结果）。
- **Hash 聚合 radix 分区**：`src/execution/radix_partitioned_hashtable.cpp:18`（`RadixPartitionedHashTable` 构造，按 hash radix 位分区并行）。
- **Join 物理算子选择（决策树）**：`src/execution/physical_plan/plan_comparison_join.cpp:23`（`PlanComparisonJoin`，按等值/范围/基数选 Hash/IE/PWMJ/NL）、`src/execution/physical_plan/plan_comparison_join.cpp:107`（`CreatePlan(LogicalComparisonJoin)` 入口）。
