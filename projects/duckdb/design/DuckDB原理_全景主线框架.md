...
> 以下 `文件:行号` 在 duckdb `src` 源码 grep 核实，为全景各主线/能力域各锚一处代表实现，作为下钻各篇的入口坐标。

- **编译链**：`src/main/client_context.cpp:488`（`planner.CreatePlan`）、`:538`（`PhysicalPlanGenerator`）；`src/planner/planner.cpp:78`（`Planner::CreatePlan`）。
- **执行引擎 / 并行**：`src/parallel/task_scheduler.cpp:61`（`ScheduleTask`，线程池 morsel 调度）。
- **存储引擎（DML 写入）**：`src/storage/data_table.cpp:1205`（`DataTable::Append`）。
- **元数据 Catalog（DDL）**：`src/catalog/catalog_set.cpp:199`（`CreateEntry`）。
- **事务与 MVCC**：`src/transaction/duck_transaction.cpp:250`（`Commit`，定 commit_id）。
- **后台 checkpoint**：`src/storage/checkpoint_manager.cpp:193`（`CreateCheckpoint`）。
- **内存与 Buffer**：`src/storage/buffer/buffer_pool.cpp:377`（`EvictBlocks`，larger-than-memory 驱逐）。
- **扩展机制**：`src/main/extension_manager.cpp:88`（`BeginLoad`）。
- **ART 索引**：`src/execution/index/art/art.cpp:50`（`ART` 构造）。
