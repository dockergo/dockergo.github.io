...
## 源码锚点（src/catalog · src/storage · src/main 精确定位）

> 以下 `文件:行号` 在 duckdb `src` 源码 grep 核实，把内存对象树、名称解析与 checkpoint 序列化落到实现。

- **Catalog 对象树**：`src/catalog/duck_catalog.cpp:96`（`CreateSchema`）、`src/catalog/duck_catalog.cpp:130`（`DropSchema`）、`src/catalog/duck_catalog.cpp:181`（`ScanSchemas`）。
- **CatalogSet 版本条目**：`src/catalog/catalog_set.cpp:199`（`CreateEntry`）、`src/catalog/catalog_set.cpp:311`（`AlterEntry`）、`src/catalog/catalog_set.cpp:447`（`DropEntry`）、`src/catalog/catalog_set.cpp:222`（`GetEntryInternal`，沿版本链取可见条目）。
- **名称解析（search_path）**：`src/catalog/catalog_search_path.cpp:159`（`Set`）、`src/catalog/catalog_search_path.cpp:208`（`Get`）、`src/catalog/catalog_search_path.cpp:219`（`GetDefaultSchema`）。
- **checkpoint 序列化 / 反序列化**：`src/storage/checkpoint_manager.cpp:409`（`CheckpointWriter::WriteEntry`）、`src/storage/checkpoint_manager.cpp:466`（`WriteSchema`）、`src/storage/checkpoint_manager.cpp:372`（`CheckpointReader::LoadCheckpoint`，打开库反序列化）。
- **多库 ATTACH 管理**：`src/main/database_manager.cpp:105`（`AttachDatabase`）、`src/main/database_manager.cpp:266`（`DetachDatabase`）、`src/main/database_manager.cpp:415`（`GetDefaultDatabase`）。
