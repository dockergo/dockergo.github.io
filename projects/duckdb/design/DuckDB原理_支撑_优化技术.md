...
## 源码锚点（src/optimizer 精确定位）

> 以下 `文件:行号` 在 duckdb `src` 源码 grep 核实，把上文 pass 与 Join 定序落到具体实现文件（各 pass 多为独立 `.cpp`，比 `optimizer.cpp` 内的 `RunOptimizer(...)` 调用点更稳）。

- **RunOptimizer 调度**：`src/optimizer/optimizer.cpp:123`（`Optimizer::RunOptimizer`，逐 pass 计时并回调）。
- **Join 定序入口**：`src/optimizer/join_order/join_order_optimizer.cpp:28`（`JoinOrderOptimizer::Optimize`）。
- **DP/贪心二选一**：`src/optimizer/join_order/plan_enumerator.cpp:532`（`SolveJoinOrder`，`:534` 处按 `NumRelations()` 与阈值分流）→ `:375`（`SolveJoinOrderExactly`，DPHyp 精确 DP）/ `:398`（`SolveJoinOrderApproximately`，贪心近似）。
- **基数与代价**：`src/optimizer/join_order/cardinality_estimator.cpp:897`（`EstimateCardinalityWithSet`）、`src/optimizer/join_order/cost_model.cpp:40`（`CostModel::ComputeCost`）。
- **统计传播**：`src/optimizer/statistics_propagator.cpp:42`（`StatisticsPropagator::PropagateStatistics`，沿计划自底向上传播）。
- **下推 / 化简 / 段裁剪**：`src/optimizer/filter_pushdown.cpp:106`（`FilterPushdown::Rewrite`）、`src/optimizer/expression_rewriter.cpp:102`（`ExpressionRewriter::VisitOperator`）、`:51`（`ApplyRules`）、`src/optimizer/row_group_pruner.cpp:18`（`RowGroupPruner::Optimize`）。
