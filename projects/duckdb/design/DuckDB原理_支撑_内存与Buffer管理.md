...
## 源码锚点（src/storage · src/storage/buffer 精确定位）

> 以下 `文件:行号` 在 duckdb `src` 源码 grep 核实，把 Pin/驱逐与阻塞算子溢写落到实现。

- **Pin / 分配 / 驱逐入口**：`src/storage/standard_buffer_manager.cpp:333`（`Pin`，命中免读盘、未命中读入）、`:198`（`Allocate`）、`:170`（`RegisterMemory`）、`:126`（`EvictBlocksOrThrow`，超限先腾再分配）。
- **BufferPool 近似 LRU**：`src/storage/buffer/buffer_pool.cpp:377`（`EvictBlocks`，挑未 pin 块腾空间）、`:271`（`AddToEvictionQueue`，Unpin 归零入队）、`:436`（`PurgeAgedBlocks`，年龄清理）。
- **阻塞算子内存额度协调**：`src/storage/temporary_memory_manager.cpp:71`（`TemporaryMemoryManager` 构造）、`:53`（`UpdateReservation`，动态调整各算子额度）。
- **溢写 / 读回临时文件**：`src/storage/temporary_file_manager.cpp:500`（`WriteTemporaryBuffer`，块写 temp_directory）、`:648`（`ReadTemporaryBuffer`，需要时读回）。
