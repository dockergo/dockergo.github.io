...
![三类写的存储形态](DuckDB原理_DML_06三类写形态.svg)

图注：INSERT 追加新行（无旧版本，靠 commit_id 控可见）、UPDATE 就地写新值且旧值旁路进 UpdateInfo 版本链、DELETE 只在 ChunkInfo 打删除标记不物理擦除；三者共性是先落本地/旁路、COMMIT 定 commit_id、旧版本按最低活跃 start_time 回收。下表补锚点。
...
## 源码锚点（src/storage 精确定位）

> 以下 `文件:行号` 在 duckdb `src` 源码 grep 核实，把本地追加、就地更新/删除标记与 WAL/Checkpoint 落到实现。

- **INSERT 本地追加三段**：`src/storage/data_table.cpp:983`（`InitializeLocalAppend`）、`:1008`（`LocalAppend`，逐 DataChunk 写本地并校验约束）、`:1040`（`FinalizeLocalAppend`）、`:1205`（`Append`，提交时并入全局 RowGroupCollection）。
- **UPDATE / DELETE 的 MVCC**：`src/storage/data_table.cpp:1722`（`Update`，新值就地、旧值入 UpdateInfo 版本链）、`:1570`（`Delete`，打 ChunkInfo 删除标记不物理擦除）。
- **提交写 WAL**：`src/storage/write_ahead_log.cpp:507`（`WriteInsert`）、`:530`（`WriteDelete`）、`:540`（`WriteUpdate`）。
- **Checkpoint 合并单文件**：`src/storage/checkpoint_manager.cpp:193`（`SingleFileCheckpointWriter::CreateCheckpoint`，WAL 超阈值时把变更写入列存块并截断日志）。
