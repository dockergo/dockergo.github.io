...
_ap = argparse.ArgumentParser(description="生成 Apache Spark 交互式核心原理图谱（离线自包含 HTML）")
...
#     新家族（Apache Spark · 大数据统一计算引擎）：元模式 = 接口 × 能力域 × 时机。
#     全景 + 1 接触面主线（编程接口层）+ 9 支撑能力域。
# ===================================================================== #
MAINLINES = [
    ("Spark原理_全景主线框架", "pano", "◇", "全景主线框架",
     "新家族计算引擎：双维模型 · driver–executor 总架构 · 依赖矩阵 · 依赖关系图 · 贯穿声明"),

    ("Spark原理_编程接口层", "iface", "⚙", "编程接口层",
     "用户与 Spark 交互的多套 API · SparkSession 统一入口 → RDD/DataFrame/SQL/Streaming → 汇入统一执行"),

    ("Spark原理_支撑_Catalyst优化", "support", "▦", "Catalyst 优化",
     "结构化查询优化器 · 分析 → 逻辑优化 → 物理规划 → CBO · RDD 不过 Catalyst"),
    ("Spark原理_支撑_Tungsten代码生成", "support", "◫", "Tungsten 代码生成",
     "whole-stage codegen 编译成 JVM 字节码 · 二进制行 UnsafeRow · 堆外内存,消虚函数开销"),
    ("Spark原理_支撑_执行模型", "support", "◉", "执行模型",
     "灵魂主线：action 触发 job → DAG 按 shuffle 切 stage → task 分区并行 → lineage 容错"),
    ("Spark原理_支撑_Shuffle", "support", "◐", "Shuffle",
     "宽依赖处数据重分布 · stage 物化边界 · map write → 落盘/索引 → reduce fetch/read · AQE"),
    ("Spark原理_支撑_调度与集群管理", "support", "⚙", "调度与集群管理",
     "DAGScheduler(stage) → TaskScheduler(task locality+重试) → backend ↔ cluster manager · YARN/K8s"),
    ("Spark原理_支撑_内存管理", "support", "▤", "内存管理",
     "UnifiedMemoryManager · execution/storage 软边界互借 · 不够就 spill 落盘 · 堆内外"),
    ("Spark原理_支撑_存储与缓存", "support", "▤", "存储与缓存",
     "BlockManager(每节点)+BlockManagerMaster(driver 目录) · 内存/磁盘/堆外多层 · 本地直读/远程拉"),
    ("Spark原理_支撑_容错", "support", "⛨", "容错",
     "靠可重算性而非副本 · lineage 重算 + checkpoint 截断 + task 重试 + 推测执行 + FetchFailed"),
    ("Spark原理_支撑_StructuredStreaming", "support", "✎", "Structured Streaming",
     "无界流建模为不断增长的表 · micro-batch/continuous → 增量执行 → state → checkpoint 保 exactly-once"),
]
...
# ===================================================================== #
PANO_NAME = "Spark原理_全景主线框架"
# 架构热区从总架构 SVG 的 data-tid 矩形自动派生(SVG = 唯一真源,消除双真源漂移)。
# (x, y, w, h, 主线name, 标签):坐标取自带 data-tid 的 <rect>,主线name = data-tid = 所属主线。
# 容器先于叶子出现在 SVG 文档序 → 派生 z 序即文档序,叶子叠在容器之上优先响应。
...
BRAND_TITLE = "一切知识皆索引"
BRAND_SUB = "Apache Spark"
HOME_DESC = ("Apache Spark 核心原理设计文档库的离线交互图谱——新家族（大数据统一计算引擎：一个 driver 协调 N 个 executor 的纯计算框架，"
             "数据在外部存储、不自管持久存储，靠 RDD lineage 可重算做容错）。"
             "11 条主线、约 50 张手绘原理图，全部回 apache/spark 源码（master · post-4.0）核实。点击项目总架构图任意模块即可下钻到对应主线。")
ARCH_SVG_NAME = "Spark原理_总架构图.svg"
# 从 SVG 派生热区(含 viewBox 除数 ARCH_W/ARCH_H),替代原硬编码坐标表。
ARCH_HOTSPOTS, ARCH_W, ARCH_H = _parse_arch_hotspots(_svg_text(ARCH_SVG_NAME))
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="Apache Spark 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
<header>
  ...
  <a href="https://github.com/apache/spark" target="_blank" rel="noopener" title="GitHub 源码仓库" style="display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M12 .5C5.7.5.5 5.7.5 12c0 5.1 3.3 9.4 7.9 10.9.6.1.8-.2.8-.6v-2c-3.2.7-3.9-1.4-3.9-1.4-.5-1.3-1.3-1.7-1.3-1.7-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1 1.8 2.7 1.3 3.4 1 .1-.8.4-1.3.7-1.6-2.6-.3-5.3-1.3-5.3-5.8 0-1.3.5-2.3 1.2-3.1-.1-.3-.5-1.5.1-3.1 0 0 1-.3 3.3 1.2a11.4 11.4 0 0 1 6 0C17.3 4.7 18.3 5 18.3 5c.6 1.6.2 2.8.1 3.1.8.8 1.2 1.8 1.2 3.1 0 4.5-2.7 5.5-5.3 5.8.4.4.8 1.1.8 2.2v3.3c0 .4.2.7.8.6 4.6-1.5 7.9-5.8 7.9-10.9C23.5 5.7 18.3.5 12 .5z"/></svg></a><a href="https://spark.apache.org" target="_blank" rel="noopener" title="项目官网" style="display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><img src="data:image/svg+xml;base64," width="18" height="18" alt="官网" style="display:block"/></a><button id="themeBtn" title="切换深色 / 浅色主题" aria-label="切换主题" style="width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);background:var(--c-panel);color:var(--c-ink2);cursor:pointer;display:inline-grid;place-items:center;font-size:16px;flex:none"><span class="tt-ico tt-moon">☾</span><span class="tt-ico tt-sun">☀</span></button>
</header>
<div class="wrap">
  <div id="home">
    {archnav}
