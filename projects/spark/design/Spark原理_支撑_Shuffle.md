...
| 结构 / 方法 | 位置 | 职责 |
|---|---|---|
| ShuffleManager | `shuffle/ShuffleManager.scala:38` | shuffle 插件接口 |
| SortShuffleManager | `shuffle/sort/SortShuffleManager.scala:73` | 默认 sort-based 实现 |
| SortShuffleWriter | `shuffle/sort/SortShuffleWriter.scala:29` | 通用 sort 写(需聚合/排序) |
| BypassMergeSortShuffleWriter | `shuffle/sort/BypassMergeSortShuffleWriter.java:84` | 少分区无聚合旁路 |
| UnsafeShuffleWriter | `shuffle/sort/UnsafeShuffleWriter.java:70` | Tungsten 序列化排序写 |
| ShuffleDependency | `Dependency.scala:84` | 宽依赖(shuffle 边界) |
| ShuffleExchangeExec | `execution/exchange/ShuffleExchangeExec.scala:190` | SQL 侧重分布算子 |
| ShuffleBlockFetcherIterator | `storage/ShuffleBlockFetcherIterator.scala:86` | reduce 端远程拉块 |
