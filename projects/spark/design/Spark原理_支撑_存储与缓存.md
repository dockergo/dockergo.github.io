...
| 结构 / 方法 | 位置 | 职责 |
|---|---|---|
| BlockManager | `storage/BlockManager.scala:183` | 每节点块存取 |
| BlockManagerMasterEndpoint | `storage/BlockManagerMasterEndpoint.scala:51` | driver 侧全局块位置目录 |
| MemoryStore | `storage/memory/MemoryStore.scala:83` | 内存块层 |
| DiskStore | `storage/DiskStore.scala:45` | 磁盘块层 |
| getOrElseUpdateRDDBlock | `storage/BlockManager.scala:1572` | cache 命中读、未命中算并存 |
| RDD.iterator | `rdd/RDD.scala:334` | 有缓存读缓存、否则 compute |
| TorrentBroadcast | `broadcast/TorrentBroadcast.scala:60` | 分块 BT 式广播 |
| ShuffleBlockFetcherIterator | `storage/ShuffleBlockFetcherIterator.scala:86` | 远程块拉取 |
