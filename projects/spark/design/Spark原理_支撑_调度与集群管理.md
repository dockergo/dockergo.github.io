...
| 结构 / 方法 | 位置 | 职责 |
|---|---|---|
| DAGScheduler | `scheduler/DAGScheduler.scala:124` | 高层 stage 调度 |
| submitStage | `scheduler/DAGScheduler.scala:1614` | 递归提交父 stage |
| TaskSchedulerImpl | `scheduler/TaskSchedulerImpl.scala:83` | 低层 task 调度 |
| resourceOffers | `scheduler/TaskSchedulerImpl.scala:512` | 按本地性把 task 分给 offer |
| TaskSetManager | `scheduler/TaskSetManager.scala:56` | 一个 taskset 的本地性/重试 |
| Task | `scheduler/Task.scala:61` | 调度的最小单元 |
| SparkContext.runJob | `SparkContext.scala:2481` | 触发 action → 提交 job |
| ShuffleDependency | `Dependency.scala:84` | 决定 stage 边界 |
