...
| 结构 / 方法 | 位置 | 职责 |
|---|---|---|
| RDD | `rdd/RDD.scala:84` | 不可变分区数据集 |
| RDD.compute | `rdd/RDD.scala:116` | 分区级计算契约 |
| ShuffleDependency | `Dependency.scala:84` | 宽依赖(切 stage 的边界) |
| DAGScheduler | `scheduler/DAGScheduler.scala:124` | 按 shuffle 边界切 stage |
| submitStage | `scheduler/DAGScheduler.scala:1614` | 递归提交父 stage |
| Stage | `scheduler/Stage.scala:56` | stage 抽象基类 |
| ShuffleMapStage / ResultStage | `scheduler/ShuffleMapStage.scala:37` / `ResultStage.scala:30` | 两类 stage |
| Task | `scheduler/Task.scala:61` | task 执行单元基类 |
