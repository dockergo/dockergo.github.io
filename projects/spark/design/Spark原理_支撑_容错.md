...
图示宽依赖容错——**丢了哪部分 shuffle 输出，就重算产生它的那个 map stage（而非整个 job）**：reduce 拉不到 map 输出时抛 `FetchFailed`（`TaskEndReason.scala:83`），`DAGScheduler.handleTaskCompletion`（`:2403`）注销丢失输出、标记 map stage ＋当前 stage 重跑、延迟重提交。连续失败上限默认 4（`config/package.scala`）。**这是"动态分配必配 external shuffle service"的根因**——否则回收 executor = 丢 shuffle = FetchFailed 连环重算（见 [[调度与集群管理]]、[[Shuffle]]）。
...
| 结构 / 方法 | 位置 | 职责 |
|---|---|---|
| handleTaskCompletion | `scheduler/DAGScheduler.scala:2403` | 成功/失败事件总入口 |
| resubmitFailedStages | `scheduler/DAGScheduler.scala:1318` | FetchFailed 后重提 stage |
| FetchFailed | `TaskEndReason.scala:83` | 拉取失败(触发重算上游) |
| TaskSetManager | `scheduler/TaskSetManager.scala:56` | task 级重试与推测 |
| RDD.checkpoint | `rdd/RDD.scala:1736` | 截断 lineage 落可靠存储 |
| RDD.iterator | `rdd/RDD.scala:334` | 重算依据 lineage |
| ShuffleMapStage | `scheduler/ShuffleMapStage.scala:37` | 保存 map 输出位置 |
| Task | `scheduler/Task.scala:61` | 失败可幂等重跑单元 |
