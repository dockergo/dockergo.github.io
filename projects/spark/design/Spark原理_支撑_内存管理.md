...
图示内存不够时 Spark **不 OOM 而是 spill（落盘）**：`TaskMemoryManager.acquireExecutionMemory`（`TaskMemoryManager.java:193`）发现紧张时触发其他 `MemoryConsumer`（`MemoryConsumer.java:32`）的 `spill`（`:86`）写盘腾地方。典型 spill 者 `ExternalSorter`（`ExternalSorter.scala:259`）、`ExternalAppendOnlyMap`（`:186`），共同经 `Spillable`。**关键不变量**：spill 让 Spark 能处理超内存数据集（代价是磁盘 IO ＋后续归并），是稳定性兜底。
...
| 结构 / 方法 | 位置 | 职责 |
|---|---|---|
| MemoryManager | `memory/MemoryManager.scala:39` | 内存管理抽象基类 |
| UnifiedMemoryManager | `memory/UnifiedMemoryManager.scala:58` | execution/storage 统一软边界 |
| acquireExecutionMemory | `memory/UnifiedMemoryManager.scala:134` | 借用/驱逐 storage 后授予 |
| acquireStorageMemory | `memory/UnifiedMemoryManager.scala:206` | storage 申请(可被 execution 挤) |
| ExecutionMemoryPool | `memory/ExecutionMemoryPool.scala:43` | task 间公平分 execution 内存 |
| StorageMemoryPool | `memory/StorageMemoryPool.scala:35` | storage 池 + 驱逐 |
| TaskMemoryManager | `memory/TaskMemoryManager.java:57` | 页式管理 + 触发 spill |
| MemoryConsumer.spill | `memory/MemoryConsumer.java:86` | 内存不足时溢写磁盘 |
