...
| 失败/边界场景 | 处理机制 | 锚点 |
|---|---|---|
| **rewrite 死循环** | POST_REWRITE 每次跳回 FIND_CONFIG 自增 `r->uri_changes`，超上限（默认 10）返 500 "rewrite or internal redirection cycle" | — |
| **无 content handler** | CONTENT 阶段无模块产内容时，末尾按默认返回 404/403、不悬挂 | `ngx_http_core_content_phase:1292` |
| **鉴权失败** | ACCESS handler 返回 403/401，checker 直接 finalize、后续阶段不执行；`satisfy any` 任一通过即放行 | POST_ACCESS 判定 |
| **子请求/内部重定向** | 重置 `r->phase_handler` 从 server_rewrite 或指定 index 重跑，主/子请求各自独立走阶段 | `:2563/:2696` `sr->write_event_handler=run_phases` |
