...
| 失败/边界场景 | 处理机制 | 锚点 |
|---|---|---|
| **上下文错误** | 指令在非法块（如 `proxy_pass` 放 http 顶层）报 "directive is not allowed here"、启动/reload 失败 | `ngx_conf_handler:356` → `:482` |
| **参数个数错误** | TAKE1 却给两参数等，报 "invalid number of arguments" | `:495` |
| **块未闭合** | `{` 无匹配 `}` 时读到 EOF 报 "unexpected end of file, expecting }" | `ngx_conf_read_token` |
| **reload 时配置错** | 新配置解析失败时 master 保留旧配置继续服务、不切换（旧 worker 不退）——故 reload 安全，先 `nginx -t` | — |
| **include 通配无匹配** | `include *.conf` 匹配 0 个文件不报错（除非具体文件名），易漏配 | — |
