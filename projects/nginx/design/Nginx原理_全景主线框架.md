...
> 统领全部原理文档：nginx 的 **2 条接触面主线（配置指令 / 信号控制）+ 8 条支撑能力域**，既无遗漏也无越界。核实基准 = 官方源码 `nginx/src`（`commit 9e32c636`，nginx 1.31.3）。nginx 属**网络服务器 / 反向代理家族**（非计算引擎）——它是**长驻守护、监听端口、事件驱动处理海量连接、配置驱动行为**的中间层。灵魂两主线：**事件驱动连接模型** 与 **HTTP 请求阶段处理**。

## 深化 · 与数据库/计算引擎的心智对照（读前必看）

nginx 与数据库内核假设完全不同——**数据库以"数据 + 查询"为中心，nginx 以"连接 + 请求流水线"为中心**：
...
| 层 | 支撑能力域 | 一句话职责 | 内核锚点 |
|---|---|---|---|
| 底座 | **进程与事件模型** | master-worker + epoll 非阻塞事件循环 | `os/unix/ngx_process_cycle.c:74`、`event/ngx_event.c:195` |
| 底座 | **内存池与缓冲** | 按请求/连接分配、一次性释放；chain buffer | `core/ngx_palloc.c:19`、`core/ngx_buf.c:13` |
| 底座 | **日志与限流** | access/error 日志缓冲写、漏桶限流 | `http/modules/ngx_http_log_module.c:254`、`limit_req_module.c:405` |
| 请求 | **HTTP 阶段处理** | 11 phase 请求生命周期（灵魂） | `http/ngx_http_core_module.h:110`、`core_module.c:884` |
| 请求 | **模块体系** | handler / filter / upstream 三类模块 | `http/ngx_http.c:564`、`core/ngx_module.h:227` |
| 后端 | **upstream 负载均衡** | 多后端分流、健康检查、连接复用 | `http/ngx_http_upstream_round_robin.c:811` |
| 后端 | **代理缓存** | proxy_cache key→磁盘 + 共享内存索引 | `http/ngx_http_file_cache.c:265` |
| 后端 | **SSL/TLS** | 握手、SNI、证书、会话复用、终止加密 | `event/ngx_event_openssl.c:2201` |
...
| # | 贯穿声明 | 含义 |
|---|---|---|
| 1 | **一切行为由配置指令声明，一切指令由模块注册** | nginx 无查询语言；`nginx.conf` 每条 directive 对应某模块解析回调，决定其在请求流水线的挂载点与参数 |
| 2 | **worker 单线程事件循环 + 非阻塞是高并发根基** | 一个 worker 一线程经 epoll 管上万连接，绝不为单连接阻塞——磁盘/后端/DNS 等阻塞点全转事件回调 |
| 3 | **请求处理 = 11 阶段 handler + filter 输出链** | 请求经 POST_READ→…→CONTENT→LOG 十一阶段，content 产出经 header/body filter 链逐级加工发出——这是理解一切 HTTP 模块的骨架 |
