...
| 失败/边界场景 | 处理机制 | 锚点 |
|---|---|---|
| **reload 配置非法** | 新 cycle 解析失败时 master 释放新 cycle、保留旧 cycle 与旧 worker 继续服务——reload 天然安全，先 `nginx -t` | — |
| **旧 worker 排空超时** | 优雅退出的旧 worker 存量请求迟迟不完时，受 `worker_shutdown_timeout` 约束到点强杀，避免旧进程僵留 | — |
| **热升级新版起不来** | exec 失败时旧 master 继续持有监听、照常服务，热升级中止不影响线上 | `ngx_exec_new_binary:265` |
| **PID 文件冲突** | 热升级期旧 master 把 pid 改名 `.oldbin`、新 master 写新 pid；回滚需还原，运维脚本要处理两个 pid | — |
| **信号丢失** | `sig_atomic_t` 标志幂等置位，短时多次同信号只触发一次动作——不累积重复 reload | — |
