...
| 失败/边界场景 | 处理机制 | 锚点 |
|---|---|---|
| **共享内存 zone 耗尽** | key 太多撑爆 zone 时 LRU 淘汰最旧节点（`ngx_queue_remove`+插头）腾位；老 key 被挤、精度降但不崩 | `ngx_http_limit_req_lookup:441-443` |
| **拒绝的返回码** | 超限走 `NGX_HTTP_LIMIT_REQ_REJECTED`(`:15`) 返 503；`nodelay` 突发立放但计水位，`delay=` 排队平滑 | — |
| **dry_run 模式** | `limit_req_dry_run on` 命中 `..._REJECTED_DRY_RUN`(`:17`) 只记日志不真拒，用于上线前观测阈值 | — |
| **单 worker 计数无意义** | 不配 `*_zone` 时限流退化为进程内、多 worker 完全不准——必须用 zone | — |
| **日志写盘失败** | 磁盘满时写失败记 error（每分钟限频告警），不阻塞请求处理 | `ngx_http_log_write:98` |
