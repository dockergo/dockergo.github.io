#!/usr/bin/env python3
"""containerd-design 交互式核心原理图谱生成器（自包含 · 离线 · 双主题）。
...
用法：
  cd containerd-design && python3 gen.py
...
_ap = argparse.ArgumentParser(description="生成 containerd 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("CONTAINERD_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("CONTAINERD_DESIGN_OUT") or os.path.join(HERE, "index.html")

# ===================================================================== #
# 一、主线注册表 —— 唯一需随项目调整的数据块
#     家族（容器运行时 · 守护进程 + 插件化子系统）：元模式 = 接触面 × 插件子系统。
#     全景 + 1 接触面主线 + 7 支撑子系统。灵魂 = 一切皆插件 + shim 进程隔离。
# ===================================================================== #
MAINLINES = [
    ("Containerd原理_全景主线框架", "pano", "◇", "全景主线框架",
     "容器运行时守护进程：插件依赖图 · 内容寻址存储 · 快照 CoW · shim 隔离 · CRI"),

    ("Containerd原理_接口_gRPC服务与客户端", "iface", "⚙", "gRPC 服务与客户端",
     "唯一接触面：gRPC/TTRPC API · services 层派发 · ctr / client · 命名空间隔离"),

    ("Containerd原理_支撑_插件化架构", "support", "❖", "插件化架构与守护进程",
     "定义性特质：一切皆插件 · Requires 依赖图拓扑排序 · Init 依赖注入 · 守护进程装配"),
    ("Containerd原理_支撑_内容寻址存储", "support", "▤", "内容寻址存储与元数据",
     "content store：摘要即路径 blobs/<algo>/<digest> · ingest→commit · bbolt 元数据 + 租约 GC"),
    ("Containerd原理_支撑_快照与CoW层", "support", "▧", "快照 snapshotter 与 CoW",
     "Prepare/View/Commit 生命周期 · Active↔Committed · overlayfs 写时复制层 · Mounts"),
    ("Containerd原理_支撑_镜像拉取与解包", "support", "⤓", "镜像拉取与解包",
     "Hot 流：resolve→fetch 落 content→unpack · diff.Applier 把层 apply 成快照"),
    ("Containerd原理_支撑_运行时shim", "support", "▦", "运行时 v2 shim",
     "每容器一 shim 进程 · task_manager→binary Start→runc/OCI · shim 存活于 daemon 重启"),
    ("Containerd原理_支撑_task与事件总线", "support", "◉", "task/exec 与事件总线",
     "tasks gRPC 派发到 v2 runtime · Create/Start/Exec/Delete · events exchange 扇出订阅"),
    ("Containerd原理_支撑_CRI插件", "support", "▩", "CRI 插件",
     "k8s 接触面：RunPodSandbox / CreateContainer · 一个 GRPC plugin 拉齐十余依赖"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接触面主线 · 用户/k8s 可见（gRPC API + ctr）"),
    ("support", "支撑主线 · 守护进程内部（7 个插件化子系统）"),
]
...
BRAND_TITLE = "containerd 核心原理"
BRAND_SUB = "containerd"
HOME_DESC = ("containerd 核心原理设计文档库的离线交互图谱——工业级容器运行时（守护进程 + 插件化子系统）范例。"
             "9 条主线、手绘原理图全部回源码核实（v2.3）。点击项目总架构图任意模块即可下钻到对应主线。")
ARCH_SVG_NAME = "Containerd原理_全景_02总架构.svg"
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="containerd 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
_ICON_SVG = _b64_svg("icon.svg")  # 官网按钮复用 containerd 官方图标(纯黑几何 mark)
...
.owico{opacity:.82;transition:opacity .15s}a:hover .owico{opacity:1}:root[data-theme="dark"] .owico{filter:invert(1)}
