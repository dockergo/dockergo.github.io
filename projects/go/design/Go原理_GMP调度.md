...
- **G**（`g`，runtime2.go:471）：一个 goroutine。关键字段（见图）`stack`/`stackguard0`（栈边界兼抢占哨兵）、`sched`（`gobuf`，切出时存 sp/pc/bp）、`atomicstatus`、`goid`、`_panic`/`_defer`、`schedlink`。
- **M**（`m`，runtime2.go:616）：一个 OS 线程。核心是 `g0`（**调度专用栈**，所有调度/GC/栈增长都在其上跑）、`curg`（当前用户 G）、`p`、`tls`、`spinning`、`lockedg`（`LockOSThread` 绑定）。
- **P**（`p`，runtime2.go:774）：逻辑处理器，数量 = `GOMAXPROCS`。持 `runq[256]`+`runnext`（本地运行队列，256 定长环形数组）、`gFree`（G 复用池）、`mcache`（无锁分配缓存）、`status`（`_Pidle`/`_Prunning`/`_Psyscall`…）。
...
**新工作唤醒**：`go` 语句创建 G（`newproc`，proc.go:5334）或 G 被 `ready` 置为就绪后,若还有空闲 P 且没有正在找活的 spinning M,运行时调 `wakep`（proc.go）→ `startm` 唤醒/新建一个 M 绑定空闲 P 来认领——保证刚就绪的 G 不会在有空闲核的情况下干等。这也是 sysmon/netpoll 注入新 G 后必调 `wakep` 的原因。
...
- OS 通知 FD 就绪 → `netpoll(delay)` 返回**一批**就绪 G → 调度器用 `injectglist`（proc.go）批量注入运行队列(当前 M 可留一个直接跑、其余进全局队列),`netpollready`（netpoll.go:494）→ `netpollgoready` 把 G 置 `_Grunnable`;随后 `wakep`（proc.go）唤醒空闲 P/M 来认领这批新就绪的 G。
...
![goroutine 状态机 · atomicstatus 六态流转](Go原理_GMP_09状态机.svg)

`_Grunning` 是中枢：进 syscall（`entersyscall`→`_Gsyscall`）、阻塞（`gopark`→`_Gwaiting`）、抢占/退出（`_Gpreempted`/`_Gdead`）都从它出发；回程一律先落 `_Grunnable`（就绪队列）再被 `execute` 选中，形成「就绪⇄运行」主循环。退出的 G 不销毁而是 `gfput` 进 `gFree` 池，`gfget` 复用（省去反复建栈）。
