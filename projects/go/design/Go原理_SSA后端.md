...
`ssa.Compile`（compile.go:30）按 `passes` 列表（compile.go:457）顺序跑约 60 遍 pass（非优化模式跳过非必需 pass），三段见图：

- **机器无关优化**（lower 前）：`opt`、`generic cse`、`nilcheckelim`、**`prove`**（值域推断消边界检查）、`branchelim`、`dse`、`generic deadcode`；随后 `expand calls` 与 **`writebarrier`**（编译期为指针写插入 GC 写屏障，【GC】接合点）。
- **`lower`**（lower.go:8）：**分水岭 pass**——把机器无关 SSA 转成**机器相关** SSA（用目标架构具体指令 Op，如 amd64 `LEAQ`/`MOVQ`）。
- **机器相关收尾**（lower 后）：`schedule`（块内指令调度）、**`flagalloc`**、**`regalloc`**（线性扫描寄存器分配）、`late lower`/`addressing modes`/`loop rotate`/`trim`。
