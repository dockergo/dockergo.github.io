#!/usr/bin/env python3
"""grpc-design 交互式核心原理图谱生成器（自包含 · 离线 · 双主题）。
...
_ap = argparse.ArgumentParser(description="生成 grpc 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("GRPC_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("GRPC_DESIGN_OUT") or os.path.join(HERE, "index.html")

# ===================================================================== #
# 一、主线注册表 —— 唯一需随项目调整的数据块
#     家族（通信中间件 · RPC 框架 · gRPC）：元模式 = 接触面 × 能力域。
#     全景 + 1 接触面主线 + 7 支撑能力域。灵魂 = 可插拔装配线 + HTTP/2 多路复用。
# ===================================================================== #
MAINLINES = [
    ("gRPC原理_全景主线框架", "pano", "◇", "全景主线框架",
     "HTTP/2 RPC：接触面契约 · 总架构装配线 · 一次调用贯穿路径 · 依赖矩阵"),

    ("gRPC原理_接口_Stub与Channel契约", "iface", "⚙", "Stub 与 Channel 契约",
     "唯一接触面：Protobuf 生成 Stub · Channel 承载 CallOpSet · 四型 RPC"),

    ("gRPC原理_支撑_调用生命周期与CallSpine", "support", "◉", "调用生命周期与 CallSpine",
     "灵魂：CallSpine promise 化调用 · picker 选 subchannel · 连接态机 + 退避重连"),
    ("gRPC原理_支撑_名字解析与负载均衡", "support", "⇄", "名字解析与负载均衡",
     "resolver 解析地址 + service config → LB 产 picker · pick_first / round_robin"),
    ("gRPC原理_支撑_Filter栈与拦截链", "support", "▤", "Filter 栈与拦截链",
     "channel stack 逐 filter 串每次调用 · v3 拦截链 · 栈底交 transport"),
    ("gRPC原理_支撑_HTTP2传输与流控", "support", "◫", "HTTP/2 传输与流控",
     "单连接多路复用 N stream · 帧化 + 读写状态机 · flow control/BDP · HPACK"),
    ("gRPC原理_支撑_CompletionQueue与EventEngine", "support", "▦", "CompletionQueue 与 EventEngine",
     "tag 驱动 CQ（Next/Pluck/Callback）· EventEngine Run/RunAfter · poller/iomgr"),
    ("gRPC原理_支撑_Deadline重试与Keepalive", "support", "⚡", "Deadline 重试与 Keepalive",
     "deadline 编码进 metadata · retryPolicy 退避 + throttle · keepalive ping 探活"),
    ("gRPC原理_支撑_凭证与安全", "support", "⛨", "凭证与安全",
     "横切：传输凭证 TLS/ALTS 握手 · 调用凭证 OAuth2/JWT 注入 metadata"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接触面主线 · 用户可见（Protobuf Stub + Channel）"),
    ("support", "支撑主线 · 框架内部（7 条能力域）"),
]
...
BRAND_TITLE = "gRPC 核心原理"
BRAND_SUB = "gRPC"
HOME_DESC = ("gRPC 核心原理设计文档库的离线交互图谱——通信中间件家族（HTTP/2 RPC 框架）范例。"
             "9 条主线、多张手绘原理图，全部回源码核实。点击项目总架构图任意模块即可下钻到对应主线。")
ARCH_SVG_NAME = "gRPC原理_全景_02总架构.svg"
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="gRPC 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
<header>
  ...
  <a href="https://github.com/grpc/grpc" target="_blank" rel="noopener" title="GitHub 源码仓库" style="display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M12 .5C5.7.5.5 5.7.5 12c0 5.1 3.3 9.4 7.9 10.9.6.1.8-.2.8-.6v-2c-3.2.7-3.9-1.4-3.9-1.4-.5-1.3-1.3-1.7-1.3-1.7-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1 1.8 2.7 1.3 3.4 1 .1-.8.4-1.3.7-1.6-2.6-.3-5.3-1.3-5.3-5.8 0-1.3.5-2.3 1.2-3.1-.1-.3-.5-1.5.1-3.1 0 0 1-.3 3.3 1.2a11.4 11.4 0 0 1 6 0C17.3 4.7 18.3 5 18.3 5c.6 1.6.2 2.8.1 3.1.8.8 1.2 1.8 1.2 3.1 0 4.5-2.7 5.5-5.3 5.8.4.4.8 1.1.8 2.2v3.3c0 .4.2.7.8.6 4.6-1.5 7.9-5.8 7.9-10.9C23.5 5.7 18.3.5 12 .5z"/></svg></a><a href="https://grpc.io" target="_blank" rel="noopener" title="项目官网" style="display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px;font-size:12px;font-weight:700;letter-spacing:-.5px">gR</a><button id="themeBtn" title="切换深色 / 浅色主题" aria-label="切换主题" style="width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);background:var(--c-panel);color:var(--c-ink2);cursor:pointer;display:inline-grid;place-items:center;font-size:16px;flex:none"><span class="tt-ico tt-moon">☾</span><span class="tt-ico tt-sun">☀</span></button>
</header>
<div class="wrap">
  <div id="home">
    {archnav}
