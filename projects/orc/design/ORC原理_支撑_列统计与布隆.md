...
**ColumnStatistics** = `numberOfValues` + 一个类型化子消息 + `hasNull`(实现 `impl/ColumnStatisticsImpl.java:63`,`serialize` 落 proto 在 `:131`):
...
- **文件级**:`Footer.statistics`(`WriterImpl.writeFileStatistics`,调用点 `WriterImpl.java:707`)——读不读整个文件。
...
写侧 `TreeWriterBase` 每列同时建 `indexStatistics`(per row group,`TreeWriterBase.java:58`)+ `fileStatistics`(`TreeWriterBase.java:60`)——边写边攒。
...
- 实现:**Murmur3 64 位**哈希 + Kirsch 双哈希技巧跑 k 个函数(`BloomFilter.java:37`);写入 `addBytes`(`util/BloomFilter.java:111`)、探测 `testBytes`(`util/BloomFilter.java:158`);`optimalNumOfHashFunctions`(`:85`)按 FPP 定 k。默认 **FPP=0.01**(`BLOOM_FILTER_FPP`,`OrcConf.java:99`);建布隆的列由 `BLOOM_FILTER_COLUMNS`(`OrcConf.java:140`)指定。
...
| ColumnStatisticsImpl | `impl/ColumnStatisticsImpl.java:63` | 各类型统计的收集/合并 |
| ColumnStatisticsImpl.serialize | `impl/ColumnStatisticsImpl.java:131` | 落 proto ColumnStatistics |
| indexStatistics / fileStatistics | `impl/writer/TreeWriterBase.java:58` / `:60` | 每列 per-group 与文件级两套统计 |
| 文件级统计 | `WriterImpl.java:707` / `:709` | writeFileStatistics 调用,跳整文件 |
| 布隆列选择 | `impl/writer/TreeWriterBase.java:92` | 按 getBloomFilterColumns[id] 决定哪列建布隆 |
...
| BloomFilter.addBytes / testBytes | `util/BloomFilter.java:111` / `:158` | 写入 / 探测 |
| BLOOM_FILTER_FPP | `OrcConf.java:99` | 假阳性率(默认 0.01) |
| BLOOM_FILTER_COLUMNS | `OrcConf.java:140` | 建布隆的列 |
