...
- **RLE v1**(Hive 0.11,`RunLengthIntegerWriter.java:31`):游程(初始字节 0x00–0x7f=长度−3 + 定长 delta + varint base)与字面量(0x80–0xff=−count + varints)两种。简单。
...
| TreeWriterBase | `impl/writer/TreeWriterBase.java:52` | 各列 writer 基类,持 index/file 统计边写边攒 |
| IntegerTreeWriter | `impl/writer/IntegerTreeWriter.java:37` | 整数列 writer,值走 IntegerWriter(RLE) |
| StringTreeWriter | `impl/writer/StringTreeWriter.java:31` | 字符串列 writer,继承字典基类 |
| BitFieldWriter | `impl/BitFieldWriter.java:27` | PRESENT null 位图 stream 写入 |
| RunLengthIntegerWriter (v1) | `impl/RunLengthIntegerWriter.java:31` | RLE v1 游程/字面量两态 |
| RunLengthIntegerWriterV2 | `impl/RunLengthIntegerWriterV2.java:121` | RLE v2 四子编码,枚举 `:123` |
| StringBaseTreeWriter.checkDictionaryEncoding | `impl/writer/StringBaseTreeWriter.java:126` | 按 `ratio ≤ 阈值` 定字典/直存 |
