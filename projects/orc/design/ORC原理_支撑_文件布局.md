...
| Footer | `WriterImpl.java:693` | 组 Footer.Builder(stripes/types/统计) |
| writeFileFooter | `PhysicalFsWriter.java:457` | 落 footer 字节 |
| writePostScript | `PhysicalFsWriter.java:475` | 落不压缩 postscript + 末字节长度 |
| StripeInformation | (proto) | offset/index/data/footer len/rows |
| PhysicalFsWriter | `PhysicalFsWriter.java:50` | 物理落盘(stripe/footer/postscript) |
| PhysicalFsWriter.finalizeStripe | `PhysicalFsWriter.java:680` | index-then-data 物理写序 |
| STRIPE_SIZE / STRIPE_ROW_COUNT | `OrcConf.java:32` / `:35` | 大小/行数双触发 flush |
| ReaderImpl 倒读 | `ReaderImpl.java:75,842` | 16KB 尾读 → postscript → footer |
| C++ 倒读镜像 | `c++/src/Reader.cc:1545` / `:1514` | readPostscript / ensureOrcFooter |
