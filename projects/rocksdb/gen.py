...
TABS = [
    # design 原理图走查(每主题一个「原理详解」走查 tab)
    ("kvwalk", "原理详解", ""),
    ("writewalk", "原理详解", ""),
    ("readwalk", "原理详解", ""),
    ("sstwalk", "原理详解", ""),
...
TAB_META = {
    # 每主题一个「原理详解」走查(权威手绘 SVG · 逐图垂直 TAB)+ 全景 + 对比
    "kvwalk":      ("◷", "原理详解 · KV 操作 API:读写两类 → Merge → Iterator → WriteBatch", "原理"),
    "writewalk":   ("✎", "原理详解 · 写入路径:WAL 追加 → MemTable → WriteThread 分组 → 写停顿", "原理"),
    "readwalk":    ("◉", "原理详解 · 读取路径:逐层归并 → SuperVersion → 布隆过滤 → 点查/扫描", "原理"),
    "sstwalk":     ("▤", "原理详解 · SST 格式:布局 → Block(前缀压缩+重启点) → 过滤器与索引", "原理"),
    "flushwalk":   ("⤓", "原理详解 · Flush:immutable→L0 SST → 触发调度 → 与写入/WAL/版本联动", "原理"),
    "compactwalk": ("▨", "原理详解 · Compaction:逐层归并 → 三风格 → Leveled → 三放大权衡", "原理"),
    "versionwalk": ("⬢", "原理详解 · 版本:Version/FileMetaData → MANIFEST/CURRENT → 恢复", "原理"),
    "cachewalk":   ("◲", "原理详解 · 缓存:三层缓存 → LRU/HyperClock → 内容与淘汰(防污染)", "原理"),
    "txnwalk":     ("⬡", "原理详解 · 事务:MVCC(seq+内部键) → Snapshot → 悲观/乐观/2PC", "原理"),
    "walwalk":     ("▦", "原理详解 · WAL:32KB 块格式 → 生命周期 → 崩溃恢复(先 MANIFEST)", "原理"),
    "cfwalk":      ("◧", "原理详解 · Column Family:各自 LSM → 共享 WAL → 独立调优", "原理"),
    "panowalk":    ("◇", "架构地图 · 双维模型 → 总架构图 → 依赖矩阵 → 能力域依赖关系图", "整体架构"),
    "compare":     ("▦", "存储引擎对比 · RocksDB vs LevelDB/InnoDB B-Tree/WiscKey/TiKV 设计取舍", "整体架构"),
}
...
THEMES = [
    # ── 接触面主线 · 用户可见 · RocksDB 是嵌入式 LSM KV 库(原型 C:嵌入式存储引擎/KV):无 SQL/网络/分布式,直调 KV API ──
    {"id": "kvapi", "icon": "◷", "title": "KV 操作 API", "cat": "iface", "ord": 0,
     "desc": "主接触面:Put/Get/Delete/Merge/Iterator/WriteBatch。design 走查——API 全景(读写两类) → Merge(读改写合一,读期合并) → Iterator(有序范围遍历,多路归并) → WriteBatch(一批写原子提交),附调优/误区/总纲",
     "tabs": ["kvwalk"]},

    # ── 支撑主线 · 引擎内部(10 条)· 严格以 design 走查为主干 ──
    {"id": "write", "icon": "✎", "title": "写入路径", "cat": "support", "ord": 0,
     "desc": "写从 API 到内存:design 走查——写入全景(WAL→MemTable) → WriteThread(leader-follower 分组提交) → MemTable(InlineSkipList 跳表插入) → 写停顿(write stall,L0/待flush 反压),附调优/误区/总纲",
     "tabs": ["writewalk"]},
    {"id": "read", "icon": "◉", "title": "读取路径", "cat": "support", "ord": 1,
     "desc": "读跨内存与多层 SST:design 走查——读取全景(MemTable→L0..Ln 逐层) → SuperVersion(钉住一致视图) → 布隆过滤(Get 短路跳 SST) → 点查 vs 扫描(GetImpl / 多路归并迭代),附调优/误区/总纲",
     "tabs": ["readwalk"]},
    {"id": "sst", "icon": "▤", "title": "SST 存储格式", "cat": "support", "ord": 2,
     "desc": "磁盘上的不可变文件:design 走查——SST 布局(data/index/filter/metaindex/footer) → Block 结构(前缀压缩+重启点) → 过滤器与索引(布隆/partitioned),附调优/误区/总纲",
     "tabs": ["sstwalk"]},
    {"id": "flush", "icon": "⤓", "title": "Flush", "cat": "support", "ord": 3,
     "desc": "内存落盘:design 走查——Flush 全景(immutable MemTable→L0 SST) → 触发与调度(write_buffer_size/手动/CF 联动) → 与写入/WAL/版本联动(FlushJob→VersionEdit),附调优/误区/总纲",
     "tabs": ["flushwalk"]},
    {"id": "compaction", "icon": "▨", "title": "Compaction", "cat": "support", "ord": 4,
     "desc": "后台整理是 LSM 的心脏:design 走查——Compaction 全景(逐层归并) → 三种风格(Level/Universal/FIFO) → Leveled 细节(层容量+选文件) → 三放大权衡(读/写/空间),附调优/误区/总纲",
     "tabs": ["compactwalk"]},
    {"id": "version", "icon": "⬢", "title": "版本与 MANIFEST", "cat": "support", "ord": 5,
     "desc": "元数据一致性根基:design 走查——Version/FileMetaData(活跃 SST 集合) → MANIFEST(VersionEdit 日志+CURRENT+原子换版) → 恢复(先 MANIFEST 重建 Version),附调优/误区/总纲",
     "tabs": ["versionwalk"]},
    {"id": "cache", "icon": "◲", "title": "缓存", "cat": "support", "ord": 6,
     "desc": "读加速:design 走查——缓存全景(block cache/table cache/OS page cache 三层) → 实现(LRUCache vs HyperClockCache,均分片) → 内容与淘汰(index/filter 优先级+防污染),附调优/误区/总纲",
     "tabs": ["cachewalk"]},
    {"id": "txn", "icon": "⬡", "title": "事务与快照", "cat": "support", "ord": 7,
     "desc": "并发与一致读:design 走查——MVCC(SequenceNumber+内部键多版本) → Snapshot(钉 seq 一致读+约束回收) → 事务(悲观锁/乐观检测/2PC),附调优/误区/总纲",
     "tabs": ["txnwalk"]},
    {"id": "wal", "icon": "▦", "title": "WAL 与崩溃恢复", "cat": "support", "ord": 8,
     "desc": "持久性根基:design 走查——WAL 记录格式(32KB 块对齐+跨块分片) → 生命周期(随 MemTable 生、sync 定强度、flush 后可删) → 崩溃恢复(先 MANIFEST 后重放 WAL),附调优/误区/总纲",
     "tabs": ["walwalk"]},
    {"id": "cf", "icon": "◧", "title": "Column Family", "cat": "support", "ord": 9,
     "desc": "组织能力域:design 走查——CF 全景(各自一棵 LSM,共享一份 WAL) → 为何共享 WAL(跨 CF 原子写 vs flush 耦合代价) → 独立调优(各设 compaction/压缩/comparator),附调优/误区/总纲",
     "tabs": ["cfwalk"]},
...
# 架构图未描绘的主题(缓存/架构地图/对比)→ 底部补充 chip,保证内容完整可达。
# 缓存无独立架构区域(在读路径③"block cache"内提及),故作 chip;overallarch 总为 chip。
...
_SVG_WALK_SPECS = {
    # ---- 接触面主线(1 条:KV 操作 API)----
    "kvwalk": [("API 全景 · 读写两类 + Options", "RocksDB原理_接触面_01API全景.svg"),
               ("Merge · 读改写合一,读期合并操作数", "RocksDB原理_接触面_02Merge.svg"),
               ("Iterator · 有序范围遍历,多路归并", "RocksDB原理_接触面_03Iterator.svg"),
               ("WriteBatch · 一批写原子提交", "RocksDB原理_接触面_04WriteBatch.svg")],
    # ---- 支撑主线(10 条)----
    "writewalk": [("写入全景 · WAL 追加 → MemTable 插入", "RocksDB原理_写入_01全景.svg"),
                  ("WriteThread · leader-follower 分组提交", "RocksDB原理_写入_02WriteThread.svg"),
                  ("MemTable · InlineSkipList 跳表插入", "RocksDB原理_写入_03MemTable.svg"),
                  ("写停顿 · write stall 反压(L0/待flush)", "RocksDB原理_写入_04写停顿.svg")],
    "readwalk": [("读取全景 · MemTable → L0..Ln 逐层归并", "RocksDB原理_读取_01全景.svg"),
                 ("SuperVersion · 钉住 {mutable+immutable+Version} 一致视图", "RocksDB原理_读取_02SuperVersion.svg"),
                 ("布隆过滤 · Get 短路,跳过不含 key 的 SST", "RocksDB原理_读取_03布隆过滤.svg"),
                 ("点查 vs 扫描 · GetImpl / 多路归并迭代器", "RocksDB原理_读取_04点查与扫描.svg")],
    "sstwalk": [("SST 布局 · data/index/filter/metaindex/footer", "RocksDB原理_SST_01布局.svg"),
                ("Block 结构 · 前缀压缩 + 重启点", "RocksDB原理_SST_02Block.svg"),
                ("过滤器与索引 · 布隆 / partitioned index", "RocksDB原理_SST_03FilterIndex.svg")],
    "flushwalk": [("Flush 全景 · immutable MemTable → L0 SST", "RocksDB原理_Flush_01全景.svg"),
                  ("触发与调度 · write_buffer_size / 手动 / CF 联动", "RocksDB原理_Flush_02触发调度.svg"),
                  ("联动 · FlushJob → VersionEdit,释放 WAL", "RocksDB原理_Flush_03联动.svg")],
    "compactwalk": [("Compaction 全景 · 后台逐层归并", "RocksDB原理_Compaction_01全景.svg"),
                    ("三种风格 · Level / Universal / FIFO", "RocksDB原理_Compaction_02三风格.svg"),
                    ("Leveled 细节 · 层容量 + 选文件", "RocksDB原理_Compaction_03Leveled.svg"),
                    ("三放大权衡 · 读 / 写 / 空间", "RocksDB原理_Compaction_04三放大.svg")],
    "versionwalk": [("Version / FileMetaData · 活跃 SST 集合", "RocksDB原理_版本_01Version.svg"),
                    ("MANIFEST · VersionEdit 日志 + CURRENT + 原子换版", "RocksDB原理_版本_02MANIFEST.svg"),
                    ("恢复 · 先重放 MANIFEST 重建 Version", "RocksDB原理_版本_03恢复.svg")],
    "cachewalk": [("缓存全景 · block / table / OS page cache 三层", "RocksDB原理_缓存_01全景.svg"),
                  ("实现 · LRUCache vs HyperClockCache(均分片)", "RocksDB原理_缓存_02实现.svg"),
                  ("内容与淘汰 · index/filter 优先级 + 防污染", "RocksDB原理_缓存_03内容淘汰.svg")],
    "txnwalk": [("MVCC · SequenceNumber + 内部键多版本", "RocksDB原理_事务_01MVCC.svg"),
                ("Snapshot · 钉 seq 一致读 + 约束回收", "RocksDB原理_事务_02Snapshot.svg"),
                ("事务 · 悲观锁 / 乐观检测 / 2PC", "RocksDB原理_事务_03事务.svg")],
    "walwalk": [("WAL 记录格式 · 32KB 块对齐 + 跨块分片", "RocksDB原理_WAL_01格式.svg"),
                ("生命周期 · 随 MemTable 生、sync 定强度、flush 后可删", "RocksDB原理_WAL_02生命周期.svg"),
                ("崩溃恢复 · 先 MANIFEST 后重放 WAL", "RocksDB原理_WAL_03恢复.svg")],
    "cfwalk": [("CF 全景 · 各自一棵 LSM,共享一份 WAL", "RocksDB原理_CF_01全景.svg"),
               ("为何共享 WAL · 跨 CF 原子写 vs flush 耦合", "RocksDB原理_CF_02共享WAL.svg"),
               ("独立调优 · 各设 compaction / 压缩 / comparator", "RocksDB原理_CF_03独立调优.svg")],
    # ---- 全局图(架构地图)----
    "panowalk": [("双维模型 · 能力域 × 执行时机", "RocksDB原理_全景_01双维模型.svg"),
                 ("总架构图 · 嵌入宿主 · 写路径/LSM/读路径", "RocksDB原理_全景_02总架构.svg"),
                 ("依赖矩阵 · 接触面操作 × 支撑能力域", "RocksDB原理_依赖矩阵.svg"),
                 ("能力域依赖关系图", "RocksDB原理_依赖关系图.svg")],
}
# 快速开始「上手总览」用独立复合渲染器 renderQsTour;RocksDB 无此素材 → 空,quickstart 主题未挂载,仅占位
_QSTOUR_OVERVIEW_B64 = _design_b64("RocksDB上手_00总览.svg")  # 无此素材 → 空 base64
...
# walk tid → prose md 文件(全局图无 prose)
_WALK_PROSE = {
    "kvwalk": "RocksDB原理_接触面_KV操作API.md",
    "writewalk": "RocksDB原理_支撑_写入路径.md",   "readwalk": "RocksDB原理_支撑_读取路径.md",
    "sstwalk": "RocksDB原理_支撑_SST存储格式.md",  "flushwalk": "RocksDB原理_支撑_Flush.md",
    "compactwalk": "RocksDB原理_支撑_Compaction.md", "versionwalk": "RocksDB原理_支撑_版本与MANIFEST.md",
    "cachewalk": "RocksDB原理_支撑_缓存.md",        "txnwalk": "RocksDB原理_支撑_事务与快照.md",
    "walwalk": "RocksDB原理_支撑_WAL与崩溃恢复.md", "cfwalk": "RocksDB原理_支撑_ColumnFamily.md",
}
...
<title>RocksDB 引擎 · 交互式核心原理图谱</title>
...
<div id="loadingOverlay" role="status" aria-live="polite">
  <div class="lo-inner">
    ...
    <div class="lo-title">RocksDB Engine Atlas</div>
...
<div id="app">
  <header>
    ...
    <a href="https://github.com/facebook/rocksdb" target="_blank" rel="noopener" title="GitHub 源码仓库" style="margin-left:auto;display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M12 .5C5.7.5.5 5.7.5 12c0 5.1 3.3 9.4 7.9 10.9.6.1.8-.2.8-.6v-2c-3.2.7-3.9-1.4-3.9-1.4-.5-1.3-1.3-1.7-1.3-1.7-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1 1.8 2.7 1.3 3.4 1 .1-.8.4-1.3.7-1.6-2.6-.3-5.3-1.3-5.3-5.8 0-1.3.5-2.3 1.2-3.1-.1-.3-.5-1.5.1-3.1 0 0 1-.3 3.3 1.2a11.4 11.4 0 0 1 6 0C17.3 4.7 18.3 5 18.3 5c.6 1.6.2 2.8.1 3.1.8.8 1.2 1.8 1.2 3.1 0 4.5-2.7 5.5-5.3 5.8.4.4.8 1.1.8 2.2v3.3c0 .4.2.7.8.6 4.6-1.5 7.9-5.8 7.9-10.9C23.5 5.7 18.3.5 12 .5z"/></svg></a><a href="https://rocksdb.org" target="_blank" rel="noopener" title="项目官网" style="display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><img src="data:image/svg+xml;base64," width="18" height="18" alt="官网" style="display:block"/></a><button class="theme-toggle" id="themeToggle" title="切换深色 / 浅色主题" aria-label="切换主题">
      ...
      <div class="nav-mode nav-arch active" id="navArch">
        <div class="arch-stage">
          <div class="arch-canvas">
            <img class="arch-img" src="data:image/svg+xml;base64,__ARCH_SVG_B64__" alt="RocksDB 总架构图" draggable="false"/>
            __ARCH_HOTSPOTS__
          </div>
        </div>
...
const COMPARE_SPEC={
  id:'compare', title:'存储引擎对比 · RocksDB 在存储引擎家族中的定位',
  note:'突出设计取舍(非优劣);同族 LSM 对手看差异,异族 B-Tree(InnoDB)看写放大与并发的根本分野。',
  rowH:40,
  cols:[{h:'维度',w:140,accent:'var(--cv-scan)'},{h:'RocksDB',w:200,accent:'var(--cv-ink)'},{h:'LevelDB',w:150,accent:'var(--cv-ink)'},{h:'InnoDB(B+Tree)',w:170,accent:'var(--cv-ink)'},{h:'WiscKey / TiKV',w:230,accent:'var(--cv-ink)'}],
  rows:[
    ['数据结构','LSM-tree(多层有序 SST)','LSM-tree(单线程)','B+Tree(原地更新)','WiscKey=键值分离 LSM;TiKV=RocksDB 之上加 Raft'],
    ['写模型','追加不覆盖(顺序写)','追加不覆盖','原地改页(随机写)','均 LSM 追加(TiKV 直接用 RocksDB)'],
    ['写放大','中(Leveled 可调)','较高(无并发压缩)','低(单次改页)','WiscKey 大 value 分离降写放大'],
    ['读放大','逐层归并 + 布隆缓解','逐层归并','B+Tree 一次定位','WiscKey value 需一次额外寻址'],
    ['并发写','WriteThread 分组提交','单写者','行锁 + MVCC(undo)','TiKV 靠 Raft 复制,单机仍 RocksDB'],
    ['MVCC','SequenceNumber + 内部键','seq + 内部键','undo log + read view','同 RocksDB(TiKV 加分布式 TSO)'],
    ['Compaction','Level/Universal/FIFO 可选','仅 Leveled','无(靠 purge/merge)','同 RocksDB;WiscKey 只压缩 key'],
    ['列族/多空间','Column Family(共享 WAL)','无','表空间/表','TiKV 用 CF 分 raft/write/lock'],
    ['事务','悲观/乐观/2PC(可选层)','无','完整 SQL 事务','TiKV=分布式事务(Percolator)'],
    ['定位','嵌入式 · 通用 KV 底座','嵌入式 · 教学原型','MySQL 默认存储引擎','分布式 KV(建在 RocksDB 上)'],
    ['可借鉴','—','RocksDB 的前身','B+Tree 读友好(点查快)','键值分离 / 分布式化 RocksDB 的范式']
  ]
};
