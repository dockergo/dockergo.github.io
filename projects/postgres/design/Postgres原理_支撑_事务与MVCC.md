...
![XID 回卷防线](Postgres原理_MVCC_03回卷防线.svg)

XID 是 32 位循环计数，freeze 是防其回卷的核心机制：VACUUM 把足够老、对所有快照都可见的 tuple 标为"已冻结"（xmin 视为永远过去、不再参与年龄计算），推高 `relfrozenxid`、把"最老未冻结年龄"拉回安全区。`GetNewTransactionId`（`access/transam/varsup.c`）按年龄设三道闸：过 **xidVacLimit**（age ≥ `autovacuum_freeze_max_age`，`:105`）→ 强制反回卷 VACUUM（即使 autovacuum 已关）；过 **xidWarnLimit**（=wrap−1亿，`:414`）→ 每次分配 XID 告警；过 **xidStopLimit**（=wrap−300万，`:400`）→ 数据库拒绝写入、进只读自保（须单用户模式 VACUUM 恢复）。**红线成因**：长事务 / 关 autovacuum / 大量导入后从不 VACUUM，都会让最老年龄单调逼近 stop 线——这是 MVCC 唯一会让整库停摆的边界。
...
| 场景 | 机理 | 后果 / 应对 |
|---|---|---|
| XID 回卷停写 | `relfrozenxid` 年龄逼近 `autovacuum_freeze_max_age` 触发强制反回卷 VACUUM | 再逼近 `xidStopLimit` 整库进只读拒写保护态，须单用户模式 VACUUM 恢复——MVCC 唯一让整库停摆的边界 |
| 长事务压住回收 | `ComputeXidHorizons` 算的全局可回收水位取决于最老快照 | 一个空闲未提交事务把水位钉死，其后死元组无法回收、膨胀失控 |
| hint bits 放大写 | hint bits 由第一个访问该 tuple 的查询回写（`SetHintBits`） | 一次大 SELECT 引发大量页变脏、连带 WAL；批量导入后首次全表扫描"莫名"变慢 |
| SSI 序列化失败 | Serializable 检测到依赖环主动 `could not serialize access` | 应用需捕获重试；跟踪读写依赖本身有内存与 CPU 开销 |
