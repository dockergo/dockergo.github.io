...
| 场景 | 机理 | 后果 / 应对 |
|---|---|---|
| 页满与更新退化 | UPDATE 新版本放不下当前页则跨页、HOT 落空、所有索引加项 | 写放大；低 `fillfactor` 预留页内空间可缓解 |
| 缓冲池抖动 | 大顺序扫描用 `BufferAccessStrategy` 环形缓冲限制占用 | 避免一次大扫描冲刷整个 shared_buffers、热数据被挤出致后续全 miss |
| TOAST 读放大 | 频繁读切片外置大字段要多次访问 TOAST 附表 | 冷热字段分表，或用 EXTERNAL（不压缩、便于随机切片读）视访问模式而定 |
| 校验和与坏页 | 开 `data_checksums` 后读到 checksum 不符的页报错 | 拒绝返回脏数据防静默损坏，代价是写时算校验和 |
