...
- `server/etcdserver/read/read.go:74` `Read.LinearizableReadNotify` — 读请求等待入口；`server/etcdserver/interface.go:46` `EtcdServer.LinearizableReadNotify`（对外委托）。
- `server/etcdserver/read/read.go:96` `LinearizableReadLoop` — 独立循环；`:98` `LeaderChanged()`（leaderChangedNotifier，leader 变更时未决读失败重试）。
- `server/etcdserver/read/read.go:117` `requestCurrentIndex` 调用 → `:148` 定义 → `:232` `sendReadIndex`（向 raft 请 ReadIndex，确认 leader+拿 commit index）。
- `server/etcdserver/read/read.go:130` `AppliedIndex()` → `:133` `if appliedIndex < confirmedIndex`（等本地 apply 追上 ReadIndex 才返回）。
- `server/etcdserver/read/read.go:33` `readIndexRetryTime = 500ms` / `:160` `retryTimer`（超时重试 ReadIndex）。
- `server/etcdserver/v3_server.go:106` `Range` — 读入口（串行读直接读本地 MVCC，与线性读对比）。
