...
- `server/storage/wal/wal.go:72` `type WAL`；`:39` `MetadataType`（记录类型 iota：Metadata/Entry/State/Crc/Snapshot）。
- `server/storage/wal/wal.go:100` `Create` / `:345` `Open` / `:471` `ReadAll`（重启回放 metadata+HardState+entries）。
- `server/storage/wal/wal.go:958` `Save(st *raftpb.HardState, ents []*raftpb.Entry)` — 保存 HardState 与日志条目（注意参数是指针）。
- `server/storage/wal/wal.go:832` `sync()` → `:844` `fileutil.Fdatasync` — fsync 落盘（写延迟决定因素）。
- `server/storage/wal/wal.go:748` `cut()` — 64MB 分段切换；`:1002` `SaveSnapshot` — 写快照记录。
- `server/etcdserver/raft.go:404` `raftNode.apply()`（Ready→先写 WAL→再 apply）；`server/etcdserver/server.go:1881` `EtcdServer.apply`（应用后触发快照判断）。
