...
- `server/storage/mvcc/watchable_store.go:56` `type watchableStore` / `:92` `newWatchableStore` — 包裹普通 store。
- `server/storage/mvcc/watchable_store.go:41` `maxWatchersPerSync = 512`；`:64` `victims []watcherBatch`（慢消费者隔离组）。
- `server/storage/mvcc/watchable_store.go:224` `syncWatchersLoop`（后台循环）→ `:347` `syncWatchers`（unsynced 追赶）→ `:285` `moveVictims`（victim 重投递）。
- `server/storage/mvcc/watchable_store.go:470` `notify(rev, evs)` — 每次写后把事件分发给 synced watcher。
- `server/storage/mvcc/watcher_group.go:149` `type watcherGroup` / `:158` `newWatcherGroup` / `:273` `watcherSetByKey`（按 key/range 索引）。
- `server/etcdserver/api/v3rpc/watch.go:130` `serverWatchStream` / `:249` `recvLoop` / `:410` `sendLoop` — 一条 gRPC 流多路复用；`api/etcdserverpb/rpc.proto:92` `service Watch`。
