...
- `server/lease/lessor.go:83` `type Lessor interface` — 租约管理接口。
- `server/lease/lessor.go:281` `Grant(id, ttl)`（走 Raft 建租约）；`:326` `Revoke(id)`（走 Raft 级联删除名下 key）；`:396` `Renew(id)`（keepalive 续租，只改 leader 内存不走 Raft）。
- `server/lease/lessor.go:620` `runLoop`（后台守护）→ `:640` `revokeExpiredLeases`（检出并发起 Revoke）→ `:728` `findExpiredLeases`（最小堆按到期时间检出）。
- `server/lease/lessor.go:667` `checkpointScheduledLeases` — 周期把剩余 TTL 作为 checkpoint 走 Raft 复制（防 leader 切换重置计时）。
- `server/lease/lessor.go:185` `checkpointInterval` / `:189` `checkpointPersist` — checkpoint 频率与持久化开关。
- `server/etcdserver/api/v3rpc/grpc.go:82` `RegisterLeaseServer(NewQuotaLeaseServer(s))`；`api/etcdserverpb/rpc.proto:106` `service Lease`。
