...
![家族对照](etcd原理_家族对照.svg)

图分三列：左两列是"别的家族"（计算引擎的 SQL、文件系统的 FileSystem API），右列高亮 etcd——接触面是一族 gRPC API、每节点存全量小数据、Raft 强一致。由此先立三条反直觉认知（不是查询而是协调 / 数据量小不分片 / CP 要正确非吞吐），后文不再重复。逐维对照（含"影响"列）见文末深化表。
...
镜像元模式（接触面 × 能力域 × 执行时机），按 etcd 社区实情命名：**1 条接触面主线**（gRPC API 族，面向用户）+ **8 条支撑主线**（面向引擎内部）——灵魂是 Raft 共识。完整登记（职责 + 核实锚点）见文末拓展表。
...
## 拓展 · 能力域主线登记（1 接触面 + 8 支撑）

| 主线 | 一句话职责 | 核实锚点 |
|---|---|---|
| **gRPC API 族**（接触面） | KV(Range/Put/DeleteRange/Txn/Compact)、Watch、Lease、Cluster、Auth、Maintenance、Election/Lock；拦截器链（认证/配额/校验） | `api/etcdserverpb/rpc.proto`、`server/etcdserver/api/v3rpc/*` |
...
| 定位 | 存海量业务数据 | 存海量文件 | 存"关于系统的关键元数据" | k8s 全部状态、服务发现、分布式锁、选主——要正确而非吞吐 |

## 源码锚点（etcd main `88fe81c`，raft `go.etcd.io/raft/v3 v3.7.0`；均本地 grep 核实）

贯穿示例 `Put` 全链路 + 读链路对应源码：

- `server/etcdserver/v3_server.go:295` `Put` — 写入口；`:325` `Txn`；`:106` `Range`（读入口）。
- `server/etcdserver/v3_server.go:1012` `raftRequest` → `:1058` `processInternalRaftRequestOnce` — 编码 `InternalRaftRequest` 提给 raft、等 applied。
- `server/etcdserver/raft.go:174` `raftNode.start`（Ready 循环）→ `:404` `raftNode.apply()` 把已提交条目送状态机。
- `server/storage/wal/wal.go:958` `WAL.Save(HardState, ents)` — 先写 WAL 落盘。
- `server/etcdserver/server.go:1881` `EtcdServer.apply` → `:1934` `applyEntryNormal` 把普通 entry 交 applierV3。
- `server/storage/mvcc/kvstore.go:72` `currentRev`（写 bump revision）；`server/storage/mvcc/index.go:39` `treeIndex`、`:45` `newTreeIndex`（内存索引）；`server/storage/backend/batch_tx.go:149` `unsafePut`、`:261` `commit`（落 boltdb）。
- `server/etcdserver/read/read.go:96` `LinearizableReadLoop` → `:148` `requestCurrentIndex`（线性读走 ReadIndex）。
