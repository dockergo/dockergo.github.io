#!/usr/bin/env python3
"""hive-design 交互式核心原理图谱生成器（自包含 · 离线 · 双主题）。
...
_ap = argparse.ArgumentParser(description="生成 Apache Hive 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("HIVE_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("HIVE_DESIGN_OUT") or os.path.join(HERE, "index.html")

# ===================================================================== #
# 一、主线注册表 —— 唯一需随项目调整的数据块
#     Hive（解耦式 SQL-on-Hadoop 数仓门面）：全景 + 4 条支撑能力域。
#     总架构图 11 个 data-tid 热区经 TID2MID 归并映射到这 5 条主线。
# ===================================================================== #
MAINLINES = [
    ("Hive原理_全景主线框架", "pano", "◇", "全景主线框架",
     "解耦式 SQL 数仓门面：双维模型 · 总架构 · 依赖矩阵 · 4 接口主线 + 5 支撑主线"),

    ("Hive原理_支撑_元数据服务", "support", "▤", "元数据服务（HMS）",
     "库表→目录+SerDe+Schema 独立目录服务 · Thrift→HMSHandler→ObjectStore→RDBMS · 全生态共享"),
    ("Hive原理_支撑_编译优化", "support", "✎", "编译优化",
     "五段流水：解析 parse→语义 SemanticAnalyzer→Calcite CBO→物理 TaskCompiler→引擎无关 DAG"),
    ("Hive原理_支撑_执行引擎", "support", "⚙", "执行引擎",
     "可插拔框架 Tez/MR/LLAP · 算子树编译成 DAG · 向量化并行 · Driver 前台指挥"),
    ("Hive原理_支撑_事务一致性", "support", "⛨", "事务一致性（ACID）",
     "base+delta 读时合并 MVCC · TxnHandler 全局事务/锁 · 后台 Compaction 合并小文件"),

    ("Hive原理_支撑_存储与序列化", "base", "▦", "存储与序列化",
     "三层可插拔契约：SerDe 行⇄字节 · InputFormat/OutputFormat 切文件 · StorageHandler 接外部存储 · ORC 列存+分桶分区"),
    ("Hive原理_支撑_会话服务", "base", "◉", "会话服务（HiveServer2）",
     "Thrift 接入门面：CLIService 门面 · SessionManager 多会话隔离 · SQLOperation 异步 doAs · JDBC/7 认证/传输模式"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("support", "支撑主线 · 引擎内部（4 条能力域）"),
    ("base", "接入与存储底座 · 门面与字节（2 条能力域）"),
]

# 总架构图 data-tid（短标识）→ 主线文档名 的归并映射：
# 11 个语义热区收敛到 5 条主线,保证图上每个模块都能下钻。
TID2MID = {
    "hs2":         "Hive原理_支撑_会话服务",
    "storage":     "Hive原理_支撑_存储与序列化",
    "hms":         "Hive原理_支撑_元数据服务",
    "compile":     "Hive原理_支撑_编译优化",
    "ddl":         "Hive原理_支撑_编译优化",
    "dml":         "Hive原理_支撑_编译优化",
    "dql":         "Hive原理_支撑_编译优化",
    "dcl":         "Hive原理_支撑_编译优化",
    "exec":        "Hive原理_支撑_执行引擎",
    "transaction": "Hive原理_支撑_事务一致性",
    "compaction":  "Hive原理_支撑_事务一致性",
}
...
# 底部兜底 chip 主线列表（原含全景主线框架；应用户要求已从导航页最下方移除,故置空）。
# 全景=导航底图本身,文档仍保留,只是不再以底部 chip 形式出现。
ARCH_ALWAYS_CHIP = []

BRAND_TITLE = "一切知识皆索引"
BRAND_SUB = "Apache Hive"
HOME_DESC = ("Apache Hive 核心原理设计文档库的离线交互图谱——解耦式 SQL-on-Hadoop 数据仓库门面"
             "（SQL 编译优化层自管 × 存储/执行/元数据解耦可插拔，存算分离数仓的鼻祖形态）。"
             "7 条主线、6 张手绘原理图，全部回 apache/hive 源码核实。点击项目总架构图任意模块即可下钻到对应主线。")
ARCH_SVG_NAME = "Hive原理_总架构图.svg"
...
def build_archnav():
    ...
    hots = []
    for (x, y, w, h, tid, lab) in ARCH_HOTSPOTS:
        mid = TID2MID.get(tid, tid)
        if mid not in meta:
            print("  ⚠ 热区指向不存在的主线:", tid, "→", mid)
            continue
        _ico, title, _s = meta[mid]
        # 热区可视标签优先用 SVG 的 data-lab（更精准的模块名），下钻 mid 用主线文档名
        tag = lab or title
        hots.append(
            '<button class="arch-hot" data-mid="{mid}" aria-label="{title}"'
            ' style="left:{l:.3f}%;top:{t:.3f}%;width:{w:.3f}%;height:{ht:.3f}%">'
            '<span class="ah-tag">{ico} {tag}</span></button>'.format(
                mid=esc(mid), title=esc(title), ico=esc(_ico), tag=esc(tag),
                l=x / _ARCH_VBW * 100, t=y / _ARCH_VBH * 100,
                w=w / _ARCH_VBW * 100, ht=h / _ARCH_VBH * 100))
    covered = {TID2MID.get(t[4], t[4]) for t in ARCH_HOTSPOTS}
    # 全景主线框架 = 导航底图本身，不作为底部 chip 呈现（应用户要求移除）
    covered |= {PANO_NAME}
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="Apache Hive 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
# 官网跳转按钮图标：复用导航主页的项目官方彩色图标 design/icon.svg
_ICON_SVG = _b64_svg("icon.svg")
...
def build_html():
    ...
    # (全景 PANO_NAME = 导航底图本身,不需要热区/chip 入口,故豁免)
    covered = {TID2MID.get(t[4], t[4]) for t in ARCH_HOTSPOTS} | set(ARCH_ALWAYS_CHIP) | {PANO_NAME}
...
<header>
  ...
  <a href="https://github.com/apache/hive" target="_blank" rel="noopener" title="GitHub 源码仓库" style="display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M12 .5C5.7.5.5 5.7.5 12c0 5.1 3.3 9.4 7.9 10.9.6.1.8-.2.8-.6v-2c-3.2.7-3.9-1.4-3.9-1.4-.5-1.3-1.3-1.7-1.3-1.7-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1 1.8 2.7 1.3 3.4 1 .1-.8.4-1.3.7-1.6-2.6-.3-5.3-1.3-5.3-5.8 0-1.3.5-2.3 1.2-3.1-.1-.3-.5-1.5.1-3.1 0 0 1-.3 3.3 1.2a11.4 11.4 0 0 1 6 0C17.3 4.7 18.3 5 18.3 5c.6 1.6.2 2.8.1 3.1.8.8 1.2 1.8 1.2 3.1 0 4.5-2.7 5.5-5.3 5.8.4.4.8 1.1.8 2.2v3.3c0 .4.2.7.8.6 4.6-1.5 7.9-5.8 7.9-10.9C23.5 5.7 18.3.5 12 .5z"/></svg></a><a href="https://hive.apache.org" target="_blank" rel="noopener" title="项目官网" style="display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><img src="data:image/svg+xml;base64,{icon}" width="20" height="20" alt="官网" style="display:block"/></a><button id="themeBtn" title="切换深色 / 浅色主题" aria-label="切换主题" style="width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);background:var(--c-panel);color:var(--c-ink2);cursor:pointer;display:inline-grid;place-items:center;font-size:16px;flex:none"><span class="tt-ico tt-moon">☾</span><span class="tt-ico tt-sun">☀</span></button>
</header>
<div class="wrap">
  <div id="home">
    {archnav}
...
<script>{js}</script>
        ...
        sub=esc(BRAND_SUB), n=total_svg, icon=_ICON_SVG,
