...
`@tf.function` 装饰的函数是一个 `Function` 对象（`polymorphic_function.py:453`）。**首次**用某种输入签名调用时触发**追踪（tracing）**：路径是 `Function.__call__` → `trace_function`（`tracing_compilation.py:144`）→ `_maybe_define_function`（`tracing_compilation.py:188`，先查缓存、未命中才真追踪），真追踪跑一遍 Python，但 tf 算子只往图里**加节点、不真算**，最终 `func_graph_from_py_func`（`tracing_compilation.py:310`）产出一张 `FuncGraph`（可序列化为 GraphDef 的静态图），封装成 `ConcreteFunction` 并按输入签名（TypeSpec：dtype+shape）存入 `FunctionCache`（`tracing_compilation.py:87`）。**后续**同签名调用直接执行缓存的图、不再跑 Python，因此更快，且图可交给 Grappler/XLA 整体优化、导出成 SavedModel 部署。

一个 `Function` 因此是**多态**的：不同输入签名各对应一份 `ConcreteFunction`（一张具体图），`FunctionCache` 用 TypeSpec 做 key 索引它们——这也是"重追踪"的直接机理：key 没命中就新追踪一份。
...
输入的 dtype/shape 变化、或传入新的 Python 值，都会导致缓存未命中、**重新追踪**一份新图。过频重追踪会拖慢甚至撑爆内存，TF 内置 `_FrequentTracingDetector`（`polymorphic_function.py:171` `called_without_tracing` / 管理器 `:205`），在 `_maybe_define_function` 里每次调用都记到检测器（`polymorphic_function.py:846` 命中缓存记 `called_without_tracing`、`:849` 未命中记 `called_with_tracing`），过频重追踪时打印 retracing 告警。治理：用 `input_signature` 固定输入 spec；或 `reduce_retracing=True`（`polymorphic_function.py:468`）让 TF 泛化 shape；避免把频繁变化的 Python 标量当参数。
