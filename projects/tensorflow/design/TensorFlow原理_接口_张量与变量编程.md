...
`tf.Tensor`（`tensor.py:139` `class Tensor(internal.NativeObject, core_tf_types.Symbol)`）是一个**不可变的多维数组**：带 `shape`（`tensor.py:423`）、`dtype`（`tensor.py:414`）、`device` 三要素，是所有算子的输入/输出，产生后不能改。eager 下它是 `EagerTensor`，持有具体值（`.numpy` 可取）；图内则是**符号张量**（占位符，执行期才有值）。图接口/追踪期用 `TensorSpec`（`tensor.py:1069`）描述一个张量的 dtype+shape 契约而不含具体值——这正是 tf.function 判定签名是否命中缓存的依据。

`tf.Variable`（`resource_variable_ops.py:377` `BaseResourceVariable`，具体类 `:1707` `ResourceVariable`）不是张量，而是一个**有状态的资源句柄**：内部持有一个 resource handle 指向设备上的缓冲。**读它**才产出一个 Tensor 参与计算——`value`（`resource_variable_ops.py:653`）/ `read_value`（`:871`）底层发一个 `ReadVariableOp`（`_read_variable_op` `:815`）从设备缓冲读出当前值；它可被 `assign`（`:1063`）/ `assign_add`（`:1028`）/ `assign_sub`（`:1000`）**原地更新**、跨调用持久存在。这套"句柄 + 显式读/写 op"的设计使变量的读写在图里是**有序的副作用节点**（靠控制依赖定序），而非普通数据流值。模型权重、优化器动量、全局步数都是 Variable。

## 二、eager：一次算子调用立即执行

![张量与变量](TensorFlow原理_张量变量_01张量与变量.svg)

TF2 默认 eager。`tf.matmul(a, b)` 的旅程：Python 调用 → 顶层分派 `EagerExecute`（`eager/execute.cc:2261`）→ 本地路径 `EagerLocalExecute`（`tensorflow/core/common_runtime/eager/execute.cc:1734`）查/建该 op 在当前设备的 `KernelAndDevice`（`GetOrCreateKernelAndDevice` `execute.cc:1264`，按签名缓存免重复查表）→ 调 `OpKernel::Compute` 真正算 → 立即返回一个 `EagerTensor`。没有图、没有跨算子优化，每行 Python 立刻见结果——直观、可调试，代价是逐次付 Python 解释与 kernel 启动开销（热路径请用 tf.function 包起来，见「图与 tf.function」）。GPU 上 op 异步下发到 stream，`.numpy()` 等需要具体值时才同步。
...
变量的每次读写在底层都是一个显式 op，这保证了图模式下有状态更新的可定序：
