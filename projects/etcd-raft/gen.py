#!/usr/bin/env python3
"""raft-design 交互式核心原理图谱生成器（自包含 · 离线 · 双主题）。
...
_ap = argparse.ArgumentParser(description="生成 HashiCorp raft 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("RAFT_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("RAFT_DESIGN_OUT") or os.path.join(HERE, "index.html")

# ===================================================================== #
# 一、主线注册表 —— 唯一需随项目调整的数据块
#     家族 6 分布式协调/共识（共识状态机核 · Go · go.etcd.io/raft/v3）：接口 × 能力域 × 时机。
#     全景 + 1 接口主线（Node 驱动循环）+ 7 支撑能力域。
# ===================================================================== #
MAINLINES = [
    ("EtcdRaft原理_全景主线框架", "pano", "◇", "全景主线框架",
     "共识状态机核：判型 · 总架构 · 宿主驱动的 Ready I/O 环 · 与 hashicorp/raft 对照 · 三条贯穿声明"),

    ("EtcdRaft原理_接口_Node驱动循环与Ready", "iface", "⚙", "Node 驱动循环与 Ready",
     "Tick/Step/Ready/Advance 四动作 · Ready 待办清单（持久+发送+应用）· 库不碰 I/O 的定义性模式"),

    ("EtcdRaft原理_支撑_Raft状态机核心", "support", "◈", "Raft 状态机核心",
     "四态 Follower/Candidate/Leader/PreCandidate · Step 单入口分派 · becomeX · 确定性无 goroutine"),
    ("EtcdRaft原理_支撑_Leader选举", "support", "⚑", "Leader 选举",
     "tickElection → hup → campaign · PreVote 不加 term 探路 · poll/TallyVotes 多数派授权"),
    ("EtcdRaft原理_支撑_日志复制", "support", "⇉", "日志复制",
     "Propose → appendEntry → bcastAppend(MsgApp) · maybeCommit 多数派 match · reject 回退 next"),
    ("EtcdRaft原理_支撑_raftLog与unstable", "support", "▤", "raftLog 与 unstable",
     "unstable 暂存新条目 → Ready.Entries · stableTo 清出 · committed/applying/applied 三水位"),
    ("EtcdRaft原理_支撑_Storage与快照", "support", "⛁", "Storage 与快照",
     "Storage 接口宿主实现（库只读）· InitialState/Entries/Term/Snapshot · MemoryStorage · 快照追平"),
    ("EtcdRaft原理_支撑_ReadIndex线性化读", "support", "◐", "ReadIndex 线性化读",
     "MsgReadIndex 记录 commit · 心跳向多数派确认领导权 · applied≥readIndex 放行 · 不写日志"),
    ("EtcdRaft原理_支撑_成员变更与联合共识", "support", "◫", "成员变更与联合共识",
     "EnterJoint → C(new,old) 两多数派并存 → LeaveJoint 收敛 · Simple 单步 · 与 hashicorp 单步对照"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接口主线 · 宿主如何驱动（Node 循环 + Ready 待办）"),
    ("support", "支撑主线 · 共识内核（7 条能力域）"),
]
...
BRAND_TITLE = "etcd Raft 核心原理"
BRAND_SUB = "etcd Raft"
HOME_DESC = ("etcd Raft 核心原理设计文档库的离线交互图谱——家族 6 分布式协调/共识（共识状态机核 · Go · go.etcd.io/raft/v3 · 驱动 etcd/CockroachDB 等）。"
             "与 hashicorp/raft「电池全含的库」相对，本库是最小共识内核：无内置传输、无存储 I/O、无驱动复制的 goroutine——"
             "宿主用 Node 循环（Tick/Step/Ready/Advance）驱动，所有磁盘与网络 I/O 由宿主承担。"
             "9 条主线全部回本地源码核实。点击项目总架构图任意模块即可下钻到对应主线。")
ARCH_SVG_NAME = "EtcdRaft原理_全景_02总架构.svg"
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="HashiCorp raft 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
<header>
  ...
  <a href="https://github.com/etcd-io/raft" target="_blank" rel="noopener" title="GitHub 源码仓库" style="display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M12 .5C5.7.5.5 5.7.5 12c0 5.1 3.3 9.4 7.9 10.9.6.1.8-.2.8-.6v-2c-3.2.7-3.9-1.4-3.9-1.4-.5-1.3-1.3-1.7-1.3-1.7-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1 1.8 2.7 1.3 3.4 1 .1-.8.4-1.3.7-1.6-2.6-.3-5.3-1.3-5.3-5.8 0-1.3.5-2.3 1.2-3.1-.1-.3-.5-1.5.1-3.1 0 0 1-.3 3.3 1.2a11.4 11.4 0 0 1 6 0C17.3 4.7 18.3 5 18.3 5c.6 1.6.2 2.8.1 3.1.8.8 1.2 1.8 1.2 3.1 0 4.5-2.7 5.5-5.3 5.8.4.4.8 1.1.8 2.2v3.3c0 .4.2.7.8.6 4.6-1.5 7.9-5.8 7.9-10.9C23.5 5.7 18.3.5 12 .5z"/></svg></a><a href="https://raft.github.io" target="_blank" rel="noopener" title="项目官网" style="display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><img src="data:image/svg+xml;base64," width="18" height="18" alt="官网" style="display:block"/></a><button id="themeBtn" title="切换深色 / 浅色主题" aria-label="切换主题" style="width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);background:var(--c-panel);color:var(--c-ink2);cursor:pointer;display:inline-grid;place-items:center;font-size:16px;flex:none"><span class="tt-ico tt-moon">☾</span><span class="tt-ico tt-sun">☀</span></button>
</header>
<div class="wrap">
  <div id="home">
    {archnav}
