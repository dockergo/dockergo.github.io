...
![可靠性三件套](Kubernetes原理_支撑_Informer与Watch_02可靠性三件套.svg)

图示"重列对账 + 背压 + resync 兜底"三件套的源码锚点：**watch 断链续传**——`watch`（reflector.go:418）channel 关闭属正常，用最后 rv 重发 watch（不回退 LIST），仅 `isExpiredError`（reflector.go:475，410 Gone）才触发 `list`（reflector.go:502）全量重列；**重列对账**——`Replace`（delta_fifo.go:636）用新列表与本地缓存做差集，**为期间被删、watch 又没收到 Deleted 的对象补发 Deleted**，是"丢事件也最终一致"的关键；**listener 背压**——`processorListener.pop`（shared_informer.go:936）用可增长环形缓冲 `pendingNotifications` 吸收突发，`run`（:966）串行消费，handler 慢只在本 listener 内积压、**不阻塞 DeltaFIFO 也不阻塞其他 listener**（故 handler 必须快进快出、只入队 key）；**resync 兜底**——`minimumResyncPeriod=1s`（shared_informer.go:579）周期对全量缓存重发 `Sync`（只走本地缓存、不打 API Server），是 level-triggered 自愈"漏处理"的底层机制；**首次同步屏障**——`HasSynced`（shared_informer.go:909）在初始 LIST 全部分发完前为 false，控制器应 `WaitForCacheSync` 后再启 worker，避免"缓存没热就 reconcile"误删。
