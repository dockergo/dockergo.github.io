...
- **demuxer**(拆):`avformat_open_input`(`libavformat/demux.c:231`)打开文件 → 识别容器格式(FFInputFormat)→ `av_read_frame`(`libavformat/demux.c:1588`)逐个读出压缩 AVPacket(带 stream_index/pts/dts);内部 `read_frame_internal`(`demux.c:1388`)→ `ff_read_packet`(`demux.c:629`)。
- **muxer**(封):`avformat_write_header`(`libavformat/mux.c:467`)写容器头 → `av_interleaved_write_frame`(`libavformat/mux.c:1223`)交织写入 AVPacket → `av_write_trailer`(`libavformat/mux.c:1238`)写尾/索引。
...
- `ffmpeg -i in.mkv -c copy out.mp4`:demux in.mkv 出 H.264 包 → 直接 mux 进 mp4(不 decode/encode);CLI 侧 `-c copy` 由 `of_streamcopy`(`fftools/ffmpeg_mux.c:280`)驱动,不交织时直写 `av_write_frame`(`libavformat/mux.c:1176`)。
...
## 四、av_read_frame 与 parser 分包

![FFmpeg parser 分包](FFmpeg原理_容器_04分包.svg)

`ff_read_packet` 从 demuxer 读出的是**原始块**,块边界未必对齐帧(裸流常半帧/多帧粘连)。若流 `need_parsing` 且有 parser,`read_frame_internal`(`libavformat/demux.c:1388`)转 `parse_packet`(`libavformat/demux.c:1178`),内部 `av_parser_parse2`(`libavformat/demux.c:1220`)循环按码流语法切出完整帧、补 pts/dts 与关键帧标记后入 parse_queue;封装容器(mp4/mkv)已按帧存储则跳过。need_parsing 分 NONE/HEADERS/FULL/FULL_ONCE/FULL_RAW 几档,裸流走 FULL/FULL_RAW。

---

## 五、seek 定位

![FFmpeg seek 定位](FFmpeg原理_容器_05seek.svg)

`av_seek_frame`(`libavformat/seek.c:641`)/`avformat_seek_file`(`libavformat/seek.c:664`)按目标时间戳定位,策略**逐级回退**:格式专属 read_seek2/read_seek → `ff_seek_frame_binary` 二分(靠 read_timestamp)→ `seek_frame_generic` 查 index_entries 索引表,命中即返回。定位后 `ff_read_frame_flush`(`libavformat/seek.c:716`)清空 packet/parse 队列、更新 cur_dts、queue_attached_pictures。`AVSEEK_FLAG_BACKWARD` 回退到目标前最近**关键帧**再顺序解到目标(P/B 帧脱离参考帧无法独立解码),`AVSEEK_FLAG_ANY` 跳过此步但可能花屏。
...
| avformat_write_header | `libavformat/mux.c:467` | 写容器头(mux 起) |
| av_interleaved_write_frame | `libavformat/mux.c:1223` | 交织写入(mux) |
| av_write_trailer | `libavformat/mux.c:1238` | 写尾/索引(mux 收) |
