...
一次转码的管线:**输入文件** → `avformat_open_input`(libavformat 打开容器,`libavformat/demux.c:231`)→ `av_read_frame`(demux 出压缩 **AVPacket**,`libavformat/demux.c:1588`)→ `avcodec_send_packet`/`receive_frame`(libavcodec 解码成原始 **AVFrame**,`libavcodec/decode.c:730` / `libavcodec/avcodec.c:720`)→ 滤镜图(libavfilter:buffersrc→scale/crop/overlay→buffersink 处理帧,`libavfilter/avfilter.c:1068`)→ `avcodec_send_frame`/`receive_packet`(编码成 AVPacket,`libavcodec/encode.c:544`)→ `av_interleaved_write_frame`(mux 封进容器,`libavformat/mux.c:1223`)→ **输出文件**。AVPacket/AVFrame 靠 AVBufferRef 引用计数零拷贝流动(`libavutil/buffer.c:103` `av_buffer_ref`);swscale/swresample 转像素/采样格式;hwaccel 可 GPU 解码(`libavutil/hwcontext.c:263`)。
