...
- **打开**:`avformat_open_input`(`libavformat/demux.c:231`)+ `avformat_find_stream_info`(`libavformat/demux.c:2607`)→ 得 AVFormatContext + streams。
- **解码器**:`avcodec_find_decoder` + `avcodec_alloc_context3` + `avcodec_open2` → AVCodecContext。
- **循环**:`av_read_frame`(读包,`libavformat/demux.c:1588`)→ `avcodec_send_packet`(`libavcodec/decode.c:730`)/`avcodec_receive_frame`(`libavcodec/avcodec.c:720`)(解码)→ 处理 → `avcodec_send_frame`(`libavcodec/encode.c:544`)/`avcodec_receive_packet`(`libavcodec/encode.c:577`)(编码)→ `av_interleaved_write_frame`(写,`libavformat/mux.c:1223`)。
...
- 关系:CLI(入口 `fftools/ffmpeg.c:991` `main`、转码主循环 `fftools/ffmpeg.c:897` `transcode`)本身用 libav API 实现;选项解析在 `fftools/ffmpeg_opt.c:1424`(`ffmpeg_parse_options`),n8 起各组件跑在 `fftools/ffmpeg_sched.c:623`(`sch_alloc` 的 Scheduler,thread-per-component)。API 是底层、CLI 是封装。
...
| 接触面 | 入口(file:line) | 职责 |
|---|---|---|
| ffmpeg CLI | `fftools/ffmpeg.c:991`(`main`)、`:897`(`transcode`) | 命令行转码工具 |
| CLI 选项/调度 | `fftools/ffmpeg_opt.c:1424`、`fftools/ffmpeg_sched.c:623` | 参数解析 + 组件调度 |
| -c copy remux | `fftools/ffmpeg_mux.c:280`(`of_streamcopy`) | 免解码转封装 |
| avformat API | `avformat_open_input`(`libavformat/demux.c:231`) | 容器打开/读写 |
| avcodec API | `avcodec_send_packet`(`libavcodec/decode.c:730`) | 编解码 |
| avfilter API | `avfilter_graph_alloc`(`libavfilter/avfiltergraph.c:85`) | 滤镜图 |
