...
## 四、时间戳:PTS/DTS 与 time_base 换算

![FFmpeg 时间戳换算](FFmpeg原理_数据_04时间戳.svg)

时间戳是**整数 tick**,乘所属流的 time_base(AVRational {num,den},`libavutil/rational.h:61`)才是秒:`秒 = tick × num ÷ den`。有 B 帧时**解码序(dts)≠显示序(pts)**——B 帧双向参考前后帧,参考帧须先解出,故 P 帧提前到 B 帧前解码。跨流/跨容器时用 `av_rescale_q`(值×源tb÷目标tb,64 位中间量防溢出)换算,`av_packet_rescale_ts`(`libavcodec/packet.c:538`)一次换 pts/dts/duration,避免音画漂移。
