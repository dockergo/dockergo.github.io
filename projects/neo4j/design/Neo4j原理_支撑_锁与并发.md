...
`LockManager` 接口(`community/lock/.../locking/LockManager.java:50`,每事务一个 `LockManager.Client` `LockManager.java:64`):每事务一个 client 抓锁;共享锁可升级、排他锁可降级;锁可栈式重入(等量 acquire/release)。

默认实现 **Forseti**(`locking/forseti/ForsetiLockManager.java:107` `public class ForsetiLockManager implements LockManager`)用 **"dreadlocks" 死锁检测**:每 client(`locking/forseti/ForsetiClient.java:75`)维护"正在等谁"的列表,`acquireShared()`(`ForsetiClient.java:178`)/`acquireExclusive()`(`ForsetiClient.java:306`)/`tryExclusiveLock()`(`ForsetiClient.java:365`)抓锁;死锁检测 `detectDeadlock()`(`ForsetiClient.java:821`)是 O(1) 本地检查(不用遍历资源分配图);最优情况抢锁 = 一次 CAS(无竞争时几乎零开销)。死锁则抛 `DeadlockDetectedException`(`ForsetiClient.java:862` 抛出,类 `kernel/DeadlockDetectedException.java:32`)。
...
| 结构 | 定义 | 职责 |
|---|---|---|
| LockManager | `lock/.../locking/LockManager.java:50` | 锁接口(升级/降级/重入) |
| ForsetiLockManager | `locking/forseti/ForsetiLockManager.java:107` | 默认实现 + dreadlocks 检测 |
| ForsetiClient | `locking/forseti/ForsetiClient.java:75` | 每事务锁 client(抓锁/detectDeadlock) |
| ResourceType | `lock/.../lock/ResourceType.java:80` | 锁粒度(NODE/REL/LABEL/…)+ 锁序 |
| DeadlockDetectedException | `lock/.../kernel/DeadlockDetectedException.java:32` | 死锁抛出 |
