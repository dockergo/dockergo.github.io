...
## 二、VerifyLeader：线性读前的多数派确认

![VerifyLeader 线性读前哨](Raft原理_支撑_可靠性与脑裂防护_02verifyLeader.svg)

`VerifyLeader`（`api.go:931`）是**主动型**读安全前哨，与被动的时间型 Leader Lease 互补。`verifyLeader`（`raft.go:966`）先自投一票、算出 `quorumSize`，随后 `asyncNotifyCh` 触发所有 follower 立刻发一轮 AppendEntries；每个 ack 令 `v.votes++` 并回到 `verifyCh`（`:849`）汇总：`votes ≥ quorumSize` 则 `respond(nil)`，确认"此刻多数派仍承认我" → 宿主安全读本地状态；`votes < quorumSize`（已有新 Leader）则 `setState(Follower)` 退位并 `respond(ErrNotLeader)`。它用一轮真实 quorum 往返换来强证据，防被分区的旧 Leader 读到过期数据，代价是每读多一轮 RTT。
