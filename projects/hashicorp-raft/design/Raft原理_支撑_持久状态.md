...
## 二、三存储抽象：LogStore / StableStore / SnapshotStore 职责边界

![三存储抽象](Raft原理_支撑_持久状态_02三存储抽象.svg)

库只定义三个存储**接口**、自身零磁盘，具体实现由宿主注入：`StableStore`（`stable.go:8`）保任期/投票不丢 → 选举安全；`LogStore`（`log.go:112`）保有序日志不丢 → 复制安全；`SnapshotStore`（`snapshot.go:46`）让日志可截断 → 控制无限增长。共识核心只调接口方法（`setCurrentTerm`/`dispatchLogs`/`takeSnapshot` 等），不认实现——测试用内存版（`InmemStore`），生产用 `raft-boltdb` + `FileSnapshotStore`。重启恢复三者合力：先从快照 `FSM.Restore`，再重放快照点之后的日志，`StableStore` 恢复任期/投票。
