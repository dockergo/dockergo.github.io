...
![两段式与截断](Raft原理_支撑_快照与日志压缩_02两段式与截断.svg)

两段式（`fsm.go:32-43`）：`FSM.Snapshot()` 只捕获状态指针秒回、不阻塞 apply；重 IO 交给 `FSMSnapshot.Persist(sink)`，且**可与后续 `FSM.Apply` 并发**——故 FSM 必须支持并发读；无论 Persist 是否被调，`Release()` 一定被调。截断窗口（`compactLogsWithTrailing`, `snapshot.go:217`）：`maxLog = min(snapIdx, lastLogIdx − TrailingLogs)`，`DeleteRange(FirstIndex, maxLog)`——两条铁律：至少保留 `TrailingLogs`（默认 10240）条尾巴，让落后不多的 follower 走增量 AppendEntries 而非昂贵的 InstallSnapshot；且绝不删快照点之后的日志（`min(snapIdx,…)` 保证边界不越界）。
