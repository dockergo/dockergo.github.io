...
![vLLM 投机解码](vLLM原理_采样_04投机解码.svg)

decode 卡在显存带宽、算力有余,投机解码用这份余量提速:**drafter**(ngram / eagle / 小 draft 模型)先便宜地 propose `k` 个草稿 token,调度器把它们作为查询位追加进批(`scheduled_spec_decode_tokens`),**目标模型一次前向**同时算出 k+1 个位置的概率,再由 **RejectionSampler** 逐位比较 draft 概率与 target 概率:接受最长匹配前缀,遇到第一个不合的位置就从修正分布重采并丢弃其后草稿,全接受则追加一个 bonus token。关键性质:rejection sampling 使**最终输出分布严格等于直接用 target 逐个采样(无损)**——草稿只决定"猜多少",猜准则一步吐 1..k+1 个,猜差则退化为每步 1 个而不出错。
...
- `vllm/v1/sample/sampler.py:20` `class Sampler(nn.Module)` → `:72` `forward`（logits→token id）→ `:243` `sample`（核心采样逻辑）。
- 贪心：`:240` `greedy_sample` → `:241` `logits.argmax(dim=-1).view(-1)`（确定性取最高分）。
- 随机整形：`:228` `apply_temperature`（温度调分布陡峭度）。
- logits 处理器：`:282` 遍历 `logitsprocs.argmax_invariant`（不改 argmax 的）、`:399` 遍历 `logitsprocs.non_argmax_invariant`（改分布的：top_k/top_p/penalties）。
- logprobs：`:305` `compute_logprobs`、`:309` `gather_logprobs`（评测/推测解码用对数概率）。
- 上下游：`vllm/v1/engine/core.py:597` step 内 `model_executor.sample_tokens`（采样触发点）；`vllm/v1/core/sched/scheduler.py:1585` `update_from_output`（采样后回填判停）。
