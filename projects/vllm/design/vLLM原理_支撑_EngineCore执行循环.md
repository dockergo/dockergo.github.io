...
- `vllm/v1/engine/core.py:98` `class EngineCore`；`:125` `self.model_executor = executor_class(...)`、`:153` `self.scheduler = Scheduler(...)`（组合调度器+执行器）。
- `vllm/v1/engine/core.py:576` `def step` — 一步 = `:587` `scheduler.schedule` → `:588` `execute_model(non_block=True)` → `:595` `future.result()` → `:597` `sample_tokens` → `:601` `scheduler.update_from_output`。
- `:431` `add_request`（入队请求）；`:617` `step_with_batch_queue`（批队列重叠调度与执行）。
- 进程化：`:995` `class EngineCoreProc(EngineCore)` → `:1358` `run_busy_loop`（busy loop 反复 step）；`:1014` `self.input_queue` / `:1015` `self.output_queue`（与前端异步通信，避开 GIL）。
- `:1844` `class DPEngineCoreProc`（数据并行进程变体）；前端 `vllm/v1/engine/async_llm.py:70` `class AsyncLLM(EngineClient)`。
- `vllm/v1/core/sched/scheduler.py:421` `schedule`（step 第一步所调）。
