...
## 四、PD 分离:prefill 与 decode 拆实例

![vLLM PD 分离](vLLM原理_并行_04PD分离.svg)

TP/PP 是把**一个实例**切到多卡;PD 分离则是把**推理的两个阶段拆成两组实例**:prefill 阶段算力密集、decode 阶段带宽密集,混在一台会互相干扰(prefill 尖峰拖慢正在解码的请求)。分离后 prefill 实例专注一次算完整段 prompt 的 KV,经 **KV Connector**(`kv_transfer`,走 NIXL/mooncake/RDMA)把 KV 传给 decode 实例;decode 实例的请求以 `WAITING_FOR_REMOTE_KVS` 状态等 KV 到齐后再进 running,只做逐 token 解码。收益:两池各选最优并行配置、按流量独立扩缩、首 token 与逐 token 延迟互不干扰;代价是引入跨实例 KV 传输开销,需高速网络撑住。
...
- `vllm/distributed/parallel_state.py:1362` `_TP` / `:1365` `get_tp_group`；`:1378` `_PP` / `:1381` `get_pp_group`（通信组全局句柄）。
- `vllm/distributed/parallel_state.py:1718` `initialize_model_parallel`（按 tp_size×pp_size 划分 GPU）→ `:1810` `_TP = init_model_parallel_group(...)`、`:1869` `_PP = init_model_parallel_group(...)`。
- 张量并行层：`vllm/model_executor/layers/linear.py:418` `class ColumnParallelLinear` → `:590` `forward` → `:601` `tensor_model_parallel_all_gather(output_parallel)`（切列+聚合）。
- `vllm/model_executor/layers/linear.py:1612` `class RowParallelLinear` → `:1747` `forward` → `:1766` `tensor_model_parallel_all_reduce(output_parallel)`（切行+归约）；Column 配 Row 成对，中间免通信。
- 通信原语：`vllm/distributed/communication_op.py:12` `tensor_model_parallel_all_reduce`、`:17` `tensor_model_parallel_all_gather`。
