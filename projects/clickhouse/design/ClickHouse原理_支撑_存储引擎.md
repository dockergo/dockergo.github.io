...
图注：逻辑到物理层次链 `Database → Table(ENGINE=MergeTree) → Partition → Part → 列 Stream(.bin/.mrk) → Granule/Mark → 压缩块`。与 Doris 的根本差异——**无 MemTable/WAL**，`INSERT` 直落一个**不可变 Part 目录**（名如 `202401_1_1_0` = `partitionID_minBlock_maxBlock_level`）。**Partition 是逻辑边界，Part 才是调度/merge/复制的最小物理单位**。
...
图注：全族由**一个枚举** `MergingParams::Mode` 区分（非类继承），每种模式对应一个 `*SortedAlgorithm`，**只在后台 merge 时被调用**。故最反直觉的一点——**变体语义不是实时的**：写入直后普通 `SELECT` 会看到未合并的重复/待聚合行，要即时正确须付 `FINAL`（读时现场归并）或 `GROUP BY`/`argMax` 的代价。
...
图注：同一引擎的 Part 有两种物理布局，由**写入时体积/行数**自动选择（`choosePartFormat`）：
...
阈值默认：`min_bytes_for_wide_part = 10MB`、`min_rows_for_wide_part = 0`。小批量 INSERT 产生的小 Part 走 Compact 避免海量小文件；merge 变大后自然转 Wide。二者读写接口统一在 `IMergeTreeDataPart`。
...
图注：ClickHouse 主键是**稀疏的**——`primary.idx` **每 granule（默认 8192 行）才存一条**主键值而非每行，故索引极小、常驻内存。

**与 Doris/OLTP 最大的认知鸿沟：主键 = 排序 + 粗粒度跳数，不去重、不是唯一约束。** 查询裁剪两步：① `KeyCondition` 把 WHERE 编译成主键区间判定；② `markRangesFromPKRange` 在稀疏索引上二分/排除得到需读的 **MarkRange** 集合。**Mark** 是 granule → 物理位置 `{压缩文件偏移, 解压块内偏移}` 的映射；granule 大小可自适应（大行表按字节封顶）。
...
图注：主键只对**排序前缀**有效；非排序列的过滤靠可选的**跳数索引**（`INDEX ... TYPE ... GRANULARITY N`），以"每 N 个 data granule"为单位存摘要，读时先判"这段能否整体跳过"。工厂注册在 `MergeTreeIndices`：
...
**GRANULARITY N** 指该索引一个条目覆盖 N 个数据 granule——越大越省空间但越粗。跳数索引是"可跳过"提示，判"可能命中"仍要读原始数据，不像主键那样定位精确区间。
...
图注：列数据落盘前经 **编码 → 压缩** 两段处理，注册在 `CompressionCodecFactory`。默认通用压缩器是 **LZ4**，可 `CODEC(...)` 按列指定：
...
编码类（Delta/DoubleDelta/Gorilla/T64/GCD）通常与压缩类（LZ4/ZSTD）用 `CODEC(Delta, ZSTD)` **链式组合**：先编码把数据变规律，再压缩。压缩块也是 IO/缓存单位（`UncompressedCache`）。
...
图注：一次 MergeTree 读取链路——**主键裁剪 → MarkRange → ReadPool 分派 → 逐 granule 解压 → PREWHERE 分步过滤 → 输出 Block**。

- **裁剪**：主键裁剪（§四）+ 跳数索引（§五）先把要读的 MarkRange 缩到最小。
- **分派**：`MergeTreeReadPoolBase` 及其子类（普通 / InOrder / parallel-replicas）把 MarkRange 分给多个执行线程，`MergeTreeSelectProcessor` 驱动。
- **PREWHERE**：ClickHouse 特有的"先读过滤列、再读投影列"优化。`optimize_move_to_prewhere` 默认开启，自动把高选择性 WHERE 条件下移为多步 PREWHERE：先只读判断列过滤掉不匹配行，**再按存活行读其余列**，省下大量列 IO。
...
图注：`writeTempPartImpl` 把内存 `Block` 变成磁盘 Part——**① 按 `PARTITION BY` 切块 → ② 按主键 `ORDER BY` 稳定排序 → ③ 经 `MergedBlockOutputStream` 流式写列 + 建索引/marks → ④ `finalizePartAsync` 落定临时 Part 再原子 rename 可见**。整条路径**无 memtable、无 WAL**，即"直落不可变 Part"。

**插入去重**：非复制表用 `MergeTreeDeduplicationLog` 按插入块 hash 去重，窗口 `non_replicated_deduplication_window` 默认 **0**（关闭）；复制表的块级幂等去重放在 Keeper（见「复制与一致性」）。
...
图注：后台 merge 是"不可变 Part"模型的代价与解药——INSERT 越碎 Part 越多、查询归并路数越多越慢，merge 把它降下来。

- **选取**：`MergeSelectorFactory` 注册 Simple/Stochastic/Trivial/All 策略；默认 `SimpleMergeSelector` 在"Part 大小相近、总量适中"时择优，由 `selectPartsToMerge` 落地。
- **执行**：`MergeTask` 合并多 Part 为一个；`chooseMergeAlgorithm` 在**水平**（整行）与**垂直**（按列分组，大宽表内存友好）间选择，`enable_vertical_merge_algorithm` 默认 **1**。变体引擎的 `*SortedAlgorithm` 在此兑现合并语义。
- **替换**：新 Part 变 active，旧 Part 转 outdated 待 GC。**全程无原地修改**——"变更"永远是"产新替旧"。
...
图注：`ALTER TABLE ... UPDATE/DELETE` 是 **Mutation**——`MutateTask` **异步整 Part 重写**（读旧 Part、应用变更、写全新 Part，绝非原地行级更新），`MutationsInterpreter` 把变更编译成读取管线。因重写整 Part，Mutation 是**重操作**、不适合高频。

**轻量 DELETE**（`DELETE FROM`）更快：改写成给隐藏掩码列 `_row_exists` 置 0，读时跳过 `_row_exists=0` 的行，物理清除延后到后续 merge；`lightweight_deletes_sync` 默认 **2**（等所有副本）。

**PatchParts** 是更新的"读期叠加补丁"机制：把变更写成轻量补丁 Part，查询时 `applyPatches` 现场叠加，避免整 Part 重写——由 `apply_mutations_on_fly`（默认 false）等开关控制。
