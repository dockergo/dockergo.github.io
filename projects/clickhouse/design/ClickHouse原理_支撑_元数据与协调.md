...
图注：`KeeperServer` 是一个 **Raft 协调器**——包装 NuRaft 的 `raft_server`，提供 ZooKeeper 兼容 API，但 C++ 原生实现、无需独立 JVM。`KeeperStateMachine` 把已提交的 Raft 日志应用到 `KeeperStorage`（内存 znode 树），`KeeperDispatcher` 路由请求。**关键认知：Keeper 只存"协调元数据"（znode），不存表数据**——它是集群"神经中枢"，不是存储节点。
...
图注：`DatabaseOnDisk` 把表定义持久化为 `.sql`；`DatabaseAtomic`（默认）在其上加 UUID + 符号链接实现原子 CREATE/DROP/RENAME；`DatabaseReplicated`（继承 Atomic）再加一层——把库内所有 DDL 写进 Keeper `/log`，各节点经 `DatabaseReplicatedDDLWorker` 自动同步。（详见「DDL · Database 引擎」篇。）
...
图注：`... ON CLUSTER` 的 DDL 经 `DDLWorker` 落地——发起端把带目标 hosts 的 entry 写进 Keeper 队列 `/clickhouse/task_queue/ddl`，各节点主循环拉取、判断是否目标、执行并回写 finished 状态。（完整链路见「DDL · 分布式 DDL」篇。）这与 Replicated 库的库级 DDL 复制是两条"DDL 播全集群"的路径。
...
图注：`system.*` 是 ClickHouse 的"元数据可观测层"，基类 `IStorageSystemOneBlock` 把内部状态暴露成可 SQL 查询的表：
