...
三种所有权模型:**`Box<T>`** 堆分配单一所有权(递归类型/大对象,唯一 owner 出作用域 Drop 释放堆内存);**`Rc<T>`** 单线程共享所有权,计数是 `Cell`(非原子),显式 `!Send/!Sync` 只能单线程;**`Arc<T>`** 跨线程共享,计数是 `Atomic`(原子),`Send/Sync where T: Sync+Send`,比 Rc 慢但线程安全。**为什么分 Rc/Arc**:共享所有权要引用计数——单线程用 Rc(非原子快)、跨线程用 Arc(原子慢但安全),编译器用 Send/Sync 强制(Rc 跨线程即编译错)。锚点见深化表。
...
**内部可变(interior mutability)**:通过 `&T`(不可变引用)也能改内部,绕过"&mut 唯一"的编译期规则。根基是 **`UnsafeCell<T>`**(`#[lang="unsafe_cell"]`,`impl !Sync`)——编译器认可的"可通过 &T 改内容"的**唯一原语**,所有内部可变类型都建在它上。**`Cell<T>`** 值进出(get/set,无引用借用,适合 Copy 小值);**`RefCell<T>`** 运行时借用检查(borrow/borrow_mut 运行时记状态,违反则 panic)——把编译期借用检查挪到运行时。**为什么 UnsafeCell 是根基**:正常 `&T` 不能改,UnsafeCell 是唯一开的口子,其 `!Sync` 是这些类型 !Sync 的根源。
...
实际数据结构常组合:**`Rc<RefCell<T>>`**(单线程共享可变)——Rc 多所有者 + RefCell 运行时借用检查的可变,如图/链表节点被多处引用且需改;**`Arc<Mutex<T>>`**(跨线程共享可变)——Arc 跨线程共享 + Mutex 运行时互斥(见并发篇),多线程共享状态的标准。**代价**:Rc/Arc 计数开销、RefCell 运行时借用检查(违反 panic)、Mutex 锁开销——比编译期检查有运行时成本,但换灵活。**为什么组合**:单指针解一维(Box 堆/Rc 共享/RefCell 可变),组合解"既共享又可变"——是"多所有者 + 可变"的标准配方。

---

## 四、引用计数生命周期与 Weak 破环

![Rust Rc 引用计数](Rust原理_指针_04refcount.svg)

Rc/Arc 共享靠计数:堆上一块 `RcInner{strong, weak, value}`,`.clone()` O(1) strong+1(不深拷 T)、`drop` strong−1,**strong=0 先析构 value T、strong 与 weak 都为 0 才回收整块内存**。风险:两 Rc 互相强引用成环,外部 drop 后彼此 strong 停在 1、永不归零 → T 永不析构 = **内存泄漏**(借用检查器不查——泄漏非内存不安全)。破环用 **`Weak`**:反向/观察边改用 Weak(只加 weak 不加 strong、不保活 T),让 strong 能归零;`upgrade()` 拿 `Option<Rc>`(已释放返回 None)。规律:所有权正向用 Rc/Arc,反向边用 Weak。
...
| cell 模块 | `library/core/src/cell.rs:1` | Shareable mutable containers |
| UnsafeCell<T> | `library/core/src/cell.rs:2323` | 内部可变根基(!Sync) |
| Cell<T> | `library/core/src/cell.rs:312` | 值进出(Copy 小值) |
| RefCell<T> | `library/core/src/cell.rs:849` | 运行时借用检查(违反 panic) |
