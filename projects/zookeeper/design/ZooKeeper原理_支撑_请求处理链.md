...
![写请求流水线](ZooKeeper原理_支撑_请求处理链_02写请求流水线.svg)

图给出一次写在 Leader 链上的流水线：主链 propose 广播与内嵌 `Sync→Ack`（Leader 自己落盘计票）并行，两路的 Ack 都汇到 `tryToCommit`，达成过半且 zxid 按序后 `CommitProcessor` 放行、`Final` apply。各处理器分工与锚点：

- **PrepRequestProcessor**：校验路径/版本、`checkACL`（create→CREATE `:664`、setData→WRITE `:383`）、把写转成幂等**事务**（记 ChangeRecord 供后续与回滚）。
- **ProposalRequestProcessor**（仅 Leader，`:68`）：转 `nextProcessor`（=CommitProcessor 等提交 `:80`）；写则 `Leader.propose`（`:85`）广播 + 内嵌 `syncProcessor` 落盘计票（`:89`）。
- **SyncRequestProcessor**：批量写 TxnLog（`flush`→`ZKDatabase.commit` `:235`）+ 周期快照（`shouldSnapshot` `:143`）——详见 [[事务日志与快照]]。
- **CommitProcessor**（`:77`）：双队列 `queuedRequests`（本地已排 `:93`）↔ `committedRequests`（ZAB 送回 COMMIT `:114`）对齐，是"本地顺序 ↔ 全局提交顺序"的汇合点。
- **FinalRequestProcessor**：链尾 `processTxn` apply 到 DataTree、组织响应、触发 watch、直接处理读（`getData/getChildren/addWatch` `:437`）。
