...
![三种 watch 模式](ZooKeeper原理_支撑_Watch机制_02三种模式.svg)

- **STANDARD（默认）**：一次性，触发即从 watchTable 移除——从触发到重挂之间的变更会漏（gap）。动机是避免服务端为海量长期订阅保持状态。
- **PERSISTENT（3.6+）**：持久，触发后仍有效、连续接收后续变更，无需重挂（无 gap）。
- **PERSISTENT_RECURSIVE（3.6+）**：持久 + 递归，一处挂载监听整棵子树的任意变更（`addWatch` 注册，`WatcherMode`）。

`WatchManagerOptimized`（`WatchManagerOptimized.java:57`）用 bitset 表示 watcher 集，海量 watch 时显著省内存；`WatcherCleaner` 后台清理死连接遗留的 watch。
...
| 论断 | 锚点 |
|---|---|
| WatchManager 实现 IWatchManager | `server/watch/WatchManager.java:46` |
| watchTable：path → 该路径的 watcher 集（触发时按 path 找） | `server/watch/WatchManager.java:50` |
| watch2Paths：watcher → 它监听的 path 集（断连按此清理） | `server/watch/WatchManager.java:52` |
| triggerWatch 精简重载（转发到全参版本） | `server/watch/WatchManager.java:135` |
| triggerWatch 主体：取出并**移除**该 path 的 watchers、逐个推送恰好一个 WatchedEvent | `server/watch/WatchManager.java:140` |
| 触发时机：写事务 apply 到内存树后由 DataTree 回调 | `server/DataTree.java:857`（processTxn）|
| 事件不带数据，客户端收到后自行重读拿新值 | `server/DataTree.java:691`（getData 重读入口）|
| 会话过期时清理该会话所有 watch | `server/SessionTrackerImpl.java:45` |
| addWatch：读操作时把 (path,watcher) 注册进两张表 | `server/watch/WatchManager.java:70` |
| WatchManagerOptimized：位图压缩的高效实现（addWatch/removeWatcher） | `server/watch/WatchManagerOptimized.java:57`、`server/watch/WatchManagerOptimized.java:152` |
