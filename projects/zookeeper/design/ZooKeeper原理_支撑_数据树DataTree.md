...
![读写分道](ZooKeeper原理_支撑_数据树DataTree_02读写分道.svg)

读（`getData` `:691`、`getChildren` `:724`）直接 `nodes.get(path)` 返回——不加写锁、不查磁盘，是读吞吐可随节点扩展的物理基础。写不直接改树，而经 ZAB Commit 后由 `processTxn`（`:857`）**按 zxid 顺序、幂等地** apply（`createNode` `:410`/`:433`、`setData` `:629` version 自增、`deleteNode` `:534`）。幂等 + 全序 ⇒ 任何副本重放同一段日志得到同一棵树——这是快照/日志恢复正确的前提。代价：读可能读到略旧值，要最新用 `sync` + read。
...
| 论断 | 锚点 |
|---|---|
| DataTree 类主体 | `server/DataTree.java:95` |
| nodes 用 NodeHashMapImpl 建（path→DataNode 并发映射） | `server/DataTree.java:285` |
| root DataNode（以 `""` 为 key） | `server/DataTree.java:266` |
| ephemerals：sessionId→临时节点路径集，供会话回收 | `server/DataTree.java:154` |
| containers / ttls 索引（容器与 TTL 节点） | `server/DataTree.java:159`、`server/DataTree.java:164` |
| createNode（两个重载，写事务 apply 入口） | `server/DataTree.java:410`、`server/DataTree.java:433` |
| deleteNode | `server/DataTree.java:534` |
| setData（改值 + bump version） | `server/DataTree.java:629` |
| getData / getChildren（读直查内存，无磁盘 IO） | `server/DataTree.java:691`、`server/DataTree.java:724` |
| processTxn：按 zxid 顺序幂等 apply 事务 | `server/DataTree.java:857` |
| DataNode 载体：data / acl（Long 引用共享 ACL 表）/ stat / children | `server/DataNode.java:40`、`:55`、`:60`、`:67` |
| ZKDatabase 内存库门面、getDataTree 暴露树 | `server/ZKDatabase.java:73`、`server/ZKDatabase.java:198` |
