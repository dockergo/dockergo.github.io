...
| 论断 | 锚点 |
|---|---|
| FileTxnLog.append：事务先写 TxnLog 缓冲（WAL 先行） | `server/persistence/FileTxnLog.java:276` |
| FileTxnLog.commit：批量 flush + fsync（channel.force） | `server/persistence/FileTxnLog.java:394` |
| forceSync 默认 yes（每次 commit 真 fsync 才回 Ack） | `server/persistence/FileTxnLog.java:155` |
| 日志文件头 magic "ZKLG" | `server/persistence/FileTxnLog.java:66` |
| FileSnap.serialize：写模糊快照（DataTree + sessions） | `server/persistence/FileSnap.java:242` |
| FileSnap.deserialize：重启时从快照恢复内存树 | `server/persistence/FileSnap.java:73` |
| 快照文件 magic "ZKSN" | `server/persistence/FileSnap.java:53` |
| 落盘由 SyncRequestProcessor 驱动（run 主循环攒批） | `server/SyncRequestProcessor.java:52`、`server/SyncRequestProcessor.java:156` |
| 恢复：快照 + 重放 snapshot zxid 之后的 TxnLog 尾部（幂等 processTxn） | `server/DataTree.java:857` |
| ZKDatabase 承接内存库的 load/commit | `server/ZKDatabase.java:73` |
