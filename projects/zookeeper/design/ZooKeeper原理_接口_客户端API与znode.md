...
![顺序保证](ZooKeeper原理_接口_04顺序保证.svg)

ZooKeeper 对单客户端提供三条强顺序保证（图）：**① 写全局有序**（所有写按 zxid 线性化，所有客户端看到相同顺序）；**② 客户端 FIFO**（同一 client 的请求按发送序执行）；**③ watch 与数据视图有序**（先收 watch 事件、再看到其后的读结果）——故"感知变化 → 重读拿新值"安全。编程上有同步 API（`ZooKeeper` 类）与异步回调；生产普遍用 **Apache Curator** 封装重连、`ephemeral_sequential` 锁、leader 选举、路径缓存等 recipe，避免自己处理 watch 重挂与会话恢复的边界。
...
| 论断 | 锚点 |
|---|---|
| 请求统一入口 submitRequest | `server/ZooKeeperServer.java:1208` |
| 写请求先经 Prep 转事务（pRequest 分类型构造 Txn + checkACL） | `server/PrepRequestProcessor.java:91`、`server/PrepRequestProcessor.java:758` |
| 写请求经 Leader.propose 走 ZAB 全序 | `server/quorum/Leader.java:1295` |
| 事务最终 apply 到内存树 processTxn | `server/DataTree.java:857` |
| create（临时节点绑 ephemeralOwner=sessionId） | `server/DataTree.java:410`、`server/DataTree.java:433` |
| delete / setData | `server/DataTree.java:534`、`server/DataTree.java:629` |
| 读直查内存 getData / getChildren（不经共识，可读到旧值） | `server/DataTree.java:691`、`server/DataTree.java:724` |
| watch 一次性：读时注册、触发即移除 | `server/watch/WatchManager.java:140` |
| 5 型 znode 载体 DataNode（data/acl/stat/children） | `server/DataNode.java:40` |
| FinalRequestProcessor 统一收尾、装配响应 | `server/FinalRequestProcessor.java:147` |
| 权限 perm 常量族（READ..ALL） | `ZooDefs.java:109` |
