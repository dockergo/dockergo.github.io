...
| 论断 | 锚点 |
|---|---|
| Leader.lead：进入 broadcast、维护 follower 复制 | `server/quorum/Leader.java:632` |
| propose：分配 zxid、广播 PROPOSAL | `server/quorum/Leader.java:1295` |
| processAck：累计 follower Ack | `server/quorum/Leader.java:1054` |
| tryToCommit：过半 Ack 且按 zxid 顺序才 COMMIT | `server/quorum/Leader.java:970` |
| Follower.followLeader：连主→同步(DIFF/SNAP)→广播 | `server/quorum/Follower.java:71` |
| Follower.processPacket：收 PROPOSAL 落盘回 Ack、收 COMMIT 提交 | `server/quorum/Follower.java:156` |
| LearnerHandler：Leader 侧每 follower 一线程 | `server/quorum/LearnerHandler.java:66` |
| 选举 FastLeaderElection.lookForLeader | `server/quorum/FastLeaderElection.java:907` |
| 选票全序 totalOrderPredicate（epoch→zxid→sid） | `server/quorum/FastLeaderElection.java:717` |
| zxid = (epoch<<32) \| counter：makeZxid / getEpochFromZxid / getCounterFromZxid | `server/util/ZxidUtils.java:29`、`:23`、`:26` |
| 过半 QuorumMaj.containsQuorum | `server/quorum/flexible/QuorumMaj.java:140` |
| Follower 落盘经 SyncRequestProcessor、apply 经 processTxn | `server/SyncRequestProcessor.java:156`、`server/DataTree.java:857` |
