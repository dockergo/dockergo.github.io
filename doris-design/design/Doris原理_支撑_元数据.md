...
复制组用 **Paxos 家族协议（BDB JE HA）**：Master 宕机后 Follower 基于已提交位点选出新 Master、对客户端透明，已提交变更不丢。
