...
写入把数据切成一批不可变 Rowset，读取则要把它们**现场还原成一份逻辑视图**：先按查询版本**选中覆盖该版本的 Rowset 集合**，在每个 Segment 上走索引裁剪跳掉无关行，再**跨 Rowset/Segment 按 key 有序堆归并**，最后**按数据模型收口**同一个 key 的多版本——这一步才是三种模型在读侧的真正区别：明细直接输出、聚合读时聚合、主键 MoR 现场取最新（读慢）、主键 MoW 用 Delete Bitmap 预先屏蔽旧行免去读时去重（读快）。这也解释了"为什么小 Rowset 多会拖慢查询"：归并路数越多越慢，Compaction 就是把它降下来。
