#!/usr/bin/env python3
"""gorm-design 交互式核心原理图谱生成器（自包含 · 离线 · 双主题）。
...
_ap = argparse.ArgumentParser(description="生成 GORM 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("GORM_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("GORM_DESIGN_OUT") or os.path.join(HERE, "index.html")

# ===================================================================== #
# 一、主线注册表 —— 唯一需随项目调整的数据块
#     新家族（ORM / 数据映射库 · Go · github.com/go-gorm/gorm）：元模式 = 接触面 × 能力域 × 时机。
#     全景 + 2 接触面 + 8 能力域。
# ===================================================================== #
MAINLINES = [
    ("Gorm原理_全景主线框架", "pano", "◇", "全景主线框架",
     "新家族 ORM/数据映射库：双维模型 · 总架构 · callbacks 灵魂 · 依赖矩阵 · 三条贯穿声明"),

    ("Gorm原理_接触面_链式查询API", "iface", "⛓", "链式查询 API",
     "chainable（Where/Select/Joins/Preload）累积 + finisher（Create/First/Find/Update/Delete）触发"),
    ("Gorm原理_接触面_模型定义", "iface", "▤", "模型定义",
     "Go struct + gorm:\"...\" tag · 约定优于配置 · db.Model() / AutoMigrate() 声明表结构"),

    ("Gorm原理_支撑_Statement与链式构建", "support", "▦", "Statement 与链式构建",
     "链式状态容器 + SQL 拼装器 · AddClause 累积 · Build 拼方言 SQL · 克隆隔离"),
    ("Gorm原理_支撑_schema反射", "support", "◱", "schema 反射",
     "struct → Schema（字段/主键/命名/关联/序列化器）· 按类型缓存 sync.Map · 只反射一次"),
    ("Gorm原理_支撑_clause_SQL构建", "support", "◈", "clause SQL 构建",
     "方言无关 SQL AST · Where/Select/Insert/Values/Set/OnConflict · AddVar 占位防注入"),
    ("Gorm原理_支撑_callbacks回调链", "support", "⚙", "callbacks 回调链",
     "灵魂：六 processor 各持有序可插拔回调链 · 主逻辑与 hook 平级 · 插件靠插回调织入"),
    ("Gorm原理_支撑_关联与Preload", "support", "⇄", "关联与 Preload",
     "has one/many · belongs to · many2many · Preload 发 IN 查询归位消 N+1 · Association API"),
    ("Gorm原理_支撑_事务与会话", "support", "◐", "事务与会话",
     "默认单条事务 · Transaction 闭包 + SavePoint · Session 派生 · PrepareStmt 预编译缓存"),
    ("Gorm原理_支撑_Migrator自动迁移", "support", "▧", "Migrator 自动迁移",
     "AutoMigrate 建表/补列/加索引 · 只增不减幂等收敛 · 方言无关骨架 + Dialector 覆盖"),
    ("Gorm原理_支撑_方言连接池与日志", "support", "◫", "方言连接池与日志",
     "Dialector 屏蔽方言（占位符/引号/类型）· ConnPool 统一执行面 · Logger.Trace 标慢查询"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接触面主线 · 应用如何用（链式查询 API / 模型定义）"),
    ("support", "支撑主线 · 库内部机制（8 条能力域）"),
]
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="GORM 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
(function(){
  ...
  var saved=localStorage.getItem('gorm-atlas-theme');
  ...
  function toggleTheme(){
    ...
    localStorage.setItem('gorm-atlas-theme',cur);
