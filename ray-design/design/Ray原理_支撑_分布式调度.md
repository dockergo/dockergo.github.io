...
2. **LocalLeaseManager（本地授租）**：`ScheduleAndGrantLeases`（`local_lease_manager.cc:126`）在本节点等待 task 参数依赖 Pull 就绪（`WaitForTaskArgsRequests`），从 WorkerPool 取 worker，把 lease 授予 CoreWorker。

**去中心化的本质**：Raylet 只决定"在哪个 worker 上跑"，跑起来后 task 由 CoreWorker **直投**该 worker，调度器不再参与数据流。
