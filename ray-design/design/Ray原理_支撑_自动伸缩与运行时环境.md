...
autoscaler（`StandardAutoscaler`，`autoscaler.py:172`）跑一个周期性 `update`（`:367`）→ `_update`（`:379`）控制环：

1. **收集需求**：GCS 聚合集群资源状态，`GcsAutoscalerStateManager::HandleGetClusterResourceState`（`gcs_autoscaler_state_manager.cc:51`）把"待满足的资源需求 + 现有供给 + gang 请求"暴露给 autoscaler。
2. **算要开多少**：`ResourceDemandScheduler.get_nodes_to_launch`（`resource_demand_scheduler.py:167`）用 bin-packing 把 pending 需求装到候选节点类型上，算出各类型要新增几台；`_get_concurrent_resource_demand_to_launch`（`:388`）限制并发启动速率避免过冲。
3. **扩容**：`launch_required_nodes`（`autoscaler.py:736`）通过 node provider（云 API/K8s/本地）**拉起新节点**。
4. **缩容**：`terminate_nodes_to_enforce_config_constraints`（`:475`）按 min/max/idle 超时回收多余节点，`terminate_unhealthy_nodes`（`:1239`）杀失活节点。
5. **回报**：autoscaler 把决策经 `HandleReportAutoscalingState`（`gcs_autoscaler_state_manager.cc:67`）写回 GCS，形成闭环。
