...
> **定位**：应用与 Ray 集群的"起点与容器"。`ray.init` 连上（或拉起）集群，确立 **driver / worker 进程模型**，用 **runtime env** 隔离依赖、用 **namespace** 隔离命名。这条主线管"程序在哪跑、依赖怎么带、名字怎么分隔"，是另外两条接口主线的运行容器。核实基准 `src/ray/core_worker/`、`python/ray/`（commit 6ff3a75）。
