#!/usr/bin/env python3
"""ray-design 交互式核心原理图谱生成器（自包含 · 离线 · 双主题）。
...
_ap = argparse.ArgumentParser(description="生成 ray 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("RAY_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("RAY_DESIGN_OUT") or os.path.join(HERE, "index.html")

# ===================================================================== #
# 一、主线注册表 —— 唯一需随项目调整的数据块
#     新家族（传输协议库 · QUIC/HTTP3 · Google RAY C++）：元模式 = 接口 × 能力域 × 时机。
#     全景 + 3 接口主线 + 8 支撑能力域。
# ===================================================================== #
MAINLINES = [
    ("Ray原理_全景主线框架", "pano", "◇", "全景主线框架",
     "通用分布式计算框架：双维模型 · 总架构 · ObjectRef 贯穿层 · 依赖矩阵 · 三条贯穿声明"),

    ("Ray原理_接口_远程任务与对象", "iface", "⚡", "远程任务与对象",
     "@ray.remote 函数 · .remote() 异步提交 · ObjectRef 分布式 future · ray.get/put"),
    ("Ray原理_接口_Actor编程模型", "iface", "◉", "Actor 编程模型",
     "@ray.remote 类 = 有状态远程对象 · handle · 方法按序串行 · 生命周期独立"),
    ("Ray原理_接口_集群与运行时", "iface", "⚙", "集群与运行时",
     "ray.init 拉起集群 · driver/worker 进程模型 · runtime env 隔离 · namespace/job"),

    ("Ray原理_支撑_远程任务提交与依赖", "support", "▤", "远程 task 提交与依赖",
     "灵魂：去中心化 worker-lease 调度 · scheduling key 排队 · spillback · 直投 worker"),
    ("Ray原理_支撑_Actor生命周期与调度", "support", "◫", "Actor 生命周期与调度",
     "GCS 集中调度定居 worker · 状态机 · max_restarts 重启 · lineage 重建 · 有序投递"),
    ("Ray原理_支撑_分布式对象存储", "support", "▦", "分布式对象存储",
     "灵魂：Plasma 共享内存零拷贝 · 两级 store · Pull/Push 传输 · spill 落盘"),
    ("Ray原理_支撑_全局控制存储GCS", "support", "⛨", "全局控制存储 GCS",
     "控制面真源 · 一进程多 manager · 表存储 + pub/sub · 对象/调度都绕开它"),
    ("Ray原理_支撑_分布式调度", "support", "✲", "分布式调度",
     "Raylet 两级 lease manager · cluster 选点/local 授租 · hybrid 打包铺开策略"),
    ("Ray原理_支撑_引用计数与容错", "support", "◐", "引用计数与容错",
     "灵魂：owner-based 引用计数（含跨进程借用回传）· lineage 递归重算 · 不可变可重跑"),
    ("Ray原理_支撑_资源管理与放置组", "support", "◪", "资源管理与放置组",
     "声明式逻辑资源账本 · placement group 两阶段提交 · bundle PACK/SPREAD 策略"),
    ("Ray原理_支撑_自动伸缩与运行时环境", "support", "⇄", "自动伸缩与运行时环境",
     "autoscaler 按 pending 需求增删物理节点 · runtime env agent 预备隔离依赖"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接口主线 · 应用如何用（远程任务与对象 / Actor / 集群与运行时）"),
    ("support", "支撑主线 · 系统内部（8 条能力域）"),
]
...
BRAND_TITLE = "一切知识皆索引"
BRAND_SUB = "Ray 核心原理 · 交互式图谱"
HOME_DESC = ("Ray 核心原理设计文档库的离线交互图谱——通用分布式计算框架（task + actor 二元模型 · Python 前端 + C++ 核心）。"
...
for d in DOCS.values():
    for _, _, svg in d["walk"]:
        _all_refs.add(svg)
_on_disk = {f for f in os.listdir(_DESIGN_DIR) if f.endswith(".svg") and f != "icon.svg"}
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="Google RAY 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
