#!/usr/bin/env python3
"""tensorflow-design 交互式核心原理图谱生成器（自包含 · 离线 · 双主题）。
...
用法：
  cd tensorflow-design && python3 gen.py
...
_ap = argparse.ArgumentParser(description="生成 tensorflow 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("TENSORFLOW_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("TENSORFLOW_DESIGN_OUT") or os.path.join(HERE, "index.html")

# ===================================================================== #
# 一、主线注册表 —— 唯一需随项目调整的数据块
#     家族 7（机器学习框架 · 张量计算 + 自动微分 · TensorFlow）：接触面 × 能力域 × 时机。
#     全景 + 3 接口主线 + 8 支撑能力域。
# ===================================================================== #
MAINLINES = [
    ("TensorFlow原理_全景主线框架", "pano", "◇", "全景主线框架",
     "家族 7 机器学习框架：双维模型 · 总架构 · 图与即时分水岭 · 依赖矩阵 · 三条贯穿声明"),

    ("TensorFlow原理_接口_张量与变量编程", "iface", "⬢", "张量与变量编程",
     "tf.Tensor 不可变值 / tf.Variable 有状态资源句柄 · eager 默认逐算子直发"),
    ("TensorFlow原理_接口_图与tf_function", "iface", "◷", "图与 tf.function",
     "灵魂分水岭：tf.function 把 Python 追踪成 FuncGraph · AutoGraph 改写控制流 · 变签名重追踪"),
    ("TensorFlow原理_接口_建模与训练", "iface", "⚙", "建模与训练",
     "Keras Model/layer 组权重 · GradientTape 求梯度 · optimizer 更新 · tf.data 输入管线"),

    ("TensorFlow原理_支撑_张量与内存", "support", "▤", "张量与内存",
     "Tensor = 形状/类型 + 共享引用计数 TensorBuffer · Allocator 分设备 · BFC 显存池"),
    ("TensorFlow原理_支撑_算子与kernel", "support", "▦", "算子与 kernel 注册",
     "REGISTER_OP 注册 OpDef schema · REGISTER_KERNEL_BUILDER 按设备注册 · Compute()"),
    ("TensorFlow原理_支撑_计算图与Grappler", "support", "◈", "计算图与 Grappler",
     "灵魂：GraphDef 静态图 · Grappler MetaOptimizer 串十余 pass 整图重写"),
    ("TensorFlow原理_支撑_自动微分引擎", "support", "∂", "自动微分引擎",
     "灵魂：GradientTape 录 op 到 tape · ComputeGradient 逆序回放 · 前向/反向双模式"),
    ("TensorFlow原理_支撑_执行引擎", "support", "⇄", "执行引擎",
     "图模式 Executor 数据流调度（ready→propagate）· eager 逐 op 直发 · 异步"),
    ("TensorFlow原理_支撑_设备与后端", "support", "◫", "设备与后端",
     "Placer 按 colocation 组 + soft placement 定设备 · CPU/GPU/TPU · send/recv 跨设备"),
    ("TensorFlow原理_支撑_XLA编译与融合", "support", "⚡", "XLA 编译与融合",
     "可选：自动聚类 _XlaCompile/_XlaRun · XlaCompiler→HLO · 算子融合成大 kernel"),
    ("TensorFlow原理_支撑_分布式训练", "support", "✲", "分布式训练",
     "tf.distribute.Strategy 镜像变量 · run 并行副本 · collective all-reduce 聚合梯度"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接口主线 · 用户如何用（张量变量 / 图与 tf.function / 建模训练）"),
    ("support", "支撑主线 · 框架内部（8 条能力域）"),
]
...
# ===================================================================== #
PANO_NAME = "TensorFlow原理_全景主线框架"
ARCH_W, ARCH_H = 1040, 772  # 必须与 ARCH_SVG_NAME 的 viewBox 一致
# (x, y, w, h, 主线name) —— 一个模块可拆多行热区，一条主线可被多个区域指向
ARCH_HOTSPOTS = [
    # 贯穿示例条 → 全景总览
    (30, 44, 980, 46, "TensorFlow原理_全景主线框架"),
    # ① Python 前端接口层 —— 三个接口主线各一子盒
    (48, 138, 304, 32, "TensorFlow原理_接口_张量与变量编程"),
    (368, 138, 304, 32, "TensorFlow原理_接口_图与tf_function"),
    (688, 138, 304, 32, "TensorFlow原理_接口_建模与训练"),
    # ② 图追踪与构建整条 → 图与 tf.function 接口
    (30, 192, 980, 74, "TensorFlow原理_接口_图与tf_function"),
    # ③ Grappler 图优化 → 计算图与 Grappler
    (30, 278, 980, 58, "TensorFlow原理_支撑_计算图与Grappler"),
    # ④ XLA 编译
    (30, 346, 980, 58, "TensorFlow原理_支撑_XLA编译与融合"),
    # ⑤ 设备放置
    (30, 416, 980, 54, "TensorFlow原理_支撑_设备与后端"),
    # ⑥ 运行时执行
    (30, 482, 980, 66, "TensorFlow原理_支撑_执行引擎"),
    # ⑦ 算子分发 OpKernel
    (30, 560, 980, 58, "TensorFlow原理_支撑_算子与kernel"),
    # ⑧ 张量与内存
    (30, 630, 980, 58, "TensorFlow原理_支撑_张量与内存"),
    # ⑨ 横切：autodiff / 分布式
    (30, 700, 475, 50, "TensorFlow原理_支撑_自动微分引擎"),
    (535, 700, 475, 50, "TensorFlow原理_支撑_分布式训练"),
]
# 没有独立架构区域、需底部 chip 兜底的主线（本项目 12 主线全部落在图上 → 空）
ARCH_ALWAYS_CHIP = []

BRAND_TITLE = "一切知识皆索引"
BRAND_SUB = "TensorFlow"
HOME_DESC = ("TensorFlow 核心原理设计文档库的离线交互图谱——家族 7 机器学习框架（张量计算 + 自动微分 · 偏静态图）。"
             "12 条主线、17 张手绘原理图，全部回本地源码核实。点击项目总架构图任意模块即可下钻到对应主线。")
ARCH_SVG_NAME = "TensorFlow原理_全景_02总架构.svg"
...
for d in DOCS.values():
    for _, _, svg in d["walk"]:
        _all_refs.add(svg)
# icon.svg 是品牌图标资产（非走查原理图），不纳入引用闭环校验
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="TensorFlow 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
(function(){
  ...
  var saved=localStorage.getItem('tensorflow-atlas-theme');
  ...
  function toggleTheme(){
    ...
    localStorage.setItem('tensorflow-atlas-theme',cur);
