#!/usr/bin/env python3
"""hudi-design 交互式核心原理图谱生成器（自包含 · 离线 · 双主题）。
...
_ap = argparse.ArgumentParser(description="生成 Apache Hudi 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("HUDI_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("HUDI_DESIGN_OUT") or os.path.join(HERE, "index.html")

# ===================================================================== #
# 一、主线注册表 —— 唯一需随项目调整的数据块
#     家族（湖仓表格式 · 事务性数据湖 · Apache Hudi，与 Iceberg 同族、upsert 导向）：元模式 = 接口 × 能力域 × 时机。
#     全景 + 1 接触面主线 + 8 支撑能力域。
# ===================================================================== #
MAINLINES = [
    ("Hudi原理_全景主线框架", "pano", "◇", "全景主线框架",
     "事务性数据湖表格式(upsert 导向)：双维模型 · 总架构 · 依赖矩阵 · 依赖关系图 · 三条贯穿声明"),

    ("Hudi原理_接触面_表读写API", "iface", "⚙", "接触面 · 表读写 API",
     "引擎经 write client upsert/insert/bulk_insert/delete + 三种查询类型读 · Spark/Flink 集成"),

    ("Hudi原理_支撑_时间线", "support", "⛨", "时间线(核心 · 灵魂)",
     "表的真相：HoodieInstant 动作×状态 · instant time 单调定序 · 决定可见性/增量起点/回滚点"),
    ("Hudi原理_支撑_表类型COW与MOR", "support", "◧", "表类型 COW / MOR",
     "写时重写 vs 读时合并的核心取舍 · CreateHandle/MergeHandle/AppendHandle · IOType"),
    ("Hudi原理_支撑_写入路径与upsert", "support", "✎", "写入路径与 upsert",
     "开 instant→索引 tag→bucket 路由→写 handle→提交 · marker 失败回滚 · 原子提交"),
    ("Hudi原理_支撑_索引", "support", "◉", "索引",
     "记录键→文件组：Bloom/Simple/Bucket/RECORD_LEVEL · 分区内 vs 全局 · 一致性哈希桶"),
    ("Hudi原理_支撑_MoR读合并", "support", "▦", "MoR 读合并",
     "base+log 读时合并(MergedLogRecordReader) · log block 格式 · 快照/读优化/增量三视图"),
    ("Hudi原理_支撑_文件布局", "support", "▤", "文件布局",
     "FileGroup→FileSlice(base+log) · 命名即元数据 · 分区/bootstrap/metadata table 布局"),
    ("Hudi原理_支撑_表服务", "support", "⚙", "表服务",
     "compaction 合 log 进 base · cleaning 按策略清旧 · clustering 重排(replacecommit)"),
    ("Hudi原理_支撑_并发控制与元数据", "support", "◐", "并发控制与元数据",
     "OCC/NBCC + 锁 provider + marker · metadata table(files/col_stats/record_index) · catalog 同步"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接触面主线 · 计算引擎如何用（表读写 API + write client）"),
    ("support", "支撑主线 · 表格式内部（8 条能力域）"),
]
...
# ===================================================================== #
PANO_NAME = "Hudi原理_全景主线框架"
ARCH_W, ARCH_H = 1080, 900  # 必须与 ARCH_SVG_NAME 的 viewBox 一致
# (x, y, w, h, 主线name) —— 一个模块可拆多行热区，一条主线可被多个区域指向
ARCH_HOTSPOTS = [
    # 标题条 → 全景总览
    (0, 0, 1080, 60, "Hudi原理_全景主线框架"),
    # ① 表读写 API 接触面
    (30, 120, 1020, 44, "Hudi原理_接触面_表读写API"),
    # ② 中层：写入路径 / 索引 / 时间线(核心)
    (30, 178, 326, 252, "Hudi原理_支撑_写入路径与upsert"),
    (368, 178, 248, 252, "Hudi原理_支撑_索引"),
    (628, 178, 422, 252, "Hudi原理_支撑_时间线"),
    # ③ 文件布局 / 表类型
    (30, 444, 510, 72, "Hudi原理_支撑_文件布局"),
    (552, 444, 498, 72, "Hudi原理_支撑_表类型COW与MOR"),
    # ④ MoR 读合并 / 表服务
    (30, 530, 510, 66, "Hudi原理_支撑_MoR读合并"),
    (552, 530, 498, 66, "Hudi原理_支撑_表服务"),
    # ⑤ 并发控制与元数据
    (30, 610, 1020, 58, "Hudi原理_支撑_并发控制与元数据"),
]
# 没有独立架构区域、需底部 chip 兜底的主线（本项目 10 主线全部落在图上 → 空）
ARCH_ALWAYS_CHIP = []

BRAND_SUB = "Apache Hudi"
HOME_DESC = ("Apache Hudi 核心原理设计文档库的离线交互图谱——事务性数据湖表格式（与 Iceberg 同族，但 upsert 导向：以时间线为真相、"
             "靠索引把记录键路由到文件组做高效 upsert，COW/MOR 两种表类型）。"
             "10 条主线、30 张手绘原理图，回 apache/hudi 源码核实。点击项目总架构图任意模块即可下钻到对应主线。")
ARCH_SVG_NAME = "Hudi原理_总架构图.svg"
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="Apache Hudi 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
(function(){
  ...
  var saved=localStorage.getItem('hudi-atlas-theme');
  ...
  function toggleTheme(){
    ...
    localStorage.setItem('hudi-atlas-theme',cur);
...
<header>
  ...
  <a href="https://github.com/apache/hudi" target="_blank" rel="noopener" title="GitHub 源码仓库" style="display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M12 .5C5.7.5.5 5.7.5 12c0 5.1 3.3 9.4 7.9 10.9.6.1.8-.2.8-.6v-2c-3.2.7-3.9-1.4-3.9-1.4-.5-1.3-1.3-1.7-1.3-1.7-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1 1.8 2.7 1.3 3.4 1 .1-.8.4-1.3.7-1.6-2.6-.3-5.3-1.3-5.3-5.8 0-1.3.5-2.3 1.2-3.1-.1-.3-.5-1.5.1-3.1 0 0 1-.3 3.3 1.2a11.4 11.4 0 0 1 6 0C17.3 4.7 18.3 5 18.3 5c.6 1.6.2 2.8.1 3.1.8.8 1.2 1.8 1.2 3.1 0 4.5-2.7 5.5-5.3 5.8.4.4.8 1.1.8 2.2v3.3c0 .4.2.7.8.6 4.6-1.5 7.9-5.8 7.9-10.9C23.5 5.7 18.3.5 12 .5z"/></svg></a><a href="https://hudi.apache.org" target="_blank" rel="noopener" title="项目官网" style="display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><img src="data:image/svg+xml;base64," width="18" height="18" alt="官网" style="display:block"/></a><button id="themeBtn" title="切换主题" aria-label="切换主题" style="width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);background:var(--c-panel);color:var(--c-ink2);cursor:pointer;display:inline-grid;place-items:center;font-size:16px;flex:none">☾</button>
</header>
<div class="wrap">
  <div id="home">
    {archnav}
