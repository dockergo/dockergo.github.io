...
delete files 存在 **delete manifest**(`ManifestFile.content == 1`),经 `ManifestFiles.readDeleteManifest` 读(`BaseSnapshot.java:275`)。
...
**merge-on-read vs copy-on-write**:MoR 写快(只写小 delete file、不重写 data)、读慢(要叠加删除);CoW 写慢(重写整个 data file)、读快(无删除叠加)。Iceberg v2 支持 MoR,按写多读少/读多写少选。
