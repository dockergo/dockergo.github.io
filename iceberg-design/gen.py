#!/usr/bin/env python3
"""iceberg-design 交互式核心原理图谱生成器（自包含 · 离线 · 双主题）。
...
用法：
  cd iceberg-design && python3 gen.py
...
_ap = argparse.ArgumentParser(description="生成 Apache Iceberg 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("ICEBERG_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("ICEBERG_DESIGN_OUT") or os.path.join(HERE, "index.html")
...
# ===================================================================== #
MAINLINES = [
    ("Iceberg原理_全景主线框架", "pano", "◇", "全景主线框架",
     "新家族湖仓表格式：双维模型 · 总架构 · 依赖矩阵 · 依赖关系图 · 三条贯穿声明"),

    ("Iceberg原理_接触面_表API与快照读", "iface", "⚙", "接触面 · 表 API 与快照读",
     "给计算引擎集成的库 API + catalog · TableScan 读/时间旅行 · SnapshotProducer 写 · 演进 DDL"),

    ("Iceberg原理_支撑_元数据树", "support", "▤", "元数据树",
     "核心：metadata.json→manifest list→manifest→data file · 自带清单+列统计,不 list 目录"),
    ("Iceberg原理_支撑_快照与提交", "support", "⛨", "快照与提交",
     "灵魂：不可变快照 + catalog 原子 CAS 换指针 + OCC 乐观重试 · 在对象存储上做 ACID"),
    ("Iceberg原理_支撑_schema演进", "support", "✎", "schema 演进",
     "列靠不可变字段 ID 追踪 · rename/加删列/类型提升全部不重写数据"),
    ("Iceberg原理_支撑_分区演进与隐藏分区", "support", "◫", "分区演进与隐藏分区",
     "分区值从源列经 transform 派生(隐藏)· 每 manifest 记 spec id · 改规则不重写老数据"),
    ("Iceberg原理_支撑_扫描规划", "support", "▦", "扫描规划",
     "两级剪枝：manifest 分区剪 + 文件列统计剪 · residual 残差 · planTasks split 切分"),
    ("Iceberg原理_支撑_行级删除", "support", "◉", "行级删除(v2 · MoR)",
     "position/equality delete file 不重写数据 · 读时 merge-on-read 叠加 · 按 seq 定作用范围"),
    ("Iceberg原理_支撑_表维护", "support", "⚙", "表维护",
     "compaction 合并小文件/物化删除 · expire snapshots 回收 · remove orphan files 清孤儿"),
    ("Iceberg原理_支撑_catalog与并发", "support", "◐", "catalog 与并发",
     "catalog 类型(Hadoop/Hive/JDBC/REST)· TableOperations 提交契约 · 原子换指针决定并发可靠性"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接触面主线 · 计算引擎如何用（表 API + catalog）"),
...
# ===================================================================== #
PANO_NAME = "Iceberg原理_全景主线框架"
ARCH_W, ARCH_H = 1080, 820  # 必须与 ARCH_SVG_NAME 的 viewBox 一致
# (x, y, w, h, 主线name) —— 一个模块可拆多行热区，一条主线可被多个区域指向
ARCH_HOTSPOTS = [
    # 标题条 → 全景总览
    (0, 0, 1080, 58, "Iceberg原理_全景主线框架"),
    # ① 接触面 · 表 API
    (30, 130, 1020, 46, "Iceberg原理_接触面_表API与快照读"),
    # ② catalog 与并发
    (30, 190, 1020, 44, "Iceberg原理_支撑_catalog与并发"),
    # ③ 不可变元数据树（核心）
    (30, 248, 1020, 196, "Iceberg原理_支撑_元数据树"),
    # ④~⑨ 能力域带（上排三 + 下排三）
    (30, 458, 330, 78, "Iceberg原理_支撑_快照与提交"),
    (375, 458, 330, 78, "Iceberg原理_支撑_schema演进"),
    (720, 458, 330, 78, "Iceberg原理_支撑_分区演进与隐藏分区"),
    (30, 548, 330, 78, "Iceberg原理_支撑_扫描规划"),
    (375, 548, 330, 78, "Iceberg原理_支撑_行级删除"),
    (720, 548, 330, 78, "Iceberg原理_支撑_表维护"),
]
# 没有独立架构区域、需底部 chip 兜底的主线（本项目 10 主线全部落在图上 → 空）
ARCH_ALWAYS_CHIP = []

BRAND_TITLE = "一切知识皆索引"
BRAND_SUB = "Apache Iceberg"
HOME_DESC = ("Apache Iceberg 核心原理设计文档库的离线交互图谱——新家族（湖仓表格式 / 开放表格式：数据文件之上的一层不可变元数据，"
             "让对象存储上的一堆文件表现得像有 ACID、schema 演进、时间旅行的表）。"
             "10 条主线、26 张手绘原理图，全部回 apache/iceberg 源码核实。点击项目总架构图任意模块即可下钻到对应主线。")
ARCH_SVG_NAME = "Iceberg原理_总架构图.svg"
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="Apache Iceberg 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
(function(){
  ...
  var saved=localStorage.getItem('iceberg-atlas-theme');
  ...
  function toggleTheme(){
    ...
    localStorage.setItem('iceberg-atlas-theme',cur);
...
<header>
  ...
  <a href="https://github.com/apache/iceberg" target="_blank" rel="noopener" title="GitHub 源码仓库" style="display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M12 .5C5.7.5.5 5.7.5 12c0 5.1 3.3 9.4 7.9 10.9.6.1.8-.2.8-.6v-2c-3.2.7-3.9-1.4-3.9-1.4-.5-1.3-1.3-1.7-1.3-1.7-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1 1.8 2.7 1.3 3.4 1 .1-.8.4-1.3.7-1.6-2.6-.3-5.3-1.3-5.3-5.8 0-1.3.5-2.3 1.2-3.1-.1-.3-.5-1.5.1-3.1 0 0 1-.3 3.3 1.2a11.4 11.4 0 0 1 6 0C17.3 4.7 18.3 5 18.3 5c.6 1.6.2 2.8.1 3.1.8.8 1.2 1.8 1.2 3.1 0 4.5-2.7 5.5-5.3 5.8.4.4.8 1.1.8 2.2v3.3c0 .4.2.7.8.6 4.6-1.5 7.9-5.8 7.9-10.9C23.5 5.7 18.3.5 12 .5z"/></svg></a><a href="https://iceberg.apache.org" target="_blank" rel="noopener" title="项目官网" style="display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><img src="data:image/svg+xml;base64," width="18" height="18" alt="官网" style="display:block"/></a><button id="themeBtn" title="切换主题" aria-label="切换主题" style="width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);background:var(--c-panel);color:var(--c-ink2);cursor:pointer;display:inline-grid;place-items:center;font-size:16px;flex:none">☾</button>
</header>
<div class="wrap">
  <div id="home">
    {archnav}
