#!/usr/bin/env python3
"""spring-boot-design 交互式核心原理图谱生成器（自包含 · 离线 · 双主题）。
...
用法：
  cd spring-boot-design && python3 gen.py
...
_ap = argparse.ArgumentParser(description="生成 Spring Boot 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("SPRINGBOOT_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("SPRINGBOOT_DESIGN_OUT") or os.path.join(HERE, "index.html")
...
# ===================================================================== #
MAINLINES = [
    ("SpringBoot原理_全景主线框架", "pano", "◇", "全景主线框架",
     "新家族 IoC 容器/应用框架：双维模型 · 总架构 · 依赖矩阵 · 依赖关系图 · 三条贯穿声明"),

    ("SpringBoot原理_接触面_注解与启动", "iface", "⚙", "接触面 · 注解与 SpringApplication 启动",
     "@SpringBootApplication 三合一 · run() 一行走完启动流程 · SpringApplicationRunListener 7 阶段 · SPI 扩展点"),

    ("SpringBoot原理_支撑_IoC容器", "support", "▤", "IoC 容器(委托 framework)",
     "核心 BeanFactory/refresh 在 spring-framework 7.0.8 · Boot 只委托 + 预配 · bean 定义两来源"),
    ("SpringBoot原理_支撑_自动配置", "support", "✦", "自动配置(灵魂)",
     "灵魂：.imports 候选 + @ConditionalOnClass/OnMissingBean 条件装配 · classpath 有什么配什么 · 读字节码不加载类"),
    ("SpringBoot原理_支撑_starter机制", "support", "▩", "starter 机制",
     "纯依赖 POM 一个拉全一套 · 带技术 jar + .imports 触发自动配置 · 自定义 starter 两模块"),
    ("SpringBoot原理_支撑_内嵌服务器", "support", "◧", "内嵌服务器",
     "jar 内 Tomcat/Jetty/Netty · 创建(onRefresh)与启动(SmartLifecycle)分离 · 优雅停机"),
    ("SpringBoot原理_支撑_配置属性绑定", "support", "◫", "配置属性绑定",
     "@ConfigurationProperties 前缀绑定 · 松散绑定(kebab/驼峰/大写都映一字段)· Environment 多源优先级"),
    ("SpringBoot原理_支撑_Actuator与AOT", "support", "◉", "Actuator 与 AOT 原生",
     "Actuator 端点(/health /metrics)配 k8s 探针 · AOT 构建期提前处理 · GraalVM 原生镜像毫秒启动"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接触面主线 · 开发者如何用(注解 + run)"),
    ("support", "支撑主线 · 框架内部(6 条能力域)"),
]

PANO_NAME = "SpringBoot原理_全景主线框架"
ARCH_W, ARCH_H = 1080, 820
ARCH_HOTSPOTS = [
    (0, 0, 1080, 58, "SpringBoot原理_全景主线框架"),
    (30, 64, 1020, 46, "SpringBoot原理_接触面_注解与启动"),
    (30, 182, 1020, 120, "SpringBoot原理_支撑_IoC容器"),
    (30, 316, 330, 80, "SpringBoot原理_支撑_自动配置"),
    (375, 316, 330, 80, "SpringBoot原理_支撑_starter机制"),
    (720, 316, 330, 80, "SpringBoot原理_支撑_内嵌服务器"),
    (30, 408, 330, 80, "SpringBoot原理_支撑_配置属性绑定"),
    (375, 408, 330, 80, "SpringBoot原理_支撑_Actuator与AOT"),
    (720, 408, 330, 80, "SpringBoot原理_支撑_Actuator与AOT"),
]
ARCH_ALWAYS_CHIP = []

BRAND_TITLE = "一切知识皆索引"
BRAND_SUB = "Spring Boot 核心原理 · 交互式图谱"
HOME_DESC = ("Spring Boot 核心原理设计文档库的离线交互图谱——新家族(IoC 容器/应用框架：建在 Spring Framework 的 IoC 容器上，"
             "加自动配置 + starter + 内嵌服务器，约定优于配置)。9 条主线、手绘原理图，全部回 spring-projects/spring-boot 源码核实。点击项目总架构图任意模块即可下钻到对应主线。")
ARCH_SVG_NAME = "SpringBoot原理_总架构图.svg"
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="Spring Boot 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
(function(){
  ...
  var saved=localStorage.getItem('springboot-atlas-theme');
  ...
  function toggleTheme(){
    ...
    localStorage.setItem('springboot-atlas-theme',cur);
