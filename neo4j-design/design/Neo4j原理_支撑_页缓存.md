...
**MuninnPageCache**(`community/io/.../pagecache/impl/muninn/MuninnPageCache.java:82`)是生产页缓存:

- **堆外分配**:页内存经 `MemoryAllocator`/`UnsafeUtil` 分配在堆外——不占 JVM 堆、不触发 GC。海量图数据可常驻内存而不拖累 GC。
- **PagedFile + PageCursor**:store 文件映射成 `PagedFile`,记录存储把 `PageCursor` 定位到 `pageIdForRecord(id)` 页的 `offsetForId(id)` 偏移读写(见记录存储篇)。
- **页大小是 2 的幂**(`isPowerOfTwo` 校验,`:25`),便于位运算定位。
- **换页**:`PageSwapperFactory`/`SingleFilePageSwapperFactory` 负责页与磁盘交换;`IOController` 做 IO 节流。
...
- **页错误(page fault)**:访问不在内存的页时,`PageFaultEvent` 触发从磁盘加载该页到一个空闲/被淘汰的页帧——类比 OS 缺页。
- **后台淘汰(eviction)**:内存满时,后台线程(`EvictionRunEvent`/`AsyncEvictionStatus`)按策略淘汰冷页,脏页先刷盘再释放页帧。这让热数据留内存、冷数据让位。
...
| 结构 | 定义 | 职责 |
|---|---|---|
| MuninnPageCache | `io/.../pagecache/impl/muninn/MuninnPageCache.java:82` | 堆外分页页缓存 |
| PagedFile / PageCursor | `io/.../pagecache/` | 文件映射 + 页内定位读写 |
| PageSwapperFactory | `io/.../pagecache/` | 页与磁盘交换 |
| PageFaultEvent | `io/.../pagecache/tracing/` | 缺页加载事件 |
