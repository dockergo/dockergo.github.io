...
回填时依 Cache-Control/`proxy_cache_valid` 决定是否可缓存 + TTL，写临时文件后原子 rename 进 `ngx_http_file_cache_name`（`http/ngx_http_file_cache.c:994`）算出的分级目录路径；`proxy_cache_lock` 让并发 MISS 只放一个回源、其余等回填或超时（防缓存击穿）。缓存文件头结构为 `ngx_http_file_cache_header_t`（校验时若 `version mismatch` `http/ngx_http_file_cache.c:570` 或 `md5 collision` `http/ngx_http_file_cache.c:576` 会判文件失效），读文件过小（`too small` `http/ngx_http_file_cache.c:562`）也视为损坏丢弃。**索引在共享内存、数据在磁盘文件**——内存只存 key→元数据映射。

存储与后台管理：磁盘按 key hash 分级目录（`levels=1:2`，用 MD5 末几位切子目录），文件头存元数据+响应头+体，`keys_zone` 存内存索引；**cache manager** 进程 `ngx_http_file_cache_manager`（`http/ngx_http_file_cache.c:2022`）按 `max_size` 触发 `ngx_http_file_cache_forced_expire`（`http/ngx_http_file_cache.c:1761`）强淘汰、按 `inactive` 走 `ngx_http_file_cache_expire`（`http/ngx_http_file_cache.c:1860`）清久未访问；**cache loader** 进程 `ngx_http_file_cache_loader`（`http/ngx_http_file_cache.c:2109`）仅启动时扫描缓存目录、把已有文件重建进内存索引。

---

## 二、失败路径与边界

- **缓存文件损坏/版本不符**：`ngx_http_file_cache_open`（`:265`）读到 `version mismatch`（`:570`）、`md5 collision`（`:576`）或文件 `too small`（`:562`）时判失效、删除并回源，绝不把坏数据返给客户端。
- **磁盘文件被外部删除**：索引仍在但 `ngx_open_cached_file`（`:359`）open 失败（`:373` 记 error），nginx 退化为 MISS 回源并重建。
- **keys_zone 索引满**：内存索引写不下新 key 时 manager 立即按 LRU 强淘汰腾位（`ngx_http_file_cache_forced_expire` `:1761`），极端下新请求缓存不命中但服务不中断。
- **max_size 超限**：manager 后台持续 LRU 淘汰把总量压回阈值；淘汰是渐进的，短时超限可容忍。
- **回源自身失败**：MISS 回源遇后端错误时，若配 `proxy_cache_use_stale` 可返回过期旧缓存兜底，否则透传后端错误。
