...
- **large 分配失败**：`ngx_palloc_large` 里 `malloc` 返回 NULL 时向上返回 NULL，调用方须判空；池本身不 OOM-kill，靠上层 finalize 请求。
- **cleanup 顺序**：`ngx_destroy_pool`（`:47`）先按注册逆序跑所有 cleanup（如关临时文件 fd）再释放内存——若模块把 fd 直接 malloc 不注册 cleanup，池销毁不会替它关，造成 fd 泄漏。
- **写缓冲背压**：输出链一次写不完（socket 缓冲满）时 buf 挂 busy 链、返回 `NGX_AGAIN`，等写事件再 `ngx_chain_update_chains`（`:185`）回收已发部分——绝不阻塞等待。
- **请求头超缓冲**：请求头超过 `large_client_header_buffers` 报 414/400，不会无限扩内存。
- **跨 worker 数据不能进池**：pool 是进程内、随生命周期释放；缓存索引/限流计数等跨 worker 状态必须放共享内存 + slab（`core/ngx_slab.c`），不能用 palloc。
