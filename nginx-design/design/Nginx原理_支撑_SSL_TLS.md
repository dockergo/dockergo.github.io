...
**SSL 上下文构建**：启动时 `ngx_ssl_create`（`event/ngx_event_openssl.c:306`）按 `ssl_protocols` 建 `SSL_CTX`；连接到来时 `ngx_ssl_create_connection`（`:2107`）为该连接建 `SSL*`。**TLS 握手嵌进事件循环**：核心是 `ngx_ssl_handshake`（`:2201`）里调 `SSL_do_handshake(c->ssl->connection)`（`:2219`）——ClientHello（含 SNI 目标域名）→ 按 SNI 选证书 → 密钥协商（证书 + ECDHE 等）。关键：`SSL_do_handshake` 返回负值且 OpenSSL 错误码为 `SSL_ERROR_WANT_READ`/`WANT_WRITE` 时挂起等对应读/写事件，与 phase 引擎的 AGAIN 机制同构——**慢握手不占线程**，worker 转去处理别的连接，事件就绪后重入 `ngx_ssl_handshake` 续跑。
...
- **握手失败**：`ngx_ssl_handshake`（`event/ngx_event_openssl.c:2201`）里 `SSL_do_handshake` 返回错误（协议不符、套件不匹配、证书校验失败）时记 SSL error 日志、关连接，绝不半开挂起。
- **SNI 未匹配**：`ngx_http_find_virtual_server`（`http/ngx_http_request.c:2491`）找不到对应 `server_name` 时回退到该 listen 的**默认 server**证书，可能触发客户端证书名不符告警——务必为每个域名配证书。
- **OCSP 取回失败**：stapling 响应拉取失败或过期时，nginx 不阻断握手、只是不下发 stapling（客户端自行校验），`ngx_ssl_ocsp_validate`（`event/ngx_event_openssl.c:2214`，另一入口 `event/ngx_event_openssl.c:2267`）容错返回。
- **0-RTT 重放风险**：TLS 1.3 early data 可被重放，nginx 对 0-RTT 数据标记 `Early-Data` 头，非幂等请求应在后端拒绝或校验。
- **私钥加载失败**：`ngx_ssl_create`（`event/ngx_event_openssl.c:306`）阶段证书/私钥不匹配或文件缺失直接启动失败，不会带病运行。
