#!/usr/bin/env python3
"""kubernetes-design 交互式核心原理图谱生成器（自包含 · 离线 · 双主题）。
...
用法：
  cd kubernetes-design && python3 gen.py
...
_ap = argparse.ArgumentParser(description="生成 kubernetes 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("KUBERNETES_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("KUBERNETES_DESIGN_OUT") or os.path.join(HERE, "index.html")

# ===================================================================== #
# 一、主线注册表 —— 唯一需随项目调整的数据块
#     家族 3（编排 / 控制平面 · Kubernetes）：元模式 = 接触面 × 能力域 × 时机。
#     全景 + 1 接触面主线 + 10 支撑能力域。灵魂 = reconcile 控制循环。
# ===================================================================== #
MAINLINES = [
    ("Kubernetes原理_全景主线框架", "pano", "◇", "全景主线框架",
     "家族3编排/控制平面：双维模型 · 总架构 · reconcile 贯穿环 · 依赖矩阵"),

    ("Kubernetes原理_接口_声明式API与期望态", "iface", "⚙", "声明式 API 与期望态",
     "唯一接触面：kubectl 提交 spec · resourceVersion 乐观并发 · apply 幂等"),

    ("Kubernetes原理_支撑_APIServer与etcd", "support", "▤", "API Server 与 etcd",
     "唯一读写 etcd 的枢纽 · REST 存储/多版本 · watch cache 扇出"),
    ("Kubernetes原理_支撑_Informer与Watch机制", "support", "⇄", "Informer 与 Watch",
     "Reflector→DeltaFIFO→Indexer→workqueue · 客户端缓存 + 增量事件"),
    ("Kubernetes原理_支撑_控制器reconcile循环", "support", "◉", "控制器 reconcile 循环",
     "灵魂：workqueue→syncHandler 对比 spec 与实际 · level-triggered · Deploy→RS→Pod"),
    ("Kubernetes原理_支撑_调度器", "support", "◈", "调度器",
     "filter 剔除 → score 打分 → assume + bind 写 pod.spec.nodeName"),
    ("Kubernetes原理_支撑_kubelet与CRI", "support", "▦", "kubelet 与 CRI",
     "syncLoop 节点级 reconcile · SyncPod 重算容器差异 · CRI→containerd"),
    ("Kubernetes原理_支撑_网络", "support", "◫", "网络",
     "Service ClusterIP · EndpointSlice · kube-proxy 整表重刷 · CNI/Ingress"),
    ("Kubernetes原理_支撑_存储", "support", "▥", "存储",
     "PVC 申领 → PV 绑定/动态供给 → CSI Provision/Attach/Mount"),
    ("Kubernetes原理_支撑_认证授权与准入", "support", "⛨", "认证授权与准入",
     "三关：认证 authn → 授权 RBAC → 准入 admission（Mutating/Validating）"),
    ("Kubernetes原理_支撑_扩展机制", "support", "✲", "扩展机制",
     "CRD 注册新类型 + 自定义控制器 · Operator = CRD + reconcile 骨架"),
    ("Kubernetes原理_支撑_控制器管理器与GC", "support", "⚡", "控制器管理器与 GC",
     "编排多控制器 · leader 选举 · ownerReferences 级联删除 · finalizers"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接触面主线 · 用户可见（声明式 API + kubectl）"),
    ("support", "支撑主线 · 控制平面内部（10 条能力域）"),
]
...
# ===================================================================== #
PANO_NAME = "Kubernetes原理_全景主线框架"
ARCH_W, ARCH_H = 1040, 720  # 必须与 ARCH_SVG_NAME 的 viewBox 一致
# (x, y, w, h, 主线name) —— 一个模块可拆多行热区，一条主线可被多个区域指向
ARCH_HOTSPOTS = [
    # 标题条 → 全景总览
    (40, 18, 960, 34, "Kubernetes原理_全景主线框架"),
    # 客户端接触面
    (40, 46, 960, 42, "Kubernetes原理_接口_声明式API与期望态"),
    # kube-apiserver hub —— 标题 + 三个子块（安全三关 / REST / watch cache）
    (64, 140, 560, 30, "Kubernetes原理_支撑_APIServer与etcd"),
    (80, 172, 168, 54, "Kubernetes原理_支撑_认证授权与准入"),
    (256, 172, 168, 54, "Kubernetes原理_支撑_APIServer与etcd"),
    (432, 172, 176, 54, "Kubernetes原理_支撑_APIServer与etcd"),
    # etcd
    (640, 140, 336, 98, "Kubernetes原理_支撑_APIServer与etcd"),
    # controller-manager 内 2×2：Informer / synchandler / workqueue / GC
    (80, 288, 256, 72, "Kubernetes原理_支撑_Informer与Watch机制"),
    (352, 288, 256, 72, "Kubernetes原理_支撑_控制器reconcile循环"),
    (80, 368, 256, 76, "Kubernetes原理_支撑_控制器reconcile循环"),
    (352, 368, 256, 76, "Kubernetes原理_支撑_控制器管理器与GC"),
    # scheduler / CRD
    (640, 256, 336, 92, "Kubernetes原理_支撑_调度器"),
    (640, 362, 336, 92, "Kubernetes原理_支撑_扩展机制"),
    # 节点面：kubelet / 网络 / 存储
    (64, 520, 296, 150, "Kubernetes原理_支撑_kubelet与CRI"),
    (376, 520, 296, 150, "Kubernetes原理_支撑_网络"),
    (688, 520, 288, 150, "Kubernetes原理_支撑_存储"),
]
# 没有独立架构区域、需底部 chip 兜底的主线（本项目 12 主线全部落在图上 → 空）
ARCH_ALWAYS_CHIP = []

BRAND_TITLE = "Kubernetes 核心原理"
BRAND_SUB = "Kubernetes"
HOME_DESC = ("Kubernetes 核心原理设计文档库的离线交互图谱——家族 3（编排 / 控制平面）范例。"
             "12 条主线、17 张手绘原理图，全部回源码核实。点击项目总架构图任意模块即可下钻到对应主线。")
ARCH_SVG_NAME = "Kubernetes原理_全景_02总架构.svg"
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="Kubernetes 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
(function(){
  ...
  var saved=localStorage.getItem('kubernetes-atlas-theme');
  ...
  function toggleTheme(){
    ...
    localStorage.setItem('kubernetes-atlas-theme',cur);
...
<header>
  ...
  <a href="https://github.com/kubernetes/kubernetes" target="_blank" rel="noopener" title="GitHub 源码仓库" style="display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M12 .5C5.7.5.5 5.7.5 12c0 5.1 3.3 9.4 7.9 10.9.6.1.8-.2.8-.6v-2c-3.2.7-3.9-1.4-3.9-1.4-.5-1.3-1.3-1.7-1.3-1.7-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1 1.8 2.7 1.3 3.4 1 .1-.8.4-1.3.7-1.6-2.6-.3-5.3-1.3-5.3-5.8 0-1.3.5-2.3 1.2-3.1-.1-.3-.5-1.5.1-3.1 0 0 1-.3 3.3 1.2a11.4 11.4 0 0 1 6 0C17.3 4.7 18.3 5 18.3 5c.6 1.6.2 2.8.1 3.1.8.8 1.2 1.8 1.2 3.1 0 4.5-2.7 5.5-5.3 5.8.4.4.8 1.1.8 2.2v3.3c0 .4.2.7.8.6 4.6-1.5 7.9-5.8 7.9-10.9C23.5 5.7 18.3.5 12 .5z"/></svg></a><a href="https://kubernetes.io" target="_blank" rel="noopener" title="项目官网" style="display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><img src="data:image/svg+xml;base64," width="18" height="18" alt="官网" style="display:block"/></a><button id="themeBtn" title="切换主题" aria-label="切换主题" style="width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);background:var(--c-panel);color:var(--c-ink2);cursor:pointer;display:inline-grid;place-items:center;font-size:16px;flex:none">☾</button>
</header>
<div class="wrap">
  <div id="home">
    {archnav}
