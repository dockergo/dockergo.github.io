...
## 〇、与 SQL 存算引擎（Doris/ClickHouse）的心智对照（读前必看）

先立三条"反直觉"认知，后文不再重复：

| 维度 | SQL 存算引擎（Doris/ClickHouse） | Spark | 影响 |
|---|---|---|---|
| 接触面 | SQL 语句族（DDL/DML/DQL/DCL） | **多 API**：RDD / DataFrame-Dataset / SQL / Structured Streaming，统一入口 SparkSession | 不是"一种语言"，是多套 API 汇入同一执行引擎 |
...
| 容错 | 事务/副本 | **lineage 重算**（丢了分区按血缘重算）+ checkpoint + task 重试 | 无事务；容错是"可重算性" |
| 优化 | RBO/CBO 产物直接执行 | **Catalyst**（逻辑→物理→CBO）+ **Tungsten** whole-stage codegen + 运行期 **AQE** | 优化跨 SQL/DataFrame 统一（RDD 不过 Catalyst） |
| "更新" | INSERT/Mutation 改数据 | RDD **不可变**，转换产生新 RDD（lineage 就是转换链） | 不可变 + 血缘 = 容错的根 |

一句话：**SQL 存算引擎以"表 + 存储"为中心，Spark 以"不可变 RDD 的 DAG + lineage 容错"为中心。**
...
- **通信/传输（性能）**：shuffle 数据交换、broadcast、block 远程拉取、driver–executor RPC 均走网络与序列化，瓶颈常落于此（尤其 shuffle）。
- **可观测性（诊断）**：事件总线（SparkListener）→ Web UI（job/stage/task/DAG 可视化）+ 指标系统 + event log（history server）。
- **运行形态（前提）**：同一套 job→stage→task 模型，跑在不同 cluster manager（Standalone/YARN/K8s）上，主线不变、资源协商方式变；client vs cluster deploy mode 决定 driver 位置。

---

## 七、运行形态（cluster manager 维度的取值）

- **Standalone**：Spark 自带 Master/Worker，最简。
- **YARN**：向 ResourceManager 申请 container 起 executor，Hadoop 生态主流。
- **Kubernetes**：executor 为 pod，云原生。
- **client vs cluster deploy mode**：driver 跑在提交端（client）还是集群内（cluster）——决定 driver 生命周期与网络。
