...
ARCH_W, ARCH_H = 1080, 760  # 必须与 ARCH_SVG_NAME 的 viewBox 一致
# (x, y, w, h, 主线name) —— 一个模块可拆多行热区，一条主线可被多个区域指向
# 注：总架构 SVG 内 <g> 只设 text-anchor/font-size，无 translate 位移 → rect 坐标皆为绝对值
ARCH_HOTSPOTS = [
    # —— 容器/大区背景（先列 → 低层，叶子模块叠其上优先响应）——
    # 标题条 → 全景总览
    (0, 0, 1080, 58, "Spark原理_全景主线框架"),
    # Driver 容器（规划协调中枢：DAG / stage / task）
    (30, 134, 620, 360, "Spark原理_支撑_执行模型"),
    # Cluster Manager（资源：Standalone / YARN / K8s）
    (670, 134, 380, 120, "Spark原理_支撑_调度与集群管理"),
    # Executors 容器（执行 · N 个）
    (670, 272, 380, 222, "Spark原理_支撑_执行模型"),
    # 外部存储（Spark 不自管持久存储）
    (30, 512, 1020, 80, "Spark原理_支撑_存储与缓存"),
    # 底部"位置即语义"总述 → 全景
    (30, 606, 1020, 126, "Spark原理_全景主线框架"),
    # —— 叶子模块（后列 → 叠在容器背景之上）——
    # 编程接口层（RDD / DataFrame / SQL / Streaming）
    (30, 66, 1020, 52, "Spark原理_编程接口层"),
    # Structured Streaming 接口芯片（压在接口层条之上）
    (740, 78, 160, 30, "Spark原理_支撑_StructuredStreaming"),
    # driver 内：Catalyst / Tungsten / 两级调度器 / 两张目录表
    (46, 188, 140, 52, "Spark原理_支撑_Catalyst优化"),
    (196, 188, 140, 52, "Spark原理_支撑_Tungsten代码生成"),
    (346, 188, 140, 52, "Spark原理_支撑_调度与集群管理"),   # DAGScheduler 切 stage
    (496, 188, 140, 52, "Spark原理_支撑_调度与集群管理"),   # TaskScheduler 分派 task
    (46, 252, 290, 46, "Spark原理_支撑_存储与缓存"),        # BlockManagerMaster 记 block 位置
    (346, 252, 290, 46, "Spark原理_支撑_Shuffle"),          # MapOutputTracker 记 shuffle 输出
    (46, 310, 590, 70, "Spark原理_支撑_执行模型"),          # driver 职责总览：构建 DAG · 切 stage 拆 task
    (46, 392, 590, 90, "Spark原理_支撑_执行模型"),          # SparkListener 事件总线 → job/stage/task 可视化
    # executor 内：统一内存 / shuffle 交换 / lineage 重算
    (690, 308, 170, 80, "Spark原理_支撑_内存管理"),         # Executor 1：统一内存
    (870, 308, 170, 80, "Spark原理_支撑_Shuffle"),          # Executor 2…N：executor 间 shuffle 交换
    (690, 398, 350, 84, "Spark原理_支撑_容错"),             # executor 职责：丢失按 lineage 重算
]
