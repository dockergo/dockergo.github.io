...
>
> 源码：`~/workdir/redis` unstable @9e5614d（`cluster.c` / `cluster_legacy.c` / `sentinel.c`）。
...
- **槽映射**（`cluster.c:33-61` `keyHashSlot`）：`slot = CRC16(key) & 16383`（即 `% 16384`）。
...
- **MOVED**（`cluster.c:1457`）：目标槽稳定属于另一节点 → 回 `-MOVED <slot> <ip:port>`，客户端**更新本地槽表**并重发（永久性纠正）。
- **ASK**（`cluster.c:1407`）：槽正在迁移中（`MIGRATING`/`IMPORTING`），且 key 已迁到目标节点 → 回 `-ASK <slot> <ip:port>`，客户端**本次**带 `ASKING` 前缀重发到目标（临时性，不更新槽表）。
...
- **消息类型**（`cluster_legacy.h:96`）：`PING`/`PONG`（心跳，携带部分节点状态）/`MEET`（加入集群）/`FAIL`（宣告某节点失败）。
- **失败检测两阶段**：
  - **PFAIL（疑似下线）**：某节点在 `cluster-node-timeout`（默认 15000ms）内未回 PONG，标记它 PFAIL。
  - **FAIL（确认下线）**：`markNodeAsFailingIfNeeded`（`cluster_legacy.c:1913`）——当 `size/2 + 1`（多数主节点）都报告某主 PFAIL，升级为 FAIL 并广播。
- **故障转移**：主库 FAIL 后，其从库发起选举，凭 `configEpoch` 争取多数主节点投票（`failover_auth_count >= 多数`），胜者晋升为新主、接管槽。
...
- **SDOWN（主观下线）**（`sentinel.c:47`）：单个 Sentinel 认为主库不可达。
- **ODOWN（客观下线）**（`sentinel.c:4654`）：达到 `quorum` 个 Sentinel 都认为主库 SDOWN → 确认下线。
...
- `cluster-node-timeout`（默认 15000ms）：失败检测超时，太小易误判、太大切换慢。
- `cluster-require-full-coverage`（默认 yes）：任一槽无主时是否整个集群停止服务。
