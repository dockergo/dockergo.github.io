...
`ResourceManager`（`.../resourcemanager/ResourceManager.java:170`）内部 `RMActiveServices`（`:750`）托管两个关键组件：`ResourceScheduler scheduler`（`:219`，由 `createScheduler:532` 按配置实例化）与 `ApplicationMasterService masterService`（`:222`）。

三方循环：① `NodeManager`（`.../nodemanager/NodeManager.java:100`）经 `NodeStatusUpdater`（`:141`）周期向 RM 心跳，报告本节点可用资源与 Container 状态；② AM 通过 `ApplicationMasterService.allocate`（`ApplicationMasterService.java:390`）向 RM 申请资源（要几个 Container、多大、位置偏好）；③ 调度器根据 NM 报上来的可用资源，把 Container 分给 AM，AM 拿到后请 NM `startContainers`（`ContainerManagerImpl.java:996`）就地拉起进程。
...
- **CapacityScheduler**（`.../scheduler/capacity/CapacityScheduler.java:177`，默认）——层级队列，每队列有**容量保证**（如队列 A 40%、B 60%），空闲时可弹性借用别队列资源，繁忙时回收到保证值。核心 `allocate`（`:1318`）响应 AM 请求，`allocateContainersToNode`（`:573`）在 NM 心跳（`handle` SchedulerEvent `:1960` 的 NODE_UPDATE）时把该节点空闲资源分给合适队列的 AM。
- **FairScheduler**（`.../scheduler/fair/FairScheduler.java:146`）——按公平份额动态均分，新应用逐步抢到平均份额；支持 `continuousScheduling`（`:189`）持续调度而非只在心跳时。
