...
`BlockPlacementPolicyDefault`（`.../server/blockmanagement/BlockPlacementPolicyDefault.java:61`）的 `chooseTarget`（`:174`/`:188`）实现经典三副本策略：

1. **第 1 副本**：`chooseLocalStorage`（`:597`）——若写入者本身是 DataNode 就放本机（写本地零网络），否则随机一台。
2. **第 2 副本**：`chooseRemoteRack`（`:570`）——放到**另一个机架**的节点（跨机架容灾）。
3. **第 3 副本**：`chooseLocalRack`（`:582`）——放到第 2 副本**同机架**的另一台（省跨机架带宽）。

这样「一近两远、两机架」：既保证单机架整体故障不丢数据（2 个机架都挂才丢），又把跨机架流量控制在 1 份。每机架副本上限 `maxNodesPerRack` 防过度集中。选点还要过 `isGoodDatanode` 校验（空间、负载、是否退役）。机架信息来自 `DatanodeManager` 的 `NetworkTopology`（`DatanodeManager.java:114`），由拓扑脚本/表解析（`resolveNetworkLocation:1070`）。

## 复制监控与欠副本重建

![复制与重建](Hadoop原理_支撑_块放置_02复制重建.svg)

副本数是**期望态**，实际态由块汇报维护，二者靠后台线程对账。`BlockManager`（`BlockManager.java:168`）里的 `RedundancyMonitor`（`:5388`，一个 `Daemon` 线程 `:343`）每 3 秒（`DFSConfigKeys.java:303`）跑一轮：

- **欠副本（under-replicated）**：块进 `neededReconstruction`（`:394`）队列，`computeBlockReconstructionWork`（`:2113`）→`computeReconstructionWorkForBlocks`（`:2145`）选源副本 + 目标节点，下发复制命令（随心跳响应带给 DataNode）。每轮处理量按活节点数 × 倍率（`dfs.namenode.replication.work.multiplier.per.iteration` 默认 2，`DFSConfigKeys.java:620`）。
