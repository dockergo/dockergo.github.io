...
DataNode 侧每个 `BPServiceActor`（`.../datanode/BPServiceActor.java:94`，一个 actor 对一个 NameNode）在 `offerService`（`:673`）循环里每 3 秒（`dfs.heartbeat.interval`，`DFSConfigKeys.java:1029`）调 `sendHeartBeat`（`:544`）上报自身容量/使用/剩余/xceiver 数。

NameNode 侧 `DatanodeManager.handleHeartbeat`（`.../blockmanagement/DatanodeManager.java:1856`）更新该节点统计，并**在心跳响应里搭载命令**（`DatanodeCommand[]`）：复制块、删除块、恢复块、重新注册等。这是 NameNode 指挥 DataNode 的**唯一通道**——NameNode 从不主动连 DataNode，全靠「DataNode 来问、NameNode 搭车回令」。

保活判死由 `HeartbeatManager`（`HeartbeatManager.java:47`）的 `Monitor`（`:538`）线程 `heartbeatCheck`（`:441`）负责：超过 `2 × recheck(5min) + 10 × heartbeat(3s)`（约 10.5 分钟）没心跳判定 DataNode 死亡，其上所有块进欠副本队列触发重建。

## 块汇报 · 全量 + 增量对账

![块汇报对账](Hadoop原理_支撑_心跳_02块汇报对账.svg)

- **全量块汇报（Full Block Report）**：`blockReport`（`BPServiceActor.java:386`）默认每 6 小时（`dfs.blockreport.intervalMsec`，`DFSConfigKeys.java:1116`）把本节点所有块列表发给 NameNode。首次汇报走 `processFirstBlockReport`（`BlockManager.java:3278`）直接建映射；后续汇报走 `reportDiff`（`:3349`）比对差异：NameNode 内存有而汇报没有→标记待删/丢失，汇报有而内存没有→加入映射或标记多余。
- **增量块汇报（IBR）**：`IncrementalBlockReportManager`（`BPServiceActor.java:125`）在块新增/删除时**即时**（不等 6 小时）上报，让映射快速收敛。
