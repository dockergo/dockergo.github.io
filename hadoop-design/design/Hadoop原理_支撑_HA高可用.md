...
Active NameNode 不再只写本地 EditLog，而是通过 `QuorumJournalManager`（`.../hdfs/qjournal/client/QuorumJournalManager.java:72`）把每批 edits **并行写到一组 JournalNode（通常 3 个）**，用 `AsyncLoggerSet loggers`（`:106`）等待**多数派（N/2+1）**确认才算成功——2/3 成功即可，容忍 1 个 JN 挂。

每个 JournalNode 的 `Journal`（`.../qjournal/server/Journal.java:85`）用 **epoch（纪元号）**防脑裂：`newEpoch`（`:339`）在 NameNode 成为 writer 时抢一个更大的 epoch，`checkRequest`（`:484`）拒绝 epoch 更小的旧 writer 的 `journal`（`:393`）写入——即使旧 Active 未死，它的 epoch 已过期，写不进 quorum，无法产生分裂的日志。

Standby 侧 `EditLogTailer`（`.../server/namenode/ha/EditLogTailer.java:73`）循环 `doTailEdits`（`:341`）从 JournalNode 拉取新 edits 重放到自己的内存树，保持与 Active 近实时同步；它同时承担检查点 `StandbyCheckpointer`（`StandbyCheckpointer.java:63`，`doCheckpoint:193`）。

## ZKFC 自动故障切换 + Fencing

![自动切换与 fencing](Hadoop原理_支撑_HA_02自动切换.svg)

每个 NameNode 旁跑一个 `ZKFailoverController`（`hadoop-common/.../ha/ZKFailoverController.java:65`，HDFS 入口 `DFSZKFailoverController.java:63`）：

- **健康监控**：本地探测 NameNode 健康。
- **选主**：用 `ActiveStandbyElector elector`（`:112`）在 ZooKeeper 上抢一个临时锁 znode，抢到的 `becomeActive`（`:411`）晋升为 Active，其余 `becomeStandby`（`:517`）。Active 崩溃→锁 znode 消失→Standby 的 ZKFC 感知并抢锁接管。
- **Fencing 防脑裂**：接管前 `fenceOldActive`（`:535`）确保旧 Active 不再写——sshfence（SSH 过去 kill 进程）或 shell fence。加上 JournalNode 的 epoch 机制，形成**双重防脑裂**：即便 fencing 失败，旧 Active 也因 epoch 过期写不进 quorum。
