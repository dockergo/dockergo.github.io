...
_ap = argparse.ArgumentParser(description="生成 Trino 引擎交互式核心原理图谱(离线自包含 HTML)")
...
THEMES = [
    # ── 接口主线 · 用户可见 · Trino 是联邦查询引擎(原型 B):DQL 为主,DCL 贯穿,DDL/DML 下推连接器 ──
    {"id": "qlife", "icon": "◷", "title": "DQL 数据查询", "cat": "iface", "ord": 0,
     "desc": "主接touch面:一条 SELECT 从文本到结果。design 走查——查询执行全景 → 编排链路(Parser→Analyzer→Planner→Optimizer→Fragmenter)→ 逻辑计划 → 优化器(迭代规则+局部CBO)→ Join 优化 → 分布式化 → 执行六级层次 → Driver 泵 → 数据交换 → 动态过滤 → 下推,附调优/误区/总纲",
     "tabs": ["qlwalk"]},
    {"id": "dcl", "icon": "⛚", "title": "DCL 数据控制", "cat": "iface", "ord": 1,
     "desc": "安全(横切所有主线):design 走查——安全全景(认证+两层访问控制)→ 认证链(有序 Authenticator)→ 两层访问控制(System+Connector)→ 行过滤/列掩码,附调优/误区/总纲",
     "tabs": ["dclwalk"]},

    # ── 支撑主线 · 引擎内部(8 条)· 严格以 design 走查为主干 ──
    {"id": "storageformat", "icon": "▤", "title": "连接器框架", "cat": "support", "ord": 0,
     "desc": "原型 B 定义性特征(数据入口 SPI):design 走查——连接器 SPI 全景(五大提供者)→ Split 生命周期 → 句柄不透明模型 → 下推契约(apply* 迭代)→ 动态过滤消费,附调优/误区/总纲",
     "tabs": ["stewalk"]},
    {"id": "opttech", "icon": "⬢", "title": "元数据与 Catalog", "cat": "support", "ord": 1,
     "desc": "最底座(schema/统计现取):design 走查——Catalog 映射(CatalogManager/MetadataManager 路由)→ 无持久元数据 → 集群协调(discovery+心跳,无共识)→ system 自省表,附调优/误区/总纲",
     "tabs": ["optwalk"]},
    {"id": "execengine", "icon": "⚡", "title": "执行引擎", "cat": "support", "ord": 2,
     "desc": "单 worker 运行时内核:design 走查——WorkProcessor 算子(Transformation 桥接)→ 关键算子(聚合/Join/扫描/交换)→ Page/Block 列式内存(sealed 三型),附调优/误区/总纲",
     "tabs": ["execwalk"]},
    {"id": "transaction", "icon": "⬡", "title": "调度与资源", "cat": "support", "ord": 3,
     "desc": "保障能力域:design 走查——Stage 调度(按 PartitioningHandle 选调度器)→ 节点选择(NodeScheduler/NodeSelector)→ 资源组(准入判定)→ 调度策略(SchedulingPolicy 四型),附调优/误区/总纲",
     "tabs": ["txnwalk"]},
    {"id": "metadata", "icon": "◱", "title": "内存管理", "cat": "support", "ord": 4,
     "desc": "查询期内存:design 走查——追踪层级(LocalMemoryContext→Query→Pool)→ 单池模型(483 无 reserved/system 池)→ 溢写(revocable→disk)→ OOM killer(集群仲裁择杀),附调优/误区/总纲",
     "tabs": ["metawalk"]},
    {"id": "workload", "icon": "◐", "title": "容错执行 FTE", "cat": "support", "ord": 5,
     "desc": "长查询容错:design 走查——RetryPolicy 三档(NONE/QUERY/TASK)→ 任务级重试(每分区独立+内存增长)→ spooling exchange(暂存去重)→ QUERY vs TASK 对比,附调优/误区/总纲",
     "tabs": ["wgwalk"]},
    {"id": "tabletsched", "icon": "◄", "title": "查询优化(内含 DQL)", "cat": "support", "ord": 6,
     "desc": "优化技术(与 DQL 优化器篇同源,单独主题便于深挖):迭代规则改写 + Memo + 局部 CBO(ReorderJoins DP / DetermineJoinDistributionType)。图见 DQL 优化器/Join 优化篇",
     "tabs": ["tswalk"]},
    {"id": "compaction", "icon": "▨", "title": "数据交换", "cat": "support", "ord": 7,
     "desc": "stage 间数据移动(与 DQL 数据交换篇同源,单独主题便于深挖):流式直连(HTTP 拉 Page)vs FTE spooling 暂存;REPARTITION/REPLICATE/GATHER 三种分区去向。图见 DQL 数据交换篇",
     "tabs": ["cpwalk"]},
...
_ARCH_SVG_B64 = open(
    os.path.join(_DESIGN_DIR, "Trino原理_总架构图.svg"),
    encoding="utf-8").read()
_ARCH_SVG_B64 = _b64.b64encode(_ARCH_SVG_B64.encode("utf-8")).decode("ascii")

# (x, y, w, h, theme_id, 标签)  y 已加上 +70 组偏移
_ARCH_HOTSPOTS = [
    # 坐标取自 Trino原理_总架构图.svg(viewBox 1080×880)。coordinator 编排链行 rect y=208;
    # 集群守护行 y=300;安全入口 y=386;worker 集群 x=690;底部外部数据生态 y=452。
    # 接口主线 DQL/DCL 未在架构图直接标注 → 作为 chip 出现(见 _ARCH_ALWAYS_CHIP)。
    # coordinator 编排链(前台):规划/优化 → 优化能力域;切分/分发 + 调度
    (306, 208, 118, 58, "tabletsched",   "规划/优化"),
    (436, 208, 118, 58, "execengine",    "执行(worker 侧内核)"),
    # 集群守护行(rect y=300):资源组 → 调度与资源;集群内存仲裁 → 内存管理
    (46,  300, 196, 52, "transaction",   "资源组/调度"),
    (254, 300, 196, 52, "metadata",      "集群内存仲裁"),
    # 安全入口(rect y=386)→ DCL
    (46,  386, 608, 36, "dcl",           "安全 · 认证+访问控制"),
    # worker 集群(rect x=690 y=132 w=360 h=300):执行/交换/连接器读
    (690, 300, 328, 52, "compaction",    "Exchange 数据交换"),
    (690, 362, 328, 52, "storageformat", "连接器读写(PageSource)"),
    (706, 216, 316, 76, "execengine",    "Task/Driver/Operator 执行"),
    # 容错(FTE)贴在 Exchange/spooling 语义上
    (690, 132, 360, 46, "workload",      "Worker 集群(FTE 可重试)"),
    # 元数据与 Catalog(底座):底部外部数据生态经连接器接入 → 元数据路由
    (30,  452, 1020, 86, "opttech",      "外部数据生态 · Catalog 接入"),
]
_arch_hotspots_html = "\n".join(
    ...
    'data-theme-id="{tid}" title="{lab} → {ttitle}"><span class="arch-hot-lab">{lab}</span></button>'.format(
        lp=x/1080*100, tp=y/880*100, wp=w/1080*100, hp=h/880*100,
...
_SVG_WALK_SPECS = {
    # ---- 接口主线(2 条:DQL 主 + DCL;DDL/DML 下推连接器,不单列)----
    "qlwalk": [("查询执行全景 · 文本到结果", "Trino原理_DQL_01执行全景.svg"),
               ("编排链路 · Parser→Analyzer→Planner→Optimizer→Fragmenter", "Trino原理_DQL_02编排链路.svg"),
               ("逻辑计划 · PlanNode 树", "Trino原理_DQL_03逻辑计划.svg"),
               ("优化器 · 迭代规则改写到定点(非 Cascades)", "Trino原理_DQL_04优化器.svg"),
               ("CBO · Join 重排(子集 DP)+ 分布决策", "Trino原理_DQL_05Join优化.svg"),
               ("分布式化 · PlanFragmenter 剪 REMOTE exchange", "Trino原理_DQL_06分布式化.svg"),
               ("执行六级 · Query→Stage→Task→Split→Driver→Operator", "Trino原理_DQL_07执行层次.svg"),
               ("Driver 泵 · 算子间推拉 Page", "Trino原理_DQL_08Driver泵.svg"),
               ("数据交换 · 流式直连 vs FTE 暂存", "Trino原理_DQL_09数据交换.svg"),
               ("动态过滤 · 构建侧谓词回推探测侧", "Trino原理_DQL_10动态过滤.svg"),
               ("下推 · applyFilter/applyProjection 迭代", "Trino原理_DQL_11下推.svg")],
    "dclwalk": [("安全全景 · 认证 + 两层访问控制", "Trino原理_DCL_01安全全景.svg"),
                ("认证链 · 有序 Authenticator(first-success-wins)", "Trino原理_DCL_02认证.svg"),
                ("两层访问控制 · System + Connector 叠加", "Trino原理_DCL_03两层控制.svg"),
                ("行过滤/列掩码 · ViewExpression 两层合并", "Trino原理_DCL_04行列安全.svg")],
    # ---- 支撑主线(8 条)----
    "stewalk": [("连接器 SPI 全景 · 五大提供者", "Trino原理_连接器_01SPI全景.svg"),
                ("Split 生命周期 · SplitManager→SplitSource→Task", "Trino原理_连接器_02Split.svg"),
                ("句柄不透明模型 · TableHandle 连接器私有", "Trino原理_连接器_03句柄.svg"),
                ("下推契约 · apply* 返回已推+剩余,可多轮", "Trino原理_连接器_04下推契约.svg"),
                ("动态过滤 SPI · 连接器读前查询裁剪", "Trino原理_连接器_05动态过滤.svg")],
    "optwalk": [("Catalog 映射 · CatalogManager/MetadataManager 路由", "Trino原理_元数据_01Catalog映射.svg"),
                ("无持久元数据 · schema/统计现取", "Trino原理_元数据_02无持久.svg"),
                ("集群协调 · discovery+心跳,无 Raft/共识", "Trino原理_元数据_03集群协调.svg"),
                ("system 连接器 · runtime/metadata 自省表", "Trino原理_元数据_04自省.svg")],
    "execwalk": [("WorkProcessor 算子 · Transformation 桥接", "Trino原理_执行_01WorkProcessor.svg"),
                 ("关键算子 · 聚合/Join/扫描/交换", "Trino原理_执行_02关键算子.svg"),
                 ("Page/Block 列式内存 · sealed 三型", "Trino原理_执行_03列式内存.svg")],
    "txnwalk": [("Stage 调度 · 按 PartitioningHandle 选调度器", "Trino原理_调度_01Stage调度.svg"),
                ("节点选择 · NodeScheduler/NodeSelector", "Trino原理_调度_02节点选择.svg"),
                ("资源组 · 准入判定(立即跑/排队/拒绝)", "Trino原理_调度_03资源组.svg"),
                ("调度策略 · SchedulingPolicy 四型 × 队列", "Trino原理_调度_04调度策略.svg")],
    "metawalk": [("内存追踪层级 · LocalMemoryContext→Query→Pool", "Trino原理_内存_01追踪层级.svg"),
                 ("单池模型 · 483 无 reserved/system 池", "Trino原理_内存_02单池模型.svg"),
                 ("溢写 · revocable 内存落盘", "Trino原理_内存_03溢写.svg"),
                 ("OOM killer · 集群仲裁择一杀之", "Trino原理_内存_04OOMkiller.svg")],
    "wgwalk": [("RetryPolicy · NONE/QUERY/TASK 与调度器映射", "Trino原理_容错_01RetryPolicy.svg"),
               ("任务级重试 · 每分区独立 + 内存增长", "Trino原理_容错_02任务重试.svg"),
               ("spooling exchange · 暂存与 committed 去重", "Trino原理_容错_03spooling.svg"),
               ("QUERY vs TASK · 粒度与代价对比", "Trino原理_容错_04对比.svg")],
    # 查询优化 / 数据交换 两个支撑主题与 DQL 篇同源,直接复用 DQL 图(避免图表重复)
    "tswalk": [("优化器 · 迭代规则改写(与 DQL 优化器篇同源)", "Trino原理_DQL_04优化器.svg"),
               ("CBO · Join 重排 + 分布决策", "Trino原理_DQL_05Join优化.svg"),
    ...
    "panowalk": [("双维模型 · 能力域 × 执行时机", "Trino原理_双维模型.svg"),
                 ...
                 ("依赖矩阵 · 接口主线 × 支撑能力域", "Trino原理_依赖矩阵.svg"),
                 ("能力域依赖关系", "Trino原理_依赖关系图.svg")],
}
# 快速开始「上手总览」用独立复合渲染器 renderQsTour(总览 SVG + 5 步选择器 + 内容区),不走通用 SVG-walk
_QSTOUR_OVERVIEW_B64 = _design_b64("Trino上手_00总览.svg")  # Trino 无此素材 → 空;quickstart 主题未挂载,仅占位
...
_WALK_PROSE = {
    "qlwalk": "Trino原理_DQL数据查询.md",  "dclwalk": "Trino原理_DCL数据控制.md",
    "stewalk": "Trino原理_支撑_连接器框架.md", "optwalk": "Trino原理_支撑_元数据与Catalog.md",
    "execwalk": "Trino原理_支撑_执行引擎.md", "txnwalk": "Trino原理_支撑_调度与资源.md",
    "metawalk": "Trino原理_支撑_内存管理.md",  "wgwalk": "Trino原理_支撑_容错执行.md",
}
...
<title>Trino 引擎 · 交互式核心原理图谱</title>
<style>
:root{
...
<div id="loadingOverlay" role="status" aria-live="polite">
  <div class="lo-inner">
    ...
    <div class="lo-title">Trino Engine Atlas</div>
...
<div id="app">
  <header>
    ...
    <a href="https://github.com/trinodb/trino" target="_blank" rel="noopener" title="GitHub 源码仓库" style="margin-left:auto;display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M12 .5C5.7.5.5 5.7.5 12c0 5.1 3.3 9.4 7.9 10.9.6.1.8-.2.8-.6v-2c-3.2.7-3.9-1.4-3.9-1.4-.5-1.3-1.3-1.7-1.3-1.7-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1 1.8 2.7 1.3 3.4 1 .1-.8.4-1.3.7-1.6-2.6-.3-5.3-1.3-5.3-5.8 0-1.3.5-2.3 1.2-3.1-.1-.3-.5-1.5.1-3.1 0 0 1-.3 3.3 1.2a11.4 11.4 0 0 1 6 0C17.3 4.7 18.3 5 18.3 5c.6 1.6.2 2.8.1 3.1.8.8 1.2 1.8 1.2 3.1 0 4.5-2.7 5.5-5.3 5.8.4.4.8 1.1.8 2.2v3.3c0 .4.2.7.8.6 4.6-1.5 7.9-5.8 7.9-10.9C23.5 5.7 18.3.5 12 .5z"/></svg></a><a href="https://trino.io" target="_blank" rel="noopener" title="项目官网" style="display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><img src="data:image/svg+xml;base64," width="18" height="18" alt="官网" style="display:block"/></a><button class="theme-toggle" id="themeToggle" title="切换深色 / 浅色主题" aria-label="切换主题">
