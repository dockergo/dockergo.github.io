...
- **解析（ANTLR4，两阶段预测）**：`SqlParser.createStatement`（`core/trino-parser/.../SqlParser.java:104`）经 `invokeParser`（同文件 `:144`）先用快速的 `SLL` 预测模式配 `BailErrorStrategy`（`SqlParser.java:162-163`），抛 `ParseCancellationException` 再 `parser.reset()` 回退 `LL` 模式重试（`SqlParser.java:167-171`）；`AstBuilder`（`core/trino-parser/.../AstBuilder.java:413`）把 ANTLR 解析树转成 `io.trino.sql.tree.*` 的 `Statement` AST。语法定义在独立的 `trino-grammar` 模块（`SqlBase.g4`）。
- **分析产出的是"旁表"不是新树**：`Analyzer.analyze`（`core/trino-main/.../sql/analyzer/Analyzer.java:91`）先 `statementRewrite.rewrite`（同文件 `:93`；展开视图、`SHOW`→`SELECT`、预处理参数），再 `StatementAnalyzer`（`StatementAnalyzer.java:449`）递归下降构建 `Scope`/`RelationType`/`Field`，`ExpressionAnalyzer` 解析类型与函数——所有结果写进一个可变的 `Analysis` 对象。planner 从 `Analysis` 读类型/函数/作用域，**AST 本身不被改写**。分析末尾做访问控制（对每个引用的表/列 `checkCanSelectFromColumns`，`Analyzer.java:105`）。
- **逻辑计划以 `OutputNode` 为根**：`LogicalPlanner.plan`（`core/trino-main/.../sql/planner/LogicalPlanner.java:244` → `planStatement` `:349`）→ `RelationPlanner`（FROM/JOIN/集合运算）+ `QueryPlanner`（每个 SELECT 的子句链）构建 `PlanNode` 树；`OutputNode` 永远是 SELECT 的逻辑根（`LogicalPlanner.java:355`）。子句成节点的顺序：FROM→WHERE(`FilterNode`)→聚合(`AggregationNode`)→HAVING→窗口(`WindowNode`)→投影(`ProjectNode`)→DISTINCT→ORDER BY/LIMIT。
...
- **`IterativeOptimizer` 是"规则改写到定点"，不是自顶向下代价搜索。** 它（`core/trino-main/.../sql/planner/iterative/IterativeOptimizer.java:66`）把计划放进 `Memo`，但 `Memo` 的每个 `Group` 只持有**单个** `PlanNode`（`Memo.java:257` 的 `private PlanNode membership;`），由 `Memo.replace`（`Memo.java:116`）就地替换——不是 Cascades 那种"一组等价表达式"。`Lookup.resolve` 的 javadoc 明写 "assumes group contains only one node"（`Lookup.java:28`）。驱动逻辑 `exploreGroup`（`IterativeOptimizer.java:151`）/`exploreNode`（`:171`）对每个节点反复套用 `RuleIndex` 里匹配的规则，直到没有规则再触发（fixpoint）。`Memo` 在这里是**改写效率结构**（避免不可变树的全量重建），不是代价搜索的解空间。
- **CBO 是"局部"的**：全局是规则改写，只有 Join 顺序与分布这两处是真正的代价枚举。
- **优化批次是有序流水线**：`PlanOptimizers`（`core/trino-main/.../sql/planner/PlanOptimizers.java:275`）把数十个 `IterativeOptimizer` 批次按固定顺序排列——谓词下推、投影下推、去关联子查询、下推进 TableScan（`applyFilter`/`applyProjection` 等 SPI）、消除 Cross Join、Join 重排（`PlanOptimizers.java:817`）、加 Exchange（`AddExchanges`，`PlanOptimizers.java:911`）、动态过滤下推……`AddExchanges` 之后代价计算器从"含预估 exchange"切换为"不含"（`PlanOptimizers.java:916` 注释 "use cost calculator without estimated exchanges after AddExchanges"）。
...
- **`ReorderJoins`（Join 顺序）**（`core/trino-main/.../iterative/rule/ReorderJoins.java:98`）：仅在 `join_reordering_strategy=AUTOMATIC` 时启用，只跑一次。先把可重排的 INNER Join 树压平成 `MultiJoinNode`（`toMultiJoinNode`，`ReorderJoins.java:576`；受 `max_reordered_joins` 上限约束），再用 **`JoinEnumerator`（`ReorderJoins.java:168`）做记忆化动态规划**：`chooseJoinOrder`（`:240`）对连通子集 `generatePartitions`（`:285`）穷举"分成两半"的所有方式，自底向上 DP，`memo` 缓存每个子集的最优结果。这是经典的子集 DP 枚举，**不是 DPhyp/超图**——别写成 DPhyp。
- **`DetermineJoinDistributionType`（Join 分布）**（`core/trino-main/.../iterative/rule/DetermineJoinDistributionType.java:51`）：`AUTOMATIC` 时枚举 `PARTITIONED`（双边按 key 重分区）vs `REPLICATED`（广播小表）两种分布 × 两种左右顺序，取代价最小；代价不可估时回退按大小的启发式 `getSizeBasedJoin`（`:114`）——`join_max_broadcast_table_size` + **8× 尺寸差阈值**翻转构建侧（`SIZE_DIFFERENCE_THRESHOLD = 8`，`DetermineJoinDistributionType.java:55`）。

`DetermineJoinDistributionType` 必须在 `AddExchanges` 之前跑（`PlanOptimizers.java:878` 注释 "Must run before AddExchanges"）。分布决定了后续插入哪种 `ExchangeNode`。
...
`PlanFragmenter`（`core/trino-main/.../sql/planner/PlanFragmenter.java:96`）把优化后的单棵逻辑计划切成 `SubPlan` 树（fragment 树）：

- **剪点是 REMOTE 的 `ExchangeNode`**。`Fragmenter.visitExchange`（`PlanFragmenter.java:508`；内部类 `Fragmenter` 见 `:245`）：`LOCAL` exchange 留在 fragment 内不切；`REMOTE`（`ExchangeNode.Scope.REMOTE`，`PlanFragmenter.java:88` import）exchange 处把子树拆成独立子 fragment，父 fragment 里对应位置替换成 `RemoteSourceNode`（`Fragmenter.visitRemoteSource`，`PlanFragmenter.java:483`；携带子 fragment id + 交换类型）。`SubPlan` 的结构不变量：父 fragment 的 `RemoteSourceNode` 源 id 多重集 == 子 fragment id 多重集。
...
- **Query**：`SqlQueryExecution.start`（`core/trino-main/.../execution/SqlQueryExecution.java:397`）驱动全程；按 `RetryPolicy` 选 `PipelinedQueryScheduler`（NONE/QUERY）或 `EventDrivenFaultTolerantQueryScheduler`（TASK）——分派在 `SqlQueryExecution.java:537` 的 `switch (retryPolicy)`。
- **Stage**：**1 fragment : 1 stage**（`StageManager.create`，`core/trino-main/.../execution/scheduler/StageManager.java:66`，广度遍历 SubPlan，每 fragment 建一个 `SqlStage`，stage id 直接取自 fragment id）。注意两个对象：`SqlStage`（持有 `RemoteTask`）vs `StageExecution`/`PipelinedStageExecution`（调度与状态机）。
- **Task**：一个 stage fan-out 成 N 个 task（数量由该 fragment 的 `PartitioningHandle` 经 `createStageScheduler` 决定：`FixedCountScheduler`（`FixedCountScheduler.java:28`）一节点一 task、`SourcePartitionedScheduler`（`SourcePartitionedScheduler.java:55`）随 split 动态、`ScaledWriterScheduler`（`ScaledWriterScheduler.java:41`）随数据量增长）。coordinator 端每 task 是一个 `RemoteTask`（`HttpRemoteTask`，`core/trino-main/.../server/remotetask/HttpRemoteTask.java:137`），经 `POST /v1/task/{taskId}` 远程创建（worker 端 REST 入口 `TaskResource.createOrUpdateTask`，`core/trino-main/.../server/TaskResource.java:142`，类上 `@Path("/v1/task")` 见 `:93`）；worker 端 `SqlTask`（`SqlTask.java:87`）容器包着 `SqlTaskExecution`（`SqlTaskExecution.java:82`）运行实例。
...
- **Driver / Operator**：一个 pipeline = 一个 `DriverFactory`（`core/trino-main/.../operator/DriverFactory.java:31`；一串 `OperatorFactory`）；一个 `Driver` = 从该 factory 实例化的一条算子链。`LocalExecutionPlanner`（`core/trino-main/.../sql/planner/LocalExecutionPlanner.java:412`）把 fragment 编译成多个 pipeline，每遇 local/remote exchange 边界起新 pipeline。
...
- **`Operator` 契约**是协作式状态机（`core/trino-main/.../operator/Operator.java:21`）：`isBlocked` 返回 future（算子从不阻塞线程，`:33`）、`needsInput`（`:41`）/`addInput(Page)`（`:47`）收输入、`getOutput` 出输出（无则返 `null`，`:53`）、`finish`（`:88`）/`isFinished` 收尾，外加 `startMemoryRevoke`（`:72`）/`finishMemoryRevoke` 溢写钩子。
- **`Driver.processInternal` 是泵**（`core/trino-main/.../operator/Driver.java:372`）：先 `processNewSources`（`Driver.java:378`）把新到的 split 喂给源算子，再遍历相邻算子对（`Driver.java:390` 的 for 循环）——若 `current` 未完成且 `next.needsInput`，就 `page = current.getOutput`（`:402`）然后 `next.addInput(page)`（`:407`），front-to-back 一次搬一个 Page；`current` 完成则 `next.finish`。单个 Driver 单线程（持独占锁，见 `Driver.java:66` 类）。
- **`Page` 是列式批**：N 个 `Block`（列）× M 行（positions）。`Block` 是 `sealed interface`，只允许 `DictionaryBlock`/`RunLengthEncodedBlock`/`ValueBlock` 三种（`core/trino-spi/.../spi/block/Block.java:21-24` 的 `permits`），编码包装经 `getUnderlyingValueBlock` 解引用。**Page 大小按字节界定（默认上限 1MB），不是固定 1024 行**——说"约 1MB 的列式批/几千行"，别写"每页 1024 行"。
- **执行器与时间片**：`Driver` 被 `TaskExecutor` 以 1 秒 `SPLIT_RUN_QUANTA` 时间片调度。Trino 483 **默认 `ThreadPerDriverTaskExecutor`（一 driver 一线程）**（`core/trino-main/.../execution/executor/dedicated/ThreadPerDriverTaskExecutor.java:60`，配额定义于 `dedicated/SplitProcessor.java:42`），可切换回经典的 `TimeSharingTaskExecutor`（多级反馈队列时间片，`timesharing/TimeSharingTaskExecutor.java:85`，配额 `timesharing/PrioritizedSplitRunner.java:49`）；两者都用 1 秒配额。
