...
`Connector` 接口是每个连接器的入口，向引擎交付五类能力：

- `getMetadata` → **`ConnectorMetadata`**：列库/表/列、表统计、以及全部下推方法。
- `getSplitManager` → **`ConnectorSplitManager`**：`getSplits` 把一次扫描拆成 `ConnectorSplitSource`（split 流）。
- `getPageSourceProvider` → **`ConnectorPageSource`**：把一个 split 读成 `Page`（列式）。
- `getPageSinkProvider` → **`ConnectorPageSink`**：写入（DML/DDL-CTAS 下推）。
- `getTransactionHandle`：连接器级事务句柄（每查询/每语句范围）。
...
`ConnectorSplitManager.getSplits` 返回 `ConnectorSplitSource`（可异步、可分批产 split）。引擎侧 `SourcePartitionedScheduler` 分批 `getNextBatch` 拉 split，经 `SplitPlacementPolicy`/`NodeScheduler` 分配到 worker 的 task，**源 pipeline 一个 split 一个 Driver**。`Split` = `CatalogHandle` + 连接器私有的 `ConnectorSplit`（引擎不解析其内容，只当句柄传递）。split 是否可远程访问、split 权重都由连接器声明。
...
`ConnectorTableHandle`/`ColumnHandle`/`ConnectorSplit` 都是**连接器私有的不透明对象**——引擎持有并传递它们，但不解析内部。下推的结果就"累积"在 `TableHandle` 里（如"已下推的 filter/limit"），最后 `PageSource` 读数据时按它裁剪。这个设计让引擎与连接器解耦：引擎只认 SPI 接口，具体语义（分区、bucket、下推能力）全封装在连接器的句柄实现中。
...
`ConnectorMetadata` 上一族 `apply*` 方法是下推的钩子：`applyFilter`/`applyProjection`/`applyAggregation`/`applyLimit`/`applyTopN`/`applyJoin`/`applyTableScanRedirect`。每个返回一个"结果"对象，含**新 TableHandle（吸收了已下推部分）+ 剩余表达式（引擎自己处理）**。优化器迭代调用：推一部分、把消化掉的从计划里摘掉、再尝试下一个。连接器决定能吃下多少——JDBC 连接器把 filter 变 SQL WHERE、Hive 变分区裁剪、不支持的连接器返回空（引擎侧全算）。（DQL 篇有跨引擎对照，本篇讲 SPI 契约本身。）
...
`io.trino.spi.connector.DynamicFilter` 是 SPI 侧接口：连接器在 `getSplits`（split 级裁剪）或 `PageSource` 读取（行组/文件级裁剪）前，查询这个运行时才就绪的谓词，跳过不匹配的数据。构建侧（Join 小表）跑完后引擎把实际值域下发，连接器据此把大表扫描量降下来。（规划期如何织入见【DQL】动态过滤篇，本篇讲连接器如何消费。）
