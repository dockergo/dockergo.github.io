#!/usr/bin/env python3
"""gin-design 交互式核心原理图谱生成器（自包含 · 离线 · 双主题）。
...
_ap = argparse.ArgumentParser(description="生成 Gin 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("GIN_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("GIN_DESIGN_OUT") or os.path.join(HERE, "index.html")
...
# ===================================================================== #
MAINLINES = [
    ("Gin原理_全景主线框架", "pano", "◇", "全景主线框架",
     "新家族 Go Web 框架：双维模型 · 总架构 · 依赖矩阵 · 依赖关系图 · 三条贯穿声明"),

    ("Gin原理_接触面_路由与ContextAPI", "iface", "⚙", "接触面 · 路由注册与 Context API",
     "r.GET(path,handler) 注册 · c.Param/Query/Bind 读请求 · c.JSON/String 写响应"),

    ("Gin原理_支撑_引擎与路由树", "support", "▤", "引擎与路由树(radix)",
     "每 method 一棵 radix 树 · node indices 首字节索引 · O(路径长)匹配 · :param/*wildcard"),
    ("Gin原理_支撑_Context与对象池", "support", "✦", "Context 与对象池(灵魂)",
     "灵魂：sync.Pool 每请求复用 Context · reset [:0] 复用 backing array · 零重分配低 GC"),
    ("Gin原理_支撑_中间件链", "support", "◧", "中间件链",
     "洋葱模型：HandlersChain + c.Next() int8 index 推进 · Abort 设 63 中断 · Use 注册"),
    ("Gin原理_支撑_请求处理流程", "support", "◉", "请求处理流程",
     "ServeHTTP：pool.Get 取 Context→路由匹配→c.Next() 跑链→pool.Put 还"),
    ("Gin原理_支撑_绑定与渲染", "support", "◫", "绑定与渲染",
     "binding 按 Content-Type 自动绑 + validator/v10 · render 按类型渲染 + 内容协商"),
    ("Gin原理_支撑_RouterGroup", "support", "▩", "RouterGroup",
     "前缀共享 + 中间件继承 · /api 组挂鉴权组内共享 · 嵌套前缀累积"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接触面主线 · 开发者如何用(路由 + Context)"),
    ("support", "支撑主线 · 框架内部(6 条能力域)"),
]

PANO_NAME = "Gin原理_全景主线框架"
ARCH_W, ARCH_H = 1080, 760
ARCH_HOTSPOTS = [
    (0, 0, 1080, 58, "Gin原理_全景主线框架"),
    # net/http 接入带 → 请求处理流程（入口）
    (30, 70, 1020, 60, "Gin原理_支撑_请求处理流程"),
    # 接触面带
    (30, 146, 1020, 58, "Gin原理_接触面_路由与ContextAPI"),
    # 请求主干 4 段（① 匹配 ② 取Context ③ 中间件 ④ handler）
    (48, 258, 216, 78, "Gin原理_支撑_引擎与路由树"),
    (286, 258, 216, 78, "Gin原理_支撑_Context与对象池"),
    (524, 258, 216, 78, "Gin原理_支撑_中间件链"),
    (762, 258, 216, 78, "Gin原理_支撑_请求处理流程"),
    # 支撑设施带
    (30, 368, 330, 150, "Gin原理_支撑_引擎与路由树"),
    (375, 368, 330, 150, "Gin原理_支撑_Context与对象池"),
    (720, 368, 330, 150, "Gin原理_支撑_中间件链"),
    # 底部
    (30, 534, 502, 150, "Gin原理_支撑_绑定与渲染"),
    (548, 534, 502, 150, "Gin原理_支撑_RouterGroup"),
]
ARCH_ALWAYS_CHIP = []

BRAND_TITLE = "一切知识皆索引"
BRAND_SUB = "Gin"
HOME_DESC = ("Gin 核心原理设计文档库的离线交互图谱——新家族(Go Web 框架：薄封装 net/http，radix 树路由 + Context 对象池 + 索引式中间件链，低分配高吞吐)。"
             "9 条主线、手绘原理图，全部回 gin-gonic/gin 源码核实。点击项目总架构图任意模块即可下钻到对应主线。")
ARCH_SVG_NAME = "Gin原理_总架构图.svg"
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="Gin 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
(function(){
  ...
  var saved=localStorage.getItem('gin-atlas-theme');
  ...
  function toggleTheme(){
    ...
    localStorage.setItem('gin-atlas-theme',cur);
