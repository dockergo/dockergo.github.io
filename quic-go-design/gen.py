#!/usr/bin/env python3
"""quic-go-design 交互式核心原理图谱生成器（自包含 · 离线 · 双主题）。
...
用法：
  cd quic-go-design && python3 gen.py
...
_ap = argparse.ArgumentParser(description="生成 quic-go 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("QUIC_GO_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("QUIC_GO_DESIGN_OUT") or os.path.join(HERE, "index.html")

# ===================================================================== #
# 一、主线注册表 —— 唯一需随项目调整的数据块
#     家族 8（传输协议库 · QUIC/HTTP3 · quic-go 纯 Go）：元模式 = 接口 × 能力域 × 时机。
#     全景 + 3 接口主线 + 8 支撑能力域。
# ===================================================================== #
MAINLINES = [
    ("quic-go原理_全景主线框架", "pano", "◇", "全景主线框架",
     "家族 8 传输协议库：双维模型 · 总架构 · 事件循环 · 依赖矩阵 · 三条贯穿声明"),

    ("quic-go原理_接口_会话与连接", "iface", "⚙", "会话与连接",
     "对象树 Transport/Listener/Conn/Stream · Go struct + io.Reader/io.Writer"),
    ("quic-go原理_接口_事件循环与IO", "iface", "⇄", "事件循环与 IO",
     "灵魂：listen 读 goroutine / run() select 循环 / sendQueue 异步发 · 自管 socket"),
    ("quic-go原理_接口_HTTP与流", "iface", "◷", "HTTP 与流",
     "http3 包实现 Handler/RoundTripper · 请求映射到 QUIC 流 · 无队头阻塞"),

    ("quic-go原理_支撑_包与帧编解码", "support", "▤", "包与帧编解码",
     "wire 逐帧解析 / packet_packer 组装 · UDP→包→帧 · varint · coalesce"),
    ("quic-go原理_支撑_TLS握手与加密", "support", "⛨", "TLS 握手与加密",
     "灵魂：TLS 1.3 内嵌 · 复用 Go crypto/tls · 三加密级 · AEAD · 密钥轮换"),
    ("quic-go原理_支撑_流与流量控制", "support", "▦", "流与流量控制",
     "一连接多流无队头阻塞 · 流级/连接级两级窗 · auto-tuning"),
    ("quic-go原理_支撑_丢包检测与恢复", "support", "◉", "丢包检测与恢复",
     "单调包号 · sentPacketHandler · 包序阈值 3 / 时间阈值 9/8 · PTO · 重传新包号"),
    ("quic-go原理_支撑_拥塞控制", "support", "⚡", "拥塞控制",
     "可插拔 SendAlgorithm · 仅内置 Cubic/Reno（无 BBR）· pacer 平滑"),
    ("quic-go原理_支撑_连接管理与迁移", "support", "◫", "连接管理与迁移",
     "Connection ID 标识非四元组 · connIDManager/generator · PATH_CHALLENGE 换网不断"),
    ("quic-go原理_支撑_HTTP3与QPACK", "support", "✲", "HTTP/3 与 QPACK",
     "请求映射到 QUIC 流 · 外部 qpack 库：静态表 + Huffman · 不用动态表"),
    ("quic-go原理_支撑_可靠性与抗攻击", "support", "◐", "可靠性与抗攻击",
     "3× 放大限制 · Retry/token 地址验证 · 入口分流限流 · stateless reset"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接口主线 · 应用如何用（会话连接 / 事件循环 / HTTP 与流）"),
...
# ===================================================================== #
PANO_NAME = "quic-go原理_全景主线框架"
ARCH_W, ARCH_H = 1040, 650  # 必须与 ARCH_SVG_NAME 的 viewBox 一致
# (x, y, w, h, 主线name) —— 一个模块可拆多行热区，一条主线可被多个区域指向
ARCH_HOTSPOTS = [
    # powers 标题条 → 全景总览
    (30, 46, 980, 42, "quic-go原理_全景主线框架"),
    # ① 应用 + http3 层
    (30, 100, 980, 22, "quic-go原理_支撑_HTTP3与QPACK"),      # http3/QPACK 层标题带
    (48, 140, 300, 26, "quic-go原理_接口_HTTP与流"),          # http3.Server
    (360, 140, 300, 26, "quic-go原理_接口_HTTP与流"),         # http3.Transport
    (672, 140, 320, 26, "quic-go原理_接口_会话与连接"),       # 应用 OpenStream/AcceptStream
    # ② Transport
    (30, 188, 980, 20, "quic-go原理_接口_会话与连接"),        # Transport 标题带（API 入口）
    (48, 214, 440, 28, "quic-go原理_接口_事件循环与IO"),      # listen 读循环 + CID demux
    (520, 214, 470, 28, "quic-go原理_支撑_可靠性与抗攻击"),   # baseServer Retry/token/放大
    # ③ Conn 左
    (30, 262, 486, 32, "quic-go原理_接口_会话与连接"),        # Conn 标题
    (48, 300, 210, 28, "quic-go原理_接口_事件循环与IO"),      # run() select 循环
    (288, 300, 210, 28, "quic-go原理_接口_会话与连接"),       # streams_map / OpenStream
    (48, 338, 210, 28, "quic-go原理_支撑_流与流量控制"),      # flow controller
    (288, 338, 210, 28, "quic-go原理_支撑_连接管理与迁移"),   # connIDManager / path
    # ④ 发送/接收路径 右
    (546, 300, 220, 28, "quic-go原理_支撑_包与帧编解码"),     # packet_packer 组包
    (778, 300, 214, 28, "quic-go原理_支撑_丢包检测与恢复"),   # sentPacketHandler
    (546, 338, 220, 28, "quic-go原理_支撑_拥塞控制"),         # cubicSender
    (778, 338, 214, 28, "quic-go原理_接口_事件循环与IO"),     # sendQueue goroutine
    # ⑤ wire + crypto
    (30, 426, 486, 120, "quic-go原理_支撑_包与帧编解码"),     # wire 线格式
    (528, 426, 482, 120, "quic-go原理_支撑_TLS握手与加密"),   # crypto 握手加密
    # ⑦ IO 底座
    (30, 560, 980, 44, "quic-go原理_接口_事件循环与IO"),      # socket + timer
]
# 没有独立架构区域、需底部 chip 兜底的主线（本项目 12 主线全部落在图上 → 空）
ARCH_ALWAYS_CHIP = []

BRAND_TITLE = "quic-go 核心原理"
BRAND_SUB = "quic-go"
HOME_DESC = ("quic-go 核心原理设计文档库的离线交互图谱——家族 8 传输协议库（QUIC + HTTP/3 · 纯 Go · batteries-included，自管 socket 与 goroutine）。"
             "12 条主线、15 张手绘原理图，全部回本地源码核实。点击项目总架构图任意模块即可下钻到对应主线。")
ARCH_SVG_NAME = "quic-go原理_全景_02总架构.svg"
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="quic-go 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
(function(){
  ...
  var saved=localStorage.getItem('quic-go-atlas-theme');
  ...
  function toggleTheme(){
    ...
    localStorage.setItem('quic-go-atlas-theme',cur);
