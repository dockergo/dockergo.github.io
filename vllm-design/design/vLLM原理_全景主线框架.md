...
1. **PagedAttention:KV 缓存分块非连续**:朴素推理为每请求预留"最大长度"的连续 KV 显存,大量浪费(内部碎片 + 为峰值预留);vLLM 把 KV 切成固定大小块(默认 16 token/块),像 OS 分页——按需分配、逻辑连续物理分散、块可共享。显存利用率从 ~20% 提到接近 100%,吞吐倍增。

2. **连续批处理:GPU 一刻不闲**:朴素静态批处理等一批全做完才收下一批,短请求做完后 GPU 空转等长请求;vLLM 连续批处理每一步(step)都重新组批——完成的请求出队、等待的立即入队,GPU 每步都满载。配合 chunked prefill(长 prompt 切块混入)进一步平滑。

3. **v1 架构:EngineCore 独立进程 + 重算式抢占**:v1(v0 已删)把调度+执行的 EngineCore 拆到独立进程,与 API 前端经队列异步通信,前端不阻塞 GPU;显存不够时**重算式抢占**(preempt 释放块、num_computed_tokens 归零、重新入等待队列)而非换出到 CPU——重算比换入换出更划算。
