...
- 每个 **RowIndexEntry** = `positions[]`(各 stream 位置)+ 该 row group 的 `ColumnStatistics`;`RowIndex` = 重复 entry。写侧在 `TreeWriterBase` 建(`rowIndex`/`rowIndexEntry`)。
...
- 只有过滤列的 row index stream 按需读(`readCurrentStripeRowIndex`,`rowIndexColsToRead` 只含 filter 列)——不为无关列读索引。
