#!/usr/bin/env python3
"""orc-design 交互式核心原理图谱生成器（自包含 · 离线 · 双主题）。
...
_ap = argparse.ArgumentParser(description="生成 Apache ORC 交互式核心原理图谱（离线自包含 HTML）")
...
_DESIGN_DIR = _first_dir(
    _args.design_dir,
    os.environ.get("ORC_DESIGN_DIR"),
    os.path.join(HERE, "design"),
)
OUT = _args.out or os.environ.get("ORC_DESIGN_OUT") or os.path.join(HERE, "index.html")

# ===================================================================== #
# 一、主线注册表 —— 唯一需随项目调整的数据块
#     新家族（列式文件格式 · 自描述列存文件 + reader/writer 库 · Apache ORC）：
#     元模式 = 接触面 × 能力域 × 时机。全景 + 1 接触面主线 + 9 支撑能力域 = 11 篇。
# ===================================================================== #
MAINLINES = [
    ("ORC原理_全景主线框架", "pano", "◇", "全景主线框架",
     "新家族列式文件格式：双维模型 · 总架构 · 依赖矩阵/关系图 · 三条贯穿声明"),

    ("ORC原理_接触面_读写API与类型系统", "iface", "⚙", "读写 API 与类型系统",
     "TypeDescription 类型树(列 ID 前序编号) · VectorizedRowBatch 列式批 · Writer/Reader 树"),

    ("ORC原理_支撑_文件布局", "support", "▤", "文件布局",
     "核心：header/stripes/footer/postscript 四段 · 倒读定位 · stripe=index+data+footer"),
    ("ORC原理_支撑_列编码", "support", "▦", "列编码",
     "一列拆多 stream(PRESENT/DATA/LENGTH/DICT) · 整数 RLE v1/v2 · 字符串字典自动选"),
    ("ORC原理_支撑_压缩与流分块", "support", "⛁", "压缩与流分块",
     "codec 默认 ZSTD · 256KB 分块 · 3 字节块头(长度<<1|isOriginal) · 压不动存原文"),
    ("ORC原理_支撑_行组与索引", "support", "◫", "行组与索引",
     "row group 默认 10000 行 · RowIndexEntry positions+统计 · stripe→组→seek 三级跳读"),
    ("ORC原理_支撑_列统计与布隆", "support", "◉", "列统计与布隆",
     "三级统计(文件/stripe/row-group) min/max/count/sum · 布隆(等值,FPP 0.01)"),
    ("ORC原理_支撑_谓词下推", "support", "◐", "谓词下推 SARG",
     "SearchArgument 传入 · SargApplier · 三态 TruthValue · 跳 stripe/row-group"),
    ("ORC原理_支撑_写路径", "support", "✎", "写路径",
     "WriterImpl 建 TreeWriter 树 · addRowBatch · 内存/行数触发 flushStripe · close 写 footer"),
    ("ORC原理_支撑_读路径", "support", "◷", "读路径",
     "倒读尾部拿全图 · 列裁剪 + pickRowGroups 谓词选块 · TreeReader 树解码填批"),
    ("ORC原理_支撑_schema演进与ACID", "support", "⇄", "schema 演进与 ACID",
     "按名/按位置映射 · 类型转换 PPD 安全 · Hive ACID 事件 schema(base+delta 择要)"),
]

CAT_ORDER = [
    ("pano", "全景框架 · 先读这一篇"),
    ("iface", "接触面主线 · 计算引擎如何用（读写 API + 类型系统 + 向量批）"),
    ("support", "支撑主线 · 文件格式内部（9 条能力域）"),
]
...
# ===================================================================== #
PANO_NAME = "ORC原理_全景主线框架"
ARCH_W, ARCH_H = 1080, 820  # 必须与 ARCH_SVG_NAME 的 viewBox 一致
# (x, y, w, h, 主线name) —— 一个模块可拆多行热区，一条主线可被多个区域指向
ARCH_HOTSPOTS = [
    # 计算引擎/表格式外部条 + ① 读写 API 层 → 接触面
    (30, 64, 1020, 52, "ORC原理_接触面_读写API与类型系统"),
    (30, 130, 1020, 46, "ORC原理_接触面_读写API与类型系统"),
    # ② 文件布局核心（大盒）——标题 + header 行 + storage 都指向文件布局
    (30, 190, 1020, 30, "ORC原理_支撑_文件布局"),
    (46, 222, 988, 26, "ORC原理_支撑_文件布局"),
    (46, 254, 988, 26, "ORC原理_支撑_文件布局"),   # stripe 标题行
    # stripe 内部：index streams → 行组与索引；data streams → 列编码；row group 带 → 行组与索引
    (66, 282, 470, 30, "ORC原理_支撑_行组与索引"),
    (546, 282, 468, 30, "ORC原理_支撑_列编码"),
    (66, 318, 948, 32, "ORC原理_支撑_行组与索引"),
    # ③~⑦ 能力域带（2 行 × 3）
    (30, 410, 330, 78, "ORC原理_支撑_列编码"),
    (375, 410, 330, 78, "ORC原理_支撑_行组与索引"),
    (720, 410, 330, 78, "ORC原理_支撑_谓词下推"),
    (30, 500, 330, 78, "ORC原理_支撑_列统计与布隆"),
    (375, 500, 330, 78, "ORC原理_接触面_读写API与类型系统"),  # 类型系统
    (720, 500, 330, 78, "ORC原理_接触面_读写API与类型系统"),  # 向量批(接触面延伸)
    # 磁盘/对象存储 → 文件布局；贯穿层 列 ID → 接触面(类型树赋 ID)
    (30, 592, 1020, 50, "ORC原理_支撑_文件布局"),
    (30, 718, 1020, 44, "ORC原理_接触面_读写API与类型系统"),
]
# 没有独立架构区域、需底部 chip 兜底的主线（全景 + 4 条跨切/子层能力域）
ARCH_ALWAYS_CHIP = [
    "ORC原理_全景主线框架",
    "ORC原理_支撑_压缩与流分块",
    "ORC原理_支撑_写路径",
    "ORC原理_支撑_读路径",
    "ORC原理_支撑_schema演进与ACID",
]

BRAND_TITLE = "一切知识皆索引"
BRAND_SUB = "Apache ORC"
HOME_DESC = ("Apache ORC 核心原理设计文档库的离线交互图谱——新家族（列式文件格式 · 自描述列存文件 + reader/writer 库 · "
             "被 Spark/Hive/Trino/Iceberg 调用）。11 条主线、30+ 张手绘原理图，全部回本地源码核实。"
             "点击项目总架构图任意模块即可下钻到对应主线。")
ARCH_SVG_NAME = "ORC原理_总架构图.svg"
...
def build_archnav():
    ...
    return (
        '<div class="arch-wrap">'
        '<img alt="Apache ORC 项目总架构图" src="data:image/svg+xml;base64,%s"/>'
...
(function(){
  ...
  var saved=localStorage.getItem('orc-atlas-theme');
  ...
  function toggleTheme(){
    ...
    localStorage.setItem('orc-atlas-theme',cur);
...
<header>
  ...
  <a href="https://github.com/apache/orc" target="_blank" rel="noopener" title="GitHub 源码仓库" style="display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" aria-hidden="true"><path d="M12 .5C5.7.5.5 5.7.5 12c0 5.1 3.3 9.4 7.9 10.9.6.1.8-.2.8-.6v-2c-3.2.7-3.9-1.4-3.9-1.4-.5-1.3-1.3-1.7-1.3-1.7-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1 1.8 2.7 1.3 3.4 1 .1-.8.4-1.3.7-1.6-2.6-.3-5.3-1.3-5.3-5.8 0-1.3.5-2.3 1.2-3.1-.1-.3-.5-1.5.1-3.1 0 0 1-.3 3.3 1.2a11.4 11.4 0 0 1 6 0C17.3 4.7 18.3 5 18.3 5c.6 1.6.2 2.8.1 3.1.8.8 1.2 1.8 1.2 3.1 0 4.5-2.7 5.5-5.3 5.8.4.4.8 1.1.8 2.2v3.3c0 .4.2.7.8.6 4.6-1.5 7.9-5.8 7.9-10.9C23.5 5.7 18.3.5 12 .5z"/></svg></a><a href="https://orc.apache.org" target="_blank" rel="noopener" title="项目官网" style="display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;border:1px solid var(--c-line);color:var(--c-ink2);text-decoration:none;margin-right:8px"><img src="data:image/svg+xml;base64," width="18" height="18" alt="官网" style="display:block"/></a><button id="themeBtn" title="切换主题" aria-label="切换主题" style="width:38px;height:38px;border-radius:50%;border:1px solid var(--c-line);background:var(--c-panel);color:var(--c-ink2);cursor:pointer;display:inline-grid;place-items:center;font-size:16px;flex:none">☾</button>
</header>
<div class="wrap">
  <div id="home">
    {archnav}
